...
## 描述
**Get-AzPolicyRemediation** Cmdlet 會取得範圍或特定補救範圍內的所有策略補救。
...
此命令會獲得在名為"我的訂閱"的訂閱或下方建立的所有修復。

### 範例 2：取得特定的策略補救和部署詳細資料
```
PS C:\> Get-AzPolicyRemediation -ResourceGroupName "myResourceGroup" -Name "remediation1" -IncludeDetail
```

此命令會從資源群組 "myResourceGroup" 獲得名為 "remediation1" 的補救。 將會包含要補救的資源詳細資料。

### 範例 3：在具有選擇性篩選的管理群組中取得 10 個策略補救
```
...
此命令最多會從名為 'mg1' 的管理群組獲得 10 個策略補救。 系統只會針對指定之策略工作分派進行政策補救。
