...
## 描述
**Get-AzRedisCache** Cmdlet 會取得指定的 Azure Redis Cache。
如果您未指定參數，此作業會針對目前的訂閱獲得每個 Redis Cache。

## 例子

### 範例 1：取得 Redis Cache by name
```
...
此命令會獲得名為 myexists 的 Redis Cache。

### 範例 2：在資源群組中取得每個 Redis Cache
```
...
此命令會獲得指定資源群組中每個 Redis Cache。

### 範例 3：取得目前訂閱中每個 Redis Cache
```
...
### -名稱
指定要取得 Redis Cache 的名稱。
使用 *ResourceGroupName* 參數。
...
### -ResourceGroupName
指定要取得包含 Redis Cache 的資源組名。
如果您只指定 *ResourceGroupName* 參數，此作業會獲得指定資源群組中每個 Redis Cache。
