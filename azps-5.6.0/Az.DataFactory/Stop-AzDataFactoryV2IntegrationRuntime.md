...
## 描述
**Stop-AzDataFactoryV2 RuntimeRuntime** Cmdlet 會以由 Start-AzDataFactoryV2IntegrationRuntime Cmdlet 啟動的受管理專用整合執行時間以 'Started' 狀態停止。 資源會釋出，而州會移轉為'已停止」。

## 例子

### 範例 1：停止位於 'Started' 狀態之受管理的整合執行時間。
```
...
受管理的整合執行時間 'test-reserlved-ir' 為 'Started' 狀態。 在 Cmdlet Stop-AzDataFactoryV2IntegrationRuntime之後，資源會釋出，而狀態會轉移到 '已停止'。
...
[Start-AzDataFactoryV2FactoryRuntime]()
