...
## 描述
Cmdlet Set-AzDataFactoryV2IntegrationRuntime更新具有特定參數的整合執行時間。
...
Cmdlet 會新增 ADF 以使用共用整合執行時間。 使用 `-SharedIntegrationRuntimeResourceId` 參數時 `-Type` ，也必須包含。 請注意，執行 Cmdlet 之前，必須授予資料工廠使用整合執行時間的許可權。

### 範例 3：將 Self-Hosted IR 設定為 ADF 中 Azure-SSIS IR 的 Proxy。
```
...
Cmdlet 會更新 Azure-SSIS 整合執行時間，以使用自我託管整合執行時間做為資料 Proxy。
...
在 Self-Hosted 與 Azure-SSIS 整合執行時間之間移動資料時，參照暫存資料存放區之 Azure Blob 儲存連結服務名稱
...
### -LicenseType
您想要為 SSIS IR 選取授權類型。 有兩種類型：LicenseIncluded 或 BasePrice。 如果您符合 Azure 混合式使用權益 (AHUB) ，請選取 BasePrice。 如果沒有，請選取 LicenseIncluded。
