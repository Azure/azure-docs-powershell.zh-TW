...
## 簡介
將資料存放區或雲端服務連結至 Azure Data Factory。
...
## 描述
**New-AzDataFactoryLinkedService** Cmdlet 將資料存放區或雲端服務連結至 Azure Data Factory。
如果您為已存在的連結服務指定名稱，此 Cmdlet 會先提示您確認，然後再取代連結的服務。
如果您指定 *Force* 參數，Cmdlet 會取代現有的連結服務，而不需要確認。
請以下列循序執行這些作業： 
- 建立資料工廠。 
- 建立連結服務。 
- 建立資料集。 
...
此命令在名為 WikiADF 的資料工廠中，建立一個名為 LinkedServiceCuratedWikiData 的連結服務。
此連結服務會連結檔案中指定的 Azure Blob 存放區至名為 WikiADF 的資料工廠。
該命令會使用管線運算子Format-List Cmdlet。
...
### -強制
表示此 Cmdlet 會取代現有的連結服務，而不會提示您確認。
