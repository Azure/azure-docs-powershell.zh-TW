...
## 描述
**New-AzDataFactoryHub** Cmdlet 會使用指定的檔案定義，在指定的 Azure 資源群組和指定的資料工廠中，為 Azure Data Factory 建立中樞。
建立中樞之後，您可以使用它來儲存及管理群組中的連結服務，也可以新增管線至中樞。
...
此命令在資源群組 ADFResourceGroup 和資料工廠中建立名為 ContosoDataHub 的中樞，名為 ADFDataFactory。

## 參數

### -DataFactory
指定 **PSDataFactory** 物件。
此 Cmdlet 會為此參數指定的資料工廠建立中樞。
...
### -檔案
指定包含中樞描述之 JSON (JSON) 之 JavaScript 物件標記法的完整路徑。
