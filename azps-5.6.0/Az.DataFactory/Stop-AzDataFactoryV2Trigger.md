...
## 描述
**Stop-AzDataFactoryV2Trigger** Cmdlet 會停止資料工廠中的觸發程式。 如果觸發程式位於 'Started' 狀態，Cmdlet 會停止觸發程式，不再會啟動管線。 如果觸發人已經進入 '已停止' 狀態，此 Cmdlet 沒有作用。 如果已指定 Force 參數，Cmdlet 不會在停止觸發之前提示。
...
在資料工廠 「WikiADF」中停止名為「ScheduledTri用」的觸發程式。
