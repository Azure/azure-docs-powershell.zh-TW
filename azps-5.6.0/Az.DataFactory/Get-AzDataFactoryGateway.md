...
## 簡介
在 Azure Data Factory 中，獲得邏輯閘道相關資訊。
...
## 描述
**Get-AzDataFactoryGateway** Cmdlet 會取得 Azure Data Factory 中邏輯閘道的資訊。
如果您指定閘道的名稱，此 Cmdlet 會獲得該閘道的資訊。
如果您未指定名稱，此 Cmdlet 會獲得資料工廠所有閘道的資訊。
如果您想要將內部部署 Microsoft SQL Server 新增為數據工廠的連結服務，您必須在內部部署電腦上安裝閘道。
...
此命令會針對名為 ADF 的資源群組中名為 WikiADF 的資料工廠，獲得所有邏輯閘道的資訊。

### 範例 2：在資料工廠中取得特定的邏輯閘道
```
...
此命令會獲得名稱為 ADF 之資源群組中名稱為 WikiADF 之資料工廠中名為 Gateway01 的邏輯閘道相關資訊。

## 參數

### -DataFactory
指定 **PSDataFactory** 物件。
此 Cmdlet 會獲得此參數指定之資料工廠中邏輯閘道的資訊。
...
### -ResourceGroupName
指定 Azure 資源組的名稱。
此 Cmdlet 會獲得屬於此參數指定之群組之邏輯閘道的資訊。
