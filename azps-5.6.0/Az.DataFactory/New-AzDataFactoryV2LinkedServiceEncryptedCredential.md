...
## 描述
此New-AzDataFactoryV2LinkedServiceEncryptedCredential Cmdlet 會使用指定的整合執行時間加密連結服務中的認證。

請確保符合下列先決條件：
* **在自行託管** 的整合執行時間上啟用遠端存取選項。
* Powershell 7.0 或更高版本用來執行 Cmdlet。

## 例子

### 範例 1：加密連結服務中的認證

此命令會加密檔案中的認證C:\samples\WikiSample\TaxiDemo1.js名為 test-selfhost-ir 的整合執行時間。
