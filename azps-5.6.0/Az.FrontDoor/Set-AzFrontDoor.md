...
## 描述
**Set-AzFrontDoor** Cmdlet 會更新前門負載平衡器。 如果沒有提供輸入參數，將會使用來自現有前門的舊參數。

## 例子

### 範例 1：使用 FrontDoorName 和資源GroupName 更新現有的前門。
```powershell
...
### 範例 4：使用 -DisableCertificateNameCheck 參數更新現有前門的 BackendPoolSetting 屬性 EnforceCertificateNameCheck
您可以使用 FrontoorName 和 ResourceGroupName、PSFrontDoor 物件或 ResourceId 來識別要更新的門。  (請參閱上述 3 個範例，例如) 範例使用 PSFrontDoor 物件。
