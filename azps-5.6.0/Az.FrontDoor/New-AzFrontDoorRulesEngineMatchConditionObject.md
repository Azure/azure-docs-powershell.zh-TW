...
改善新的 PSRulesEngineMatchCondition 物件。
...
### -MatchValue
比對要比對的值。
運算子會以 OR 語義來適用于這裡的每個值。
如果其中任何一個符合變數與給定運算子，則此比對條件會視為相符。
...
### -MatchVariable
Match Variable。
可能的值為 IsMobile、RemoteAddr、RequestMethod、QueryString、PostArg、RequestUri、RequestPath、RequestFileName、RequestfilenameExtension、RequestHeader、RequestBody、RequestS方程式
...
### -運算子
描述要適用于比對條件的運算子。
可能的值為 Any、IPMatch、GeoMatch、Equal、Contains、LessQual、GreaterQualEqual、LessQualOrEqual、GreaterQualOrEqual、BeginsWith、EndsWith。
...
### -轉換
在比對之前套用哪些轉換的清單。 可能的個別轉換值為小寫、大寫、修剪、UrlDecode、UrlEncode、RemoveNulls。
