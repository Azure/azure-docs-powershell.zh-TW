...
## 描述
**New-AzFrontDoorBackendpoolsSettingObject** Cmdlet 會建立一個新的 PSBackendPoolsSettings 物件，用於建立前門。

## 例子

### 範例 1：使用預設值建立後端幕後處理物件
```powershell
...
是否要針對所有後端資料庫的 HTTPS 要求強制執行憑證名稱檢查。
對非 HTTPS 要求沒有影響。
...
### -SendRevbTimeoutIn用秒
將轉寄要求傳送及接收超時到後端。 到達超時時，要求會失敗並返回。
