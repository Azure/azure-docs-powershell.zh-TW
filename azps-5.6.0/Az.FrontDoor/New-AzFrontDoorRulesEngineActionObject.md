...
使用 Cmdlet "New-AzFrontDoorHeaderActionObject" 來建立 PSHeaderObjects，以進入參數 "-RequestHeaderActions" 和 "-ResponseHeaderActions"。
...
### -CustomQueryString
這是要放在重新導向 URL 中的查詢字串集。
設定此值會取代任何現有的查詢字串;保留空白以保留傳入查詢字串。
查詢字串必須採用 \<key\> = \<value\> 格式。
第一個？
並且&自動新增，因此請不要在最前面加入查詢字串，但請以不同的查詢字串&。
