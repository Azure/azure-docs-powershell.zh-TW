...
### 範例 1：使用轉轉規則建立適用于前門的 PSRoutingRuleObject
```powershell
...
### 範例 2：使用重新導向規則建立適用于前門的 PSRoutingRuleObject
```powershell
...
### -CustomFragment
要新加入重新導向 URL 的片段。 片段是 #之後 URL 的一部分。 請勿包含 #。
...
### -CustomPath
重新導向的完整路徑。 路徑不能是空白的，必須以 /開始。 保留空白，以使用內入路徑做為目的地路徑。
...
### -CustomQueryString
這是要放在重新導向 URL 中的查詢字串集。 設定此值會取代任何現有的查詢字串;保留空白以保留傳入查詢字串。 查詢字串必須 <key>=<value> 格式。 第一個？ 並且&自動新增，因此請不要在最前面加入查詢字串，但請以不同的查詢字串&。
...
### -ForwardingProtocol
將流量轉往後端時，此規則將使用的通訊協定預設值為 MatchRequest。
...
### -PatternToMatch
規則的路由模式，不得有任何 *，除非可能在路徑結尾的最後 /之後。 預設值為 /*
...
### -RedirectProtocol
重新導向流量目的地的通訊協定。 預設值為 MatchRequest
