...
## 描述
**Restore-AzKeyVaultSecret** Cmdlet 會建立指定金鑰庫中的機密。
此秘訣是輸入檔案中備份機密的複本，其名稱與原始密碼相同。
如果金鑰庫已經有相同名稱的機密，此 Cmdlet 會失敗，而不會覆寫原始的機密。
如果備份包含多個版本的機密，所有版本會還原。
您還原機密的金鑰庫可能會與備份該機密的金鑰庫不同。
...
此命令會從名為 Backup.blob 的備份檔案還原為名為 contoso 的金鑰保存庫，包括其所有版本。
