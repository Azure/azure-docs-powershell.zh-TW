...
## 描述
**Backup-AzKeyVaultManagedStorageAccount** Cmdlet 可下載並儲存于檔案中，以備份金鑰保存庫中指定的受管理儲存帳戶。
由於下載的內容已加密，因此無法在 Azure 金鑰保存庫外部使用。
只要保存庫位於相同的 Azure 地理位置，您即可以將備份的儲存空間帳戶還原到訂閱中任何備份的金鑰保存庫。
使用此 Cmdlet 的一般原因如下： 
- 您想要保留儲存空間帳戶的離線副本，以防您不小心從保存庫刪除原始檔案。
 
- 您使用 Key Vault 建立受管理的儲存帳戶，而現在想要將物件複製至不同的 Azure 區域，以便從分散式應用程式的所有實例使用它。
使用 **Backup-AzKeyVaultManagedStorageAccount** Cmdlet 以加密格式來取回受管理的儲存帳戶，然後使用 **Restore-AzKeyVaultManagedStorageAccount** Cmdlet，並指定第二個地區的金鑰保存庫。

## 例子

### 範例 1：使用自動產生的檔案名備份受管理的儲存空間帳戶
```powershell
...
此命令會從名為 MyKeyVault 的金鑰保存庫中，從名為 MyMSAK 的受管理儲存帳戶，並儲存該受管理儲存帳戶的備份至自動為您命名的檔案，並顯示檔案名。

### 範例 2：將受管理的儲存空間帳戶備份到指定的檔案名
```powershell
...
此命令會從名為 MyKeyVault 的金鑰保存庫中，從名為 MyMSAK 的受管理儲存帳戶，並儲存該受管理儲存帳戶的備份至名為 Backup.blob 的檔案。

### 範例 3：將先前取回的受管理的儲存空間帳戶備份到指定的檔案名，覆寫目標檔案而不提示。
```powershell
...
此命令會建立名為 $msak 的受管理儲存帳戶$msak。名稱為 $msak。保存庫名稱為名為 Backup.blob 的檔案，如果檔案已存在，會以無提示方式覆寫檔案。
