...
## 描述
**Restore-AzKeyVaultCertificate Cmdlet** 會從備份檔案，于指定的金鑰保存庫中建立憑證。
此憑證是輸入檔案中備份憑證的複本，其名稱與原始憑證相同。
如果金鑰庫已經包含相同名稱的憑證，此 Cmdlet 會失敗，而不會覆寫原始憑證。
如果備份包含多個版本的憑證，會還原所有版本。
您還原憑證的金鑰庫可能會與備份憑證的金鑰庫不同。
...
此命令會從名為 Backup.blob 的備份檔案還原憑證 ，包括所有版本，到名為 MyKeyVault 的金鑰保存庫。
