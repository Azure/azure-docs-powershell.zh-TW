...
## 描述
復原 **-AzKeyVaultManagedStorageAccountRemoval** 命令會復原先前已刪除的受管理儲存空間帳戶，但此保存庫必須啟用柔刪除，且復原期間會嘗試復原。
...
此命令順序會決定指定的儲存空間帳戶是否存在於保存庫中的已刪除狀態;後續的命令會復原已刪除的儲存空間帳戶，讓該帳戶恢復為使用中狀態。
...
### -InputObject
已刪除 Managed Storage Account 物件
...
### -名稱
KeyVault 管理儲存帳戶的名稱。
Cmdlet 會從保存庫名稱、目前選取的環境及受管理的儲存帳戶名稱建構目標 FQDN。
