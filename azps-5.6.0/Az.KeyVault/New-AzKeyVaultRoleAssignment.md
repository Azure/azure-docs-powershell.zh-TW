...
## 描述
使用命令 `New-AzKeyVaultRoleAssignment` 來授予存取權。
...
指派的角色必須使用 RoleDefinitionName pr RoleDefinitionId 參數指定。 您可以指定授予存取權的範圍。 它會預設為選取的訂閱。
...
此範例將角色「受管理的 HSM 策略管理員」指派給使用者」在 user1@microsoft.com 最上方的範圍。
