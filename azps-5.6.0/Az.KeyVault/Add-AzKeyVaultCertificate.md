...
## 描述
**Add-AzKeyVaultCertificate Cmdlet** 會啟動在 Azure 金鑰庫金鑰庫註冊憑證的過程。
...
第一個命令使用 New-AzKeyVaultCertificatePolicy Cmdlet 建立憑證策略，然後將它儲存在 $Policy 變數中。
第二個命令使用 **Add-AzKeyVaultCertificate** 來啟動建立憑證的過程。
第三個命令Get-AzKeyVaultCertificateOperation Cmdlet 來輪詢作業，以確認作業已完成。
最後一個命令會使用 Get-AzKeyVaultCertificate Cmdlet 取得憑證。
