...
## 描述
**New-AzKeyVaultCertificateOrganizationDetail** Cmdlet 會建立記憶體內憑證組織詳細資料物件。
...
第一個命令會建立憑證系統管理員詳細資料物件，然後將它儲存在$AdminDetails變數中。
第二個命令會建立憑證組織詳細資料物件，然後將它儲存在$OrgDetails變數中。
