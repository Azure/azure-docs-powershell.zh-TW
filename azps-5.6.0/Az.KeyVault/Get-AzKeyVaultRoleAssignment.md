...
## 簡介
取得或列出受管理 HSM 的角色指派。 使用各自的參數列出指派給特定使用者或角色定義。
...
## 描述
使用命令 `Get-AzKeyVaultRoleAssignment` 列出在範圍中有效的所有角色指派。
若沒有任何參數，此命令會返回受管理 HSM 下進行的所有角色指派。
...
指派的角色必須使用 RoleDefinitionName 或 RoleDefinitionId 參數指定。
您可以指定授予存取權的範圍。 它預設為 "/"。
...
此範例會列出 「/keys」範圍中「my進一步」的所有角色指派，並按使用者登錄名稱篩選結果。
