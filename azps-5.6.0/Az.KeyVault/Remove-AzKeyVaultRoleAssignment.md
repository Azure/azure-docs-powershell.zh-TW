...
## 描述
使用 `Remove-AzKeyVaultRoleAssignment` Cmdlet 在給定範圍和給定角色中撤銷任何主體的存取權。 工作分派的物件，即必須指定主體。 主體可以是使用者 (使用 SignInName 或 ObjectId 參數來識別使用者) ，安全性群組 (會使用 ObjectId 參數來識別群組) 或服務主體 (使用 ApplicationId 或 ObjectId 參數來識別 ServicePrincipal。 必須使用 RoleDefinitionName 或 RoleDefinitionId 參數指定指派給主體的角色。
...
此範例在 "/keys" 範圍中撤銷 "" 的「受管理 HSM 策略系統管理員 user1@microsoft.com 」角色。
...
此範例在所有範圍中撤銷 user1@microsoft.com "" 的所有角色。
