...
## 描述
**Get-AzKeyVaultSecret** Cmdlet 會取得金鑰庫中的機密。
此 Cmdlet 會獲得金鑰庫中的特定機密或所有機密。

## 例子

### 範例 1：取得金鑰庫中所有機密的所有目前版本
```powershell
...
此命令會獲得名稱為 Contoso 之金鑰庫中所有機密的最新版本。
...
此命令會獲得名稱為 Contoso 之金鑰庫之機密的所有版本。
...
此命令會獲得名稱為 Contoso 之金鑰庫之機密的目前版本。
...
此命令在名稱為 Contoso 的金鑰庫內，會獲得名為 secret1 的特定版本。

### 範例 5：取得特定機密目前版本的純文字值
```powershell
PS C:\> $secretText = Get-AzKeyVaultSecret -VaultName 'Contoso' -Name 'ITSecret' -AsPlainText
```

Cmdlet 會以字串形式在已使用時 `-AsPlainText` ，將密碼做為字串。

### 範例 6：取得此金鑰庫的所有已被刪除但並未清除的機密。
```powershell
...
此命令會獲得先前在名稱為 Contoso 的金鑰庫中刪除但並未清除的所有機密。

### 範例 7：針對此金鑰庫，獲得已被刪除但並未清除的機密 ITSecret。
```powershell
...
此命令會獲得先前在名稱為 Contoso 的金鑰庫中刪除但並未清除的機密'機密 1。」
此命令會返回中繼資料，例如刪除日期，以及此已刪除之機密的排程清除日期。

### 範例 8：使用篩選在金鑰庫中取得所有機密的所有目前版本
```powershell
...
此命令會獲得名稱為 Contoso 之金鑰庫中所有機密的最新版本，以「機密」做為開始。

## 參數

### -AsPtext
設定之後，Cmdlet 會將安全字串中的機密轉換為解密純文字字串做為輸出。
...
### -IncludeVersions
表示此 Cmdlet 會獲得所有版本的機密。
目前版本的機密是清單中的第一個版本。
如果您指定此參數，您也必須指定 *Name* 和 *VaultName* 參數。
如果您未指定 *IncludeVersions* 參數，此 Cmdlet 會以指定的名稱獲得目前版本的 *機密*。
...
### -VaultName
指定該機密所屬的金鑰庫名稱。
此 Cmdlet 會根據此參數指定的名稱 (您目前的環境，建構金鑰庫的 FQDN) 完整功能變數名稱。
...
### -版本
指定機密版本。
此 Cmdlet 會依據金鑰庫名稱、您目前選取的環境、機密名稱和金鑰版本來建構機密的 FQDN。
