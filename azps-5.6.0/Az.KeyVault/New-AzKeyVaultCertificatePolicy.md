...
## 描述
**New-AzKeyVaultCertificatePolidlet** 會為 Azure Key Vault 建立記憶體內憑證策略物件。
...
### -DnsName
指定憑證中的 DNS 名稱。 主題替代名稱 (SAN) 可指定為 DNS 名稱。
...
> [!NOTE]
> 如果您必須在參數的屬性內使用逗號 (、) 或句號 (.) ，則必須以引號括住 `SubjectName` 屬性欄位。 例如，您可以使用 O="Contoso， Ltd." 在組織名稱欄位中。
