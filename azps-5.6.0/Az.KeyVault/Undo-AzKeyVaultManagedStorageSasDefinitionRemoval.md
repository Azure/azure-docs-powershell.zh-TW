...
## 簡介
復原先前已刪除的 KeyVault 管理儲存空間 SAS 定義。
...
## 描述
復原 **-AzKeyVaultManagedStorageSasDefinitionRemoval** 命令會復原先前已刪除的受管理儲存空間 SAS 定義，但此保存庫必須啟用柔刪除，且復原間隔期間會嘗試復原。
...
此命令順序會決定指定的儲存空間 SAS 定義是否存在於保存庫中的已刪除狀態;後續命令會復原已刪除的 sas 定義，讓該定義恢復為使用中狀態。

## 參數

### -AccountName
KeyVault 管理的儲存空間帳戶名稱。
Cmdlet 會從保存庫名稱、目前選取的環境及受管理的儲存帳戶名稱，建構受管理的儲存空間 SAS 定義的 FQDN。
...
### -名稱
KeyVault 管理的儲存空間 SAS 定義名稱。
Cmdlet 會從保存庫名稱、目前選取的環境、受管理儲存帳戶的名稱以及 SAS 定義的名稱建構目標 FQDN。
