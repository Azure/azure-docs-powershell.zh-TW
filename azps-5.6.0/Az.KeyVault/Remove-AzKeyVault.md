...
## 描述
**Remove-AzKeyVault** Cmdlet 會刪除指定的金鑰庫。
它也會刪除該實例中所包含的所有金鑰和機密。
...
此命令會從您目前的訂閱移除名為 Contoso03Vault 的金鑰庫。
...
此命令會從命名的資源群組移除名為 Contoso03Vault 的金鑰庫。
如果您未指定資源組名，Cmdlet 會搜尋命名的金鑰保存庫，以在目前的訂閱中刪除。
...
此命令會從您目前的訂閱移除名為 testManaged除法的受管理 hsm。
