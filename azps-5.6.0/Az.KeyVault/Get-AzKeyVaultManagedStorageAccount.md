...
## 描述
如果指定帳戶名稱，且帳戶金鑰是由指定的保存庫管理，則獲得金鑰保存庫管理的 Azure 儲存帳戶。 如果未指定帳戶名稱，會列出所有由指定儲存庫管理之金鑰的帳戶。

## 例子

### 範例 1：列出所有金鑰保存庫受管理的儲存帳戶
```powershell
...
如果金鑰是由保存庫 'myvault' 管理，則獲得金鑰保存庫管理儲存帳戶的'mystorageaccount'詳細資料

### 範例 3：使用篩選列出所有金鑰保存庫受管理的儲存帳戶
```powershell
...
列出所有以「測試」為開始由「測試」開始由「myvault」管理之儲存庫「myvault」的帳戶
