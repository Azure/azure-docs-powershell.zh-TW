...
### 範例 1：根據名稱建立 Windows 虛擬桌面 HostPool
```powershell
...
### -SsAuthority
客戶 ADFS 伺服器的 URL，用於簽署 WVD SSO 憑證。
...
### -SsoClientId
用來發行 WVD SSO 憑證之已註冊的信賴方 ClientId。
...
儲存用於與 ADFS 通訊之機密的 Azure KeyVault 路徑。
