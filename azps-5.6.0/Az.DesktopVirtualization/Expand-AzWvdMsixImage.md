...
### 範例 1：展開指定的圖像路徑，並取回在圖像中找到的套件中繼資料AppxManifest.xml
```powershell
...
### -MsixImageUri
代表參照 MSIX 圖像至建構的 URI，請參閱 MSIXIMAGEURI 屬性的 NOTES 區段，並建立雜湊表。
...
MSIXIMAGEURI： <IMsixImageUri> 代表參照 MSIX 圖像的 URI
  - `[Uri <String>]`：URI 對圖像
