...
此命令在工作階段主機中會獲得 Windows 虛擬桌面使用者部署。

### 範例 2：列出 Windows 虛擬桌面使用者
```powershell
...
此命令會列出工作階段主機中的 Windows 虛擬桌面使用者清單。

### 範例 3：在主機集中列出 Windows 虛擬桌面使用者
```powershell
...
此命令會列出主機集中工作階段主機中的 Windows 虛擬桌面使用者清單。
...
### -篩選
OData 篩選運算式。
篩選的有效屬性為 userprincipalname 和會話狀態。
