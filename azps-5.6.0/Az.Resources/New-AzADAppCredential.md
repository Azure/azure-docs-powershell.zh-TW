...
## 描述
New-AzADAppCredential Cmdlet 可用來新增或匯總應用程式的認證。
提供應用程式物件識別碼或應用程式識別碼以識別應用程式。
...
新的密碼認證會新增到現有應用程式，物件識別碼為 '1f89cf81-0146-4f4e-beae-2007d0668416'。
...
提供的 base64 編碼公用 X509 憑證 ("myapp.cer") 會新增到現有的應用程式，其應用程式識別碼為 '4589cd6b-3d79-4bb4-93b8-a0b99f3bfc58'。
...
使用物件識別碼 '1f89cf81-0146-4f4e-beae-2007d0668416' 和管道到 New-AzADAppCredential 的應用程式，以使用給定密碼為該應用程式建立新應用程式認證。
...
### -EndDate
認證使用量的有效結束日期。 預設結束日期為自今天起一年。  對於「非對稱」類型認證，這必須設定為 X509 憑證有效日期當天或之前。
...
### -StartDate
認證使用量的有效開始日期。 預設的開始日期值是今天。 對於「非對稱」類型認證，這必須設定為 X509 憑證的有效日期當天或之後。
