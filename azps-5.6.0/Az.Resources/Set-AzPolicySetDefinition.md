...
## 描述
**Set-AzPolicySetDefinition** Cmdlet 會修改一個策略定義。
...
該命令會儲存該物件在$PolicySetDefinition變數中。
第二個命令會更新由 **resourceId** 屬性識別之 $PolicySetDefinition。
...
此命令會更新名為 VMPolicySetDefinition 之策略集定義的中繼資料，指出其類別為「虛擬機器」。
...
此命令會更新名為 VMPolicySetDefinition 之策略集定義的群組。

### 範例 4：使用雜湊表更新策略集定義的群組
```
$groupsJson = ConvertTo-Json @{ name = "group1", displayName = "Virtual Machine Security" }, @{ name = "group2" }
PS C:\> Set-AzPolicySetDefinition -Name 'VMPolicySetDefinition' -GroupDefinition $groupsJson
```

此命令會使用雜湊表更新名為 VMPolicySetDefinition 之策略集定義的群組，以建構群組。
...
### -GroupDefinition
已更新之策略集定義之策略定義群組。 這可以是包含群組之檔案的路徑，或群組為 JSON 字串。
...
### -InputObject
此策略會設定定義物件，以更新從另一個 Cmdlet 輸出的結果。
...
### -中繼資料
更新之策略集定義的中繼資料。 這可以是包含中繼資料之檔案名的路徑，或是 JSON 字串的中繼資料。
...
### -Parameter
更新原則集定義的參數宣告。 這可以是檔案名或包含參數宣告的 uri 路徑，或參數宣告為 JSON 字串。
...
### -PolicyDefinition
該政策定義。 這可以是包含策略定義的檔案名路徑，或 JSON 字串中的策略定義。
