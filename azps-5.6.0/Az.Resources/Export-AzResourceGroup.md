...
## 描述
**Export-AzResourceGroup** Cmdlet 會將指定的資源群組視為範本，並存到 JSON 檔案。當您已在資源群組中建立一些資源，然後想要運用使用範本支援部署的好處時，這項功能就很實用。
此 Cmdlet 提供您一個簡單的起點，為資源群組中的現有資源產生範本。
在某些情況下，此 Cmdlet 無法產生範本的某些部分。
警告訊息會通知您資源失敗。
系統仍然會針對成功的部分產生範本。
...
此命令會將名為 TestGroup 的資源群組當做範本，並存到目前目錄中的 JSON 檔案。
...
此命令會從 "TestGroup" 資源群組中，將名為 "TestVirtualMachine" 的虛擬機器資源當做範本，並存到目前目錄中的 JSON 檔案。
...
此命令會以範本從 「TestGroup」資源群組中捕獲兩個資源，並存到目前目錄中的 JSON 檔案。 產生的範本不會包含任何產生的參數。
...
### -Pre
表示此 Cmdlet 會在自動決定要使用哪個 API 版本時，使用測試版 API 版本。
