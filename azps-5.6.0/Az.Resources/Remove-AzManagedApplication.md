...
## 描述
**Remove-AzManagedApplication** Cmdlet 會移除受管理的應用程式

## 例子

### 範例 1：根據資源識別碼移除受管理的應用程式
```
PS C:\>$Application = Get-AzManagedApplication -Name "myApp" -ResourceGroupName "myRG"
PS C:\>Remove-AzManagedApplication -Id $Application.ResourceId -Force
```

第一個命令會使用 Get-AzManagedApplication Cmdlet，獲得名為 myApp 的受管理應用程式。
命令會儲存在$Application變數中。
第二個命令會移除 **由 resourceId** 屬性識別的 managed $Application。
