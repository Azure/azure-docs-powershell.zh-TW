...
## 描述
**New-AzPolicySetDefinition** Cmdlet 會建立一個策略集定義。

## 例子

### 範例 1：使用策略集檔案建立包含中繼資料的組定義
```
[
   {
...
此命令會建立名為 VMPolicySetDefinition 的一個策略集定義，其中中繼資料指出其類別為「虛擬機器」，其中包含 C:\VMPolicy.js中指定的規則定義。 上方提供VMPolicy.js的範例內容。
...
此命令會建立名為 VMPolicySetDefinition 的參數化C:\VMPolicy.js定義。 上方提供VMPolicy.js的範例內容。

### 範例 3：使用策略定義群組建立一個策略集定義
```
[
   {
      "policyDefinitionId": "/providers/Microsoft.Authorization/policyDefinitions/2a0e14a6-b0a6-4fab-991a-187a4f81c498",
...
此命令會建立一個名為 VMPolicySetDefinition 與群組原則定義組在C:\VMPolicy.js定義。 上方提供VMPolicy.js的範例內容。
...
### -GroupDefinition
新策略集定義之策略定義群組。 這可以是包含群組之檔案的路徑，或群組為 JSON 字串。
...
### -中繼資料
設定定義之中繼資料。 這可以是包含中繼資料之檔案名的路徑，或是 JSON 字串的中繼資料。
...
### -Parameter
原則集定義的參數宣告。
這可以是包含參數宣告的檔案名路徑，或參數宣告為 JSON 字串。
