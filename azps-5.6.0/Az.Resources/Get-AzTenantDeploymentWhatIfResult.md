...
## 描述
**Get-AzTenantDeploymentWhatIfResult** Cmdlet 會取得 ARM 範本 What-If 指定租使用者範圍中範本部署的結果。 它會回一份變更清單，指出如果部署已執行，且不會對實際資源進行任何變更，將會更新哪些資源。 若要指定傳回結果的格式，請使用 *ResultFormat* 參數。
...
此命令會使用What-If範本檔案和磁片上的參數檔案，在租使用者範圍取得一個結果。
...
### 範例 2：使用 ResourceIdOnly What-If租使用者範圍取得結果
```powershell
PS C:\> Get-AzTenantDeploymentWhatIfResult `
    -DeploymentName "deploy-01" `
    -Location "West US" `
...
### -名稱
要建立之部署的名稱。
如果未指定，則提供範本檔案時預設為範本檔案名;預設為提供範本物件的目前時間，例如"20131223140835"。
