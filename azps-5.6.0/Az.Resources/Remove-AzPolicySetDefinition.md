...
## 描述
**Remove-AzPolicySetDefinition** Cmdlet 會移除一個策略定義。
...
第一個命令會使用 Cmdlet Get-AzPolicySetDefinition定義。
命令會儲存在$PolicySetDefinition變數中。
第二個命令會移除由 **resourceId** 屬性識別$PolicySetDefinition。
...
### -InputObject
此策略會設定定義物件，以移除從另一個 Cmdlet 輸出的物件。
