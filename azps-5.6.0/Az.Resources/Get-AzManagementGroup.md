...
## 描述
此Get-AzManagementGroup Cmdlet 會獲得全部或特定的管理群組。
...
透過 `Expand` 標號，您可以流覽 `Children` 陣列，並取得每個子女的詳細資訊。 例如， `Children[0]` 會以顯示名稱提供群組詳細資料 `TestGroup1DisplayName` 。

### 範例 4：取得特定的管理群組和階層的所有層級
```
