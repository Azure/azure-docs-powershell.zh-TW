...
在資源組 DS-TestRG 中，以名稱 MyDeploymentScript 來記錄部署腳本。
...
在資源組 DS-TestRG 中，使用名稱為 MyDeploymentScript 的部署腳本記錄的最後 3 行。
...
第一個命令會獲得資源群組 DS-TestRG 中名稱為 MyDeploymentScript 的部署腳本。
第二個命令會獲得給定部署腳本的記錄。
