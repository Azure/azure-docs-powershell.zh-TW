...
## 簡介
Get-AzPolicyAlias會取及輸出已定義別名且符合指定參數值的 Azure 提供者資源類型。 如果沒有提供參數，將會輸出包含別名的所有提供者資源類型。
-ListAvailable 切換器會列出所有符合的資源類型，包括沒有別名的資源類型，以修改此行為。
...
## 描述
**Get-AzPolicyAlias Cmdlet** 會取得一個策略別名清單。
Azure Policy 會使用策略別名來參照資源類型屬性。
提供的參數可比對資源類型或別名的各種屬性，以限制清單中專案。
如果目標字串包含目標字串，使用不區分大小寫的比較，指定相符值會相符。
...
列出命名空間符合 'compute' 且包含別名的所有提供者資源類型。
...
列出資源類型符合'虛擬'的所有提供者資源類型，包括沒有別名的提供者資源類型。
...
列出命名空間符合 'compute' 的所有提供者資源類型，而資源類型符合'虛擬'且包含別名。
注意：-命名空間Match 和 -ResourceTypeMatch 提供獨佔比對，而其他則包含。
...
列出所有包含別名符合'虛擬'或路徑符合'網路'的別名的提供者資源類型。
...
列出具有 Alpha api 版本或包含 Alpha api 版本別名的所有提供者資源類型。
...
### -ApiVersion
設定時，表示要使用資源提供者 API 的版本。 如果未指定，API 版本會自動判斷為最新可用。
...
* 若要展開 Aliases 或其他任何屬性，請將輸出移至 `select -ExpandProperty <property>` 。 例如： `Get-AzPolicyAlias -NamespaceMatch 'Microsoft.Cache' -ApiVersionMatch 'alpha' | select -ExpandProperty Aliases | select -Property Name -ExpandProperty Paths`

* 其他屬性可在輸出中使用，並可以輸出到的管道顯示 `Format-List` 。 例如： `Get-AzPolicyAlias -NamespaceMatch 'Web' -ResourceTypeMatch site -PathMatch cert | Format-List`
