...
## 描述
**Set-AzPolicyDefinition** Cmdlet 會修改一個策略定義。
...
該命令會儲存該物件在$PolicyDefinition變數中。
第二個命令會更新由 **resourceId** 屬性識別之 $PolicyDefinition。
...
此命令會使用 Set-AzPolicyDefinition Cmdlet 將 mode 屬性設為 'All'，來更新名為 VMPolicyDefinition 的策略定義。
...
此命令會更新名為 VMPolicyDefinition 之策略定義的中繼資料，指出其類別為「虛擬機器」。
...
### -Parameter
原則定義的參數宣告。 這可以是檔案名或包含參數宣告的 uri 路徑，或參數宣告為字串。
...
### -策略
指定原則定義的新原則規則。
您可以使用 JSON 格式的 JavaScript 物件標記法指定 .json 檔案或包含 (字串) 路徑。
