...
## 描述
**New-AzManagementGroup** Cmdlet 會建立管理群組。
...
建立新群組，並且 `DisplayName` `ParentId` 設定為 `null` 。 `DisplayName`與群組的父項相同，且 `GroupName` 其父項為租使用者。  
...
在此案例中，群組的父項即為租使用者， `DisplayName` 且會設定為所提供值。

### 範例 3：建立具有父項和顯示名稱的管理群組
```
