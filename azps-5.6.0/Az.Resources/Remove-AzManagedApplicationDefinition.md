...
## 描述
**Remove-AzManagedApplicationDefinition** Cmdlet 會移除受管理的應用程式定義

## 例子

### 範例 1：根據資源識別碼移除受管理的應用程式定義
```
...
第一個命令會使用 Get-AzManagedApplicationDefinition Cmdlet，獲得名為 myAppDef 的受管理應用程式定義。
命令會儲存在$ApplicationDefinition變數中。
第二個命令會移除由 **resourceId** 屬性識別的受管理應用程式定義$ApplicationDefinition。
