...
## 描述
**Save-AzTenantDeploymentTemdlet** 會將部署範本儲存至 JSON 檔案，以用於租使用者範圍中的部署。
...
此命令會從 TestDeployment 中獲得部署範本，並會將它另存為目前目錄中的 JSON 檔案。
...
此命令會獲得目前租使用者範圍中的部署「RolesDeployment」，並保存其範本。
