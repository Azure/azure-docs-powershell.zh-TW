...
## 描述
**Invoke-AzResourceAction** Cmdlet 會針對指定的 Azure 資源來調用動作。
若要取得支援的動作清單，請使用 Azure 資源管理器工具。

## 例子

### 範例 1：使用 ResourceId 啟動 VM
...
此命令會以 {ResourceId} 啟動虛擬機器。

### 範例 2：使用 ResourceName 啟動 VM
...
此命令會停止使用 {ResourceId} 的虛擬機器。
命令會指定 *Force* 參數，因此不會提示您確認。

### 範例 3：使用 ResourceId 來要求註冊資源提供者
...
### -參數
指定參數 #A0 為雜湊表格 #C1，用於此 Cmdlet 所調用的動作。
...
### -ResourceId
指定此 Cmdlet 會以該 Cmdlet 所調用之資源之完整資源識別碼。
