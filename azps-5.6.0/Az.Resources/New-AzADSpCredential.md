...
## 描述
您可以使用 New-AzADSpCredential Cmdlet 來新增認證或匯總服務主體的認證。
服務主體是提供物件識別碼或服務主體名稱來識別。

## 例子

### 範例 1：使用產生的密碼建立新服務主體認證
...
新的密碼認證會新增到現有服務主體，物件識別碼為 '1f99cf81-0146-4f4e-beae-2007d0668476'。
...
提供的 base64 編碼公用 X509 憑證 ("myapp.cer") 會使用 SPN 新增到現有的服務主體。
...
使用物件識別碼 '1f99cf81-0146-4f4e-beae-2007d0668476' 和服務管道到 New-AzADSpCredential，以產生密碼為該服務主體建立新服務主體認證的服務主體。
...
認證使用量的有效結束日期。
預設結束日期為自今天起一年。
對於「非對稱」類型認證，這必須設定為 X509 憑證有效日期當天或之前。
...
認證使用量的有效開始日期。
預設的開始日期值是今天。
對於「非對稱」類型認證，這必須設定為 X509 憑證的有效日期當天或之後。
...
### Microsoft.Azure.Commands.Resources.models.Authorization.PSADCredential方apper
