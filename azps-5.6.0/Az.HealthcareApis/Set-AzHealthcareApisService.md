...
### 範例 1：使用db OfferThroughput = 500 更新資源群組 MyResourceGroup 中名為 MyService 的現有醫療保健apis 服務。
...
### 範例 2：更新資源群組 MyResourceGroup 中名為 MyService 的現有醫療保健 apis 服務，其為HTTPs:// \<my-keyvault> \<my-key> .vault.azure.net/keys/"
...
指定可存取此 API 的來源網站的 URL，或使用 " * " 允許從任何網站存取。
...
### -外星KeyVaultKeyUri
HealthcareApis Fhir ServiceAultsKeyVaultKeyUri.
支援資料庫之客戶管理金鑰的 URI。
...
### -一些高能專案
HealthcareApis FhirServiceServiceServiceOfferThroughput.
...
HealthcareApis FhirService CorsCredentials Not alloweds.
...
### -InputObject
來自 Get-AzHealthcareApisFhirService 的 HealthcareApis fhirService 服務。
...
### -PublicNetworkAccess
Fhir 服務的網路存取類型。 一般 `Enabled` 或 `Disabled` .
...
### -標記
HealthcareApis Fhir Service 帳戶標記。
