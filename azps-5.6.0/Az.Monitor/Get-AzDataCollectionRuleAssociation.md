...
## 描述
**Get-AzDataCollectionRuleAssociation** Cmdlet 會取得 DCRA (一) 。
...
### 範例 1：根據目標資源識別碼取得資料收集規則關聯 (關聯的虛擬機器) 
```
...
此命令會列出虛擬機器或虛擬機器中之目標資源識別碼 (的所有) 。

### 範例 2：依規則關聯資料集合規則關聯 (DCR) 
```
...
此命令會列出給定資源群組的資料收集規則關聯，以及 DCR (規則) 。

### 範例 3：根據 PSDataCollectionRuleResource (輸入物件取得資料收集規則關聯) 
```
...
此命令會列出給定輸入物件的資料收集規則關聯。

### 範例 4：依目標資源識別碼取得資料收集規則關聯 (相關聯的虛擬機器) 關聯名稱
```
