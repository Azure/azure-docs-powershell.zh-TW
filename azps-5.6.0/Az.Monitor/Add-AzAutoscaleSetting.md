...
## 描述
**Add-AzAutoscaleSetting** Cmdlet 會建立自動縮放設定。
...
前兩個命令使用 [New-AzAutoscaleRule](https://docs.microsoft.com/powershell/module/az.monitor/new-azautoscalerule) 建立兩個自動縮放規則，$Rule 1 和 $Rule 2。
第三個和第四個命令使用 [New-AzAutoscaleProfile，](https://docs.microsoft.com/powershell/module/az.monitor/new-azautoscaleprofile) 使用 $Profile 1 和 $Profile 2 建立自動縮放設定檔$Rule 1 和 $Rule 2。
最後一個命令會使用 $Profile 1 和 $Profile 2 中的設定檔建立自動縮放設定。

## 參數

### -AutoscaleProfile
指定要新增到自動縮放設定之配置檔案清單，或指定$Null設定檔的清單。
