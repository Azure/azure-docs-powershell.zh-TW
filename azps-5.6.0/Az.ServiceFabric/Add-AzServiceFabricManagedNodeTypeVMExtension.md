...
## 描述
新增 vm 擴充功能至節點類型。 這會將擴充功能新增到不足的虛擬機器縮放集資源。
...
### -Publisher
擴充處理常式發行者的名稱。
這可以使用 Get-AzVMImagePublisher Cmdlet 取得發行者。
...
您可以使用 Get-AzVMExtensionImageType Cmdlet 取得副檔名類型。
