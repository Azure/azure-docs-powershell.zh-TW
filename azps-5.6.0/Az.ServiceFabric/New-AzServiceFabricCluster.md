...
**New-AzServiceFabricCluster 命令** 會使用您提供的憑證，或系統產生的自我簽署憑證來設定新的服務結構叢集。 使用的範本可以是預設範本或您提供的自訂範本。 您可以選擇指定資料夾來匯出自我簽署憑證，或稍後從金鑰庫抓取。
如果您要指定自訂範本和參數檔案，則不需要在參數檔案中提供憑證資訊，系統會填入這些參數。
以下詳述四個選項。 向下卷起以尋找每個參數的說明。

## 例子

### 範例 1：只指定組大小、認證主體名稱，以及部署該組組的作業系統
...
此命令只會指定要部署該組集的組大小、認證主體名稱，以及作業系統。

### 範例 2：在金鑰庫中指定現有的憑證資源，以及自訂範本以部署組
...
此命令會指定金鑰庫中的現有憑證資源，以及部署組狀圖的自訂範本。

### 範例 3：使用自訂範本建立新組。 為金鑰庫指定不同的資源組名，然後讓系統上傳新的憑證至它
...
此命令會使用自訂範本建立新組。 為金鑰庫指定不同的資源組名，然後讓系統上傳新的憑證至它

### 範例 4：攜帶您自己的憑證和自訂範本，然後建立新組
...
現有的 Azure 金鑰庫密碼 URL，例如 https://mykv.vault.azure.net:443/secrets/mysecrets/55ec7c4dc61a462bbc645ffc9b4b225f '
...
### -拇指列印
憑證的指紋會套用至 SecretIdentifier。 如果憑證未管理，因為金鑰保存庫只會將憑證儲存為機密，且 Cmdlet 無法重新處理指紋，請使用這個方法。
