...
## 簡介
從組群移除服務結構應用程式類型。 這會移除此資源下的所有類型版本。 僅支援 ARM 部署的應用程式類型。
...
## 描述
此 Cmdlet 會移除該組集中的應用程式類型，並移除此資源下的所有類型版本，但應先移除其下的所有應用程式，再執行此命令。
...
此範例會移除應用程式類型 "testAppType" 及其下的所有版本。 如果有使用此類型建立的任何應用程式，該命令會引發例外。
