...
使用 5 個節點建立主節點類型，並指定位置屬性、容量、應用程式和暫時埠。
...
### -容量
將節點類型中的節點所使用容量標記當做金鑰/值組時，集區資源管理員會使用這些標記來瞭解節點有多少資源。
更新此選項會優先于目前的值。
...
### -PlacementProperty
在節點類型中，將節點所使用的位置標記當做金鑰/值組，可用來指出應在何處執行特定服務 (工作負載) 位置。
更新此選項會優先于目前的值。
...
### -VmImageOffer
Azure 虛擬機器 Marketplace 映射的優惠類型。
預設值：WindowsServer。
