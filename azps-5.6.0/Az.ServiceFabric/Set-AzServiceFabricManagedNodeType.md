...
## 描述
使用 -Reimage 參數設定節點類型資源屬性，或在節點類型的特定數個數上執行重新映射動作。 重新映射作業時，服務布節點會先停用，然後再重新為 vm 重新製作成動畫，並再次啟用它們一旦恢復使用。 如果是在主要節點類型上完成此工作，可能需要一些時間，因為可能不會同時重新製作所有節點的映射。 即使服務結構無法停用節點，使用 -ForceReimage 還是會強制執行作業，但請小心使用，因為如果節點上執行有狀態的工作量，可能會導致資料遺失。
...
更新節點類型的位置適當位置。 這將會覆寫較舊的放置適當位置 ，如果有。
...
### -容量
將節點類型中的節點所使用容量標記當做金鑰/值組時，集區資源管理員會使用這些標記來瞭解節點有多少資源。 更新此選項會優先于目前的值。
...
### -ForceReimage
使用此旗標會強制移除，即使服務結構無法停用節點。
請小心使用，如果節點上執行狀態工作負載，可能會導致資料遺失。
...
### -PlacementProperty
在節點類型中，將節點所使用的位置標記當做金鑰/值組，可用來指出應在何處執行特定服務 (工作負載) 位置。 更新此選項會優先于目前的值。
