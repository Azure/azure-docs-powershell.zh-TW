...
## 描述
此 Cmdlet 允許根據指定的應用程式建立程式設計或狀態服務。 服務應該會從應用程式清單中退出，而且類型應該與清單中的類型相同。 應用程式名稱應該是服務名稱的首碼。
...
此範例將在所有節點上建立實例計數 -1 的新服務「testApp~testService1」， (節點上) 。
...
此範例會建立一個新的狀態服務"testApp~testService2"，目標為 5 個節點。
...
### -DefaultMoveCost
指定移動的預設成本。
成本越高，當嘗試在組群之間取得平衡時，組組資源管理員移動複本的可能性較小
...
表示服務使用已命名的分區分配。
使用此模型的服務通常會有一些資料可以在一組界限內進行容器處理。
一些常用資料欄位做為命名的分割鍵，例如區域、郵遞區號、客戶群組或其他商務邊界。
...
表示服務使用 UniformInt64 分割方案。
這表示每個磁碟分割區都有一系列 int64 鍵。
