...
## 簡介
建立一個新的管理中心DB IndexingPolicy 物件。
...
## 描述
**New-AzCosmosDBGremlinIndexingPolidlet** 會建立 PSIndexingPolicy 類型的新物件。
...
### -CompositePath
類型為 Microsoft.Azure.Commands.AzsDB.PSCompositePath 的物件陣列
...
### -ExcludedPath
包含排除Path 的字串陣列 (指定 JSON 檔內要排除在 Azure 一些 Db service.) 元素中的路徑。
...
### -IncludedPath
包含 includedPath 的字串陣列 (指定要包含在 AzureAzings DB service.) 元素中的 JSON 檔路徑。
...
### -空間分析
類型為 Microsoft.Azure.Commands.AzsDB.PSSpatialSpec 的物件陣列
