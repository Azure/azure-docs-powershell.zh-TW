...
## 描述
**Get-AzNotificationHubPNSCredential Cmdlet** 會取得通知中樞的平臺 (PNS) 認證。
每個通知中樞都有一組 PNS 認證。
這些認證會適用于個別推入通知服務，例如，但不限於;iOS 推入通知服務、Android 推入通知服務和 Windows Phone 8。

## 例子

### 範例 1：取得特定通知中樞的 PNS 認證
```
...
此命令會為名稱為 ContosoInternalHub 的通知中樞獲得 PNS 認證，該通知中樞屬於名為 ContosoNotificationsGroup 的資源群組。
