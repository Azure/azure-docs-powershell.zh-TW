...
## 描述
**Remove-AzNotificationHub** Cmdlet 會移除現有的通知中樞。
通知中樞可用來傳送推入通知給多個用戶端，無論這些用戶端使用哪個平臺。
平臺包括但不限於：iOS、Android、Windows Phone 8 和 Windows Store。
通知中樞大致相當於個別的應用程式：每一個 App 通常會有自己的通知中樞。
您可以使用 **Remove-AzNotificationHub** Cmdlet 移除現有的通知中樞。
移除中樞後，您無法再使用該中樞來傳送推入通知給使用者。
...
此命令會移除名為 ContosoInternalHub 的通知中樞。
若要移除中樞，您必須指定中樞所在的命名空間，以及中樞指派給的資源群組。
