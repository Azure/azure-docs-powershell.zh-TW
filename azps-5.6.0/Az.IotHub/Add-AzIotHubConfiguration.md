...
## 描述
組組內容為 json，並且會依據裝置或模組意圖而稍有不同。 裝置組配置的形式為 {"deviceContent"：{...}}
模組組式是 {"moduleContent"：{...}}
您可使用使用者提供的隨需評估度量來定義群組原則。
使用者計量為 json 且為 {"queries"：{...}} 或 {"metrics"：{"queries"：{...}}}。

注意：模組的目標條件必須以「從 devices.modules where」開始。 請參閱 https://docs.microsoft.com/azure/iot-hub/iot-hub-automatic-device-management 詳細資訊。
...
建立優先順序為 3 的裝置組態，此為在建築物 9 中標記裝置且環境為'測試'的條件。
