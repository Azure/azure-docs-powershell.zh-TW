...
### 範例 2：取得 BotService by ResourceGroupName 和 Name
```powershell
...
按 ResourceGroupName 和 Name 取得 BotService

### 範例 3：使用 ResourceGroupName 取得所有 BotServices
```powershell
...
根據 ResourceGroupName 取得所有 BotServices

### 範例 4：取得 BotService by inputObject
```powershell
...
INPUTOBJECT： <IBotServiceIdentity> 身分識別參數
  - `[ChannelName <ChannelName?>]`：通道資源的名稱。
  - `[ConnectionName <String>]`：Bot 服務連接設定資源的名稱
  - `[Id <String>]`：資源識別路徑
