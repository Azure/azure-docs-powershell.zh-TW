...
### -InputObject
指定需要啟動測試移移清理的複製伺服器。
伺服器物件可以使用 Cmdlet Get-AzMigrateServerReplication結構來取取，請參閱 INPUTOBJECT 屬性的 NOTES 區段，並建立雜湊表。
...
INPUTOBJECT：指定需要啟動測試移移清理的複製 <IMigrationItem> 伺服器。 伺服器物件可以使用 Cmdlet Get-AzMigrateServerReplication取
