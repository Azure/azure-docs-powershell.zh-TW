...
### -DiskToInclude
指定要包含在來源伺服器上用於複本的磁片。
若要建構，請參閱 DISKTOINCLUDE 屬性的附注區段，然後建立雜湊表。
...
### -InputObject
指定要移移的已發現伺服器。
伺服器物件可以使用 Cmdlet Get-AzMigrateServer取。
...
指定要用於 VM 建立的可用性集S指定要用於建立 VM 的可用性集。
...
### -TargetNetworkId
指定伺服器需要移移的目標 Azure 訂閱內的虛擬網路識別碼。
...
### -TargetSubnetName
指定目的地 Virtual Netowk 中伺服器需要移移的子網名稱。
...
DISKTOINCLUDE <IVCbt一文[]>：指定要包含在來源伺服器上用於複本的磁片。
  - `DiskId <String>`：磁片識別碼。
  - `IsOSDisk <String>`：指出磁片是否為作業系統磁片的值。
  - `LogStorageAccountId <String>`：記錄儲存空間帳戶 ARM 識別碼。
  - `LogStorageAccountSasSecretName <String>`：記錄儲存帳戶的金鑰保存庫密碼。
  - `[DiskEncryptionSetId <String>]`：DiskEncryptionSet ARM 識別碼。
  - `[DiskType <DiskAccountType?>]`：磁片類型。

INPUTOBJECT： <IVMwareMachine> 指定要移移的已發現伺服器。 伺服器物件可以使用 Cmdlet Get-AzMigrateServer取。
  - `[GuestOSDetailOstype <String>]`：作業系統類型。
