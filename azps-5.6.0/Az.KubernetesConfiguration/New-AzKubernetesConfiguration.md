...
### 範例 1：建立 kubernetes cluster 的組式
```powershell
...
此命令會建立 kubernetes cluster 的組式。

### 範例 1：使用指定 paramter OperatorNamespace 為 kubernetes cluster 建立組式
```powershell
...
此命令會針對 kubernetes cluster 的新運算子命名空間建立一個組式。
請注意，無法在現有的運算子命名空間中建立組組。
