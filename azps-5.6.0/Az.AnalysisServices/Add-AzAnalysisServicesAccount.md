...
## 描述
Cmdlet Add-AzAnalysisServicesAccount用來登入 Azure Analysis Services 伺服器的實例
...
此範例會新增由 $UserCredential 變數指定的帳號westcentralus.asazure.windows.net Analysis Services 環境。
...
第一個命令會獲得應用程式服務主體認證，然後將這些認證儲存在$ApplicationCredential變數中。
第二個命令會新增由 $ApplicationCredential 變數和 TenantId 指定的應用程式服務主體帳戶westcentralus.asazure.windows.net Analysis Services 環境。
...
此範例會新增 ApplicationId、TenantId 和 CertificateThumbprint 指定的應用程式服務主體帳戶至 westcentralus.asazure.windows.net Analysis Services 環境。
...
### -RolloutEnvironment
要登入的 Azure Analysis Services 環境名稱。 假設伺服器的完整名稱 ，例如 asazure://westcentralus.asazure.windows.net/testserver，此變數的正確值將會westcentralus.asazure.windows.net
...
### Microsoft.Azure.Commands.AnalysisServices.Dataservices.Dataservice.AsAzureProfile
