...
### 範例 1：建立 AzDigitalTwinsEndpoint for Eventhub
```powershell
...
使用 connectionStringPrimaryKey 建立 AzDigitalTwinsEndpoint for Eventhub

### 範例 2：建立 AzDigitalTwinsEndpoint for EventGrid
```powershell
...
使用 TopicEndpoint 和 accessKey1 建立 AzDigitalTwinsEndpoint for Eventhub

### 範例 3：建立 AzDigitalTwinsEndpoint for ServiceBus
```powershell
...
建立 AzDigitalTwinsEndpoint for ServicBus by PrimaryConnectionString
...
若要建構，請參閱 ENDPOINTDESCRIPTION 屬性的附注區段，並建立雜湊表。
...
ENDPOINTDESCRIPTION <IDigitalTwinsEndpointResource> ：DigitalTwinsInstance 端點資源。
  - `EndpointType <EndpointType>`：數位星型端點的類型
  - `[DeadLetterSecret <String>]`：無信儲存空間的秘訣。 在閱讀期間會混淆。
