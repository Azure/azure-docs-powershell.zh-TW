...
建立具有提供者和對等位置的對等服務物件。 用於與 `Get-AzPeeringServiceProvider``Get-AzPeeringServiceLocation`
...
### -對等位置
與 Azure 區域不同的實體位置。 使用 Get-AzPeeringServiceLocation [-Country <country> ]
...
對等服務提供者名稱。
將Get-AzPeeringServiceProvider Cmdlet 用於清單
...
### -標記
與 Microsoft InputObject Service 關聯的標記。
