...
## 描述
為訂閱建立 ARM 對等。 請參閱 [New-AzPeeringDirectConnectionObject](https://docs.microsoft.com/powershell/module/az.peering/new-azpeeringdirectconnectionobject) 或 [New-AzPeeringExchangeConnectionObject，](https://docs.microsoft.com/powershell/module/az.peering/new-azpeeringexchangeconnectionobject) 以瞭解有關建立連線物件的資訊。
...
使用 PeerAsn 65000 在西雅圖工廠建立單一連接的新直接對等
...
### -DirectConnection
使用命令的管道和New-AzExchangePeeringConnection來建立新的直接連接。
...
### -ExchangeConnection
使用這個命令的New-AzExchangePeeringConnection和管道來建立新的 Exchange 連結。
...
### -PeerAsnResourceId
對等 Asn 資源識別碼。Get-AzPeerAsn來取回識別碼。
...
### -對等位置
與 Azure 區域不同的實體位置。
使用 Get-AzPeeringLocation \<kind\> -Kind 使用城市名稱做為金鑰。
...
### -SKU
選取Basic_Direct_Free或Premium_Direct_Free，除非明確要求選取另一個選項。
