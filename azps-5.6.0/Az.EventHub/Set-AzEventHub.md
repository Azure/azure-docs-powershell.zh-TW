...
## 描述
Cmdlet Set-AzEventHub更新指定事件中樞的屬性。

## 例子

### 範例 1
若要使用捕獲描述屬性更新 Eventhub，請遵循下列步驟。 
...
更新 \` MyEventHubHub 物件所代表的 \` 事件中樞 \` MyEventHubName， \` 設定 CaptureDescription 屬性
...
更新 \` \` \` MyCreaedEventHub 物件所代表的事件中樞 MyEventHubName，將郵件保留期間設定為 4 天，以及磁碟分割數目設定 \` 為 2。
