...
## 描述
Cmdlet Remove-AzEventHubConsumerGroup會從指定的事件中樞移除並刪除指定的消費者群組。
...
從事件中樞 \` MyEventHubName 刪除消費者群組 MyConsumerGroupName ，此名稱範圍為 \` \` \` \` MyNamespaceName \` 命名空間。
