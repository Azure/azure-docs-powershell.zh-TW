...
## 描述
Cmdlet New-AzEventHubAuthorizationRule會建立新的事件中樞授權規則。
...
使用資源群組 \` \` \` \` \` MyResourceGroupName 建立授權規則 MyAuthRuleName，授予命名空間 MyNamespaceName 的聆聽和傳送許可權 \` 。
...
使用資源群組 \` \` \` \` \` \` MyResourceGroupName，在命名空間 MyNamespaceName 中建立授權規則 MyAuthRuleName，將聆聽和傳送許可權授予事件中樞 \` MyEventHubName。 \`
