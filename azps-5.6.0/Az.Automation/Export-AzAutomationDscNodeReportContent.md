...
## 簡介
將從 DSC 節點所送出之 DSC 報表的原始內容匯出至自動化。
...
## 描述
**Export-AzAutomationDscNodeReportContent** Cmdlet 會匯出 APS 想要狀態組態 (DSC) 報表的原始內容。
DSC 節點會傳送 DSC 報表至 Azure Automation。
...
這組命令會從名為 Computer14 的 DSC 節點匯出最新的報表至桌面。

## 參數

### -AutomationAccountName
指定自動化帳戶的名稱。
此 Cmdlet 會針對屬於此參數指定的自動化帳戶的 DSC 節點匯出報表內容。
...
### -強制
表示此 Cmdlet 會以名稱相同的新檔案取代現有的本地檔案。
...
### -NodeId
指定此 Cmdlet 匯出報表內容之 DSC 節點的唯一識別碼。
...
### -ResourceGroupName
指定資源組的名稱。
此 Cmdlet 會匯出屬於此 Cmdlet 指定之資源群組之 DSC 節點之報表的內容。
