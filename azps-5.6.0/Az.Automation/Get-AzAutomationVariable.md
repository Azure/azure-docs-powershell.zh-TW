...
## 描述
**Get-AzAutomationVariable** Cmdlet 會取得一或多個 Azure 自動化變數。
若要取得特定變數，請指定其名稱。
...
第一個命令在名為 Contoso17 的帳戶中，會獲得一個名為 Variable06 的自動化變數。
該命令會儲存該物件在$Variable變數中。
第二個命令使用標準點標記法來參照 **$Variable。**
命令會儲存值$value變數。

## 參數

### -AutomationAccountName
指定包含此 Cmdlet 所獲得變數的自動化帳戶名稱。
