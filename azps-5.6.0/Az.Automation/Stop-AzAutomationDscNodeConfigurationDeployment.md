...
## 簡介
在自動化中停止 DSC 節點組組部署。 它只會停止目前的部署工作，但是不會取消指派已指派的節點群組原則。
...
## 描述
**Stop-AzAutomationDscNodeConfigurationDeployment** Cmdlet 會停止 Azure Automation 中所需狀態組態 (DSC) 節點組態的部署。 它會停止指派節點組至節點群組的節點組 ，如果有剩餘部分要指派，但是不會取消指派已指派的節點。 若要取消註冊已排程的工作，請使用 [Ungister-AzAutomationScheduledRunbook](./Unregister-AzAutomationScheduledRunbook.md) 與 JobScheduleId 來取消分配現有的已排程工作。

## 例子

### 範例 1：在自動化中部署 Azure DSC 節點組
```
...
上述命令會停止傳遞 jobId 的 DSC 節點組配置部署工作。

## 參數

### -AutomationAccountName
指定包含此 Cmdlet 編譯之 DSC 配置的自動化帳戶名稱
