...
## 描述
**New-AzAutomationVariable** Cmdlet 在 Azure Automation 中建立變數。
若要加密變數，請指定 *Encrypted* 參數。
您無法在建立後修改變數的加密狀態。
...
此命令會建立一個名為 StringVariable22 的變數，其字串值在自動化帳戶中名為 Contoso17。
...
第一個命令會使用 Get-AzVM Cmdlet。
命令會儲存在$VirtualMachine變數中。
第二個命令在自動化帳戶中建立名為 Contoso17 的 ComplexVariable01 變數。
此命令會使用複雜的物件來表示其值，在此例中為 $VirtualMachine。
