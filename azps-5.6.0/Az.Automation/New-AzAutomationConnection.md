...
## 描述
**New-AzAutomationConnection** Cmdlet 在 Azure Automation 中建立一個連接。

## 例子

### 範例 1：建立 ConnectionTypeName=Azure 的連接
```
PS C:\> $FieldValues = @{"AutomationCertificateName"="ContosoCertificate";"SubscriptionID"="81b59010-dc55-45b7-89cd-5ca26db62472"}
PS C:\> New-AzAutomationConnection -Name "Connection12" -ConnectionTypeName Azure -ConnectionFieldValues $FieldValues -ResourceGroupName "ResourceGroup01" -AutomationAccountName "AutomationAccount01"
```

第一個命令會指派域值的雜湊表給$FieldValue變數。
第二個命令在自動化帳戶名為 AutomationAccount01 中建立名為 Connection12 的 Azure 連接。
命令使用 $FieldValues 中的連接域$FieldValues。

### 範例 2：建立 ConnectionTypeName=AzureServicePrincipal 的連接
```
...
該命令使用 $RunAsAccountConnectionFieldValues 和 ConnectionTypeName=AzureServicePrincipal，在自動化帳戶中建立名為 Connection13 的 Azure 連接，名為 AutomationAccount01。
此 ConnectionTypeName=AzureServicePrincipal 主要用於 Azure Run As Account。

### 範例 3：建立 ConnectionTypeName=AzureClassicCertificate 的連接
```
...
該命令使用 $ClassicRunAsAccountConnectionFieldValues 和 ConnectionTypeName=AzureClassicCertificate，在自動化帳戶中建立名為 Connection14 的 Azure 連接，名為 AutomationAccount01。
此 ConnectionTypeName=AzureClassicCertificate 主要用於 Azure 傳統執行 As 帳戶。

## 參數

### -AutomationAccountName
指定此 Cmdlet 建立連接的自動化帳戶名稱。
