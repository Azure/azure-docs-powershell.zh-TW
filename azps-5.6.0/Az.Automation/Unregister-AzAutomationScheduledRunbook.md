...
## 描述
取消 **註冊-AzAutomationScheduledRunbook** Cmdlet 會移除 Azure 自動化執行手冊和排程之間的關聯。
排程不會再啟動執行簿。

## 例子

### 範例 1：移除 runbook 與排程之間的關聯
```
...
此命令會移除名為 Runbk01 的 Runbook 與名為 Runbk01Sched 的排程之間的關聯。
