...
## 描述
**Remove-AzAutomationWebtomation Cmdlet** 會從 Azure 自動化執行手冊移除 Webaz。
網頁設計會被刪除。
...
此命令會移除自動化帳戶中名為 AutomationAccount01 的 Web一個名為 Web一11 的網頁。
命令會指定 *Force* 參數。
因此，它不會提示您確認。
