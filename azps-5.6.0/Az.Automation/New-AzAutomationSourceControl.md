...
## 描述
此New-AzAutomationSourceControl Cmdlet 會建立一個組塊，將 Azure 自動化帳戶與 VSTS TFVC、VSTS Git 或 GitHub 連結。

## 例子

### 範例 1
建立原始程式碼管理組，將 Azure 自動化帳戶與 VSTS TFVC 專案連結。 TFVC 專案沒有分支，因此未指定分支參數。
...
### 範例 2
建立原始程式碼管理組，將 Azure 自動化帳戶與 VSTS Git 專案連結。
...
### 範例 3
建立原始程式碼管理組配置，將 Azure 自動化帳戶與 GitHub 專案連結。
...
### -DoNotPublishRunbook
原始程式碼的 publishRunbook 屬性。
如果 DoNotPublishRunbook 已設定，這表示使用者 runbooks 會以'草稿'方式輸入。
