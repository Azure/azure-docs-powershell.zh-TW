...
## 簡介
獲得 Azure Automation 原始程式碼管理同步作業。
...
## 描述
Cmdlet Get-AzAutomationSourceControlSyncJob Azure Automation 原始程式碼管理同步作業。 若要取得特定的原始程式碼管理同步作業，請指定其識別碼。

## 例子

### 範例 1
此命令會獲得原始程式碼管理 VSTSNative 的所有自動化原始程式碼同步作業。
...
### 範例 2
此命令會針對原始程式碼管理 VSTSNative 使用 id 08d6d266-27b6-463c-beea-bc48a67ace15 執行原始程式碼同步處理工作。 
...
獲得 Azure Automation 原始程式碼管理同步作業。  (自動) 
