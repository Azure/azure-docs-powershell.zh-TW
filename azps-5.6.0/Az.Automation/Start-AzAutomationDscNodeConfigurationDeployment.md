...
## 描述
**Start-AzAutomationDscNodeConfigurationDeployment** Cmdlet 在 Azure Automation 中部署想要的狀態組態 (DSC) 節點組態。
...
上述命令將名為「Config01.Node1」的 DSC 節點組式部署至指定的二維節點名稱陣列。 部署會以階段的方式執行。

### 範例 2：在自動化中排程 Azure DSC 節點組組部署
```
...
上述命令會排程將名為 "Config01.Node1" 的 DSC 節點組式部署排程至指定的二維節點名稱陣列。 部署會以階段的方式執行，並且會根據排程執行。
