...
## 描述
Cmdlet Import-AzContext載入來自檔案的驗證資訊，以設定 Azure 環境和上下文。
您目前會話執行 Cmdlet 會使用此資訊向 Azure Resource Manager 驗證要求。

## 例子

### 範例 1：從 AzureRmProfile 中輸入內容
```
...
此範例會從傳遞至 Cmdlet 的 PSAzureProfile 中，將內容導入。
...
此範例會從 JSON 檔案選取傳遞至 Cmdlet 的內容。 此 JSON 檔案可以從 Save-AzCoNtext 建立。
