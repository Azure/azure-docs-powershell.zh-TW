...
## 描述
此Set-AzContext Cmdlet 會針對您目前會話執行 Cmdlet 設定驗證資訊。
內容包括租使用者、訂閱及環境資訊。
...
### -訂閱
內容應設定為訂閱的名稱或識別碼。 此參數具有 -SubscriptionName 和 -SubscriptionId 的別名，因此為了清楚起見，您可以分別在指定名稱和識別碼時，使用其中一個來取代 -Subscription。
