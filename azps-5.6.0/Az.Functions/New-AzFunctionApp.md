...
### 範例 1：在中美國建立消費 PowerShell 函數應用程式。
```powershell
...
此命令會在美國中央建立一個消費 PowerShell 函數應用程式。

### 範例 2：建立將在服務方案中託管的 PowerShell 函數應用程式。
```powershell
PS C:\> New-AzFunctionApp -Name MyUniqueFunctionAppName `
                          -ResourceGroupName MyResourceGroupName `
                          -PlanName MyPlanName `
...
此命令會建立一個 PowerShell 函數應用程式，它將託管在服務方案中。

### 範例 3：使用私人 ACR 影像建立函數應用程式。
```powershell
PS C:\> New-AzFunctionApp -Name MyUniqueFunctionAppName `
                          -ResourceGroupName MyResourceGroupName `
                          -PlanName MyPlanName `
...
### -DockerImageName
僅適用于 Linux。
來自 Docker Registry 的容器圖像名稱，例如 publisher/image-name：tag。
