...
INPUTOBJECT： <ISite> 
  ...
  - `[CloningInfoTrafficManagerProfileId <String>]`：要使用流量管理員設定檔的 ARM 資源識別碼 ，如果有。 流量管理員資源識別碼是表單 /subscriptions/{subId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Network/trafficManagerProfiles/{profileName}。
  ...
  - `[Config <ISiteConfig>]`：應用程式的組配置。
    ...
    - `[DetailedErrorLoggingEnabled <Boolean?>]`： <code>true</code> 如果已啟用詳細的錯誤記錄，否則為 <code>false</code> 。
    ...
    - `[ExperimentRampUpRule <IRampUpRule[]>]`：快速上線規則清單。
      ...
      - `[MinReroutePercentage <Double?>]`：指定 ReroutePercentage 會維持在上方的下限。
    ...
    - `[IPSecurityRestriction <IIPSecurityRestriction[]>]`：主要 IP 安全性限制。
      ...
      - `[Tag <IPFilterTag?>]`：定義此 IP 篩選的用用方式。 這是為了支援對代理程式進行 IP 篩選。
    ...
    - `[SlowRequestTimeTaken <String>]`：需要一些時間。
    ...
    - `[Use32BitWorkerProcess <Boolean?>]`： <code>true</code> 使用 32 位員工程式;否則 <code>false</code> ，。
