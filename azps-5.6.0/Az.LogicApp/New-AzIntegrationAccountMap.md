...
## 描述
**New-AzCountAccountMap** Cmdlet 會建立整合帳戶地圖。
此 Cmdlet 會返回代表整合帳戶地圖的物件。
指定整合帳戶名稱、資源組名、地圖名稱和地圖定義。
...
此命令會建立指定資源群組中的整合帳戶地圖。
命令會指定由上一個命令$MapContent變數中儲存的地圖定義。
...
### -MapFilePath
指定整合帳戶地圖定義之檔案路徑。 指定此參數或 *MapDefinition* 參數。
...
[Remove-Az有AccountMap](./Remove-AzIntegrationAccountMap.md)
