...
## 描述
**Get-AzAsseAccountAssembly** Cmdlet 在整合帳戶中建立新元件。
...
使用位於「$localAssemblyFilePath」中包含的檔案路徑的本地檔案來建立新元件。
...
使用 "$assemblyContent" 中包含的位元組陣列來建立新元件。
...
使用位於 URL "$assemblyUrl" 的位元組資料來建立新元件。 這是建立大型元件的建議方法。
