...
## 簡介
此 Cmdlet 會針對每一份協定及控制項編號值，來取回特定的接收交換控制項編號。
...
## 描述
此 Cmdlet 旨在用於災害復原案例，以驗證收到的交換控制項號碼是否存在，並且選擇性地使用 Remove-Az ScenarioAccountReceivedIcn 移除該實體。
...
此命令會以協定名稱和控制項編號值，獲得 X12 整合帳戶收到的交換控制編號。
...
此命令會以協定名稱和控制項編號值，獲得 Edifact 整合帳戶收到的交換控制編號。
