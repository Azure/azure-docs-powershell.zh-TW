...
**Get-AzLogicAppRunHistory** Cmdlet 會取得邏輯應用程式的執行歷程記錄。
此 Cmdlet 會返回 **WorkflowRun 物件** 的集合。
指定邏輯應用程式和資源群組。
此模組支援動態參數。
若要使用動態參數，請于命令中輸入。
...
此命令會獲得名為 LogicApp03 的邏輯應用程式的執行歷程記錄。
...
此命令會針對名為 LogicApp03 的邏輯應用程式執行特定的邏輯應用程式。
...
此命令會遵循 NextPageLink，獲得名為 LogicApp03 的邏輯應用程式完整執行歷程記錄。
...
此命令會遵循 NextPageLink，將結果大小限制為兩頁，以取得邏輯應用程式邏輯 App 的前兩頁執行歷程記錄。
每頁包含三十個結果。
