...
## 簡介
此 Cmdlet 會根據協定，從產生的交換控制項編號中，取回目前的值。
...
## 描述
此 Cmdlet 旨在用於災害復原案例，以取回產生的交換控制項編號目前的值，以便使用 Set-Az ScenarioAccountGeneratedIcn 回寫增加的值。
交換控制號碼應該增加，以避免在作用中地區發生災害時，無法複製到被動區域的數位重複的交換控制編號。
...
此命令會根據協定名稱，獲得整合帳戶產生的 X12 交換控制編號。 請確定指定的協定類型為 "X12"
...
此命令會以協定名稱獲得整合帳戶產生的 Edifact 交換控制編號。 請確定指定的協定類型為"Edifact"
...
此命令會根據整合帳戶名稱，獲得所有產生的 X12 交換控制項編號。
...
[Set-AzCountGeneratedIcn](./Set-AzIntegrationAccountGeneratedIcn.md)
