...
## 描述
**Get-Az有AccountBatchConfiguration** Cmdlet 會從整合帳戶取得批次組配置。
...
取得名為"sampleBatchConfig"的批次組組，該組組位於整合帳戶"sampleSourceAccount"中，該群組包含在資源群組"sampleResourceGroup"中。

### 範例 2：以參數列出整合帳戶中的所有批次設定
```powershell
...
取得位於整合帳戶 "sample準備Account"的所有批次組式組，此帳戶包含在資源群組"sampleResourceGroup"中。
