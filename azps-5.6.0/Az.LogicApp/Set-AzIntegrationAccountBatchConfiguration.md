...
## 描述
**Set-AzCountBatchConfiguration** Cmdlet 會修改整合帳戶批次設定。
...
使用位於 "$batchConfigurationFilePath" 中所包含的檔案路徑的本地檔案，修改名為 "sampleBatchConfig" 的批次組$batchConfigurationFilePath。

### 範例 2：使用 JSON 字串修改批次組配置
```powershell
...
使用 "$batchConfigurationContent" 中包含的 JSON 字串修改名為 "sampleBatchConfig" 的批次組$batchConfigurationContent。
...
手動提供所有必要的參數，以修改名為「sampleBatchConfig」的批次設定。
...
### Microsoft.Azure.Commands.LogicApp.models.PS方AccountBatchConfiguration
