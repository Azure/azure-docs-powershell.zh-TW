...
## 描述
**Get-Az一Account** Cmdlet 會從資源群組取得整合帳戶。 指定整合帳戶名稱和資源組名。
此模組支援動態參數。
若要使用動態參數，請于命令中輸入。
...
此命令會從指定的資源群組中，獲得一個名為 IntegrationAccount31 的整合帳戶。
...
此命令會從名為 ResourceGroup11 的資源群組中，獲得整合帳戶。
...
[Get-AzCountAccountCallbackUrl](./Get-AzIntegrationAccountCallbackUrl.md)
