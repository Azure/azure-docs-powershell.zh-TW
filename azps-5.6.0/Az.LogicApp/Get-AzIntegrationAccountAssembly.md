...
## 描述
**Get-AzAsseCountAssembly** Cmdlet 會從整合帳戶取得一個元件。
...
取得名稱為"sampleAssembly"的程式集，位於整合帳戶 "sampleAsseAccount" 中，此元件包含在資源群組"sampleResourceGroup"中。

### 範例 2：根據參數列出整合帳戶中的所有元件
```powershell
...
取得位於整合帳戶 "sample準備Account"的所有元件，此元件包含在資源群組"sampleResourceGroup"中。
