...
## 描述
**Get-Az有AccountBatchConfiguration** Cmdlet 會建立整合帳戶中的新批次組組。
...
使用位於「$batchConfigurationFilePath」中包含的檔案路徑的本地檔案，來建立新的批次組$batchConfigurationFilePath。

### 範例 2：使用 JSON 字串建立新批次組組
```powershell
...
使用 "$batchConfigurationContent" 中包含的 JSON 字串來建立新的批次組$batchConfigurationContent。
