...
## 描述
**New-AzEmentAccountAgreement** Cmdlet 會建立整合帳戶協定。
此 Cmdlet 會返回代表整合帳戶協定的物件。
指定整合帳戶名稱、資源組名、協定名稱、類型、合作夥伴名稱、合作夥伴限定詞及協定內容。
...
指定此參數或 *AgreementContentFilePath* 參數。
...
[Set-AzEmentAccountAgreement](./Set-AzIntegrationAccountAgreement.md)
