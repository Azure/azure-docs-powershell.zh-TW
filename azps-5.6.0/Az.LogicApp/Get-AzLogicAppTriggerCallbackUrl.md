...
## 描述
**Get-AzLogicAppTriallbackUrl** Cmdlet 會從資源群組取得邏輯 App 觸發程式 URL。
此 Cmdlet 會傳回代表轉寄 URL 的 **WorkflowTri一CallbackUrl** 物件。
指定資源組名、邏輯應用程式名稱及觸發程式名稱。
此模組支援動態參數。
若要使用動態參數，請于命令中輸入。
