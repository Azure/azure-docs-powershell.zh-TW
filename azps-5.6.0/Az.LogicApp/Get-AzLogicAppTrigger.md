...
## 描述
**Get-AzLogicAppTrigger Cmdlet** 會從邏輯應用程式觸發程式。
此 Cmdlet 會返回 **WorkflowTrigger 物件** 。
指定工作流程、資源群組和觸發。
此模組支援動態參數。
若要使用動態參數，請于命令中輸入。
...
此命令會從名為 LogicApp05 的邏輯應用程式獲得名為 Trigger01 的觸發程式。

### 範例 2：取得邏輯應用程式的所有觸發程式
```
...
[Get-AzLogicAppTritoryHistory](./Get-AzLogicAppTriggerHistory.md)
