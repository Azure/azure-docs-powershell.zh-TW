...
第一個命令會獲得 ContosoADL 帳戶根目錄的 ACL，然後將它儲存在$ACL變數。
第二個命令會設定檔案的 ACL，Test.txt中該檔案的$ACL。
...
第一個命令會獲得 ContosoADL 帳戶目錄資料夾1 的 ACL，然後將它儲存在$ACL變數。
第二個命令會以遞迴方式將 ACL 設定為 Folder2 及其子目錄和檔案，$ACL。
