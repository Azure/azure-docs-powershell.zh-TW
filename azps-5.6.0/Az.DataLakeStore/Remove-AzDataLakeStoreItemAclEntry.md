...
## 描述
**Remove-AzDataLakeStoreItemAclEntry Cmdlet** 會從 Data Lake Store 檔案或資料夾的存取控制清單 (ACL) 移除專案 (ACE) 。
...
此命令會從 ContosoADL 帳戶移除使用者 ACE for Patti Fuller。
...
此命令會從根目錄移除使用者 ACE for Patti Fuller，並遞迴從帳戶 ContosoADL 的所有子目錄和檔案。
...
### -路徑
指定要從移除 ACE 之專案的 Data Lake Store 路徑，從根目錄開始 (/) 。
...
### -ShowProgress
如果通過，則顯示進度狀態。 只有在完成遞迴 Acl 移除時，才適用。
