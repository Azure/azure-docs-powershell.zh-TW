...
此命令會從 ContosoADL 帳戶中MyFile.txt檔案的內容。
...
此命令會從 ContosoADL 帳戶的檔案中MyFile.txt兩個新行分隔列。
...
### -Head
從檔案開頭 (到預覽) 分隔的列數。 如果前 4mb 的資料沒有遇到新的一行，則只會將該資料退回。
...
### -路徑
指定檔案的 Data Lake Store 路徑，從根目錄開始 (/) 。
...
### -Tail
從檔案結尾 (以) 行分隔的列數。 如果前 4mb 的資料沒有遇到新的一行，則只會將該資料退回。
