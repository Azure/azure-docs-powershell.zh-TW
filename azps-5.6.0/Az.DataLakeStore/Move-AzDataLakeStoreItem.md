...
## 描述
**Move-AzDataLakeStoreItem** Cmdlet 會移動或重新命名 Data Lake Store 中的檔案或資料夾。
...
此命令會重新命名要File.txtRenamedFile.txt，並將其移至不同的資料夾。
...
### -目的地
指定要移動專案的 Data Lake Store 路徑，從根目錄或 / (開始) 。
...
### -路徑
指定要移動或重新命名之專案的 Data Lake Store 路徑，從根目錄開始 (/) 。
