...
## 描述
若要深入瞭解查詢語法，請在這裡： https://aka.ms/resource-graph/learntoquery
...
```
具有所有虛擬機器的查詢，這些虛擬機器不是使用受管理的磁片，而且包含訂閱顯示名稱。
