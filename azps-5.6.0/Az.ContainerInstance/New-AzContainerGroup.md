...
## 描述
**New-AzContainerGroup** Cmdlet 會建立容器群組。
...
這個命令會使用最新的 nginx 影像建立容器群組，並要求開啟埠 8000 的公用 IP 位址。
...
這個命令會建立一個容器群組，該群組會列印出"hello"並停止。

### 範例 4：使用 Azure Container Registry 中的圖像建立容器群組
```
...
這個命令會使用 Azure Container Registry 中的 nginx 圖像建立容器群組。

### 範例 5：使用自訂容器圖像註冊表中的圖像建立容器群組
```
PS C:\> $secpasswd = ConvertTo-SecureString "PlainTextPassword" -AsPlainText -Force
...
這個命令會使用自訂容器圖像註冊表中的自訂圖像來建立容器群組。

### 範例 6：建立安裝 Azure 檔案音量的容器群組
```
PS C:\> $secpasswd = ConvertTo-SecureString "PlainTextPassword" -AsPlainText -Force
PS C:\> $mycred = New-Object System.Management.Automation.PSCredential ("username", $secpasswd)
...
這個命令會建立容器群組，將提供的 Azure 檔案共用安裝至 `/mnt/azfile` 。
...
這個命令會建立一個容器群組，其系統會使用最新的 nginx 影像指派身分識別，並要求開啟埠 8000 的公用 IP 位址。
...
這個命令會建立一個容器群組，其系統已指派，且使用者已使用最新的 nginx 影像指派身分識別，並要求開啟埠 8000 的公用 IP 位址。
...
要安裝的 Azure 檔案共用儲存帳號憑證，使用者名稱是儲存帳戶名稱，而金鑰是儲存帳戶金鑰。
