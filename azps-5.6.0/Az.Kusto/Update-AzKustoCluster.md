...
上述命令會更新在資源群組 "testrg" 中找到的 Kusto 叢集 「testnewkustocluster」SKU。
...
上述命令會使用客戶管理的金鑰更新資源群組 "testrg" 中找到的叢集「testnewkustocluster」。
...
### -EnableCryptEncryption
這是一個布林值，指出該組群的磁片是否經過加密。
...
### -IdentityType
使用的受管理身分識別類型。
'SystemAssigned， UserAssigned' 類型包含隱含建立身分識別，以及一組使用者指派的身分識別。
類型為 "None" 會移除所有身分。
...
與 Kusto 組群相關聯的使用者身分清單。
使用者身分識別字典的金鑰參照為表單中的 ARM 資源識別碼：'/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/{identityName}'。
...
語言延伸模組清單。
若要建構，請參閱 LANGUAGEEXTENSIONVALUE 屬性的 NOTES 區段，然後建立雜湊表。
...
該群的外部租使用者。
若要建構，請參閱 TRUSTEDEXTERNALTENANT 屬性的 NOTES 區段，然後建立雜湊表。
...
TrustEDEXTERNALTENANT <ITrustedExternalTenant[]>：該群的外部租使用者。
  - `[Value <String>]`：代表外部租使用者 GUID。
