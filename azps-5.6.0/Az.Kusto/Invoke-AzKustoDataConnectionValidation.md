...
上述命令會驗證名為 "myeventhubdc" 的 EventHub 資料連線，以用於叢集"testnewkustocluster"中的資料庫"mykustodatabase"。

### 範例 2：驗證 EventGrid 資料連線參數
```powershell
...
上述命令會驗證名為 "myeventgriddc" 的 EventGrid 資料連線，以用於叢集"testnewkustocluster"中的資料庫"mykustodatabase"。

### 範例 3：驗證 IotHub 資料連線參數
```powershell
...
上述命令會驗證名為 "myiothubdc" 的 IotHub 資料連線，以用於叢集"testnewkustocluster"中的資料庫"mykustodatabase"。

### 範例 4：透過身分識別驗證 EventHub 資料連線參數
```powershell
...
### 範例 5：透過身分識別驗證 EventGrid 資料連線參數
```powershell
PS C:\> $database = Get-AzKustoDatabase -ResourceGroupName "testrg" -ClusterName "testnewkustocluster" -DatabaseName "mykustodatabase"
