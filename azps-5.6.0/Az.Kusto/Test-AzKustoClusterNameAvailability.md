...
上述命令會返回名稱為"testnewkustocluster"的 Kusto 叢集是否存在於「美國東部」地區。

### 範例 2：檢查未使用之 Kusto 組名的可用性
```powershell
...
上述命令會返回名稱為「availablekustocluster」的 Kusto 叢集是否存在於「美國東部」地區。
