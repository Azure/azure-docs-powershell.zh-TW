...
### 範例 1：列出一個集中的所有 AttachedDatabaseConfigurations
```powershell
...
上述命令會列出叢集中的所有 AttachedDatabaseConfigurations"testnewkustoclusterf"。

### 範例 2：在一個組集中取得特定的 AttachedDatabaseConfiguration
```powershell
...
上述命令會返回名為「testnewkustoclusterf」叢集中「myfollollconfiguration」的 AttachedDatabaseConfigurations。
