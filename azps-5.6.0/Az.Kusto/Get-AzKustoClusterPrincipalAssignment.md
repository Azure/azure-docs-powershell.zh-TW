...
### 範例 1：列出所有 Kusto cluster principalAssignment
```powershell
...
上述命令會列出叢集中 「testnewkustocluster」中所有主體的Assignment。

### 範例 2：根據名稱獲得 Kusto cluster principalAssignment
```powershell
...
上述命令會返回名為 "testnewkustocluster" 叢集中"kustoprincipal1" 的 Kusto 叢集主體Assignment。
