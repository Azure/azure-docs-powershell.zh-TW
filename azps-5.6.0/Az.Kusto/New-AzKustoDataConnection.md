...
上述命令會針對叢集中「testnewkustocluster」中的「mykustodatabase」資料庫，建立名為「myeventhubdc」的新 EventHub 資料連線。

### 範例 2：建立新的 EventGrid 資料連線
```powershell
...
上述命令會為「testnewkustocluster」中的資料庫「mykustodatabase」建立名為「myeventgriddc」的新 EventGrid 資料連線。

### 範例 3：建立新 IotHub 資料連線
```powershell
...
上述命令會針對叢集中「testnewkustocluster」中的「mykustodatabase」資料庫，建立名為「myiothubdc」的新 IotHub 資料連線。
