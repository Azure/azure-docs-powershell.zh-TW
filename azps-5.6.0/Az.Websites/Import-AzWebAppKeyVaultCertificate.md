...
## 簡介
從 Key Vault 將 SSL 憑證匯出至 Web App。
...
## 描述
**Import-AzWebAppKeyVaultCertificate** Cmdlet 會從 Key Vault 將 SSL 憑證導入至 Web App。
...
此命令會從 Key Vault 將 SSL 憑證導入至 Web App。
...
此命令會使用資源識別碼將 SSL 憑證從 Key Vault (到 Web App，通常是在金鑰庫位於另一個訂閱中時) 。

## 參數

### -CertName
在 keyvault 中建立之憑證的 KeyVaultCertName
