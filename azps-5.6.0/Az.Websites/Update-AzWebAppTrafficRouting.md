...
## 描述
**Update-AzWebAppTrafficRoudlet** 會更新 Azure Web App Slot 的路由規則組式。

## 例子

### 範例 1 更新路由規則，將 15% 生產流量轉移到 Stg slot
```powershell
PS C:\>Update-AzWebAppTrafficRouting -ResourceGroupName "Default-Web-WestUS" -WebAppName "ContosoSite" 
- RoutingRule @{AtionHostName='XXXX.azurewebsites.net';ReroutePercentage=15;Name='Stg'}
```

此命令會更新路由規則，將 15% 生產流量轉移到 Stg slot。

### 範例 2 更新路由規則，以增量方式將生產流量轉移到 Stg slot 範圍從 50% 到 90%。
```powershell
PS C:\>Update-AzWebAppTrafficRouting -ResourceGroupName "Default-Web-WestUS" -WebAppName "ContosoSite" 
...
此命令會更新路由規則，以增量方式將生產流量傳輸至 Stg 插槽的範圍從 50% 到 90%。
...
### -RoutingRule
Web App 路由規則。
範例：-RoutingRule @{ActionHostName=$slot。DefaultHostName ;ReroutePercentage=$ReroutePercentage ;Name=$slotName}
