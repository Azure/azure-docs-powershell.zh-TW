...
## 描述
**Get-AzWebAppSnapshot** Cmdlet 會返回 Web 應用程式的所有快照。 快照是 Web App 檔案和設定自動備份。 您可以使用 **Restore-AzWebAppSnapshot Cmdlet** 還原快照。
...
取得名為 「ContosoApp」的 Web App 的快照，以及 "Default-Web-WestUS" 資源群組中名為「暫存」的時段
