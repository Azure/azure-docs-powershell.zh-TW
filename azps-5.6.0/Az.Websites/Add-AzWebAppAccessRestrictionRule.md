...
## 描述
**Add-AzWebAppAccessRestrictionRule** Cmdlet 會新增存取限制規則至 Azure Web App。

## 例子

### 範例 1：新增 IpAddress Access 限制規則至 Web App
```
PS C:\>Add-AzWebAppAccessRestrictionRule -ResourceGroupName "Default-Web-WestUS" -WebAppName "ContosoSite" 
-Name IpRule -Priority 200 -Action Allow -IpAddress 10.10.0.0/8
```

此命令會將優先順序為 200 和 ip 範圍的存取限制規則新增到屬於資源群組 Default-Web-WestUS 的名為 ContosoSite 的 Web App。

### 範例 2：新增子網服務端點存取限制規則至 Web App
```
PS C:\>Add-AzWebAppAccessRestrictionRule -ResourceGroupName "Default-Web-WestUS" -WebAppName "ContosoSite" 
-Name SubnetRule -Priority 300 -Action Allow -SubnetName appgw-subnet -VirtualNetworkName corp-vnet
```

此命令會將優先順序為 300 且具有 corp-vnet 子網 appgw-子網的存取限制規則新增到屬於資源群組 Default-Web-WestUS 的 Web App，名為 ContosoSite。

### 範例 3：新增 ServiceTag 存取限制規則至 Web App
```
PS C:\>Add-AzWebAppAccessRestrictionRule -ResourceGroupName "Default-Web-WestUS" -WebAppName "ContosoSite" 
-Name ServiceTagRule -Priority 200 -Action Allow -ServiceTag AzureFrontDoor.Backend
```

此命令會將優先順序為 200 的存取限制規則，以及代表 Azure Front Door ip 範圍的服務標籤新增到屬於資源群組 Default-Web-WestUS 的 ContosoSite Web App。

### 範例 4：新增多重位址存取限制規則至 Web App
```
PS C:\>Add-AzWebAppAccessRestrictionRule -ResourceGroupName "Default-Web-WestUS" -WebAppName "ContosoSite" 
-Name MultipleIpRule -Priority 200 -Action Allow -IpAddress "10.10.0.0/8,192.168.0.0/16"
```

此命令會將優先順序為 200 和兩個 ip 範圍的存取限制規則新增到屬於資源群組 Default-Web-WestUS 的名為 ContosoSite 的 Web App。

### 範例 5：新增具有 HTTP 標頭的存取限制規則至 Web App
```
PS C:\>Add-AzWebAppAccessRestrictionRule -ResourceGroupName "Default-Web-WestUS" -WebAppName "ContosoSite" 
...
此命令會針對服務標記 AzureFrontDoor.後端新增優先順序為 400 的存取限制規則，進一步限制對屬於資源群組 Default-Web-WestUS 的 Web App 存取特定值的 HTTP 標頭。
...
### -HTTPHeader
HTTP 標頭限制。 範例：-HTTPHeader @{'x-azure-fdid' = '7acacb02-47ea-4cd4-b568-5e880e72582e';'x-forwarded-host' = 'www.contoso.com'， 'app.contoso.com'}
