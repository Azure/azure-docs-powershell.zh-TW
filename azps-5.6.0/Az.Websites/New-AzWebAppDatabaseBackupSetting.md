...
## 描述
**New-AzWebAppDatabaseBackupSetting** Cmdlet 會建立新的 Azure Web App 備份設定。
...
針對位於資源群組 Default-Web-WestUS 內) 的指定應用程式 ContosoWebApp 建立資料庫備份設定 (類型 SqlAzure 的連接字串。

### 範例 2

此New-AzWebAppDatabaseBackupSetting Cmdlet 會建立新的 Azure Web App 備份設定。  (自動) 
