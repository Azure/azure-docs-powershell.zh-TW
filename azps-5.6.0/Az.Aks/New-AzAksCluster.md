...
在 AKS (中) Azure Kubernetes Service。
...
若要在 AKS 上建立 Windows Server 容器，您必須在建立 AKS 時指定至少四個下列參數，且其值必須和分別 `NetworkPlugin` `NodeVmSetType` `azure` `VirtualMachineScaleSets` 指定。
...
### -AcrNameToAttach
將指定 ACR 的 'acrpull' 角色授予 AKS 服務主體，例如 myacr
...
### -DnsNamePrefix
該組群的 DNS 名稱首碼。 如果使用者打算新增視窗容器<長度必須等於 9。
...
### -EnableRbac
是否要啟用 Kubernetes Role-Based Access
...
### -NodeOs以Size
節點資料庫每個節點的作業系統磁片大小 ，以 GB 為單位。 最低 30 GB。
...
要用來指定低優先順序虛擬機器縮放集的部署策略的 ScaleSetEvictionPolicy。 預設為 Delete。
...
### -NodeSetPriority
用來指定虛擬機器縮放比例集優先順序的 ScaleSetPriority。 預設為一般。
...
### -NodeEvSetType
AgentPoolType 代表代理程式清單類型。 可能的值包括：「VirtualMachineScaleSets'、'AvailabilitySet'
...
與 AAD 應用程式/服務主體相關聯的用戶端識別碼與用戶端金鑰。
...
Windows 虛擬機器的系統管理員密碼長度必須至少為 12 個，包含至少一個小寫字元，即一個特殊 `[a-z]` `[A-Z]` 字元 `[!@#$%^&*()]` 。
