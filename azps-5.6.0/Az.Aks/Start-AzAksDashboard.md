...
## 描述
在受管理之組儀表板上建立 Kubectl SSH 星型。 SSH 的外包裝是在名為 Kubectl-Tunnel 的 PowerShell 工作中設定，而且可以執行來找到 `Get-Job` 。 應該可以透過 . [http://127.0.0.1:8001](http://127.0.0.1:8001)

## 例子

### 啟動 SSH 儀表板，並開啟瀏覽器至 Kubernetes 儀表板
```
...
### Microsoft.Azure.Commands.Aks.KubeTunureJob
