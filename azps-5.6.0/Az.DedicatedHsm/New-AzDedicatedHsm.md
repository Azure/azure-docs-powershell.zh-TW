...
此命令會建立專用 HSM 至現有的虛擬網路。

**注意：** HSM 在 Azure 上完整設置之前，目前會 `New-AzDedicatedHsm` 先返回限制。
因此，建立新 HSM 之後，請先查詢其狀態， `Get-AzDedicatedHsm` 確認其 `Provisioning State` 狀態後 `Succeeded` 再使用它。
...
### -NetworkInterface
指定與專用 HSM 相關聯的網路介面之資源識別碼清單。
若要建構，請參閱 NETWORKINTERFACE 屬性的附注區段，並建立雜湊表。
...
NETWORKINTERFACE <INetworkInterface[]>：指定與專用 HSM 相關聯的網路介面資源識別碼清單。
