...
## 描述
此New-AzRecoveryServicesAsrProtectionContainer Cmdlet 會建立指定 Azure 網站修復結構下的保護容器。
...
使用指定的參數開始建立保護容器，並返回用來追蹤作業的 ASR 工作。
...
### -InputObject
在指定的輸入物件中建立禁止複製容器 (Azure Fabric) 。
