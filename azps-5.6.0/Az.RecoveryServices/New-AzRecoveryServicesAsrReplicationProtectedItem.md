...
### VTypeToAzureWithAzUreWithAzType
```
...
## 描述
**New-AzRecoveryServicesAsrReplicationProtecedItem** Cmdlet 會建立一個新的複製受保護的專案。
使用此 Cmdlet 啟用 ASR 可保護專案的複製。
...
針對指定的 ASR 可保護專案啟動複製受保護的專案建立作業，並返回用來追蹤作業的 ASR 工作。
...
針對指定的 ASR 可保護專案啟動複製受保護的專案建立作業，並返回用來追蹤作業的 ASR 工作 (vmWare 到 Azure 案例) 。
...
針對指定的 ASR 可保護專案啟動複製受保護的專案建立作業，並返回用來追蹤作業的 ASR 工作 (Azure 到 Azure 案例) 。
...
為指定的 VmId 啟動複製受保護的專案建立作業，並返回用來追蹤 Azure (Azure 案例作業的 ASR) 。
...
針對指定的 ASR 可保護專案啟動複本受保護的專案建立作業，包括選擇性磁片選取，並返回用來追蹤作業 (vmWare 到 Azure 案例的 ASR 工作，) 選取的磁片。
...
為指定的 VmId 啟動複製受保護的專案建立作業，並返回用來追蹤 Azure (Azure 案例作業的 ASR) 。針對在 Cmdlet 中傳遞加密的容錯移轉 VM 詳細資料，將會使用。
...
在鄰近位置群組內啟動虛擬機器的複製受保護的專案建立作業，並返回用來追蹤作業的 ASR 工作 (Azure 到 Azure 案例) 。

## 參數

### -帳戶
需要時，以帳戶執行以推入安裝行動服務。 必須是 ASR 結構中以帳戶執行清單中的帳戶。
...
### -AzureToAzure
參數參數指定在 Azure 中建立複製的專案至 azure 案例。
...
### -AzureToAzureAzlicationConfiguration
指定用於 Azure 的 Vm 到 Azure 災害復原案例的磁片組配置。
...
指定 Azure 磁片加密的磁片加密 (密碼庫識別碼) 容錯移轉後要用於復原 VM。
...
### -HyperVToAzure
切換參數以指定複製的專案是正在複製到 Azure 的 Hyper-V 虛擬機器。
...
### -InMageAzureV2AgeInput
指定 vMWare 磁片識別碼的磁片組配置輸入，以防範指定的可保護專案。
...
### -KeyEncryptionKeyUrl
指定具有版本 (Azure 磁片加密的磁片加密金鑰 URL，) 容錯移轉後用來做為修復 VM。
...
指定 Azure 磁片加密的磁片 (金鑰庫識別碼) 容錯移轉後要用於復原 VM。
...
### -ProcessServer
要用於複製此電腦的流程伺服器。 使用 ASR 結構中對應至 Configuration 伺服器的流程伺服器清單來指定一個。
...
指定對應到要用於複製之複製之複寫原則的 ASR 保護容器對應物件。
...
AvailabilitySet 的識別碼，以在容錯移轉時將電腦復原到該電腦。
...
在容錯移轉時，應附加容錯移轉到虛擬機器之復原 Azure 虛擬網路內的子網。
...
指定要用於建立多 VM 一致修復點的複寫組名。 根據預設，每個複本受保護的專案都是以一組自己的複本建立，且不會產生多 VM 一致的復原點。 只有在您需要保護所有電腦至同一個複本群組，以跨一組機器建立多 VM 一致的修復點時，才使用此選項。 
...
### -UseManaged可使用
指定在容錯移轉上建立 Azure 虛擬機器時，應該使用受管理的磁片。 它接受 True 或 False。
...
### -VmmToVmm
切換參數以指定複製的專案是正在 VMM 管理的 Hyper-V 網站之間複製的 Hyper-V 虛擬機器。
...
### -VAzToAzure
切換參數以指定複製的專案是 V是否虛擬機器或實體伺服器，會複製到 Azure。
