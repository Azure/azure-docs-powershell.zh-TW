...
## 描述
**New-AzRecoveryServicesAsrFabric** Cmdlet 會建立指定類型的 Azure 網站修復結構。
...
以傳遞的名稱開始建立布料，並傳回用來追蹤布料建立作業的 ASR 工作。
...
以傳遞的名稱開始建立 Azure 結構，並傳回用來追蹤布料建立作業的 ASR 工作。

## 參數

### -Azure
Switch 參數指定建立 Azure 結構。
...
### -位置
指定與要建立之結構物件對應的 Azure 區域。 Azure 網站復原結構物件代表區域。 針對在兩個 Azure 區域之間複製的虛擬機器，主要結構代表主要的 Azure 區域及復原結構。
