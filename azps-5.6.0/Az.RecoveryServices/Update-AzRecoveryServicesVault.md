...
## 描述
此 Cmdlet 用來從復原服務儲存庫新增或移除 MSI。 使用 -IdentityType param 將 SystemAssigned 身分識別指派給 RSVault。 使用 IdentityType None 從儲存庫移除 MSI。
...
此 Cmdlet 用來將 SystemAssigned 身分識別新加到復原服務儲存庫。
