...
## 描述
**Update-AzRecoveryServicesAsrPolicy** Cmdlet 會更新指定的 Azure 網站復原複本策略。
...
使用指定的參數啟動更新複寫原則作業，並返回用來追蹤作業的 ASR 工作。
...
使用指定的參數啟動 Azure 至 Azure 複寫原則作業，並返回用來追蹤作業的 ASR 工作。
...
使用指定的參數將 Azure 更新為 Azure 複寫原則，並返回用來追蹤作業的 ASR 工作。

## 參數

### -ApplicationConsistentSnapshotFrequencyInHours
指定以小時 () 建立應用程式一致修復點的頻率。
...
### -HyperVToAzure
切換參數，指出指定的策略是用來將 Hyper-V 虛擬機器複製到 Azure。
...
### -InputObject
Cmdlet 的輸入物件：指定對應至要更新之複寫原則的 ASR 複寫原則物件。
...
指定複製目標的 Azure 儲存空間帳戶識別碼。 在啟用使用 Cmdlet 進行複製時，若未提供替代專案，則用來New-AzRecoveryServicesASRReplicationProtectedItem帳戶。
...
### -ReplicaDeletion
指定在停用 VMM 受管理網站的複本時，是否應刪除複本虛擬機器。
...
### -VmmToVmm
切換參數，指出指定的策略是用來在兩個 Hyper-V 網站之間複製 VMM 管理的 Hyper-V 虛擬機器。
...
### -VAzToAzure
切換參數，指出指定用於將 VAa 虛擬機器複製到 Azure。
