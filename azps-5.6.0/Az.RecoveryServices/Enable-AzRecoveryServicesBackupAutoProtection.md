...
## 簡介
**Enable-AzRecoveryServicesBackupAutoProtection** Cmdlet 會使用提供之策略，自動保護給定實例內目前和未來的任何 SQL DB。
...
## 描述
此命令可讓使用者自動保護所有現有的未受保護的 SQL DB 以及稍後會以給定政策新增的任何 DB。  由於指示是備份所有未來的 DBs，因此作業是在 SQLInstance 層級完成，因此 Azure 備份服務會定期掃描任何新 DBS 的自動保護容器，並自動保護它們。
...
第二個 Cmdlet 會抓取相關的 SQLInstance，這是可保護的專案。 接著，第 3 個命令會使用此實例中的$Pol。

### 範例 2

這個命令可讓使用者自動保護所有現有的未受保護的 DBs 和任何 DB，這些 DB 稍後會以給定政策新增。  (自動) 
...
受保護的資源類別。 此 Cmdlet 目前支援的值為 MAB、AzureWorkload、AzureCM
...
### -InputItem
指定可傳遞為輸入的可保護專案物件。 目前支援的值是類型為 "SQLInstance" 的可保護的Item 物件。 
...
### -WorkloadType
資源的工作量類型。 目前支援的值為 AzureVM、WindowsServer、MSSQL
