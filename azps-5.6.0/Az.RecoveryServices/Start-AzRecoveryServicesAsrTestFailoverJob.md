...
### ByRPObjectWithAzureEVNETworkId
```
Start-AzRecoveryServicesAsrTestFailoverJob -RecoveryPlan <ASRRecoveryPlan> -Direction <String>
...
### ByRPIObjectWithAzureEVNETworkId
```
Start-AzRecoveryServicesAsrTestFailoverJob -ReplicationProtectedItem <ASRReplicationProtectedItem>
...
## 描述
**Start-AzRecoveryServicesrTestFailoverJob** Cmdlet 會啟動 Azure 網站復原複本保護專案或復原計畫的測試容錯移轉。
您可以使用 Cmdlet 檢查工作是否Get-AzRecoveryServicesAsrJob成功。
...
使用指定的參數啟動復原計畫的測試容錯移轉作業，並返回用來追蹤作業的 ASR 工作。
...
指定是否應該建立新的雲端服務，或應該使用為 VM 所配置的復原雲端服務進行測試容錯移轉。
