...
## 描述
建立將 vMWare 虛擬機器磁片與緩存儲存帳戶及目標受管理磁片類型 (修復區域) 以用於複製磁片的磁片地圖物件。
...
建立要複製之 vMWare 虛擬機器磁片的磁片映射物件。在啟用 vMWare 電腦保護期間使用。
...
### Microsoft.Azure.Commands.RecoveryServices.SiteRecovery.AsrInMageAzureV2CoverInput
