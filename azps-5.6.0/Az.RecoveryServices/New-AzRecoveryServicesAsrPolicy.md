...
## 描述
**New-AzRecoveryServicesAsrPolicy** Cmdlet 會建立 Azure 網站復原複本策略。
複寫原則是用來指定複本設定，例如複本頻率及復原點數目。
...
使用指定的參數啟動複寫原則建立作業，並返回用來追蹤作業的 ASR 工作。
...
### -AzureToVPv
參數參數會指定 azure 到 vMWare 建立之策略的情境。
...
### -HyperVToAzure
切換參數以指定用於將 Hyper-V 虛擬機器複製到 Azure 的策略
...
### -VmmToVmm
切換參數以指定策略，以在 VMM 伺服器管理的 Hyper-V 網站之間複製。
...
### -VAzToAzure
切換參數，指定要建立之複寫原則將用來將 V是否虛擬機器和/或實體伺服器複製到 Azure。
