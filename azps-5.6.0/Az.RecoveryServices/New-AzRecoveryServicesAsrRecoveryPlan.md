...
## 描述
**New-AzRecoveryServicesAsrRecoveryPlan** Cmdlet 在修復服務庫中建立 Azure 網站復原、復原計畫。

復原計畫會將屬於應用程式的虛擬機器收集到一個單位中，以便一起復原。
...
使用指定的參數啟動復原計畫建立作業，並返回用來追蹤作業的 ASR 工作。
...
針對 Azure 區域開始復原計畫建立作業，以將複本專案分區，並返回用來追蹤作業的 ASR 工作。

## 參數

### -Azure
參數參數會指定 Azure 到 Azure 的災害復原、復原計畫建立案例。
...
### -AzureZoneToZone
參數參數指定在 Azure 區域中建立複製的專案至區域分析藍本。
...
指定傳統 (或 Resource Manager) 屬於此復原計畫之複本保護專案的容錯移轉部署模型。
...
### -路徑
指定復原計畫定義 json 檔案的路徑。 修復計畫定義 json 可用來建立復原計畫。
...
### -PrimaryFabric
指定屬於此復原計畫之複本受保護專案之主要 ASR 結構物件的 ASR 結構物件。
...
### -RecoveryFabric
指定將屬於此復原計畫之複本保護專案的 ASR 結構復原 ASR 結構物件。
