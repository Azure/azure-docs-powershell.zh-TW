...
## 簡介
更新指定複本受保護的專案或復原計畫的複本方向。 用來重新保護/反向複製已複製的專案或復原計畫失敗。
...
## 描述
完成 **提交容錯移轉作業後，Update-AzRecoveryServicesrProtectionDirection** Cmdlet 會更新指定 Azure 網站修復物件的複本方向。
...
為指定的復原計畫啟動更新方向作業，並返回用來追蹤作業的 ASR 工作物件。
...
在目標 Azure 地區中，針對保護容器的映射和使用與 VM (相同的區域使用快 (定義的指定複製受保護的專案，開始更新方向) 。
...
針對由保護容器映射和提供的磁碟複製組定所定義之目標 Azure 區域指定複製受保護的專案，開始更新方向作業。
...
針對由保護容器地圖和提供的磁碟複製組定所定義之目標 Azure 區域指定的加密複製受保護的專案，開始更新方向作業。
...
在目標 Azure 區域，針對由保護容器映射和使用快件儲存空間 (在與 VM) 和鄰近位置群組相同的地區中定義之指定複製受保護的專案，開始更新方向作業。
...
### -KeyEncryptionKeyUrl
指定 Azure 磁片加密 (的磁片加密金鑰 URL，) 容錯移轉後用來做為修復 VM。
...
### -VmmToVmm
在兩個 VMM 受管理的 Hyper-V 網站之間保護的 Hyper-V 虛擬機器上失敗的更新複製方向。
