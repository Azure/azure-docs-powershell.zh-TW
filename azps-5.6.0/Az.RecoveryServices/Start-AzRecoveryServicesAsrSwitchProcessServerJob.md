...
## 描述
**Start-AzRecoveryServicesAsrSwitchProcessServerJob** 會切換指定虛擬機器或指定程式伺服器的複製資料移動至指定的目標流程伺服器。 用於在程式伺服器之間負載平衡或切換複製。
...
工作以追蹤從來源到目標流程伺服器之所有複製受保護專案的切換程式伺服器。
...
針對從來源到目標流程伺服器傳遞的複製受保護專案，執行追蹤切換程式伺服器的工作。
...
### -布料
對應到 Configuration Server 的網站復原結構。
