...
## 描述
**Get-AzRecoveryServicesAsrEvent** 會依據指定的選取篩選，取得資料庫中的事件清單。
...
時間開始時間和結束時間之間的事件清單、嚴重性臨界值及健康情況類型 VmHealth。
...
### -EndTime
指定搜尋視窗的結束時間。 使用此參數僅取得指定時間之前發生的事件。
