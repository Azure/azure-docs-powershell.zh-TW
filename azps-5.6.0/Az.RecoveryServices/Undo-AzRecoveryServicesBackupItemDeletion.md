...
## 簡介
如果備份專案被刪除，且呈現為柔刪除狀態，此命令會讓專案回到資料永遠保留的狀態 
...
## 描述
Cmdlet Undo-AzRecoveryServicesBackupItemDeletion將柔刪除的專案還原為保護已停止，但資料會永久保留的狀態。
...
第三個命令會停用 $PI 0 中專案的備份保護，並將專案置於 \[ \] 柔式狀態。
第四個命令會獲得狀態為柔和的專案。
最後一個命令將 Softdeleted VM 帶到保護已停止的狀態，但資料會永久保留。
