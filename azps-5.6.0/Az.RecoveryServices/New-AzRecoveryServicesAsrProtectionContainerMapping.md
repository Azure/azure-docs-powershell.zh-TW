...
## 簡介
將指定的複本策略與指定的 ASR 保護容器建立關聯，以建立 Azure 網站復原保護容器的映射。
...
## 描述
**New-AzRecoveryServicesAsrProtectionContainerMadlet** Cmdlet 會建立 Azure 網站修復保護容器的關聯，將指定的複本策略與指定的保護容器建立關聯。
...
開始建立具有指定參數的保護容器，並返回用來追蹤作業的 ASR 工作。
...
指定要用於復原保護容器的 ASR 保護容器物件，用於 (如果複製到非 Azure.) 的復原位置時所使用的復原容器) 
