...
## 簡介
此命令會建構備份專案的復原組態，例如 SQL DB。 設定物件會儲存所有詳細資料，例如復原模式、還原的目標目的地，以及應用程式特定參數 ，例如 SQL 的目標實體路徑。
...
## 描述
該命令會傳回傳遞至還原 Cmdlet 的 AzureWorkload 專案的復原設定檔。
...
第一個 Cmdlet 是用來取得修復點物件。
第二個 Cmdlet 會為原始位置還原建立復原計畫。
第三個 Cmdlet 會針對替代位置還原建立復原計畫。

### 範例 2

此命令會建構備份專案的復原組態，例如 SQL DB。  (自動) 
...
### -TargetItem
指定需要還原 DB 的目標。 針對 SQL 還原，它只需要為可保護的專案類型 SQLInstance。
