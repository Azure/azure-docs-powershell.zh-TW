...
## 描述
建立一個磁片地圖物件，將 Azure 虛擬機器磁片與緩存儲存帳戶及目標儲存帳戶 (修復) 用於複本磁片。
...
為要複製的 Azure 虛擬機器磁片建立磁片映射物件。在 Azure 期間用於 Azure EnableDr 及重新保護作業。
...
為要複製的 Azure 虛擬機器磁片建立受管理的磁片地圖物件。在 Azure 期間用於 Azure EnableDr 及重新保護作業。
...
針對要複製的 Azure 虛擬機器磁片，使用一次加密設定建立受管理的磁片地圖物件。在 Azure 期間用於 Azure EnableDr 及重新保護作業。
...
使用目標磁片加密集 Id 建立受管理的磁片地圖物件，以複製 Azure 虛擬機器磁片。在 Azure 期間用於 Azure EnableDr 及重新保護作業。
...
### -RecoveryCryptEncryptionSetId
指定要用於修復磁片的 Azure 磁片加密集識別碼。
...
### -RecoveryTargetCountAccountType
指定複本受管理磁片的復原目標磁片。
...
### Microsoft.Azure.Commands.RecoveryServices.SiteRecovery.ASRAzuretoAzureAzureReplicationConfig
