...
## 描述
**Set-AzRecoveryServicesBackupProtectionPolicy** Cmdlet 會修改現有的 Azure 備份保護原則。
您可以修改備份排程和保留原則元件。
您進行的任何變更都會影響與策略關聯的專案的備份和保留。
...
以下是修改保護政策時要遵循之步驟的高層級描述： 
1.  取得基準 SchedulePolicyObject 和 Base RetentionPolicyObject。 將它們儲存在一些變數中。
2.  根據需求設定排程和保留原則物件的不同參數。 例如，在上述範例腳本中，我們嘗試設定每週保護政策。 因此，我們將排程頻率變更為「每週」，並更新排程執行時間。 在保留原則物件中，我們已更新每週保留持續時間，並設定正確的「啟用每週排程」標標。 若要設定每日策略，請設定 「啟用每日排程」標號為 True，並指派適當的值給其他物件參數。
3.  取得您想要修改的備份保護原則，並儲存在變數中。 在上例中，我們取了一個名稱為 "TestPolicy" 的備份策略，而我們想要修改該名稱。
4.  使用修改的排程策略物件和保留原則物件修改步驟 3 中所取回的備份保護原則。
...
切換參數，指出是否要重試失敗專案的 Policy Update。
...
### -SchedulePolicy
指定基準排程策略物件。
若要取得 **SchedulePolicy 物件** ，請使用Get-AzRecoveryServicesBackupSchedulePolicyObject物件。
