...
## 簡介
此命令會在特定容器內或跨所有已登錄的容器，來取回所有可保護的專案。 它會包含應用程式階層的所有元素。 會返回 DB 及其上層實體，例如 Instance、AvailabilityGroup 等。
...
## 描述
**Get-AzRecoveryServicesBackupProtectableItem** Cmdlet 會取得容器中可保護專案的清單，以及專案的保護狀態。
...
第一個命令會獲得 MSSQL 類型的容器，然後將它儲存在$Container變數中。
第二個命令會獲得 $Container 備份可保護的專案，然後將它儲存在$Item變數中。
...
### -ItemType
指定可保護專案的類型。 適用值： (SQLDataBase、SQLInstance、SQLAvailabilityGroup) 。
