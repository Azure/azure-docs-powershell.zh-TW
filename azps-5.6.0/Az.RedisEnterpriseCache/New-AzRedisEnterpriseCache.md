...
## 簡介
建立 Redis Enterpise 緩存組和相關聯的資料庫
...
### 範例 1：建立 Redis Enterprise Cache
```powershell
...
此命令會建立名為 MyCache 的 Redis Enterprise Cache。

### 範例 2：使用一些選擇性參數建立 Redis Enterprise Cache
```powershell
...
此命令會使用一些選擇性參數建立名為 MyCache 的 Redis Enterprise Cache。
...
### -容量
RedisEnterprise 集區的大小。
根據 SKU，預設為 2 或 3。
有效的值 (2、4、6、...) 企業 SKUS 和 (3、9、15、...) Flash SK。
...
### -ClientProtocol
指定重新發送用戶端是否可以使用 TLS 加密或純文字重新文本通訊協定進行連結。
預設值為 TLS 加密。
...
### -模組
在此資料庫中啟用的選擇性重新模組組 - 模組只能在建立時新增。
若要建構，請參閱 MODULE 屬性的 NOTES 區段，並建立雜湊表。
...
### -SKU
要部署的 RedisEnterprise 集區類型。
可能的值： (Enterprise_E10、EnterpriseFlash_F300等) 
...
MODULE <IModule[]>：在此資料庫中啟用的選擇性重新模組組 - 模組只能在建立時新增。
  - `Name <String>`：模組的名稱，例如'RedisBloom'，'RediSearch'， 'RedisTimeSeries'
  - `[Arg <String>]`：模組的組ERROR_RATE，例如'ERROR_RATE 0.00 INITIAL_SIZE 400'。
