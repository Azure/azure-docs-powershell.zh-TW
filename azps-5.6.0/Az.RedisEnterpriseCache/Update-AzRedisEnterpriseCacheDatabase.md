...
此命令會更新名為 MyCache 的 Redis Enterprise Cache 資料庫的用戶端通訊協定。
...
此命令會更新名為 MyCache 之 Redis Enterprise Cache 之資料庫的用戶端通訊協定和修正策略。
...
INPUTOBJECT： <IRedisEnterpriseCacheIdentity> 身分識別參數
  - `[ClusterName <String>]`：RedisEnterprise 組名。
  - `[DatabaseName <String>]`：資料庫的名稱。
  - `[Id <String>]`：資源識別路徑
  - `[Location <String>]`：作業所位於的區域。
  - `[OperationId <String>]`：作業的唯一識別碼。
  - `[PrivateEndpointConnectionName <String>]`：與 Azure 資源相關聯的私人端點連接名稱
