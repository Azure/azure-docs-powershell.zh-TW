...
### 範例 1：移除 Redis Enterprise Cache 並返回結果
```powershell
PS C:\> Remove-AzRedisEnterpriseCache -Name "MyCache" -ResourceGroupName "MyGroup" -PassThru
True
```

此命令會移除 Redis Enterprise Cache，並顯示作業是否成功。

### 範例 2：移除 Redis Enterprise Cache 且不顯示結果
```powershell
PS C:\> Remove-AzRedisEnterpriseCache -Name "MyCache" -ResourceGroupName "MyGroup"
```

此命令會移除 Redis Enterprise Cache。
由於未指定 PassThru 參數，因此不會顯示作業的結果。
