...
## 描述
**New-AzServiceBusAuthorizationRule** Cmdlet 會為指定的 Service Bus 命名空間或佇列或主題建立新授權規則。
...
使用 `AuthoRule1` 命名空間 **的 Listen** 和 **Send** 許可權建立 `SB-Example1` 。
...
使用 `AuthoRule1` 佇列 **的聆聽** 和 **傳送** 許可權建立 `SBQueue` 。
...
使用 `AuthoRule1` 主題 **的聆聽** 和 **傳送** 許可權建立 `SBTopic` 。
