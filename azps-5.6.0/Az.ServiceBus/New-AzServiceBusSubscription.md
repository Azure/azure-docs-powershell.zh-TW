...
## 描述
**New-AzServiceBusSubscription** Cmdlet 會建立指定 Service Bus 主題的新訂閱。
...
建立指定 `SB-TopicSubscription-Example1` Service Bus 主題的訂閱 `SB-Topic_exampl1` 。

## 參數

### -AutoDeleteOnIdle
指定 [時間間隔，](https://msdn.microsoft.com/library/system.timespan.aspx) 之後會自動刪除訂閱。 最短持續時間為 5 分鐘。
