...
## 描述
此命令會從同步處理群組中刪除指定的雲端端點。 共用雲端端點參照的 Azure 檔案會不受此程式觸控。 此命令僅適用于解除授權。 移除雲端端點是一項破壞性的作業。 伺服器端點必須至少有一個雲端端點，就無法同步處理。 此作業不應執行以解決同步處理問題。 如果這個 Azure 檔案共用再次新增到與雲端端點相同的同步處理群組，可能會導致衝突檔案和其他非預期的結果。
