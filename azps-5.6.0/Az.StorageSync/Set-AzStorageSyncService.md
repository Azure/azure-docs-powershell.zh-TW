...
## 描述
儲存同步服務是 Azure 檔案同步的頂層資源。此命令會設定資源群組中的儲存空間同步服務。 建議您盡可能少建立儲存同步處理服務，以區分貴組織中不同的伺服器群組。 儲存同步處理服務包含同步處理群組，也可以做為註冊伺服器的目標。 伺服器只能註冊單一儲存同步處理服務。 如果伺服器需要參與同步處理同一組檔案，請將它們註冊到相同的儲存同步處理服務。
