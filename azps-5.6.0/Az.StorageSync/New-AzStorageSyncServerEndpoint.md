...
## 描述
此命令在已登錄的伺服器上建立新伺服器端點。 這可讓伺服器上指定的路徑開始同步處理群組中與其他端點的檔案。 如果同步處理群組中其他端點已有檔案，且新新增的位置也包含檔案，對帳程式會嘗試判斷檔案實際上是否與其他端點位於相同的資料夾中。 命名空間會合並及對帳，有助於防止衝突檔案。 如果其他伺服器端點有檔案，通常最好從這個伺服器上空白的位置開始，讓雲端的檔案以稱為快速災害復原的自動程式下拉至伺服器。 命名空間中繼資料會先向下同步處理，然後下載每個檔案的資料流程。 如果使用者或應用程式在下載順序外要求檔案，該檔案會優先回收，以滿足存取要求。 您可以選擇性地在此伺服器端點使用雲端分層，以判斷此端點是否應成為來自雲端之完整檔案集的緩存。 如果使用雲端分層，檔案內容下載會從您可以設定之雲端分層策略定義的點停止。
...
此命令在已登錄的伺服器上建立新伺服器端點，並將它插入同步處理群組。 THis 方式，它是其他端點之拓撲的一部分，而檔案中繼資料和內容會立即開始在同步處理群組中參照為端點的所有位置之間同步處理。
