...
## 描述
此命令會建立 Azure 檔案同步處理雲端端點。 雲端端點是現有 Azure 檔案共用參照。 它代表檔案共用，並定義它參與同步處理雲端端點已建立之同步處理群組的所有檔案部分。
...
雲端端點是同步處理群組中不可或缺的成員，這是在名為「mySyncGroupName」的現有同步處理群組內建立一個同步處理群組的範例。
...
### -名稱
雲端端點的名稱。 透過 Azure 入口網站建立時，名稱會設定為 Azure 檔案共用參照的名稱。
