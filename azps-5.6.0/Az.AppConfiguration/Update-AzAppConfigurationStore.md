...
### 範例 1：根據系統指派的受管理身分識別啟用應用程式組配置存放區的資料加密
```powershell
...
此命令可讓儲存在 Azure 金鑰保存庫中的金鑰使用系統指派的受管理身分識別進行資料加密。
該儲存庫必須啟用柔刪除和清除保護，而受管理的身分識別必須擁有以下重要許可權：get、wrapKey、unwrapKey。

### 範例 2：根據使用者指派的受管理身分識別啟用應用程式組配置存放區的資料加密
```powershell
PS C:\> $key = Add-AzKeyVaultKey -VaultName kv-Name -Name key-Name -Destination 'Software'
...
此命令可讓儲存在 Azure 金鑰保存庫中的金鑰使用使用者指派的受管理身分識別進行資料加密。
使用者指派的身分識別必須設定為 `-UserAssignedIdentity` 。
該儲存庫必須啟用柔刪除和清除保護，而受管理的身分識別必須擁有以下重要許可權：get、wrapKey、unwrapKey。
...
### 範例 3：根據管道更新 App 組配置存放區 SKU 和標記。
```powershell
...
### -IdentityType
使用的受管理身分識別類型。
"SystemAssignedAndUserAssigned" 類型包括隱含建立身分識別，以及一組使用者指派的身分識別。
類型為 "None" 會移除任何身分。
...
與資源相關聯的使用者指派身分驗證清單。
使用者指派的身分識別字典金鑰為表單中的 ARM 資源識別碼：'/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ManagedIdentity/userAssignedIdentity/{identityName}'。
