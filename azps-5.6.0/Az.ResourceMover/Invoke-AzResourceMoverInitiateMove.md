...
### 範例 2：使用「MoveResource Name」做為輸入，為 Move 集合中的資源集合初始化移動。
```powershell
...
使用「MoveResource 名稱」做為輸入，為 Move 集合中的資源集合初始化移動。

### 範例 3：在 Move 集合中，使用 "SourceARMID" 做為輸入，為一組資源初始化移動。
```powershell
...
使用 「SourceARMID」 做為輸入，為 Move 集合中的資源集合初始化移動。
