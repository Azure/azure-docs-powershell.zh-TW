...
### 範例 2：使用「MoveResource Name」做為輸入，在 Move 集合中確定一組資源。
```powershell
...
使用「MoveResource 名稱」做為輸入，在移動集合中確定一組資源。

### 範例 3：使用 "SourceARMID" 做為輸入，在 Move 集合中確定一組資源。
```powershell
...
使用「SourceARMID」做為輸入，在移動集合中承諾一組資源。
