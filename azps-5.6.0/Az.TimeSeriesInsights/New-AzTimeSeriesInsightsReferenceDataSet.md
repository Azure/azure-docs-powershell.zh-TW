...
您可以使用此屬性設定參照資料集鍵比較行為。
根據預設，值為 "Ordinal" -這表示在將參照資料與事件聯聯或在新增參照資料時，會執行區分大小寫的金鑰比較。
設定好 "OrdinalIgnoreCase" 時，將會使用不區分大小寫的比較。
...
### -KeyProperty
參照資料集的重要屬性清單。
若要建構，請參閱 KEYPROPERTY 屬性的 NOTES 區段，然後建立雜湊表。
...
KEYPROPERTY <IReferenceDataSetKeyProperty[]>：參照資料集的重要屬性清單。
  - `[Name <String>]`：鍵屬性的名稱。
  - `[Type <ReferenceDataKeyPropertyType?>]`：鍵屬性的類型。
