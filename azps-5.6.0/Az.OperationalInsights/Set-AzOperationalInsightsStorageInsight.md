...
## 描述
**Set-AzOperationalInsightsStorageInsight** Cmdlet 會變更儲存深入資訊設定。
...
此命令會修改名為 MyStorageInsight 的儲存深入資訊讀取資料表。

### 範例 2：使用工作區物件修改儲存深入資訊
```
...
第二個命令會修改名為 MyStorageInsight 的 Storage Insight 讀取的容器。
