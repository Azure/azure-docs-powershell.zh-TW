...
## 描述
**New-AzOperationalInsightsDatasyslogDataSource** Cmdlet 會將 syslog 資料來源新增到工作區中已連接的 Linux 電腦。
Azure 營運深入資訊可以收集 syslog 資料。
...
### -名稱
指定資料來源的名稱。 名稱不會在 Azure 入口網站中公開，而且任何字串都可以使用，只要它是唯一的。
...
[Enable-AzOperationalInsights](./Enable-AzOperationalInsightsLinuxSyslogCollection.md)
