...
## 描述
**Remove-AzOperationalInsightsWorkspace** Cmdlet 會刪除現有的工作區。
如果此工作區是在建立時透過 *CustomerId* 參數連結至現有帳戶，則原始帳戶不會在營運深入資訊入口網站中刪除。
...
此命令會從名為 ContosoResourceGroup 的資源群組移除名為 MyWorkspace 的工作區。
...
此命令使用 Get-AzOperationalInsightsWorkspace Cmdlet 取得名為 MyWorkspace 的工作區，然後使用管線運算子將其移除，將其傳遞給 **Remove-AzOperationalInsightsWorkspace** Cmdlet。
由於 *已指定 Force* 參數，因此移除工作區之前，命令不會提示您。
