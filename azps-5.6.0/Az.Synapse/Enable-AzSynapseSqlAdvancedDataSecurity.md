...
## 描述
**Enable-AzSynapseSqlAdvancedDataSecurity** Cmdlet 可在工作區上啟用進位資料安全性。 進位資料安全性是一個統一的安全性套件，其中包含適用于您工作區的資料分類、弱點評估及進一威脅防護。  (儲存弱點評定時，系統會自動建立新的儲存空間帳戶。 如果儲存空間帳戶先前已建立為此用途，將會改為) 
...
此命令可透過管線啟用工作區進位資料安全性，而且不會自動啟用弱點評定。
