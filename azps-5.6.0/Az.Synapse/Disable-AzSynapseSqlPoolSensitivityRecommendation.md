...
## 描述
Cmdlet Disable-AzSynapseSqlPoolSensitivityRecommendation停用 (會) SQL 集區中欄的敏感度建議。

## 例子

### 範例 1：停用 Azure Synapse SQL 資料庫中特定資料行的敏感度建議。
```powershell
...
### 範例 2：停用在 Azure Synapse SQL 集區使用管道提供敏感度建議的資料行敏感度建議。
```powershell
...
### 範例 3：使用管道在 Azure Synapse SQL 資料庫中停用特定資料行的敏感度建議。
```powershell
