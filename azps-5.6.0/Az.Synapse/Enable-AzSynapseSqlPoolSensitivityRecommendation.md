...
## 簡介
啟用欄的敏感度建議 (在 SQL 資料庫內的所有資料行上預設) 啟用建議。
...
## 描述
此Enable-AzSynapseSqlPoolSensitivityRecommendation Cmdlet 可在 SQL 資料庫的欄上啟用敏感度建議。

## 例子

### 範例 1：在 Azure Synapse SQL 資料庫的一個給定資料行上啟用敏感度建議。
```powershell
...
### 範例 2：使用管道在給定資料行 Azure Synapse SQL 資料庫上啟用敏感度建議。
```powershell
