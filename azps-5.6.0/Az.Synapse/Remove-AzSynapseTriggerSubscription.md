...
## 描述
**Remove-AzSynapseTriggerSubscription** Cmdlet 會取消訂閱觸發程式所指定外部服務事件的事件觸發程式。
...
此命令會取消訂閱名為 ContosoTrigger 的觸發程式，以從觸發程式觸發指定事件。
...
此命令會從觸發程式透過管道取消訂閱，將名為 ContosoTrigger 的事件觸發至指定的事件。
