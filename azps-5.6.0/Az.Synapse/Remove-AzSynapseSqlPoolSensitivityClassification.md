...
## 描述
Cmdlet Remove-AzSynapseSqlPoolSensitivityClassification移除 SQL 資料庫中欄的資訊類型和敏感度標籤。

## 例子

### 範例 1：移除 Azure Synapse SQL 資料庫中欄的資訊類型和敏感度標籤。
```powershell
...
### 範例 2：使用管道移除 Azure Synapse SQL 集區中欄的目前資訊類型和敏感度標籤。
```powershell
...
### 範例 3：使用管道移除 Azure Synapse SQL 資料庫中欄的資訊類型和敏感度標籤。
```powershell
