...
## 描述
**Start-AzSynapseTrigger Cmdlet** 會啟動工作區中的觸發程式。 如果觸發程式為 '已停止' 狀態，Cmdlet 會啟動觸發程式，最後會根據定義來叫用管線。 如果觸發已進入 '開始' 狀態，此 Cmdlet 將無效。
...
在工作區 ContosoWorkspace 中透過管線啟動名為 ContosoTrigger 的觸發程式。
