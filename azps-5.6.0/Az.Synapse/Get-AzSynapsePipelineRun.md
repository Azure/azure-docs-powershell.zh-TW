...
## 描述
**Get-AzSynapsePipelineRun** 命令會針對指定的管線，會返回執行相關資訊。 如果指定 PipelineRunId，它會以該識別碼顯示執行詳細資料。 如果未指定 PipelineRunId，則會顯示 RunStartedAfter 和 RunStartedBefore 值之間發生之管線的所有執行資訊。
...
此命令會獲得使用 ID "61eb095a-fe23-4591-8a97-fade6c65ca72" 執行管線的詳細資訊。
