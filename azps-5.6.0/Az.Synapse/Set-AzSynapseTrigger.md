...
## 描述
**Set-AzSynapseTrigger Cmdlet** 會建立工作區中的觸發程式。 觸發會以 '已停止' 狀態建立，這表示它們不會立即開始叫用他們參照的管道。
...
在工作區 ContosoWorkspace 中建立名為 ContosoTrigger 的新觸發程式。 觸發會以 '已停止' 狀態建立，這表示它不會立即開始。 可以使用 `Start-AzDataFactoryV2Trigger` Cmdlet 啟動觸發程式。
...
透過管線在工作區 ContosoWorkspace 中建立名為 ContosoTrigger 的新觸發程式。 觸發會以 '已停止' 狀態建立，這表示它不會立即開始。 可以使用 `Start-AzDataFactoryV2Trigger` Cmdlet 啟動觸發程式。
