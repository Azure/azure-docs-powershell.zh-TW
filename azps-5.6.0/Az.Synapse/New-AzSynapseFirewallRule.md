...
## 描述
**New-AzSynapseFirewallRule** Cmdlet 會建立 Azure Synapse Analytics Firewall Rule。
...
此命令在工作區 ContosoWorkspace 下建立名為 ContosoFirewallRule 的防火牆規則，並命名為 ContosoFirewallRule。
...
此命令會透過管線在工作區下建立名為 ContosoFirewallRule 的防火牆規則。
...
此命令會建立允許工作區下所有 azure ip 的防火牆規則。

## 參數

### -AllowAllAzureIP
建立允許所有 Azure IP 存取的特殊防火牆規則。
