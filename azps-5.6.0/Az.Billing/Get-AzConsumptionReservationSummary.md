...
## 描述
**Get-AzConsumptionReservationSummary Cmdlet** 會取得每日或每月谷類的預約摘要。

## 例子

### 範例 1：取得具有每月谷類預定訂單識別碼的保留摘要
```powershell
...
### 範例 2：取得保留訂單識別碼的預約摘要，以及每月谷類的預約識別碼
```powershell
...
### 範例 3：取得包含每日谷類提供日期範圍的保留訂單識別碼的預約摘要
```powershell
...
### 範例 4：取得包含預訂訂單識別碼的預約摘要，以及每日提供日期範圍的保留識別碼
```powershell
...
### -EndDate
保留摘要 (UTC) YYYY-MM-DD 的結束資料，僅適用于每日谷類。
...
### -StartDate
保留摘要 (UTC) YYYY-MM-DD 的起始資料，僅適用于每日谷類。
