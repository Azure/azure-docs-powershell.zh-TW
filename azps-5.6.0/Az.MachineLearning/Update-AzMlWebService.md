...
在這裡，我們變更描述、主存取鍵，並啟用 Web 服務執行時間期間所有追蹤的診斷集合。
...
範例會先建立只包含要更新之欄位的 Web 服務定義，然後呼叫 Update-AzMlWebService使用 ServiceUpdates 參數來將它們進行申請。

## 參數

### -資產
資產集合 (例如模組、資料集) 組成 Web 服務。
...
### -IsReadOnly
指定此 Web 服務為唯讀。
設定完成後，Web 服務可以更新更久，包括變更此屬性的值，而且只能刪除。
...
### -參數
為 Web 服務定義的一組全域參數值，以全域參數名稱做為預設值 \> 集合。
如果沒有指定預設值，則視為需要參數。
...
### -ServiceUpdates
一組更新，以套用至以 Web 服務定義實例提供的 Web 服務。
只有非靜態欄位會修改。
