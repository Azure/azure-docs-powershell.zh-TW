...
## 描述
**Update-AzSentinelBookmark** Cmdlet 會更新指定工作區中的書簽。
您可以傳遞 **書簽** 物件做為參數，或是使用管線運算子，或者您也可以指定所需的 *BookmarkId* 參數。
...
命令會設定 Notes 屬性來 *更新書簽* 。  所有其他的排泄都保持不變。
...
第一個命令會從指定的工作區獲得 *BookmarkId* 書簽，然後將它儲存在$Bookmark變數中。
第二個命令會更新 Notes 屬性。   所有其他的排泄都保持不變。
