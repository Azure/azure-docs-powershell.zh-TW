...
## 描述
**Get-AzSentinelAlertRuleTemplate** Cmdlet 會從指定的工作區取得警示規則範本。
如果您指定 *AlertRuleTemplateId* 參數，會返回單 **一 AlertRuleTemplate** 物件。
如果您未指定 *AlertRuleTemplateId* 參數，則會返回包含指定工作區中所有警示規則範本的陣列。
您可以使用 **AlertRuleTemplate 物件** 來建立新警示規則。
...
此範例會獲得指定工作區中所有 **AlertRuleTemplates，** 然後將它儲存在 $AlertRuleTemplates 變數中。
...
此範例會獲得指定工作區中的 **AlertRuleTemplate，** 然後將它儲存在 $AlertRuleTemplate 變數中。
