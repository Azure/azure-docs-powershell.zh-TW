...
## 描述
**Update-AzSentinelDataConnector** Cmdlet 會更新指定工作區中的資料連線器。
您可以將 **DataConnector 物件** 傳遞為參數，或是使用管線運算子，或者您也可以指定所需的參數。
...
該命令會按 *DataConnectorId* 獲得資料連線器，並且將 *通知狀態設定* 為 *已停用*。  所有其他屬性會保持相同。
