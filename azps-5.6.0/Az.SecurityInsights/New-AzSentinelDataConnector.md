...
您必須指定其中一個參數 ，例如 -AzureActiveDirectory，以指定要建立的警示規則類型。  每種類型都有不同的所需參數。
您可以使用確認 *參數$ConfirmPreference* Windows PowerShell 變數來控制 Cmdlet 是否提示您確認。
注意：並非入口網站中所有可用的資料連線器都可透過 API 進行連接。
...
此範例會針對指定的工作區建立 Azure 資訊安全中心的 **DataConnector，** 然後將它儲存在 $DataConnector 變數中。
...
此範例會為 Microsoft Cloud App Security 在指定的工作區中建立 **DataConnector，** 然後將它儲存在$DataConnector變數。
...
### -MicrosoftCloudAppSecurity
Data Connector Microsoft Cloud App 安全性
...
### -MicrosoftDefenderAdvancedThreatProtection
Data Connector Microsoft Defender 進位威脅防護
