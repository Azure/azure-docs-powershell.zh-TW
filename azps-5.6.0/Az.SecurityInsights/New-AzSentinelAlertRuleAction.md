...
## 描述
**New-AzSentinelAlertRuleAction** Cmdlet 會建立指定工作區中警示規則的自動回應。
您必須提供邏輯應用程式重新連接識別碼和觸發 Uri，您可以使用邏輯應用程式模組找到。
...
此範例會使用邏輯應用程式的屬性，為指定的警示規則建立 **AlertRuleAction，** 然後將它儲存在 $AlertRuleAction 變數中。
