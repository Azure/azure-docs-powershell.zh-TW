...
## 描述
**Get-AzSentinelBookmark** Cmdlet 會從指定的工作區取得書簽。
如果您指定 *BookmarkId* 參數，會返回單 **一書簽** 物件。
如果您未指定 *BookmarkId* 參數，會返回包含指定工作區中所有書簽的陣列。
您可以使用書簽 **物件** 來更新書簽，例如，您可以新增 **筆記書簽。**
...
此範例會獲得指定工作區中所有的書簽，然後將它儲存在 $Bookmarks 變數中。
...
此範例 **會獲得指定** 工作區中的書簽，然後將它儲存在$Bookmark變數。
