...
使用已定義參數和資源群組字典，在指定的訂閱中建立藍圖定義 `$blueprintObject` 的新藍圖指派。 使用系統指派的身分識別。 位置會定義建立受管理身分識別的區域。
...
使用已定義參數和資源群組字典，並設定資源鎖定至 `$blueprintObject` **AllResources，** 來建立指定訂閱中藍圖定義的新藍圖指派。 預設為使用系統指派的身分識別。  位置會定義建立受管理身分識別的區域。
...
使用已定義參數和資源群組字典，使用指定的使用者指派身分識別識別碼，在指定的訂閱內建立藍圖定義 `$blueprintObject` 的新藍圖指派。
...
透過作業檔案建立藍圖作業。 工作分派檔案格式可在要求/回應範例中找到： https://github.com/Azure/azure-rest-api-specs/tree/master/specification/blueprint/resource-manager/Microsoft.Blueprint/preview/2018-11-01-preview/examples
...
使用已定義參數建立藍圖定義的新藍圖指派，以指定管理群組內的指定 `$blueprintObject` 訂閱為目標。

## 參數

### -AssignmentFile
作業檔案在磁片上以 JSON 格式顯示的位置。
