...
## 描述
**Get-AzSqlSyncGroup** Cmdlet 會返回有關一或多個 Azure SQL 資料庫同步群組的資訊。
指定同步組的名稱，只查看該同步組的資訊。

## 例子

### 範例 1：取得指派給 Azure SQL Database 的 Azure SQL 同步處理群組的所有實例
```
...
此命令會獲得所有指派給 Azure SQL Database 的 Azure SQL 資料庫同步群組相關資訊。

### 範例 2：取得 Azure SQL 資料庫同步組相關資訊
```
...
此命令會獲得名稱為「SyncGroup01」的 Azure SQL 資料庫同步處理群組相關資訊

### 範例 3：使用篩選將 Azure SQL 同步處理群組的所有實例指派給 Azure SQL 資料庫
```
...
此命令會獲得指派給以「同步處理群組」為開始之 Azure SQL 資料庫之所有 Azure SQL 資料庫同步處理群組的資訊。
