...
## 描述
Cmdlet Remove-AzSqlDatabaseSensitivityClassification移除資料庫中資料行的資訊類型和敏感度標籤。

## 例子

### 範例 1：移除 Azure SQL Database 中欄的資訊類型和敏感度標籤。
```powershell
...
### 範例 2：使用管道移除 Azure SQL 資料庫中資料行的目前資訊類型和敏感度標籤。
```powershell
...
### 範例 3：使用管道移除 Azure SQL 資料庫中資料行的資訊類型和敏感度標籤。
```powershell
