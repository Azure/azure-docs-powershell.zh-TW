...
## 描述
**Set-AzSqlDatabaseBackupLongTermRetentionPolidlet** 會設定已登錄至此資料庫的長期保留原則。
此策略是 Azure 備份資源，用來定義備份儲存策略。

## 例子

### 範例 1：設定目前長期保留政策版本的每週保留
```powershell
...
這會設定資料庫01 的長期保留原則，將每週完整備份儲存 2 周

### 範例 2：設定目前長期保留政策版本的每月保留
```powershell
...
這會設定資料庫01 的長期保留政策，將每個月的第一次完整備份儲存 5 年

### 範例 3：設定目前長期保留政策版本的每年保留
```powershell
...
這會設定資料庫01 的長期保留政策，以將一年的第 26 周所執行的完整備份儲存 10 年

### 範例 4：針對目前版本的長期保留政策設定每個保留
```powershell
...
這會設定資料庫01 的長期保留原則，將每個完整備份儲存 14 天、每個月的第一次完整備份為 24 周，以及 10 年一年的第 26 周執行完整備份
...
移除資料庫01 的策略，如此一來就不會再存下任何長期保留備份。
這不會影響已採取的備份
...
這是移除資料庫01 之策略的另一種方式，因此不會再節省任何長期保留備份。
這不會影響已採取的備份
...
### -MonthlyRetention
每月保留。
如果只傳遞數位而非 ISO 8601 字串，則天數會假設為單位。
最短為 7 天，最多 10 年。
