...
## 簡介
更新同步成員資料庫或同步中樞資料庫的同步架構。
它會從實際資料庫取得最新的資料庫架構，然後使用它重新組織同步中繼資料資料庫所快回的架構。
如果指定 「SyncMemberName」，它會重新組織成員資料庫架構;如果沒有，它會重新組織中樞資料庫架構。
...
## 描述
**Update-AzSqlSyncSchema** Cmdlet 會更新同步成員資料庫或同步中樞資料庫的同步架構。
...
此命令會更新同步群組 SyncGroup01 中中樞資料庫的同步架構
...
此命令會更新同步成員 SyncMember01 中成員資料庫的同步架構
