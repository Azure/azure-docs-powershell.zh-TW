...
## 簡介
移除 Azure SQL 受管理實例資料庫中的資料行資訊類型和敏感度標籤。
...
## 描述
此Remove-AzSqlInstanceDatabaseSensitivityClassification Cmdlet 會移除 Azure SQL 受管理實例資料庫中欄的資訊類型和敏感度標籤。

## 例子

### 範例 1：在 Azure SQL 受管理的實例資料庫中移除資料行的資訊類型和敏感度標籤。
```powershell
...
### 範例 2：在 Azure SQL 受管理的實例資料庫中使用管道移除欄的目前資訊類型和敏感度標籤。
```powershell
...
### 範例 3：在 Azure SQL 受管理的實例資料庫中，使用管道移除資料行的資訊類型和敏感度標籤。
```powershell
