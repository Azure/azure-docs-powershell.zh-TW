...
## 簡介
移除 Azure SQL 伺服器的 Microsoft 支援作業稽核設定。
...
## 描述
**Remove-AzSqlServerMSSupportAudit** Cmdlet 會移除 Azure SQL 伺服器的 Microsoft 支援作業稽核設定。
若要使用 Cmdlet，請使用 *ResourceGroupName* 和 *ServerName* 參數來識別伺服器。

## 例子

### 範例 1：移除 Azure SQL Server 的 Microsoft 支援作業稽核設定
```powershell
PS C:\>Remove-AzSqlServerMSSupportAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01"
```

### 範例 2：透過管線移除 Azure SQL Server 的 Microsoft 支援稽核設定
```
