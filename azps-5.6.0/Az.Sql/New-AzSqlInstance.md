...
## 描述
**New-AzSqlInstance** Cmdlet 會建立 Azure SQL Database Managed 實例。
...
此命令會使用 Edition 和 ComputeGeneration 參數來建立新實例。

### 範例 3：使用實例資料庫物件在實例資料庫內建立新實例
```powershell
...
此命令會使用實例資料庫物件在實例資料庫內建立新實例。

### 範例 4：在實例資料庫使用實例資料庫資源識別碼建立新實例
```powershell
...
此命令會使用實例資料庫的資源識別碼，在實例資料庫內建立新實例。
...
此命令會使用 name instancePool0 在實例資料庫建立新實例
...
### -AssignIdentity
為此 Managed 實例產生並指派 Azure Active Directory 身分識別，以用於 Azure KeyVault 等重要管理服務。
...
用來儲存 Sql Azure Managed 實例之備份的備份儲存備份的備份儲存空間。 選項為：本地、區域及地理位置
...
### -DnsZonePartner
從受管理實例建立繼承 DnsZone 屬性的合作夥伴 Managed Server 資源識別碼
...
### -LicenseType
決定要使用哪一種授權類型。 可能的值有：
- BasePrice - Azure 混合式權益 (AHB) 現有 SQL Server 授權擁有者的折扣價格。 受管理的實例服務價格會針對現有的 SQL Server 授權擁有者折扣。
- LicenseIncluded - Azure 混合式權益 (AHB) 現有 SQL Server 授權擁有者的折扣價格不會適用。 受管理的實例服務價格會包含新的 SQL Server 授權成本。
...
### -TimezoneId
要設定實例的時區識別碼。 時區識別碼清單會透過 Transact-SQL sys.time_zone_info (顯示) 顯示。
