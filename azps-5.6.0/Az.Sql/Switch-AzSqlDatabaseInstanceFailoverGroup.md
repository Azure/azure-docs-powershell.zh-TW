...
## 描述
此命令會容錯移轉到指定的次要區域，以交換實例容錯移轉群組中受管理實例的角色，使其成為新的主要區域。 所有連接到主要端點的新 TDS 會話都會自動重新路由至新的主要區域。 
...
### -AllowDataLoss
即使執行此操作可能會導致資料遺失，也請完成容錯移轉。
這樣即使主資料庫無法使用，也允許容錯移轉繼續進行。
