...
## 簡介
在 Azure SQL Database 中獲取彈性資料庫及其屬性值。
...
## 描述
**Get-AzSqlElasticPool** Cmdlet 會取得配置區及其屬性值。
指定現有資料庫資料庫的名稱，僅查看該資料庫的屬性值。
...
此命令會在伺服器上名為 Server01 的一個名為Pool0127 的彈性資料庫。

### 範例 3：取得 Azure SQL 準備資料庫的度量
```
...
此命令會針對名為 CommandPool01 的 Azure SQL 彈性資料庫，會返回計量。

### 範例 4：使用篩選取得所有篩選資料庫 -PoolName "PoolPool*"
```
PS C:\>Get-AzSqlElasticPool -ResourceGroupName "ResourceGroup01" -ServerName "Server01"
ResourceId        : /subscriptions/00000000-0000-0000-0000-000000000001/resourceGroups/resourcegroup01/providers/Microsoft.Sql/servers/server01/elasticPools/elasticpool01
ResourceGroupName : resourcegroup01
...
此命令會獲得伺服器名稱為 Server01 的所有以 「PoolPool」為起始的彈性資料庫。
