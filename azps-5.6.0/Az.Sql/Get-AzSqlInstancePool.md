...
### GetInstancePoolByStancePoolResourceIdentifierParameterSet
```
Get-AzSqlInstancePool [-ResourceId] <String> [-DefaultProfile <IAzureContextContainer>] [<CommonParameters>]
```

## 描述
**Get-AzSqlInstancePool** Cmdlet 會傳回一或多個 Azure SQL 實例資料庫相關資訊。
指定實例資料庫的名稱，只查看該實例資料庫的資訊。
...
此命令會獲得實例資料庫實例Pool0 相關資訊。

### 範例 4：使用實例資料庫資源識別碼取得實例資料庫相關資訊
```powershell
