...
## 描述
**Enable-AzSqlServerAdvancedDataSecurity** Cmdlet 可在伺服器上啟用進位資料安全性。 進位資料安全性是一個統一的安全性套件，其中包含適用于伺服器的資料分類、弱點評估及進一威脅防護。  (儲存弱點評定時，系統會自動建立新的儲存空間帳戶。 如果儲存空間帳戶先前已建立為此用途，將會改為) 
