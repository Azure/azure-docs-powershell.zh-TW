...
## 描述
**Set-AzSqlInstancePool** Cmdlet 會修改 Azure SQL 實例集區的屬性。
...
此命令會設定名稱為 instancePool0 的實例集區之授權類型和/或標籤。

### 範例 2：使用實例集區物件設定實例集區授權類型
```powershell
...
此命令會使用實例集區物件來設定實例集區授權類型和/或標記。

### 範例 3：使用實例資料庫資源識別碼設定實例集區授權類型
```powershell
...
### -LicenseType
決定要使用哪一種授權類型。
可能的值為 BASEPrice (具有AHB 折扣) 且 LicenseIncluded (不含AHB 折扣) 。
