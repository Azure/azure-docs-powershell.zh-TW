...
## 描述
**Get-AzSqlServer** Cmdlet 會返回一或多個 Azure SQL 資料庫伺服器相關資訊。
指定伺服器名稱，只查看該伺服器的資訊。

## 例子

### 範例 1：取得指派給資源群組之 SQL Server 的所有實例
```
...
此命令會獲得指派給資源群組 ResourceGroup01 的所有 Azure SQL 資料庫伺服器相關資訊。

### 範例 2：取得 Azure SQL Database 伺服器相關資訊
```
...
此命令會獲得名稱為 Server01 的 Azure SQL Database 伺服器相關資訊。

### 範例 3：取得訂閱中 SQL Server 的所有實例
```
...
此命令會獲得目前訂閱中所有 Azure SQL Database 伺服器的資訊。

### 範例 4：使用篩選將 SQL Server 的所有實例指派給資源群組
```
...
此命令會獲得指派給以「伺服器」為起始資源群組 ResourceGroup01 之所有 Azure SQL 資料庫伺服器的資訊。
