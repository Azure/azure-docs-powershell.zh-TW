...
## 描述
**Set-AzSqlInstance Cmdlet** 會修改 Azure SQL Database Managed 實例的屬性。

## 例子

### 範例 1：使用 -AdministratorPassword、-LicenseType、-StorageSizeInGB、-VCore 和 -Edition 的新值設定現有實例
```powershell
...
### 範例 2：使用 -ComputeGeneration 的新值變更現有的實例硬體產生
```powershell
...
此命令會使用 -AdministratorPassword、-LicenseType、-StorageSizeInGB 和 -VCore 的新值來設定現有實例

### 範例 3：針對實例集區中的實例，使用 -AdministratorPassword、-LicenseType、-StorageSizeInGB 和 -VCore 的新值設定現有實例
```powershell
PS C:\>$InstancePassword = "Newpassword1234"
PS C:\> $SecureString = ConvertTo-SecureString $InstancePassword -AsPlainText -Force
...
此命令會針對實例集區中的實例，使用 -AdministratorPassword、-LicenseType、-StorageSizeInGB 和 -VCore 的新值來設定現有實例
...
### -AssignIdentity
為此實例產生並指派 Azure Active Directory 身分識別，以用於 Azure KeyVault 等重要管理服務。
