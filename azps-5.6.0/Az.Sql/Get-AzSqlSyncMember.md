...
## 描述
**Get-AzSqlSyncMember Cmdlet** 會返回有關一或多個 Azure SQL 資料庫同步成員的資訊。
指定同步成員的名稱，只查看該同步成員的資訊。

## 例子

### 範例 1：取得指派給同步處理群組之 Azure SQL 同步處理成員的所有實例
```
...
此命令會獲得所有指派給同步群組 SyncGroup01 的 Azure SQL 資料庫同步成員相關資訊。

### 範例 2：取得 Azure SQL 資料庫同步成員相關資訊
```
...
此命令會獲得名稱為「SyncMember01」的 Azure SQL 資料庫同步處理成員相關資訊

### 範例 3：使用篩選將 Azure SQL 同步處理成員的所有實例指派給同步處理群組
```
...
此命令會獲得所有指派給同步處理群組 SyncGroup01 的 Azure SQL 資料庫同步處理成員相關資訊，該群組以「SyncMember」為開始。
