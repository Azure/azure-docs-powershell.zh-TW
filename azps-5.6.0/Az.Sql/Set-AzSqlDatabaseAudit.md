...
## 描述
**Set-AzSqlDatabaseAudit** Cmdlet 會變更 Azure SQL 資料庫的稽核設定。
若要使用 Cmdlet，請使用 *ResourceGroupName、ServerName* 和 *DatabaseName* 參數來識別資料庫。
當 Blob 儲存體是稽核記錄的目標時，請指定 *StorageAccountResourceId* 參數，以決定稽核記錄中的儲存帳戶，以及 *StorageKeyType* 參數來定義儲存金鑰。 您也可以設定 *RetentionInDays* 參數的值來定義稽核記錄期間，以定義稽核記錄保留。

## 例子

### 範例 1：啟用 Azure SQL 資料庫的 Blob 儲存稽核策略
```powershell
...
### 範例 2：停用 Azure SQL 資料庫的 Blob 儲存稽核策略
```powershell
...
### 範例 3：使用 T-SQL 詞詞啟用 Azure SQL 資料庫的 Blob 儲存稽核策略與篩選
```powershell
...
### 範例 5：啟用 Azure SQL 資料庫的事件中樞稽核策略
```powershell
...
### 範例 6：停用 Azure SQL 資料庫的事件中樞稽核策略
```powershell
...
### 範例 7：啟用 Azure SQL 資料庫的記錄分析稽核策略
```powershell
...
### 範例 8：停用 Azure SQL 資料庫的記錄分析稽核策略
```powershell
...
### 範例 9：透過管線停用 Azure SQL 資料庫的記錄分析稽核策略
```powershell
...
### 範例 10：停用將 Azure SQL 資料庫的稽核記錄傳送至 Blob 儲存空間，並啟用傳送記錄分析。
```powershell
...
### 範例 11：啟用將 Azure SQL 資料庫的稽核記錄傳送至 Blob 儲存體、事件中樞和記錄分析。
```powershell
...
收到  
引用  
定義要稽核之動作的一般形式為：[action] ON [object] BY [principal] 請注意，上述格式的 [物件] 可參照資料表、視圖或儲存程式等物件，或是整個資料庫或架構。 在後一種情況下，會分別使用表單 DATABASE：：[dbname] 和 SCHEMA：：[schemaname]。
例如：  
在 dbo.myTable 上按公用 SELECT  
DATABASE 上的 SELECT：myDatabase by public  
SCHEMA 上的 SELECT：mySchema by public  
詳細資訊請參閱 https://docs.microsoft.com/sql/relational-databases/security/auditing/sql-server-audit-action-groups-and-actions#database-level-audit-actions 。
...
### -AuditActionGroup
建議使用的動作群組組合為下列組合：這會稽核對資料庫執行的所有查詢和儲存程式，以及成功和失敗的登入：  
  
"BATCH_COMPLETED_GROUP"，  
"SUCCESSFUL_DATABASE_AUTHENTICATION_GROUP"，  
「FAILED_DATABASE_AUTHENTICATION_GROUP」  
上述組合也是預設設定的組合。 這些群組涵蓋針對資料庫執行的所有 SQL 語句和儲存程式，而且不應與其他群組搭配使用，因為這樣會導致重複的稽核記錄。
詳細資訊請參閱 https://docs.microsoft.com/sql/relational-databases/security/auditing/sql-server-audit-action-groups-and-actions#database-level-audit-action-groups 。
...
### -EventHubName
事件中樞的名稱。 如果在提供 EventHubAuthorizationRuleResourceId 時未指定任何專案，將會選取預設的事件中樞。
...
### -WorkspaceResourceId
工作區識別碼 (Log Analytics 工作區的資源識別碼) Log Analytics 工作區的資源識別碼，以傳送稽核記錄。 範例：/subscriptions/4b9e8510-67ab-4e9a-95a9-e2f1e570ea9c/resourceGroups/insights-integration/providers/Microsoft.OperationalInsights/workspaces/viruela2
