...
## 描述
**New-AzSqlServer** Cmdlet 會建立 Azure SQL Database 伺服器。

## 例子

### 範例 1：建立新 Azure SQL Database 伺服器
```
...
此命令會建立版本 12 Azure SQL Database 伺服器。
...
### -ServerVersion
指定新伺服器的版本。 此參數可接受的值為：2.0 和 12.0。
指定 2.0 以建立版本 11 伺服器，或指定 12.0 以建立版本 12 伺服器。
...
指定新伺服器的 SQL Database 伺服器系統管理員認證。 若要取得 **PSCredential 物件** ，請使用 Get-Credential Cmdlet。 如需要詳細資訊，請鍵入 `Get-Help
Get-Credential` 。
