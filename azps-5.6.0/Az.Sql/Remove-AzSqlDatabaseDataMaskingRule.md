...
## 描述
**Remove-AzSqlDatabaseDataMaskingRule** Cmdlet 會從 Azure SQL 資料庫移除特定的資料遮罩規則。
您可以使用 ResourceGroupName、ServerName、DatabaseName和 *RuleId* 參數來移除資料遮罩規則，以識別此 Cmdlet 移除的規則。 
...
此命令會移除為資料庫 Database01 定義的規則名稱 Rule01。
資料庫位於 Server01，並指派給資源群組 ResourceGroup01。
