...
## 描述
**Get-AzSqlServerAdvisor** Cmdlet 會為 Azure SQL Server 取得一或多個 Azure SQL Server 建議程式。
...
此命令會獲得名稱為 wi-runs-australia-east 伺服器的所有建議程式清單，該伺服器屬於名為 WIRunnersProd 的資源群組。
...
此命令會針對名為 wi-runner-australia-east 的伺服器，獲得名為 CreateIndex 的顧問。
...
此命令會獲得伺服器名稱為 wi-runner-australia-east 的所有建議程式。
由於命令使用 *ExpandRecomactionActions* 參數，因此 Cmdlet 會獲得回應中包含的建議動作。
...
此命令會獲得名稱為 wi-run-australia-east 的伺服器之所有顧問清單，該伺服器所屬的資源群組名為 WIRunnersProd，其開頭為 "d"。
