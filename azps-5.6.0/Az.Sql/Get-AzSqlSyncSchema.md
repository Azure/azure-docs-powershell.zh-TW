...
## 描述
**Get-AzSqlSyncSchema Cmdlet** 會返回成員資料庫或中樞資料庫的同步架構相關資訊。
...
此命令會獲得同步群組同步組同步組01 中中樞資料庫的同步架構。

### 範例 2：取得中樞資料庫的同步架構，然後展開資料表
```powershell
...
此命令會針對同步群組 SyncGroup01 中的中樞資料庫，獲得同步架構架構，並展開資料表屬性。
...
此命令會獲得同步成員 SyncMember01 中成員資料庫的同步架構。
