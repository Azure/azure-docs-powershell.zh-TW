...
## 描述
**New-AzSqlDatabaseCopy** Cmdlet 會建立使用目前資料快照的 Azure SQL 資料庫的一份副本。 使用此 Cmdlet 而非 Start-AzSqlDatabaseCopy Cmdlet 來建立一次資料庫副本。 此 Cmdlet 會 **返回複製的 Database** 物件。
注意：使用 New-AzSqlDatabaseSecondary Cmdlet 設定資料庫的異地複製。
...
### -標記
以雜湊表的形式指定要與 Azure SQL 資料庫副本建立關聯的索引鍵值組。 例如：@{key0="value0";key1=$null;key2="value2"}
...
### -VCore
Azure Sql Database 副本的 Vcore 數位。
