...
## 描述
Cmdlet Invoke-AzSqlElasticPoolFailover容錯移轉會建立一個資料庫。 執行此 Cmdlet 之後，資料庫集中的所有資料庫都會發生容錯移轉。
...
此命令將會容錯移轉名為「Server01」伺服器上名為「Pool01」的集中資料庫。  這表示容錯移轉會發生在名為「Pool01」的集中資料庫的所有資料庫中。
...
### -ServerName
位於 Azure SQL Server 的 Azure Sql Server 中 ，即是其中一個資料庫。
