...
## 描述
**Get-AzSqlDatabaseAdvisor** Cmdlet 會針對 Azure SQL 資料庫取得一或多個 Azure SQL 資料庫建議程式。
...
此命令會列出名為 WIRunner 之資料庫的所有建議程式，該伺服器名稱為 wi-run-australia-east。
...
此命令會針對名為 WIRunner 的資料庫，獲得名為 CreateIndex 的顧問。
...
此命令會獲得名為 "WIRunner" 之資料庫的所有建議程式，並提供回應中的建議動作。
由於命令使用 *ExpandRecomactionActions* 參數，因此 Cmdlet 會隨著回應獲得建議的動作。
...
此命令會從名為 WIRunner 的資料庫獲得名為 CreateIndex 的建議程式，其建議動作會包含在回應中。
由於命令使用 *ExpandRecomactionActions* 參數，因此 Cmdlet 會隨著回應獲得建議的動作。

### 範例 5：使用篩選列出指定資料庫的所有建議程式
```
...
此命令會列出名為 WIRunner 之資料庫的所有建議程式，該伺服器名稱為 wi-run-australia-east，開頭為 "d"。
...
## 筆記
* 關鍵字：azure、azurerm、arm、resource、management、manager、sql、database、mssql、advisor
...
[Get-AzSqlDatabaseRecomactionAction](./Get-AzSqlDatabaseRecommendedAction.md)
