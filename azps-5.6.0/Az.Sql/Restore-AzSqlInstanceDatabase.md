...
## 描述
**Restore-AzSqlInstanceDatabase** Cmdlet 會從異地重複備份、即時資料庫中的時間點或長期保留備份還原實例資料庫。
還原的資料庫會建立為新的實例資料庫。
...
該命令會從指定的時間點備份將實例資料庫 Database01 還原到名為 Database01_restored 的實例資料庫。

### 範例 2：將實例資料庫從時間點還原至不同資源群組上的另一個實例
```
...
該命令會還原資源群組 ResourceGroup01 上的實例 managedInstance1 實例資料庫 Database01，從指定的時間點備份還原到資源群組 ResourceGroup02 上實例 managedInstance2 上名為 Database01_restored 的實例資料庫。

### 範例 3：Geo-Restore實例資料庫
```
...
第二個命令會還原備份$GeoBackup名為 Database01_restored。

### 範例 4：從時間點還原已刪除的實例資料庫
```
...
第一個命令會獲得實例 'managedInstance1' 上名為 'DB1' 的已刪除實例資料庫。
第二個命令會從指定的時間點備份還原到名為 Database01_restored 的實例資料庫。

### 範例 5：從時間點還原已刪除的實例資料庫
```
PS C:\> $deletedDatabase = Get-AzSqlDeletedInstanceDatabaseBackup -ResourceGroupName "ResourceGroup01" -InstanceName "managedInstance1" -DatabaseName "DB1"
...
第一個命令會獲得實例 'managedInstance1' 上名為 'DB1' 的已刪除實例資料庫。
第二個命令會還原所取資料庫，從指定的時間點備份還原到使用輸入Database01_restored實例資料庫。
...
使用提供的資源識別碼還原 LTR 備份 (執行 Get-AzSqlInstanceDatabaseLongTermRetentionBackup) 。
