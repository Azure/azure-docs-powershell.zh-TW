...
## 簡介
會返回 Azure SQL Managed 實例資料庫相關資訊。
...
## 描述
**Get-AzSqlInstanceDatabase** Cmdlet 會從 Azure SQL Database Managed 實例取得一或多個 Azure SQL 資料庫。
...
此命令會獲得名為 managedInstance1 的實例上的所有資料庫。

### 範例 2：在 Managed 實例上按名稱取得資料庫
```
...
此命令會從 managedInstance1 實例中，獲得名為 managedDatabase1 的資料庫。

### 範例 3：使用篩選取得實例上的所有資料庫
```
...
此命令會獲得名為 managedInstance1 之實例上以 "managedDatabase" 做為開始的所有資料庫。
