...
## 描述
Cmdlet 會Disable-AzSqlInstanceDatabaseSensitivityRecommendation Azure SQL 受管理實例 (中) 欄的敏感度建議。

## 例子

### 範例 1：停用 Azure SQL 受管理實例資料庫中的特定資料行的敏感度建議。
```powershell
...
### 範例 2：停用在 Azure SQL 受管理的實例資料庫中具有使用管道的敏感度建議欄的敏感度建議。
```powershell
...
### 範例 3：在 Azure SQL 受管理的實例資料庫中，使用管道停用特定資料行的敏感度建議。
```powershell
