...
## 描述
此Remove-AzSqlInstanceKeyVaultKey Cmdlet 會從指定的受管理實例移除金鑰庫金鑰。 請注意，SQL 受管理實例對金鑰庫的許可權不會變更。 若要變更許可權，請使用 Set-AzKeyVaultAccessPolicy。 請注意，此 Cmdlet 不會變更金鑰保存庫。 若要從 Key Vault 移除金鑰，請使用 Remove-AzureKeyVaultKey。
...
此命令會從指定的受管理實例移除具有 Id ' ' https://contoso.vault.azure.net/keys/contosokey/01234567890123456789012345678901 的 Key Vault 金鑰。 
