...
## 描述
**Start-AzSqlInstanceDatabaseLogReplay** Cmdlet 會開機記錄重播服務。
...
此命令會建立新的受管理資料庫，並開始從給定容器還原備份，last_backup.bak 還原。
...
此命令會建立新的受管理資料庫，並開始從給定容器還原備份，直到Complete-AzSqlInstanceDatabaseLogReplay最後一次要求備份時再重新叫用。
