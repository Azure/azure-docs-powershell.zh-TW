...
## 簡介
從 Azure SQL Database 容錯移轉群組移除一或多個資料庫。
...
## 描述
從指定的 Azure SQL Database 容錯移轉群組移除一或多個資料庫。 資料庫和複製關係會保持不變，但無法再透過容錯移轉群組端點來便於使用。
...
此命令會從容錯移轉群組移除一個集中資料庫的所有資料庫。

## 參數

### -資料庫
從容錯移轉群組中移除容錯移轉群組主伺服器的一或多個 Azure SQL 資料庫。
