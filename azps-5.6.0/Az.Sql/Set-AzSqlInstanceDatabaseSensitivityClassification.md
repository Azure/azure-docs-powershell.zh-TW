...
## 簡介
在 Azure SQL 受管理的實例資料庫中設定資料行的資訊類型和敏感度標籤。
...
## 描述
此Set-AzSqlInstanceDatabaseSensitivityClassification Cmdlet 會設定 Azure SQL 受管理實例資料庫中欄的資訊類型和敏感度標籤。

## 例子

### 範例 1：在 Azure SQL 受管理的實例資料庫中設定資料行的資訊類型和敏感度標籤。
```powershell
...
### 範例 2：在 Azure SQL 受管理的實例資料庫中設定資料行的建議資訊類型和敏感度標籤。
```powershell
...
### 範例 3：在 Azure SQL 受管理的實例資料庫中，使用管線設定資料行的資訊類型和敏感度標籤。
```powershell
