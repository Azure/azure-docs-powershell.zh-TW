...
## 描述
**New-AzSqlDatabaseImport** Cmdlet 會從 Azure 儲存帳戶將 bacpac 檔案導入新的 Azure SQL 資料庫。
系統可能會送出取得匯出資料庫狀態要求，以取得此要求的狀態資訊。

## 例子

### 範例 1：建立 bacpac 檔案的輸入要求
```
...
### -AuthenticationType
指定用來存取伺服器的驗證類型。
如果未設定驗證類型，此參數會預設為 SQL。
此參數可接受的值為：
- Sql。
SQL 驗證。
將 *AdministratorLogin 和* *AdministratorLoginPassword* 參數設定為 SQL 系統管理員使用者名稱和密碼。 
- ADPassword。
Azure Active Directory 驗證。
將 *AdministratorLogin 和* *AdministratorLoginPassword* 設定為 Azure Active Directory 系統管理員使用者名稱和密碼。
...
指定要指派給 Azure SQL Database 的服務目標名稱。
