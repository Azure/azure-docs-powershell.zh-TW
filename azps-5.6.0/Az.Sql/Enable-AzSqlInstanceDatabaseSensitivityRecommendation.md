...
## 簡介
啟用欄的敏感度建議 (Azure SQL 受管理實例資料庫中所有資料行上預設) 啟用建議。
...
## 描述
Cmdlet Enable-AzSqlInstanceDatabaseSensitivityRecommendation Azure SQL 受管理實例資料庫中的資料行啟用敏感度建議。

## 例子

### 範例 1：在 Azure SQL 受管理的實例資料庫中，為一個資料行啟用敏感度建議。
```powershell
...
### 範例 2：在 Azure SQL 受管理的實例資料庫中，使用管道在一個資料行上啟用敏感度建議。
```powershell
