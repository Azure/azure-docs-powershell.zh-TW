...
## 描述
Cmdlet 會Disable-AzSqlDatabaseSensitivityRecommendation停用 (關閉) 欄的敏感度建議。

## 例子

### 範例 1：停用 Azure SQL Database 中特定資料行的敏感度建議。
```powershell
...
### 範例 2：停用在 Azure SQL 資料庫中使用管道提供敏感度建議的資料行敏感度建議。
```powershell
...
### 範例 3：在 Azure SQL 資料庫中的特定資料行使用管道停用敏感度建議。
```powershell
