...
## 簡介
會返回 Azure SQL Managed Instance 資料庫的重複備份相關資訊。
...
## 描述
**Get-AzSqlInstanceDatabaseGeoBackup** Cmdlet 會從 Azure SQL Database Managed 實例取得一或多個 Azure SQL 資料庫的重複備份。

## 例子

### 範例 1：取得實例上的所有資料庫重複備份
```
...
此命令會為 managedInstance1 實例上的所有資料庫重複備份。

### 範例 2：在 Managed 實例上根據名稱取得資料庫多餘的備份
```
...
此命令會從名為 managedInstance1 的實例獲得名為 managedDatabase1 的資料庫多餘的備份。

### 範例 3：使用篩選在實例上取得所有資料庫多餘的備份
```
...
此命令會獲得名為 managedInstance1 之實例上以 "managedDatabase" 做為開始的所有資料庫多餘的備份。
