...
## 簡介
會返回 Azure SQL Managed 資料庫實例相關資訊。
...
## 描述
**Get-AzSqlInstance Cmdlet** 會傳回一或多個 Azure SQL Managed 實例相關資訊。
指定實例的名稱，只查看該實例的資訊。
...
此命令會獲得指派給資源群組 ResourceGroup01 的所有實例相關資訊。
...
此命令會獲得 managedInstance1 實例相關資訊。

### 範例 3：使用篩選將所有實例指派給資源群組
```powershell
...
此命令會獲得指派給資源群組 ResourceGroup01 以 "managedInstance" 為起始之所有實例的資訊。
...
此命令會獲得實例資料庫 "instancePool0" 內所有實例的資訊。

### 範例 5：使用實例資料庫物件取得實例資料庫內的所有實例
```powershell
...
### 範例 6：使用實例資料庫資源識別碼取得實例資料庫內的所有實例
```powershell
