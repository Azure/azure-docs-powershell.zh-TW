...
## 簡介
此命令會建立一個新的 Azure SQL 資料庫實例容錯移轉群組。
...
## 描述
使用指定的受管理實例組，在指定的區域之間建立新的 Azure SQL 資料庫實例容錯移轉群組。

在 Name.SqlDatabaseDnsSuffix (建立兩個 Azure SQL Database TDS 端點，例如 Name.database.windows.net) 和 Name.secondary.SqlDatabaseDnsSuffix。 這些端點可能分別用來連接容錯移轉群組的主要和次要區域。 如果主要區域受到中斷影響，端點和資料庫的自動容錯移轉就會根據實例容錯移轉群組的容錯移轉政策與寬限期所指示來觸發。

在實例容錯移轉群組功能預覽期間，'-GracePeriodWithDataLossHours'參數僅支援大於或等於 1 小時的值。
...
此命令會針對受管理的實例組建立一個新的實例容錯移轉群組，其容錯移轉策略為 '自動'。
...
此命令會建立一個新的實例容錯移轉群組，並針對受管理的實例組使用容錯移轉策略的'手動'。

### 範例 3

此命令會建立一個新的 Azure SQL 資料庫實例容錯移轉群組。  (自動) 
...
實例容錯移轉群組次要受管理實例的訂閱識別碼。 此參數僅適用于跨訂閱設定
