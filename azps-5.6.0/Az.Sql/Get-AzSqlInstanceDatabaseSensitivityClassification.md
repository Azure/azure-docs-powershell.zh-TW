...
## 簡介
在 Azure SQL 受管理的實例資料庫中，獲得資料行的目前資訊類型和敏感度標籤。
...
## 描述
Cmdlet Get-AzSqlInstanceDatabaseSensitivityClassification Azure SQL 受管理的實例資料庫中，會返回目前的資訊類型和欄的敏感度標籤。

## 例子

### 範例 1：取得 Azure SQL 受管理實例資料庫的目前資訊類型和敏感度標籤。
```powershell
...
### 範例 2：取得 Azure SQL Managed Instance 資料庫的目前資訊類型和敏感度標籤與管道。
```powershell
...
### 範例 3：取得 Azure SQL 受管理實例資料庫特定資料行的目前資訊類型和敏感度標籤。
```powershell
...
### 範例 4：使用管道取得 Azure SQL 受管理實例資料庫特定資料行的目前資訊類型和敏感度標籤。
```powershell
