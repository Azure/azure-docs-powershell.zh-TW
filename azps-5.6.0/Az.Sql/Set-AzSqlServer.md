...
## 描述
**Set-AzSqlServer** Cmdlet 會修改 Azure SQL Database 伺服器的屬性。
...
此命令會重設 AzureSQL Server 上名為 server01 的系統管理員密碼。

### 範例 2

修改 SQL Database 伺服器的屬性。  (自動) 
...
### -PublicNetworkAccess
使用已啟用/停用的標號來指定是否允許公用網路存取伺服器。
停用時，只有透過私人連結所建立的連結可以連至此伺服器。
...
### -ServerVersion
指定此 Cmdlet 變更伺服器的版本。 此參數可接受的值為：2.0 和 12.0。
...
### -SqlAdministratorPassword
指定資料庫伺服器系統管理員的新密碼 ，做為 **SecureString。** 若要取得 **SecureString，** 請使用 Get-Credential Cmdlet。 如需要詳細資訊，請鍵入 `Get-Help
...
### -標記
指定此 Cmdlet 與伺服器關聯之標籤字典。 以雜湊表格形式設定為伺服器上標記的索引鍵值組。 例如：@{key0="value0";key1=$null;key2="value2"}
