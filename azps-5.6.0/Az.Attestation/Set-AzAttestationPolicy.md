...
## 描述
此Set-AzAttestationPolicy Cmdlet 會從 Azure 的租使用者中設定該策略。
...
使用預設格式的文字策略格式設定 TEE 類型 *SgxEnclave* for *AShtest* (定義) 。
...
使用 JWT 策略格式設定 TEE 類型 *SgxEnclave* for證明提供者 *pshtest* 的使用者定義策略。
...
### -策略
指定要設定的政策檔。  此策略格式可以是文字或 JSON Web Token (JWT) 。
...
### -PolicyFormat
指定規則的格式，包括文字或 JWT (JSON Web Token) 。  預設政策格式為文字。
...
### -Tee
指定信任的執行環境類型。
支援四種環境類型：SgxEnclave、OpenEnclave、CyResComponent 和 VBSEnclave。
