...
## 簡介
為指向網站連線性建立新 VpnServerConfiguration。
...
## 描述
**New-Az VpnServerConfiguration** Cmdlet 可讓您使用不同的 VpnProtocols、VpnAuthenticationTypes、IpsecPolicies 來建立新的 VpnServerConfiguration，並針對客戶對點到網站連接的需求，設定選取的 vpn 驗證類型相關參數。
...
上述命令會使用 VpnAuthenticationType 作為憑證來建立新 VpnServerConfiguration。

### 範例 2

為指向網站連線性建立新 VpnServerConfiguration。  (自動) 
