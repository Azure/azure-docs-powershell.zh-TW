...
## 描述
**Set-AzVirtualNetworkGateway** Cmdlet 會更新虛擬網路閘道。
...
第一個命令會獲得一個名為 Gateway01 的虛擬網路閘道，該閘道屬於資源群組 ResourceGroup001，並儲存到名為 $Gateway 的變數。第二個命令會更新儲存在變數 $Gateway 中的虛擬網路閘道。
命令也會將 ASN 設定為 1337。

### 範例 2：新增 IPsec 策略至虛擬網路閘道
```
PS C:\>$Gateway = Get-AzVirtualNetworkGateway -ResourceGroupName "ResourceGroup001" -Name "Gateway001"
...
第一個命令會獲得一個名為 Gateway01 的虛擬網路閘道，該閘道屬於資源群組 ResourceGroup001，並儲存到名為 $Gateway 的變數。第二個命令會根據指定的 ipsec 參數建立 Vpn ipsec 策略物件。
第三個命令會更新儲存在變數變數中的$Gateway。
該命令也會設定在虛擬網路閘道上$vpnclientipsecpolicy的自訂 vpn ipsec 策略。

### 範例 3：新增/更新標籤至現有的虛擬網路閘道
```
PS C:\>$Gateway = Get-AzVirtualNetworkGateway -ResourceGroupName "ResourceGroup001" -Name "Gateway001"
...
第一個命令會獲得一個名為 Gateway01 的虛擬網路閘道，該閘道屬於資源群組 ResourceGroup001，並儲存到名為 $Gateway 的第二個命令會以標籤 @{ testtagKey="SomeTagKey"; testtagValue="SomeKeyValue" }更新虛擬網路閘道閘道01。

### 範例 4：新增/更新現有虛擬網路閘道 VpnClient 的 AAD 驗證組
```
PS C:\>$Gateway = Get-AzVirtualNetworkGateway -ResourceGroupName "ResourceGroup001" -Name "Gateway001"
...
第一個命令會獲得一個名為 Gateway01 的虛擬網路閘道，該閘道屬於資源群組 ResourceGroup001，並儲存到名為 $Gateway 的變數。第二個命令會以 AAD 驗證組組參數更新虛擬網路閘道閘道01：aadTenantUri、aadAudienceId、aadIssuerUri for VpnClient。
第三個命令會從虛擬網路閘道的 VpnClient 移除 AAD 驗證組式。

### 範例 5：新增/更新 IpConfigurationBgpPeeringAddresses 到現有的虛擬網路閘道
```
PS C:\>$Gateway = Get-AzVirtualNetworkGateway -ResourceGroupName "ResourceGroup001" -Name "Gateway001"
...
第一個命令會獲得一個名為 Gateway01 的虛擬網路閘道，該閘道屬於資源群組 ResourceGroup001，並將它儲存至名為 $Gateway 的變數。第二個命令會指派虛擬網路閘道 Gateway01 IpConfiguration Id 的值至變數 ipconfigurationId1。
第三個命令會指派地址清單至地址清單1。
第四個命令會建立 PSIpConfigurationBgpPeeringAddress 物件。
此新建立 PSIpConfigurationBgpPeeringAddress 到 IpConfigurationBgpPeeringAddresses 及更新閘道的第五個命令集。
...
### -Asn
虛擬網路閘道的 ASN，用來在 IPsec 內設定 BGP 會話
...
### -GatewayDefaultSite
用於強制強制設置的預設網站。
如果已指定預設網站，閘道 vnet 的所有網際網路流量會路由至該網站。
...
做為修改基礎的虛擬網路閘道物件。
這可以使用資料Get-AzVirtualNetworkGateway
...
### -VpnClientRevokedCertificates
已撤銷 VPN 用戶端憑證的清單。
如果 VPN 用戶端展示的憑證符合其中一項，系統將會要求離開。
...
### -VpnClientRootCertificates
VPN 用戶端根憑證清單，用於 VPN 用戶端驗證。
連接 VPN 用戶端必須呈現這些根憑證之一產生的憑證。
