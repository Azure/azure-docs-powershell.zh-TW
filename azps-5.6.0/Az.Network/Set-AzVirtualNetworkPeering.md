...
## 描述
**Set-AzVirtualNetworkPeering** Cmdlet 會設定虛擬網路對等。
...
將此屬性變更為$True，就可以使用對等的 VNet 閘道。
不過，對等 VNet 必須擁有一個閘道， **而 AllowGatewayTransit** 必須有 $True。
如果已經設有閘道，就無法使用此屬性。
