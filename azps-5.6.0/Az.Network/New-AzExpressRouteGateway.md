...
New-AzExpressRouteGateway建立可縮放的 ExpressRoute 閘道。 這是針對 VirtualHub 內部部署至 Azure 的軟體定義連接。 

此閘道可以依據在此或 Cmdlet 中指定的縮放比例Set-AzExpressRouteGateway縮放。 

從內部部署 ExpressRoute 回路設定到可縮放閘道的連接。

ExpressRouteGateway 會位於與參照的 VirtualHub 相同的位置。
...
上述步驟將在 Azure 的 「testRG」資源群組中，建立資源群組、虛擬 WAN、虛擬網路、美國西部的虛擬中樞。 在此之後，將會以 2 個縮放單位在虛擬中心中建立 ExpressRoute 閘道。
...
### -MaxScaleUnits
此 ExpressRouteGateway 的縮放單位上限。 有效範圍 > 2
...
### -MinScaleUnits
此 ExpressRouteGateway 的最小刻度單位數目。 有效範圍 > 2
...
### -VirtualHub
這個 VpnGateway 的 VirtualHub 需要關聯。
