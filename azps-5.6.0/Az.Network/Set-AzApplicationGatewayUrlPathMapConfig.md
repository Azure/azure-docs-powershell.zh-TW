...
## 描述
**Set-AzApplicationGatewayUrlPathMapConfig** Cmdlet 會設定後端伺服器集區 URL 路徑的陣列。
...
第一個命令會取得名為 appGwName 的應用程式閘道，並儲存在$appgw變數中。
第二個命令會更新應用程式閘道中名為 map01 的 URL 路徑映射。
第三個命令會更新應用程式閘道。

## 參數

### -ApplicationGateway
指定此 Cmdlet 設定 URL 路徑地圖設定的應用程式閘道。
