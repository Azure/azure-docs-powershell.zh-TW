...
## 描述
**New-AzLoadBalancerInboundNatRuleConfig** Cmdlet 會為 Azure 負載平衡器建立輸入網路位址轉譯 (NAT) 規則組式。

## 例子

### 範例 1：為負載平衡器建立內入 NAT 規則組
```
...
第三個命令會使用 $frontend 中的前端物件，建立名為 MyInboundNatRule 的輸入 NAT 規則$frontend。
已指定 TCP 通訊協定，且前端埠為 3389，與這種情況中的後端埠相同。
若要 *建立輸入 NAT 規則設定，需要 FrontendIpConfiguration、Protocol、FrontendPort* 和 *後端Port* 參數。  
