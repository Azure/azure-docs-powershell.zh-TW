...
## 描述
**Get-AzApplicationGatewayFrontendPort** Cmdlet 取得應用程式閘道的前端埠。
...
第二個命令會從 $AppGw 取名為 FrontEndPort01 的前端埠，並儲存在 $FrontEndPort 變數中。
...
第一個命令會從名為 ResourceGroup01 的資源群組中，獲得名為 ApplicationGateway01 的應用程式閘道，並儲存在$AppGw變數。
第二個命令會從命令中$AppGw前端埠清單，並儲存在$FrontEndPorts變數。
