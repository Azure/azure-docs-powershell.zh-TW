...
## 簡介
建立可傳遞為參數的 VHubRoute 物件至New-AzVHubRouteTable命令。
...
上述命令會建立一個 VHubRoute 物件，其中 nextHop 是指定的防火牆，然後可新加入 VHubRouteTable 資源。
...
上述命令會建立一個 VHubRoute 物件，其中 nextHop 是指定的 hubVnetConnection，然後可新加到 VHubRouteTable 資源。
...
```
上述命令會取得現有 AzVHubRoute 的 RouteConfiguration，然後在連接上新增靜態路由。 或者，如果您想要與其中靜態路由建立新連接，請參閱此處的範例 [1。](New-AzRoutingConfiguration.md)
## 參數
