...
## 描述
**New-AzApplicationGatewayBackendAddressPool** Cmdlet 會為 Azure 應用程式閘道建立後端位址集區。
後端位址可指定為 IP 位址、FQDN (完整功能變數名稱) IP 組組識別碼。

## 例子

### 範例 1：使用後端伺服器的 FQDN 建立後端位址資料庫
```
PS C:\>$Pool = New-AzApplicationGatewayBackendAddressPool -Name "Pool01" -BackendFqdns "contoso1.com", "contoso2.com"
```

此命令會使用後端伺服器的 FQDN 建立名為 Pool01 的後端位址資料庫，並儲存在 $Pool 變數中。

### 範例 2：使用後端伺服器的 IP 位址建立後端位址資料庫
```
PS C:\>$Pool = New-AzApplicationGatewayBackendAddressPool -Name "Pool02" -BackendFqdns "10.10.10.10", "10.10.10.11"
```

此命令會使用後端伺服器的 IP 位址建立名為 Pool02 的後端位址資料庫，並儲存在 $Pool 變數中。

## 參數

### -後端Fqdns
指定此 Cmdlet 與後端伺服器資料庫關聯之後端 FQDN 的清單。
...
### -後端IPAddresses
指定此 Cmdlet 與後端伺服器資料庫關聯之後端 IP 位址的清單。
