...
## 描述
**Get-AzExpressRouteCrossConnectionARPTable** Cmdlet 會從 ExpressRoute 交互連接的兩個介面中，取得 ARP 資料表。 ARP 資料表提供特定對等的 IPv4 位址與 MAC 位址的映射。 您可以使用 ARP 資料表驗證第 2 層組配置和連接。
