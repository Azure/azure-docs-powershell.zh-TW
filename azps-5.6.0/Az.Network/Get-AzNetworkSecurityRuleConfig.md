...
## 描述
**Get-AzNetworkSecurityRuleConfig** Cmdlet 會取得 Azure 網路安全性群組的網路安全性規則組。
...
此命令會從資源群組 "rg1" 中名為 "nsg1" 的 Azure 網路安全性群組中，從名稱為 "AllowInternetOutBound" 的預設規則

### 2：僅使用名稱來取回網路安全性規則設定檔
```
Get-AzNetworkSecurityGroup -Name nsg1 -ResourceGroupName rg1 
    | Get-AzNetworkSecurityRuleConfig -Name "rdp-rule"
```

此命令會從資源群組 "rg1" 中名為 "nsg1" 的 Azure 網路安全性群組，從名為 "rdp-rule" 的使用者定義規則
...
### -DefaultRules
指出此 Cmdlet 是否獲得使用者建立的規則組式或預設規則組式。
