...
## 描述
**Remove-AzLoadBalancerInboundNatRuleConfig** Cmdlet 會從 Azure 負載平衡器移除輸入網路位址轉譯 (NAT) 規則組式。

## 例子

### 1：刪除 Azure 負載平衡器中的內入 NAT 規則
```
...
第一個命令會載入一個稱為「mylb」的現有負載平衡器，並儲存在變數中$load平衡器。 第二個命令會移除與此負載平衡器相關聯的輸入 NAT 規則。
...
### -LoadBalancer
指定 **LoadBalancer 物件** ，該物件包含此 Cmdlet 移除的傳入 NAT 規則組。
