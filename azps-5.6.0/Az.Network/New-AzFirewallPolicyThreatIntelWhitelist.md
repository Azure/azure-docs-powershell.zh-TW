...
## 描述
**New-AzFirewallPolicyThreatIntelWhitelist** Cmdlet 會建立威脅 intel whitelist 物件，可用於建立或設定 Azure 防火牆策略。
...
此範例會建立威脅 Intel Whitelist，其中包含一個輸入的 FQDN 白名單和兩個輸入的 Ip 位址白名單
