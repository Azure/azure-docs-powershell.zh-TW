...
## 描述
**Set-AzLoadBalancerRuleConfig** Cmdlet 會更新負載平衡器的規則設定。
...
第二個命令使用管線運算子，將 $slb 中的負載平衡器傳遞到 Add-AzLoadBalancerRuleConfig，這會將名為 NewRule 的規則新增到其中。
第三個命令會將負載平衡器傳遞至 **Set-AzLoadBalancerRuleConfig，** 以設定新的規則設定。
請注意，此組配置不會啟用由上一個命令啟用的浮動 IP 位址。

## 參數

### -後端AddressPool
指定 **要與負載平衡器規則關聯的後端AddressPool** 物件。
