...
## 描述
**New-AzFirewallNetworkRule** Cmdlet 會建立 Azure 防火牆的網路規則。
...
此範例會建立所有 TCP 流量的規則。 使用者根據與規則關聯的規則集合，強制執行規則是否允許或拒絕流量。
...
此範例會針對從 10.0.0.0 到 60.1.5.0：4040 的所有 TCP 流量建立規則。 使用者根據與規則關聯的規則集合，強制執行規則是否允許或拒絕流量。

### 範例 3：針對從任何來源到 10.0.0.0/16 的所有 TCP 和 ICMP 流量建立規則
```powershell
$rule = New-AzFirewallNetworkRule -Name "tcp-and-icmp-rule" -Description "Rule for all TCP and ICMP traffic from any source to 10.0.0.0/16" -Protocol TCP,ICMP -SourceAddress * -DestinationAddress "10.0.0.0/16" -DestinationPort *
```

此範例會針對從任何來源到 10.0.0.0/16 的所有 TCP 流量建立規則。 使用者根據與規則關聯的規則集合，強制執行規則是否允許或拒絕流量。
...
### -通訊協定
指定此規則要篩選的流量類型。 可能的值為 TCP、UDP、ICMP 和 Any。
