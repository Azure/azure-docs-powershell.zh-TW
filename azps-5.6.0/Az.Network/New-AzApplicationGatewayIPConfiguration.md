...
## 描述
**New-AzApplicationGatewayIPConfiguration** Cmdlet 會為應用程式閘道建立 IP 組式。
IP 群組原則包含部署應用程式閘道的子網。

## 例子

### 範例 1：建立應用程式閘道的 IP 組配置。
```
...
第一個命令會獲得名為 VNet01 的虛擬網路，該虛擬網路屬於名為 ResourceGroup01 的資源群組。
第二個命令會獲得上一個命令中虛擬網路所屬子網的子網組$Subnet變數。
第三個命令會使用 $Subnet。
