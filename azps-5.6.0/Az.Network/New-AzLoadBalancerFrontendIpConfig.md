...
## 描述
**New-AzLoadBalancerFrontendIpConfig** Cmdlet 會為 Azure 負載平衡器建立前端 IP 組式。

## 例子

### 範例 1：為負載平衡器建立前端 IP 組
```
PS C:\> $publicip = New-AzPublicIpAddress -ResourceGroupName "MyResourceGroup" -Name "MyPublicIP" -Location "West US" -AllocationMethod "Dynamic"
PS C:\> New-AzLoadBalancerFrontendIpConfig -Name "FrontendIpConfig01" -PublicIpAddress $publicip
```

第一個命令在名為 MyResourceGroup 的資源群組中建立名為 MyPublicIP 的動態公用 IP 位址，然後將它儲存在 $publicip 變數中。
第二個命令會使用 $publicip 中的公用 IP 位址，建立名為 FrontendIpConfig01 的前端 IP $publicip。

### 範例 2：使用 ip 首碼為負載平衡器建立前端 IP 組
```
...
第一個命令在名為 MyResourceGroup 的資源群組中，建立長度為 28 的公用 ip 首碼，然後將它儲存在 $publicipprefix 變數中。
第二個命令會使用 $publicipprefix 中的公用 IP 首碼，建立名為 FrontendIpConfig01 的前端 IP $publicipprefix。
...
### -PublicIpAddress
指定要與前端 IP 群組原則關聯的 **PublicIpAddress** 物件。
...
### -PublicIpAddressId
指定要與前端 IP 群組原則關聯的 **PublicIpAddress** 物件的識別碼。
