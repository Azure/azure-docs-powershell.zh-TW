...
## 描述
**Remove-AzApplicationGatewayIPConfiguration** Cmdlet 會從 Azure 應用程式閘道移除 IP 組式。

## 例子

### 範例 1：從 Azure 應用程式閘道移除 IP 組
```
...
第二個命令會從儲存在 $AppGw 的應用程式閘道移除名為子網02 的 IP $AppGw。

## 參數

### -ApplicationGateway
指定要從中移除 IP 群組原則的應用程式閘道。
