...
## 描述
**Remove-AzApplicationGatewaySslCertificate** Cmdlet 會從 Azure 應用程式閘道移除安全通訊端層 (SSL) 憑證。
...
第一個命令會取得名為 ApplicationGateway01 的應用程式閘道，並儲存在名為 $AppGW 的變數中。
第二個命令會從儲存在此變數的應用程式閘道移除名為 Cert02 的 SSL $AppGW憑證。

## 參數

### -ApplicationGateway
指定此 Cmdlet 會移除 SSL 憑證的應用程式閘道。
