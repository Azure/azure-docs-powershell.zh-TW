...
## 描述
**Remove-AzApplicationGatewayCustomError** Cmdlet 會從應用程式閘道的 HTTP 聆聽者移除自訂錯誤。

## 例子

### 範例 1：移除 HTTP 聆聽者中的自訂錯誤
```powershell
PS C:\> $updatedlistener = Remove-AzApplicationGatewayCustomError -HttpListener $listener01 -StatusCode HttpStatus502
```

此命令會從 HTTP 聆聽程式 $listener 01 移除 HTTP 狀態碼 502 的自訂錯誤，並返回更新的聆聽者。
