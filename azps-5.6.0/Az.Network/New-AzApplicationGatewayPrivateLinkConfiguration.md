...
## 描述
**New-AzApplicationGatewayPrivateLinkConfiguration** Cmdlet 會為 Azure 應用程式閘道建立 PrivateLink 組式。
私人連結組配置必須與前端 IP 組配置相關聯，才能啟用私人連結功能。
一個私人連結組組最可能只與應用程式閘道上的一個前端 ip 組定相關聯。

## 例子

### 範例 1：使用單一 Ip 組配置建立私人連結組
```powershell
...
此命令會建立名為 'privateLinkConfig01' 的 PrivateLink 組$PrivateLinkConfiguration。

### 範例 2：使用多個 Ip 組配置的建立私人連結組
```powershell
...
此命令會使用兩個 ipConfiguration 建立名為 'privateLinkConfig01' 的 PrivateLink 組$PrivateLinkConfiguration。 
