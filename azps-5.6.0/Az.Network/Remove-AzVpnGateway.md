...
## 簡介
Cmdlet Remove-AzVpnGateway移除 Azure VPN 閘道。 這是 Azure Virtual WAN 軟體定義之連接的特定閘道。
...
此範例會在美國中建立資源群組、虛擬 WAN、虛擬中樞、可縮放 VPN 閘道，然後立即刪除。 若要隱藏刪除虛擬閘道時提示，請使用 -Force 旗標。
這會刪除 VpnGateway 及其上的所有 VpnConnection。
...
此範例會在美國中建立資源群組、虛擬 WAN、虛擬中樞、可縮放 VPN 閘道，然後立即刪除。 此刪除會使用 powershell 管線執行，此管道使用由 Get-AzVpnGateway 命令所返回的 VpnGateway 物件。
...
[Get-Az VpnGateway](./Get-AzVpnGateway.md)
