...
## 描述
**Add-AzApplicationGatewayTrustedClientCertificate** Cmdlet 會將信任的用戶端 CA 憑證鏈新增到 Azure 應用程式閘道。
...
第一個命令會獲得應用程式閘道，並儲存在$gw變數中。 第二個命令會建立一個新的信任的用戶端 CA 憑證鏈，以用戶端 CA 憑證的路徑做為輸入。 第三個命令會使用信任的用戶端憑證建立 SSL 設定檔。 第四個命令會更新應用程式閘道。
