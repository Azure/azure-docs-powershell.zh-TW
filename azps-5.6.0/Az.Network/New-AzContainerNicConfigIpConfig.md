...
## 簡介
建立容器 nic configuration ip 組組物件。
...
## 描述
**New-AzContainerNicConfigIpConfig Cmdlet** 會建立一個新的容器網路介面組式 ip 組式。 
...
前兩個命令會建立並初始化 vnet 和子網。 第三個命令會建立參照所建立子網的容器 nic ip 組設定檔。 第四個命令會建立容器網路介面組配置，提供在上一個命令中建立之 ip 組設定檔。 最後，第五個命令會建立一個初始化的網路設定檔，其為儲存在 $containerNicConfig 中的容器網路介面$containerNicConfig。
