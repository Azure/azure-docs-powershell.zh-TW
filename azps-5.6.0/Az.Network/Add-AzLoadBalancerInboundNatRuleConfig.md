...
## 描述
**Add-AzLoadBalancerInboundNatRuleConfig** Cmdlet 會將輸入網路位址轉譯 (NAT) 規則組配置新增到 Azure 負載平衡器。

## 例子

### 範例 1：將內入 NAT 規則組組新增到負載平衡器
```
...
第一個命令會取得名為 MyloadBalancer 的負載平衡器，然後將它儲存在變數 $slb。
第二個命令會使用管線運算子，將 $slb 中的負載平衡器傳遞到 **Add-AzLoadBalancerInboundNatRuleConfig，** 這會將輸入 NAT 規則組式新增到負載平衡器。
最後一個命令會設定為載入平衡器，如果您沒有執行 Set-AzLoadBalancer，您的變更將不會套用至載入平衡器。
...
### -LoadBalancer
指定 **LoadBalancer** 物件。
此 Cmdlet 會新增輸入 NAT 規則設定至此參數指定的負載平衡器。
