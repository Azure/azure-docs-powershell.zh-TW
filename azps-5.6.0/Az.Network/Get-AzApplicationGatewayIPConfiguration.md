...
## 描述
**Get-AzApplicationGatewayIPConfiguration** Cmdlet 取得應用程式閘道的 IP 組式。
...
第一個命令會獲得應用程式閘道，並儲存在$AppGw變數中。第二個命令會從儲存在 $AppGw 的閘道中，獲得名為 GateSubnet01 的 IP $AppGw。
...
第一個命令會獲得應用程式閘道，並儲存在$AppGw變數中。第二個命令會獲得所有 IP 組配置的清單。

## 參數

### -ApplicationGateway
指定包含 IP 群組原則的應用程式閘道物件。
