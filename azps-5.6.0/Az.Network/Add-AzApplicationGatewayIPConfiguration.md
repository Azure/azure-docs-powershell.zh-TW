...
## 描述
**Add-AzApplicationGatewayIPConfiguration** Cmdlet 會新增 IP 設定檔至應用程式閘道。
IP 群組原則包含部署應用程式閘道的子網。

## 例子

### 範例 1：新增虛擬網路組配置至應用程式閘道
```
...
第一個命令會獲得一個虛擬網路。
第二個命令會使用先前建立之虛擬網路來建立子網。
第三個命令會獲得應用程式閘道，並儲存在$AppGw變數。
第四個命令會將 IP 組$AppGw。

## 參數

### -ApplicationGateway
指定此 Cmdlet 新增 IP 群組原則的應用程式閘道。
