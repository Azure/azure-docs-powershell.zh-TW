...
## 描述
**Add-AzApplicationGatewayCustomError** Cmdlet 會新增自訂錯誤至應用程式閘道。

## 例子

### 範例 1：新增自訂錯誤至應用程式閘道層級
```powershell
...
此命令會將 HTTP 狀態碼 502 的自訂錯誤新$appgw應用程式閘道，並返回更新的閘道。

### 範例 2：新增自訂錯誤至應用程式閘道聆聽者層級
```powershell
...
此命令會將 HTTP 狀態碼 502 的自訂錯誤新$appgw至聆聽者層級的應用程式閘道，並返回更新的閘道。
