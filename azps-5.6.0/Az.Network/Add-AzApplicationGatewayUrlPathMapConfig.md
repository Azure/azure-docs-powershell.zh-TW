...
## 描述
**Add-AzApplicationGatewayUrlPathMapConfig** Cmdlet 會將 URL 路徑的陣列新增到後端伺服器集區。

## 例子

### 範例 1：新增應用程式閘道的 URL 路徑。
```
...
第二個命令會獲得後端位址資料庫，並儲存在$pool變數。
第三個命令會獲得後端 HTTP 設定，並儲存在$poolSettings變數。
第四個命令會建立名為 rule01 的新路徑規則組$pathRule變數。
第五個命令會新增 URL 路徑，將名稱為 url01 的 URL 路徑轉換到應用程式閘道。
第六個命令會更新應用程式閘道。

## 參數

### -ApplicationGateway
指定此 Cmdlet 新增 URL 路徑地圖組配置的應用程式閘道。
...
指定要路由的預設後端位址資料庫，以防 *pathRules* 參數中指定的規則沒有相符。
...
指定在 *pathRules* 參數中指定的規則與規則不相符時所使用的預設後端 HTTP 設定。
...
應用程式閘道的預設 RedirectConfiguration
...
### -名稱
指定此 Cmdlet 新增到後端伺服器資料庫的 URL 路徑地圖名稱。
