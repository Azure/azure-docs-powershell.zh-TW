...
此命令會建立名為 NetworkInterface001 的網路介面，其虛擬網路中名為 VirtualNetwork1 的子網 1 動態指派私人 IP 位址。 該命令也會將兩個 DNS 伺服器指派給網路介面。 IPConfiguration 子資源會自動使用名稱 IPConfiguration1 建立。

### 範例 2：使用 IP 組組物件建立 Azure 網路介面
```powershell
...
此範例會使用 IP 組設定物件建立新網路介面。 IP 群組原則物件會指定靜態私人 IPv4 位址。
第一個命令會取回現有的指定虛擬網路，用來指派第二個命令中的子網。
第二個命令會建立名為 IPConfig1 的網路介面 IP 組$IPconfig。
第三個命令會建立名為 NetworkInterface1 的網路介面，該網路介面會使用儲存在名為 networkInterface 的變數中的網路介面 IP $IPconfig。
...
指定網路介面 IP 組配置應所屬的應用程式安全性群組參照集合。
...
### -EnableIPForwarding
表示此 Cmdlet 啟用網路介面的 IP 轉寄。
IP 轉轉可讓虛擬機器接收目的地為其他目的地的流量。
...
### -PublicIpAddress
指定 **要指派給網路介面的 PublicIPAddress** 物件。
...
### -PublicIpAddressId
指定要指派給 **網路介面的 PublicIPAddress** 物件的識別碼。
