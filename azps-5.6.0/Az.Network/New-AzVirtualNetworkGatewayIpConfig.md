...
## 描述
**New-AzVirtualNetworkGatewayIpConfig** Cmdlet 會建立指派給虛擬網路閘道的組 (先前根據子網識別碼建立) 公用 IP 位址。
...
設定具有公用 IP 位址的虛擬網路閘道。 變數 $myGWsubnet是使用您建立虛擬網路閘道之 「GatewaySubnet」上的 **Get-AzVirtualNetworkSubnetConfig** Cmdlet 取得。 此變數$myGWpip **New-AzPublicIpAddress** Cmdlet 取得。
