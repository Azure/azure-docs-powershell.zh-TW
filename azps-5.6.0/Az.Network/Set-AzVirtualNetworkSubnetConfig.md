...
## 描述
**Set-AzVirtualNetworkSubnetConfig** Cmdlet 會更新虛擬網路的子網設定。
...
此範例會建立一個子網的虛擬網路。 接著是呼叫Set-AzVirtualNetworkSubnetConfig修改子網的 AddressPrefix。 這只會影響虛擬網路的記憶體中表示。 Set-AzVirtualNetwork，系統隨即會要求修改 Azure 中的虛擬網路。
...
此範例會建立一個資源群組，其中一個虛擬網路只包含一個子網。 接著，它會建立一個網路安全性群組，並針對 RDP 流量建立允許規則。 Cmdlet Set-AzVirtualNetworkSubnetConfig修改前端子網的記憶體內表示方式，以指向新建立的網路安全性群組。 接著Set-AzVirtualNetwork Cmdlet，將修改後的狀態寫入服務。
