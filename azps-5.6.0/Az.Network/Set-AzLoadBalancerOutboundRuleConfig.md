...
## 描述
**Set-AzLoadBalancerOutboundRuleConfig** Cmdlet 會針對 Azure 負載平衡器設定輸出規則設定。

## 例子

### 範例 1：修改負載平衡器上的外發規則組式
```powershell
...
第二個命令使用管線運算子，將 $slb 中的負載平衡器傳遞到 Add-AzLoadBalancerOutboundRuleConfig，為它新增輸出規則組式。
第三個命令會將負載平衡器傳遞至 **Set-AzLoadBalancerOutboundRuleConfig，** 這可節省和更新輸出規則設定。
...
### -EnableTcpReset
在 TCP 流程閒置超時或非預期的連接終止時接收雙向 TCP 重設。
只有在通訊協定設定為 TCP 時，才能使用此元素。
