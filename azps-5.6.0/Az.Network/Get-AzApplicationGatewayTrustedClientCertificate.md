...
## 描述
此Get-AzApplicationGatewayTrustedClientCertificate Cmdlet 會從應用程式閘道獲得具有特定名稱的受信任的用戶端 CA 憑證鏈。
...
第一個命令會獲得應用程式閘道，並儲存在$gw變數中。 第二個命令會從應用程式閘道獲得具有指定名稱的受信任用戶端 CA 憑證鏈。
