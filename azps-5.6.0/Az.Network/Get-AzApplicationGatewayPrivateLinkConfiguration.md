...
## 描述
**Get-AzApplicationGatewayPrivateLinkConfiguration** Cmdlet 會取得應用程式閘道的私人連結組式。
...
第一個命令會從名為 ResourceGroup01 的資源群組中，獲得名為 ApplicationGateway01 的應用程式閘道，並儲存在$AppGw變數。
第二個命令會從命令中$AppGw名為 privateLinkConfig01 的私人連結組$PrivateLinkConfiguration變數。
...
第一個命令會從名為 ResourceGroup01 的資源群組中，獲得名為 ApplicationGateway01 的應用程式閘道，並儲存在$AppGw變數。
第二個命令會從 $AppGw 中獲得所有私人連結組$PrivateLinkConfigurations變數。
