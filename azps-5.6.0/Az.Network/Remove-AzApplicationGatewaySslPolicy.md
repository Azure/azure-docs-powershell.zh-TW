...
## 描述
此Remove-AzApplicationGatewaySslPolicy Cmdlet 會從 Azure 應用程式閘道移除 SSL 策略。

## 例子

### 範例 1：從應用程式閘道移除 SSL 策略
```
PS C:\>$AppGW = Get-AzApplicationGateway -Name "ApplicationGateway01" -ResourceGroupName "ResourceGroup01"
PS C:\> $AppGW = Remove-AzApplicationGatewaySslPolicy -ApplicationGateway $AppGW
```

此命令會從名為 ApplicationGateway01 的應用程式閘道移除 SSL 策略。

## 參數

### -ApplicationGateway
指定此 Cmdlet 會移除 SSL 策略的應用程式閘道。
