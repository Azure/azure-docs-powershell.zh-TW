...
## 描述
**New-AzApplicationGatewayWebApplicationFirewallConfiguration** Cmdlet 會為 Azure 應用程式閘道建立 Web 應用程式防火牆 (WAF) 組式。

## 例子

### 範例 1：為應用程式閘道建立 Web 應用程式防火牆組
```
...
第一個命令會針對名為「REQUEST-942-APPLICATION-ATTACK-SQLI」的規則群組，建立一個新的已停用規則組，並停用規則 942130 和規則 942140。
第二個命令會為名為「REQUEST-921-PROTOCOL-ATTACK」的規則群組建立另一個已停用的規則組組。 系統不會特別傳遞規則，因此會停用規則組的所有規則。
最後一個命令接著會建立 WAF 群組原則，且防火牆規則會以 $disabledRuleGroup 1 和 $disabledRuleGroup 2 中的$disabledRuleGroup停用。 新的 WAF 組$firewallConfig變數。
