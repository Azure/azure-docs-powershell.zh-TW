...
## 描述
**New-AzVirtualNetworkTap** Cmdlet 會建立 Azure 虛擬網路點符資源。

## 例子

### 範例 1：建立 Azure 虛擬網路點兩下
```
PS C:\>New-AzVirtualNetworkTap -Name "VirtualNetworkTap1" -ResourceGroupName "ResourceGroup1" -Location "centralus" -DestinationPort 8888 -DestinationNetworkInterfaceIPConfiguration $destinationNic.IpConfigurations[0]
```

此命令會建立名為「VirtualNetworkTap1」的虛擬網路點，其中具有目的地 VM 組 (DestinationIpConfiguration、DestinationPort) 。
點一下所配置之 VM 流量的所有來源都會路由至此目的地 IP + 埠。

### 範例 2：使用 LoadBalancer IP 建立 Azure 虛擬網路點一下
```
# Create LoadBalancer
...
此命令會建立名為「VirtualNetworkTap1」的虛擬網路點 (FrontEndIpConfiguration) 。
所有來源點一下所配置的 VM 流量都會路由到此 LoadBalancer IpConfiguration。 預設目的地埠為 4789。
