...
## 簡介
從 P2S VpnGateway 獲得目前點至網站連結的詳細資訊。
...
## 描述
**Get-AzP2s VpnGatewayDetailedConnectionHealth** Cmdlet 可讓您從 P2S VpnGateway 取得目前點至網站連結的詳細資訊。 客戶需要傳遞 SAS URL，我們可以在此輸入詳細的健康情況資訊。
...
**Get-AzP2s VpnGatewayDetailedConnectionHealth** Cmdlet 可讓您從 P2S VpnGateway 取得目前點至網站連結的詳細資訊。 客戶可以從傳遞的 SAS URL 下載下載健康情況詳細資料。 這會顯示每個點到網站連結的資訊，包括使用者名稱、位元組數、位元組數、已配置 IP 位址等。
...
### -OutputBlobSasUrl
p2s vpn 連接健康狀態要寫入的 OutputBlob Sas URL。
...
### Microsoft.Azure.Commands.Network.models.PSP2S VpnConnectionHealth
