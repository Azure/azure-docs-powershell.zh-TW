...
## 描述
**Add-AzApplicationGatewayHttpListener** Cmdlet 會將 HTTP 聆聽者新增到應用程式閘道。
...
第一個命令會獲得應用程式閘道，並儲存在$AppGw變數。第二個命令會將 HTTP 聆聽者新增到應用程式閘道。

### 範例 2：使用 SSL 新增 HTTPS 聆聽者
```
PS C:\>$AppGw = Get-AzApplicationGateway -Name "ApplicationGateway01" -ResourceGroupName "ResourceGroup01"
...
第二個命令會將使用 HTTPS 通訊協定的聆聽者新增到應用程式閘道。

### 範例 3：使用 SSL 和 HostNames 新增 HTTPS 聆聽者
```
PS C:\>$AppGw = Get-AzApplicationGateway -Name "ApplicationGateway01" -ResourceGroupName "ResourceGroup01"
