...
## 描述
**Set-AzRouteConfig** Cmdlet 會更新路由資料表的路由設定。
...
此命令會使用 Cmdlet 的路由資料表，Get-AzRouteTable路由資料表。
該命令會使用管線運算子，將表格傳遞至目前的 Cmdlet。
目前的 Cmdlet 會修改名為 Route02 的路由，然後將結果傳遞至 **Set-AzRouteTable** Cmdlet，此 Cmdlet 會更新資料表以反映您的變更。
...
Azureserver-to-server 虛擬專用網路閘道。 
- VnetLocal。
本地虛擬網路。
