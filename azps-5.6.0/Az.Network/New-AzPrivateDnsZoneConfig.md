...
## 描述
**New-AzPrivateDnsZoneConfig** Cmdlet 可讓您建立一個新的 DNS 區域設定物件，物件會設定在 DNS 區域群組上。
...
上述範例會建立 DNS 區域，然後建立 DNS 區域組配置。 `New-AzPrivateDnsZone` 模組 Az.PrivateDns 已驗證 Cmdlet。
