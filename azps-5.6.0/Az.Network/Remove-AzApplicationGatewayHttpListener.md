...
## 描述
**Remove-AzApplicationGatewayHttpListener** Cmdlet 會從 Azure 應用程式閘道移除 HTTP 聆聽者。

## 例子

### 範例 1：移除應用程式閘道 HTTP 聆聽者
```
...
第二個命令會從儲存在 $AppGw 的應用程式閘道移除名為 Listener02 的 HTTP $AppGw。

## 參數

### -ApplicationGateway
指定要從中移除 HTTP 聆聽者的應用程式閘道。
