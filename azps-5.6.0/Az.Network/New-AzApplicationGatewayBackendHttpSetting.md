...
## 描述
Cmdlet New-AzApplicationGatewayBackendHttpSetting建立應用程式閘道的後端 HTTP 設定。
...
此命令會使用 HTTP 通訊協定，在埠 80 上建立名為 Settings01 的後端 HTTP 設定，並停用 Cookie 型關聯。
這些設定會儲存在$Setting變數中。

### 範例 2

為應用程式閘道建立後端 HTTP 設定。  (自動) 
