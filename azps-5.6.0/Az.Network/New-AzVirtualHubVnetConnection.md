...
## 簡介
Cmdlet New-AzVirtualHubVnetConnection建立 HubVirtualNetworkConnection 資源，將虛擬網路對等至 Azure 虛擬中樞。
...
Cmdlet New-AzVirtualHubVnetConnection建立 HubVirtualNetworkConnection 資源，將虛擬網路對等至 Azure 虛擬中樞。  (自動) 
...
```
上述步驟會建立新的路由組式，在路由設定檔中建立靜態路由，下一個躍點會做為指定的 IP 位址。 然後，這個路由設定可以傳遞至 New-AzVirtualHubVnetConnection 命令做為參數 -RoutingConfiguration。
