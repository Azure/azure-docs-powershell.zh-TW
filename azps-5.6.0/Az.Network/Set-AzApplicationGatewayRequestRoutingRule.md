...
## 描述
**Set-AzApplicationGatewayRequestRoutingRule** Cmdlet 會修改要求路由規則。
...
第一個命令會獲得名為 ApplicationGateway01 的應用程式閘道，並儲存在$AppGw變數。
第二個命令會修改應用程式閘道的要求路由規則，以使用 $Setting 變數中指定的後端 HTTP 設定、在 $Listener 變數中指定的 HTTP 聆聽程式，以及 $Pool 變數中指定的後端位址集區。

## 參數

### -ApplicationGateway
指定此 Cmdlet 與要求路由規則關聯的應用程式閘道物件。
