...
## 描述
**Remove-AzLoadBalancerFrontendIpConfig** Cmdlet 會從 Azure 負載平衡器移除前端 IP 組式。

## 例子

### 範例 1：從負載平衡器移除前端 IP 組
```
PS C:\>$loadbalancer = Get-AzLoadBalancer -Name "MyLoadBalancer" -ResourceGroupName "MyResourceGroup"
PS C:> Remove-AzLoadBalancerFrontendIpConfig -Name "frontendName" -LoadBalancer $loadbalancer
```

第一個命令會取得與您想要移除的前端 IP 組配置相關聯的負載平衡器，然後將它儲存在$loadbalancer變數。
第二個命令會從 $loadbalancer 中的負載平衡器移除相關聯的前端 IP $loadbalancer。
