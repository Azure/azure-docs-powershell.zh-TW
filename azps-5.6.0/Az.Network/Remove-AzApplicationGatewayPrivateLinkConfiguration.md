...
## 描述
**Remove-AzApplicationGatewayPrivateLinkConfiguration** Cmdlet 會從 Azure 應用程式閘道移除 privateLink 組式。

## 例子

### 範例 1：移除應用程式閘道 PrivateLink Configuration
```
...
第二個命令會從儲存在 $AppGw 的應用程式閘道移除名為 privateLinkConfig01 的 privateLink $AppGw。
