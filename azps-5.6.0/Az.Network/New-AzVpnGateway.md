...
## 描述
New-AzVpnGateway建立可縮放的 VPN 閘道。
這是針對 VirtualHub 內網站與網站連結的軟體定義之連接。

此閘道會依據此或 Cmdlet 中指定的縮放比例單位，Set-AzVpnGateway縮放。

從稱為 VPNSite 的分支/網站設定到可縮放閘道的連接。
每個連接由 2 個Active-Active組成。

VpnGateway 會位於與參照的 VirtualHub 相同的位置。
...
在此 VpnGateway 上啟用路由喜好設定網際網路的標標。
...
[Remove-Az VpnGateway]()

[Update-Az VpnGateway]()
