...
## 描述
**Add-AzApplicationGatewaySslCertificate** Cmdlet 會新增 SSL 憑證至應用程式閘道。

## 例子

### 範例 1：使用 pfx 新增 SSL 憑證至應用程式閘道。
```
...
此命令會獲得名為 ApplicationGateway01 的應用程式閘道，然後將名為 Cert01 的 SSL 憑證新增到它。

### 範例 2：使用 KeyVault Secret (version-less secretId) 新增 SSL 憑證至應用程式閘道。
```
PS C:\> $AppGW = Get-AzApplicationGateway -Name "ApplicationGateway01" -ResourceGroupName "ResourceGroup01"
...
取得機密，並參照到 ， `Add-AzApplicationGatewaySslCertificate` 以將其新增到應用程式閘道，並加上名稱 `Cert01` 。
