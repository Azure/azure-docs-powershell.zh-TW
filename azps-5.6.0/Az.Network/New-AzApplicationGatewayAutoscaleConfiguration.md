...
## 描述
**New-AzApplicationGatewayAutoscaleConfiguration** Cmdlet 會為 Azure 應用程式閘道建立自動縮放組配置。
...
第一個命令會建立容量最低為 3 的自動縮放配置。
第二個命令會使用自動縮放設置建立應用程式閘道。
