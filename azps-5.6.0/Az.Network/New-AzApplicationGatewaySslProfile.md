...
## 描述
**New-AzApplicationGatewaySslProfile** Cmdlet 會建立應用程式閘道的 SSL 設定檔。 SSL 設定檔是在 HTTPS 聆聽程式上所配置。
...
```
第一個命令會建立新的 SSL 策略，並儲存在$sslPolicy變數中。
第二個命令會建立信任的用戶端 CA 憑證鏈，並儲存在 $ClientCert 01 變數中。
第三個命令會使用 ssl 策略和信任的用戶端 CA 憑證鏈來建立新的 SSL 設定檔。
