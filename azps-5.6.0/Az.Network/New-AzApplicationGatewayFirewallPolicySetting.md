...
## 描述
**New-AzApplicationGatewayFirewallPolicySetting** 會為防火牆策略建立一個策略設定。
...
命令會建立一個以 $enabledState、模式為 $enabledMode、RequestBodyCheck 為 false、FileUploadLimitInMb 為 $fileUploadLimitInMb 且 MaxRequestBodySizeInKb 為 $$maxRequestBodySizeInKb。
...
防火牆政策設定中的 MaxRequestBodySizeInKb。
