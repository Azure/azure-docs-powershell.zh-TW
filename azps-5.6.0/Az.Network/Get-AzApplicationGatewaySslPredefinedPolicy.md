...
## 描述
**Get-AzApplicationGatewaySlPredefinedPolidlet** 會取得應用程式閘道提供的預先定義 SSL 策略。
...
這個命令會以名稱 AppGwSslPolicy20170401 來返回預先定義的策略。
...
這個命令會以 「AppGwSslPolicy2017」開始，以名稱為起始值，以回預先定義之規則。
