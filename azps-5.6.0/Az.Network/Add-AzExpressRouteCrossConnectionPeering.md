...
## 描述
**Add-AzExpressRouteCrossConnectionPeering** Cmdlet 會新增對等組配置至 ExpressRoute 交叉連接。 請注意，在啟動 **Add-AzExpressRouteCrossConnectionPeering** 之後，您必須呼叫 Set-AzExpressRouteCrossConnection Cmdlet 以啟用組配置。

## 例子

### 範例 1：新增對等至現有的 ExpressRoute 交叉連接
```
...
要修改的 ExpressRoute 交叉連接。 這是 **Get-AzExpressRouteCrossConnection Cmdlet 所返回的** Azure 物件。
...
### -MicrosoftConfigAdvertvertlicPublicPrefix
...
### -PeerASN
ExpressRoute 交叉連接的 AS 號碼。 當對等類型為 AzurePublicPeering 時，這必須是公用 ASN。
