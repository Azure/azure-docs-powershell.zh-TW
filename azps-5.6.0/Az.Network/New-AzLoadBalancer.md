...
## 描述
**New-AzLoadBalancer** Cmdlet 會建立 Azure 負載平衡器。
...
部署負載平衡器需要您先建立多個物件，而前七個命令會顯示如何建立這些物件。
第八個命令在名為 MyResourceGroup 的資源群組中，建立名為 MyLoadBalancer 的負載平衡器。
第九個及最後一個命令會取得新的負載平衡器，以確保已成功建立。
請注意，此範例只會顯示如何建立負載平衡器。 您也必須使用 Add-AzNetworkInterfaceIpConfig Cmdlet 進行設定，以將 NIC 指派給不同的虛擬機器。
...
部署全域負載平衡器需要您先建立多個物件，而前五個命令會顯示如何建立這些物件。
第六個命令在名為 MyResourceGroup 的資源群組中，建立名為 MyLoadBalancer 的負載平衡器。
第七個及最後一個命令會取得新的負載平衡器，以確保已成功建立。
請注意，此範例只會顯示如何建立全域負載平衡器。 您也必須使用 New-AzLoadBalancerBackendAddressConfig Cmdlet 進行設定，以將地區負載平衡器 frontend ipconfig 識別碼指派至其後端位址集區
...
### -強制
表示此 Cmdlet 會建立負載平衡器，即使已存在相同名稱的負載平衡器。
...
### -InboundNatRule
指定 NAT 的輸入網路位址轉譯 (NAT) 與負載平衡器關聯的規則。
