...
## 描述
**Set-AzApplicationGatewaySslCertificate** Cmdlet 會更新應用程式閘道的 SSL 憑證。
...
更新名為 ApplicationGateway01 之應用程式閘道的現有 SSL 憑證。

### 範例 2：在應用程式閘道上使用 KeyVault Secret (version-less secretId) 現有的 SSL 憑證
```
...
取得金鑰，然後使用更新現有的 SSL 憑證 `Set-AzApplicationGatewaySslCertificate` 。

### 範例 3：在應用程式閘道使用 KeyVault Secret 更新現有的 SSL 憑證
```
PS C:\> $secret = Get-AzKeyVaultSecret -VaultName "keyvault01" -Name "sslCert01"
