...
## 描述
**Add-AzDelegation** Cmdlet 會將服務委派新增到 Azure 子網。
...
第一個命令會取回子網所位於的虛擬網路。 第二個命令會隔離您想要委派的子網。 第三個命令會新增委派至子網。 當您想要啟用 SQL 在此子網中建立介面端點時，這個特定範例會很有用。 最後一個命令會傳送更新的子網至伺服器，以實際更新您的子網。
