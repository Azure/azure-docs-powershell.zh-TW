...
## 描述
**New-AzFirewallPolicyApplicationRule** Cmdlet 會建立 Azure 防火牆原則的應用程式規則。
...
此範例會建立具有來源位址、通訊協定和目標 fqdns 的應用程式規則。
...
此範例會建立具有來源位址、通訊協定和 Web 類別的應用程式規則。
