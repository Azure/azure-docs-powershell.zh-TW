...
## 描述
**Remove-AzApplicationGatewayUrlPathMapConfig** Cmdlet 會移除後端伺服器集區 URL 路徑的映射。

## 例子

### 範例 1：移除應用程式閘道的 URL 路徑地圖
```
...
第二個命令會從應用程式閘道移除名為 map01 的 URL 路徑映射。
第三個命令會更新應用程式閘道。

## 參數

### -ApplicationGateway
指定此 Cmdlet 會移除 URL 路徑地圖組配置的應用程式閘道。
...
### -名稱
指定此 Cmdlet 從後端伺服器移除的 URL 路徑地圖名稱。
