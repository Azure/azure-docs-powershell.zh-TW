...
## 描述
Cmdlet New-AzDdosProtectionPlan建立 DDoS 保護計劃。

## 例子

### 範例 1：建立 DDoS 保護計劃與新虛擬網路的關聯
```
...
首先，我們使用 **New-AzDdosProtectionPlan** 命令來建立新的 DDoS Protection 方案。
然後，我們使用 **New-AzVirtualNetwork** 建立新虛擬網路，然後在參數 **DdosProtectionPlanId** 中指定新建立計畫的識別碼。 在這個案例中，由於我們要將虛擬網路與計畫建立關聯，所以我們也可以指定 **EnableDdoSProtection 參數**。

### 範例 2：建立 DDoS 保護計劃與現有虛擬網路的關聯
```
D:\> $ddosProtectionPlan = New-AzDdosProtectionPlan -ResourceGroupName ResourceGroupName -Name DdosProtectionPlanName -Location "West US"
...
首先，我們使用 **New-AzDdosProtectionPlan** 命令來建立新的 DDoS Protection 方案。
第二，我們會取得與計畫建立關聯的最新版本的虛擬網路。 我們會使用 **PSResourceId** 物件更新 **屬性 DdosProtectionPlan，** 該物件包含新建立之計畫識別碼的參照。 在這種情況下，如果我們將虛擬網路與 DDoS 保護計劃建立關聯，也可以將 **EnableDdosProtection** 標為 True。
最後，我們將本地變數放入 **Set-AzVirtualNetwork** 中，以持續新的狀態。
