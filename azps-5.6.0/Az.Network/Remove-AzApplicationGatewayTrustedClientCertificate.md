...
## 描述
**Remove-AzApplicationGatewayTrustedClientCertificate** Cmdlet 會從應用程式閘道移除信任的用戶端 CA 憑證鏈物件。
...
第一個命令會獲得應用程式閘道，並儲存在$gw變數。 第二個命令會從儲存在 $gw 的應用程式閘道移除名為 "TrustedClientCertificate01" 的受信任用戶端 CA 憑證鏈。
