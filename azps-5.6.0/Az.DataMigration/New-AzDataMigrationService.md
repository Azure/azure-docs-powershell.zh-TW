...
## 描述
此New-AzDataMigrationService Cmdlet 會建立 Azure 資料庫移轉服務的新實例。 此 Cmdlet 會取用現有 Azure 資源群組的名稱、要建立之 Azure 資料庫移轉服務新實例的唯一名稱、實例的所在地區、DMS 員工 SKU 的名稱，以及服務所在的 Azure 虛擬子網名稱。 訂閱名稱沒有參數，因為使用者預期會指定 Azure 登入會話的預設訂閱，或執行 Get-AzSubscription -SubscriptionName "MySubscription" |Select-AzSubscription選取另一個訂閱。
...
上述範例顯示如何在美國中地區建立名為 TestService 的 Azure 資料庫移移服務新實例。
...
### -位置
要建立 Azure 資料庫移移服務實例的位置，對應到 Azure 區域。
...
### -SKU
Azure 資料庫移移服務實例的 SKU。 目前可能的值Basic_1vCore，Basic_2vCores，GeneralPurpose_4vCores
...
### -VirtualSubnetId
指定虛擬網路下要用於 Azure 資料庫移移服務實例的子網名稱。
