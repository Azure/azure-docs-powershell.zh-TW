...
## 描述
**Remove-AzRmStorageContainerLegalHold** Cmdlet 會移除儲存 Blob 容器中的法律保留標記

## 例子

### 範例 1：移除儲存 Blob 容器中具有儲存帳戶名稱和容器名稱的法律保留標記
```
PS C:\>Remove-AzRmStorageContainerLegalHold -ResourceGroupName "myResourceGroup" -AccountName "myStorageAccount" -ContainerName "myContainer" -Tag  tag1
```

此命令會移除儲存 Blob 容器中具有儲存帳戶名稱和容器名稱的法律保留標記。

### 範例 2：移除儲存 Blob 容器中具有儲存帳戶物件和容器名稱的法律保留標記
```
PS C:\>$accountObject = Get-AzStorageAccount -ResourceGroupName "myResourceGroup" -AccountName "myStorageAccount"
PS C:\>Remove-AzRmStorageContainerLegalHold -StorageAccount $accountObject -ContainerName "myContainer"  -Tag  tag1,tag2
```

此命令會移除儲存 Blob 容器中具有儲存帳戶物件和容器名稱的法律保留標記。

### 範例 3：移除儲存帳戶中具有管線之所有儲存 Blob 容器的法律保留標記
```
PS C:\>Get-AzStorageContainer -ResourceGroupName "myResourceGroup" -AccountName "myStorageAccount" | Remove-AzRmStorageContainerLegalHold -Tag  tag1
```

此命令會從具有管線的儲存空間帳戶內的所有儲存 Blob 容器移除法律保留標記。
