...
## 描述
**New-AzStorageEncryptionScope** Cmdlet 會為儲存空間帳戶建立加密範圍。
...
### 範例 2：使用 Keyvault Encryption 和 RequireInfracryptEncryption 建立加密範圍
```
...
此命令會使用 Keyvault Encryption 和 RequireInfracryptEncryption 建立加密範圍。
儲存帳戶身分識別需要取得 keyvault 金鑰的 wrapkey、unwrapkey 許可權。
...
### -KeyvaultEncryption
使用 KeySource 建立加密範圍做為 Microsoft.Keyvault
...
### -StorageEncryption
使用 KeySource 建立加密範圍做為 Microsoft.Storage。
