...
## 描述
**Start-AzStorageFileCopy** Cmdlet 會開始將來源檔案複製到目的地檔案。

## 例子

### 範例 1：使用共用名稱稱和檔案名開始從檔案複製到檔案
```
...
### 範例 2：使用容器名稱和 Blob 名稱，開始從 Blob 到檔案的複製作業
```
...
### -DestCoNtext
指定目的地的 Azure 儲存內容。
若要取得上下文，請使用 **New-AzStorageCoNtext。**
...
### -SrcBlob
指定 **CloudBlob** 物件。
您可以使用 Cmdlet 建立雲端 blob 或取得Get-AzStorageBlob Blob。
...
### -SrcContainer
指定雲端 Blob 容器物件。
您可以建立雲端 Blob 容器物件，或使用 Get-AzStorageContainer Cmdlet。
...
### -SrcFile
指定 **CloudFile** 物件。
您可以建立雲端檔案，或使用 **Get-AzStorageFile 取得一個**。
...
### -SrcShare
指定雲端檔案共用物件。
您可以使用 Cmdlet 建立雲端檔案共用，或Get-AzStorageShare共用。
