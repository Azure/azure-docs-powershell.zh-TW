...
## 描述
**New-AzStorageBlobSAsToken Cmdlet** 會為 Azure 儲存 blob 產生共用存取簽名 (SAS) 權杖。
...
此範例會產生具有生命週期的 Blob SAS 權杖。

### 範例 3：根據 OAuth 驗證產生具有儲存內容的使用者身分識別 SAS 權杖
```
...
此範例會根據 OAuth 驗證產生具有儲存內容的使用者身分識別 Blob SAS 權杖
...
### -CloudBlob
指定 **CloudBlob** 物件。
若要取得 **CloudBlob 物件** ，請使用 [Get-AzStorageBlob](./Get-AzStorageBlob.md) Cmdlet。
...
### -許可權
指定儲存 Blob 的許可權。 請注意，這是一個字串，例如用於讀取 (寫入和 `rwd` 刪除) 。 
