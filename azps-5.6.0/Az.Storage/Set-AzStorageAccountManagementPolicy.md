...
## 描述
**Set-AzStorageAccountManagementPolicy** Cmdlet 會建立或修改 Azure 儲存體帳戶的管理原則。

## 例子

### 範例 1：使用 ManagementPolicy 規則物件建立或更新儲存空間帳戶的管理原則。
```
...
此命令會先建立 2 個 ManagementPolicy 規則物件，然後使用 2 ManagementPolicy 規則物件建立或更新儲存空間帳戶的管理原則。

### 範例 2：使用 Json 格式策略建立或更新儲存空間帳戶的管理原則。
```
...
此命令會使用 json 格式策略來建立或更新儲存空間帳戶的管理原則。

### 範例 3：從儲存空間帳戶取得管理原則，然後設定為另一個儲存空間帳戶。
```
...
此命令會先從儲存空間帳戶獲得管理原則，然後設定為另一個儲存空間帳戶。
...
### -規則
管理原則規則。 使用 Cmdlet New-AzStorageAccountManagementPolicyRule物件。
