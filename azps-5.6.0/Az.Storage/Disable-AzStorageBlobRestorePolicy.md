...
## 描述
**Disable-AzStorageBlobRestorePolicy** Cmdlet 會停用 Azure 儲存體 Blob 服務的 Blob 還原策略。

## 例子

### 範例 1：停用儲存帳戶上 Azure 儲存體 Blob 服務的 Blob 還原策略
```powershell
PS C:\> Disable-AzStorageBlobRestorePolicy -ResourceGroupName "myresourcegoup" -StorageAccountName "mystorageaccount"
```

此命令會停用儲存帳戶上 Azure 儲存體 Blob 服務的 Blob 還原策略。
