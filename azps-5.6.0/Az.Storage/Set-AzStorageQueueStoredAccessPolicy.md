...
## 描述
**Set-AzStorageQueueStoredAccessPolicy** Cmdlet 會設定 Azure 儲存佇列的儲存存取策略。

## 例子

### 範例 1：在具有完整許可權的佇列中設定儲存的存取策略
```
PS C:\> Set-AzStorageQueueStoredAccessPolicy -Queue "MyQueue" -Policy "Policy07" -Permission arup
```

此命令會針對名為 MyQueue 的儲存佇列設定名為 Policy07 的存取策略。
