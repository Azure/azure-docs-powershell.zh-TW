...
## 描述
**Remove-AzStorageCORSRule** Cmdlet 會移除 Azure 儲存 (CORS) 的跨來源資源分享。
此 Cmdlet 會刪除儲存服務類型中所有的 CORS 規則。
此 Cmdlet 的儲存服務類型為 Blob、資料表、佇列和檔案。

## 例子

### 範例 1：移除 Blob 服務的 CORS 規則
```
PS C:\>Remove-AzStorageCORSRule -ServiceType Blob
```

此命令會移除 Blob 服務類型的 CORS 規則。
...
### -內容
指定 Azure 儲存內容。
若要取得儲存內容，New-AzStorageContext Cmdlet。
...
### -ServiceType
指定此 Cmdlet 會移除規則的 Azure 儲存體服務類型。
此參數可接受的值為：
- Blob 
- 表 
- 佇列 
