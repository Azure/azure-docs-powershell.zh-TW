...
## 描述
**Get-AzStorageFileCopyState** Cmdlet 會取得 Azure 儲存體檔案複製作業的狀態。
它應在複製目標檔案上執行。
...
第一個命令會開始將檔案 "contosofile" 複製到 "contosofile_copy"，然後輸出 destiantion 檔案物件。 第二個命令管線將 destiantion 檔案物件用於 Get-AzStorageFileCopyState，以取得檔案複製狀態。
