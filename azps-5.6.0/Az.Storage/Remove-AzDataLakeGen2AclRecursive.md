...
## 描述
**Remove-AzDataLakeGen2AclRecursive** Cmdlet 會移除指定路徑上的 ACL 遞迴。 原始 ACL 中的 ACL 專案，其具有相同 AccessControlType、DefaultScope 和 EntityId 與輸入 ACL 專案 (即使具有不同的許可權) 會移除。

## 例子

### 範例 1：移除檔案系統根目錄上的 ACL 遞迴
```
...
此命令會先建立一個包含 2 個 acl 專案之 ACL 物件，然後在檔案系統的根目錄上遞迴移除 ACL。
...
此命令會先移除目錄中的 ACL 遞迴，但失敗，然後在使用者修正失敗檔案後繼續使用 ContinuationToken。
...
此腳本會以 BatchSize * MaxBatchCount 的區塊大小，以遞迴的目錄區塊移除 ACL。 此腳本中的區塊大小為 50000。

### 範例 4：在目錄和 ContinueOnFailure 上移除 ACL 遞迴，然後從失敗中一一繼續
```
...
此命令會先以 ContinueOnFailure 將 ACL 遞迴移除至目錄，而有些專案失敗，然後再一個一個繼續失敗的專案。
