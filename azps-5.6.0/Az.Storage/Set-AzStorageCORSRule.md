...
## 描述
**Set-AzStorageCORSRule** Cmdlet 會針對一種 Azure 儲存服務類型 (CORS) 設定跨來源資源分享規則。
此 Cmdlet 的儲存服務類型為 Blob、資料表、佇列和檔案。
此 Cmdlet 會覆寫現有的規則。
若要查看目前規則，請使用 Get-AzStorageCORSRule Cmdlet。
...
第一個命令會指派規則陣列給$CorsRules變數。
此命令使用標準會延伸至此程式碼區塊中的多行。
第二個命令會指派規則$CorsRules Blob 服務類型。

### 範例 2：變更 Blob 服務 CORS 規則的屬性
```
...
第一個命令會使用 **Get-AzStorageCORSRule Cmdlet** 取得 Blob 類型的目前 CORS 規則。
該命令將規則儲存在陣列$CorsRules中。
第二個和第三個命令會修改 $CorsRules。
最後一個命令會指派規則$CorsRules Blob 服務類型。
修訂的規則會覆寫目前的 CORS 規則。
...
### -CorsRules
指定 CORS 規則的陣列。
您可以使用 Cmdlet 來Get-AzStorageCORSRule規則。
...
### -ServiceType
指定此 Cmdlet 指派規則的 Azure 儲存體服務類型。
此參數可接受的值為：
- Blob 
- 表 
- 佇列 
