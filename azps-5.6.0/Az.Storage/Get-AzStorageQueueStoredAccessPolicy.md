...
## 描述
**Get-AzStorageQueueStoredAccessPolicy** Cmdlet 會列出 Azure 儲存佇列的儲存存取策略或策略。
...
此命令會取得名為 MyQueue 的儲存佇列中名為 Policy12 的存取策略。

### 範例 2：在佇列中取得所有儲存的存取策略
```
PS C:\>Get-AzStorageQueueStoredAccessPolicy -Queue "MyQueue"
```

此命令會取得佇列中名為 MyQueue 的所有儲存存取策略。
