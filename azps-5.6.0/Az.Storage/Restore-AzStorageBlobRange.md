...
## 描述
**Restore-AzStorageBlobRange** Cmdlet 會還原儲存帳戶中特定 Blob 範圍中的 Blob。
包含開始範圍，而 Blob 還原會排除結束範圍。

## 例子

### 範例 1：開始還原具有特定 Blob 範圍之儲存體帳戶中的 Blob
```powershell
...
此命令會先建立 2 個 Blob 範圍，然後從 1 天前的 2 個 Blob 範圍開始還原儲存帳戶中的 Blob。 使用者可以稍後Get-AzStorageAccount追蹤還原狀態。

### 範例 2：還原後端中儲存空間帳戶中的所有 Blob
```powershell
...
此命令會從 30 分鐘前還原儲存空間帳戶中的所有 Blob，並等待還原完成。 由於還原 Blob 可能需要很長的時間，因此請以 -Asjob 參數在後端執行，然後等待工作完成，然後顯示結果。

### 範例 3：直接根據輸入 Blob 範圍還原 Blob，並等待完成
```powershell
...
此命令會直接從 Cmdlet 輸入 2 個 blob 範圍，以還原儲存Restore-AzStorageBlobRange Blob。 此命令會等待還原完成。
