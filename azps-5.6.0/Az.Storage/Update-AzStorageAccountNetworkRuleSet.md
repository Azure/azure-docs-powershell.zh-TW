...
## 描述
**Update-AzStorageAccountNetworkRuleSet** Cmdlet 會更新儲存帳戶的 NetworkRule 屬性

## 例子

### 範例 1：更新 NetworkRule 的所有屬性，使用 JSON 輸入規則
```
...
此命令會更新 NetworkRule 的所有屬性，以及使用 JSON 輸入規則。

### 範例 2：NetworkRule 的更新旁路屬性
```
PS C:\> Update-AzStorageAccountNetworkRuleSet -ResourceGroupName "myResourceGroup" -AccountName "mystorageaccount" -Bypass AzureServices,Metrics
```

此命令更新 NetworkRule 的 Bypass 屬性 (不會變更其他屬性) 。

### 範例 3：清理儲存空間帳戶的 NetworkRule 規則
```
...
此命令會清除儲存空間帳戶的 NetworkRule 規則， (不會變更其他) 。
...
### -旁路
要更新至儲存空間帳戶的 NetworkRule 屬性的 Bypass 值。
允許的值不是或任何組合：• Logging • 計量 • Azureservices
...
### -DefaultAction
要更新至儲存空間帳戶 NetworkRule 屬性的 DefaultAction 值。
允許的選項：• 允許 • 拒絕
...
### -IPRule
要更新至儲存帳戶 NetworkRule 屬性的 IpRule 物件的陣列。
...
### -VirtualNetworkRule
要更新為儲存帳戶 NetworkRule 屬性的 VirtualNetworkRule 物件的陣列。
