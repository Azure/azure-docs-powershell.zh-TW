...
## 描述
**New-AzRmStorageShare** Cmdlet 會建立儲存空間檔案共用。

## 例子

### 範例 1：使用儲存空間帳戶名稱和共用名稱稱建立儲存空間檔案共用，中繼資料和共用配額為 100 GiB。
```
...
此命令會建立包含中繼資料的儲存空間檔案共用，並共用配額為 100 GiB。

### 範例 2：使用 Storage 帳戶物件建立儲存空間檔案共用
```
...
此命令會使用儲存空間帳戶物件和共用名稱稱建立儲存空間檔案共用。

### 範例 3：使用 Accesstier 建立儲存空間檔案共用為 Hot
```
...
此命令會建立與 Accesstier 的儲存空間檔案共用為 Hot。
