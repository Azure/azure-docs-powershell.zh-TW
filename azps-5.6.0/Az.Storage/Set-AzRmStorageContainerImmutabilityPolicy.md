...
## 描述
**Set-AzRmStorageContainerImmutabilityPolidlet** 會建立或更新儲存 Blob 容器的服務性

## 例子

### 範例 1：使用儲存帳戶名稱和容器名稱來建立或更新儲存 Blob 容器的項項
```
PS C:\>Set-AzRmStorageContainerImmutabilityPolicy -ResourceGroupName "myResourceGroup" -AccountName "myStorageAccount" -ContainerName "myContainer" -ImmutabilityPeriod 10
```

此命令會使用儲存帳戶名稱和容器名稱來建立或更新儲存 Blob 容器的一項作業。

### 範例 2：使用 Storage 帳戶物件擴充儲存 Blob 容器的更新更新
```
...
此命令會擴充儲存 Blob 容器的指令性，以及儲存帳戶物件。 擴充的可操作性Policy 只能在鎖定了一個功能之後才能執行。

### 範例 3：更新儲存 Blob 容器的更新服務性
```
...
此命令會使用儲存容器物件更新 3 次儲存 Blob 容器的指令性：第一次更新至 12 天不含 etag 的 12 天，然後使用 etag 將 9 天更新為 9 天，最後啟用 AllowProtectedAppendWrite。

### 範例 4：擴充 Storage Blob 容器的擴充更新性Policy，使用一個可及性Policy 物件
```
PS C:\>Get-AzRmStorageContainerImmutabilityPolicy -ResourceGroupName "myResourceGroup" -AccountName "myStorageAccount" -ContainerName "myContainer" | Set-AzRmStorageContainerImmutabilityPolicy -ImmutabilityPeriod 15 -ExtendPolicy
```

此命令會擴充儲存 Blob 容器的一項服務性，並包含一個與一個資料標籤物件。 擴充的可操作性Policy 只能在鎖定了一個功能之後才能執行。

## 參數

### -AllowProtectedAppendWrite
此屬性只能針對解除鎖定的時間型保留原則變更。 啟用此屬性後，新區塊可以寫入至附加 Blob，同時維持可讀性保護與合規性。 只能新增區塊，而且無法修改或刪除任何現有的區塊。
...
### -Etag
可移動性政策 etag。 如果未指定 -ExtendPolicy，則 Etag 為選擇性;否則必須輸入 Etag。
...
### -ExtendPolicy
表示延伸策略以延伸現有的客戶服務。  鎖定了一個可移動策略之後，它只能延伸。 
