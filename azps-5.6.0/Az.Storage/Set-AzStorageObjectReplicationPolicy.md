...
## 描述
**Set-AzStorageObjectReplicationPolicy** Cmdlet 會建立或更新儲存帳戶中指定的物件複寫原則。

## 例子

### 範例 1：將物件複寫原則設定為目的地和來源帳戶。
```
...
此命令會同時將物件複寫原則設定為目的地和來源帳戶。
首先建立 2 個物件複製原則規則，然後使用 2 個規則將原則設定為目的地帳戶。 接著，將目標帳戶的物件複寫原則設定為來源帳戶。
...
### -PolicyId
物件複寫原則識別碼。它應該是 GUID 或 'default'。
如果不輸入 PolicyId，將會使用 'default'，這表示要建立新策略，而新策略的識別碼會以已建立之策略中返回。
