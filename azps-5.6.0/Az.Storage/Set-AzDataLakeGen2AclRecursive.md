...
## 描述
**Set-AzDataLakeGen2AclRecursive** Cmdlet 會設定指定路徑上的 ACL 遞迴。 輸入 ACL 會完全取代原始的 ACL。
...
此命令會先建立一個包含 3 個 acl 專案之 ACL 物件，然後在目錄中以遞迴的方法設定 ACL。

### 範例 2：在檔案系統的根目錄上設定 ACL 遞迴
```
...
此命令會先將 ACL 遞迴設定為根目錄並失敗，然後在使用者修正失敗檔案之後，使用 ContinuationToken 繼續。

### 範例 3：將 ACL 遞迴區塊設定成區塊
```
$token = $null
...
此腳本會以區塊將 ACL 以遞迴的方法設定為目錄區塊，區塊大小為 BatchSize * MaxBatchCount。 此腳本中的區塊大小為 200。

### 範例 4：在目錄和 ContinueOnFailure 上設定 ACL 遞迴，然後從失敗中一一繼續
```
...
此命令會先將 ACL 遞迴設定為具有 ContinueOnFailure 的目錄，而有些專案失敗，然後再一個一個繼續失敗的專案。

## 參數

### -Acl
POSIX 存取控制清單可針對檔案或目錄設定遞迴。
...
### -BatchSize
如果資料集大小超過批次大小，則作業會分割成多個要求，以便追蹤進度。
批次大小應該介於 1 到 2000 之間。
預設值為 2000。
...
### -ContinueOnFailure
將此參數設定為忽略失敗，並繼續進行目錄其他子實體上的作業。 預設作業在遇到失敗時會快速終止。
...
### -MaxBatchCount
單一變更存取控制作業可以執行的批次數上限。 如果資料集大小超過 MaxBatchCount 乘 BatchSize，將會返回延續權杖。
...
### -路徑
指定 FileSystem 中以遞迴方法變更 Acl 的路徑。
可以是檔案或目錄。
在格式為 'directory/file.txt' 或 'directory1/directory2/'。
跳過設定此參數，以從檔案系統的根目錄以遞迴的方法變更 Acl。
