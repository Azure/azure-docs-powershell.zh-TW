...
## 簡介
建立 ManagementPolicy 規則物件，可用於 Set-AzStorageAccountManagementPolicy。
...
## 描述
**New-AzStorageAccountManagementPolicyRule** Cmdlet 會建立 ManagementPolicy 規則物件，可用於 Set-AzStorageAccountManagementPolicy。

## 例子

### 範例 1：建立 ManagementPolicy 規則物件，然後設定為儲存空間帳戶
```
...
此命令會建立 ManagementPolicy 規則物件，其中 ManagementPolicy 動作群組物件包含 4 個動作 ，即 ManagementPolicy 規則篩選物件，然後將規則設定為儲存帳戶。

## 參數

### -動作
定義動作集的物件。
使用 Cmdlet 取得物件Add-AzureStorageAccountManagementPolicyAction
...
### -篩選
定義篩選集的物件。
使用 Cmdlet 取得物件New-AzureStorageAccountManagementPolicyFilter
