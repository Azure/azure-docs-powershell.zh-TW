...
## 描述
**Add-AzRmStorageContainerLegalHold** Cmdlet 會將法律保留標記新增到儲存 Blob 容器

## 例子

### 範例 1：新增具有儲存帳戶名稱和容器名稱之儲存空間 Blob 容器的法律保留標記
```
...
此命令會新增具有儲存帳戶名稱和容器名稱之儲存空間 Blob 容器的法律保留標記。

### 範例 2：新增具有儲存帳戶物件和容器名稱之儲存空間 Blob 容器的法律保留標記
```
PS C:\>$accountObject = Get-AzStorageAccount -ResourceGroupName "myResourceGroup" -AccountName "myStorageAccount"
PS C:\>Add-AzRmStorageContainerLegalHold -StorageAccount $accountObject -ContainerName "myContainer"  -Tag  tag1
```

此命令會新增具有儲存帳戶物件和容器名稱之儲存空間 Blob 容器的法律保留標記。

### 範例 3：在具有管線的儲存空間帳戶中，將法律保留標記新增到所有儲存 Blob 容器
```
PS C:\>Get-AzStorageContainer -ResourceGroupName "myResourceGroup" -AccountName "myStorageAccount" | Add-AzRmStorageContainerLegalHold -Tag  tag1,tag2,tag3
```

此命令會將法律保留標記新增到具有管線的儲存空間帳戶內的所有儲存 Blob 容器。
