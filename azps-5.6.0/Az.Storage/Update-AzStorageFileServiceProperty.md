...
## 描述
**Update-AzStorageFileServiceProperty** Cmdlet 會修改 Azure 儲存檔案服務的服務屬性。

## 例子

### 範例 1：啟用檔案共用 softdelete
```powershell
...
設定為 $true，以針對儲存空間帳戶啟用共用刪除保留$false。
...
### -ShareRetentionDays
設定共用 DeleteRetentionPolicy 的保留天數。
只有在啟用共用刪除保留原則時，才應設定該值。
