...
## 描述
**Get-AzStorageBlobContent** Cmdlet 會下載指定的儲存 Blob。
如果 Blob 名稱對本地電腦無效，此 Cmdlet 會盡可能自動解析。
...
此範例先取得 Blob 物件並將其儲存為變數，然後下載包含 Blob 物件的 Blob 內容。 
...
您可以建立它，或使用 Get-AzStorageContainer Cmdlet。
...
## 筆記
* 如果 Blob 名稱對本地電腦無效，此 Cmdlet 會盡可能自動解決它。
