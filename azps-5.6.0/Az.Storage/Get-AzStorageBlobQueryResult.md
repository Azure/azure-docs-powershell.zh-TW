...
## 簡介
在 blob 的內容上 (SQL) SQL 查詢語言，並僅將資料查詢子集儲存至本地檔案。
...
## 描述
**Get-AzStorageBlobQueryResult** Cmdlet 會針對 Blob 的內容，將簡單的結構化查詢語言 (SQL) 語句，並儲存查詢的資料子集至本地檔案。
...
此命令會以 csv 的輸入方式查詢 blob 處理，並輸出 config 為 json，並儲存輸出到「c:\resultfile.js」。
...
此命令會先取得 Blob 快照的 Blob 物件，然後查詢 Blob 快照，然後顯示結果包含查詢錯誤。
...
用來處理查詢輸入文字的組配置。 使用 New-AzStorageBlobQueryConfig 建立組組物件。
...
用來處理查詢輸出文字的組配置。 使用 New-AzStorageBlobQueryConfig 建立組組物件。
...
### -QueryString
查詢字串，請參閱以下位置的更多詳細資料： https://docs.microsoft.com/azure/storage/blobs/query-acceleration-sql-reference
