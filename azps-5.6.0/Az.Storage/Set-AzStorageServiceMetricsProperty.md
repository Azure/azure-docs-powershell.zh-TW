...
## 描述
**Set-AzStorageServiceMetricsProperty Cmdlet** 會修改 Azure 儲存體服務的計量屬性。

## 例子

### 範例 1：修改 Blob 服務的度量屬性
```
C:\PS>Set-AzStorageServiceMetricsProperty -ServiceType Blob -MetricsType Hour -MetricsLevel Service -PassThru -RetentionDays 10 -Version 1.0
```

此命令將 Blob 儲存空間的版本 1.0 計量修改為服務層級。
Azure 儲存體服務計量會保留專案 10 天。
由於此命令會指定 *PassThru 參數* ，因此該命令會顯示已修改的度量屬性。
...
### -MetricsType
指定度量類型。
此 Cmdlet 將 Azure 儲存體服務計量類型設定為此參數指定的值。
此參數可接受的值為：小時和分鐘。
