...
## 描述
**Update-AzDataLakeGen2AclRecursive** Cmdlet 會以遞迴的方法更新指定路徑上的 ACL。 輸入 ACL 會合並原始的 ACL：如果具有相同 AccessControlType/EntityId/DefaultScope 的 ACL 專案存在，請更新許可權;否則會新增 ACL 專案。

## 例子

### 範例 1：在檔案系統的根 directiry 上更新 ACL 遞迴
```
...
此命令會先建立一個包含 3 個 acl 專案之 ACL 物件，然後在檔案系統的根目錄上以遞迴的方法更新 ACL。

### 範例 2：更新目錄中的 ACL 遞迴，然後使用 ContinuationToken 從失敗中繼續
```
...
此命令會先將 ACL 遞迴更新至目錄並失敗，然後在使用者修正失敗檔案之後，使用 ContinuationToken 繼續。
...
此腳本會以 BatchSize * MaxBatchCount 的區塊大小，以遞迴的目錄區塊更新 ACL。 此腳本中的區塊大小為 5000。

### 範例 4：在目錄和 ContinueOnFailure 上更新 ACL 遞迴，然後從失敗中一一繼續
```
...
此命令會先以 ContinueOnFailure 將 ACL 遞迴更新至目錄，而有些專案失敗，然後再一個一個繼續失敗的專案。
