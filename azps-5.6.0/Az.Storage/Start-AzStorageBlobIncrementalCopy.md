...
## 描述
啟動從頁面 Blob 快照到指定目的地頁面 Blob 的增量複製作業。
在 中查看此功能的更多詳細資料 https://docs.microsoft.com/rest/api/storageservices/fileservices/incremental-copy-blob 。

## 例子

### 範例 1：根據 Blob 名稱和快照時間啟動增量複製作業
```
...
此命令會啟動按 Blob 名稱和快照時間執行增量複製作業

### 範例 2：使用來源 uri 啟動增量複製作業
```
...
此命令會使用來源 uri 啟動增量複製作業

### 範例 3：從 GetAzureStorageContainer 使用容器管線開始增量複製作業
```
...
此命令會使用來自 GetAzureStorageContainer 的容器管線啟動增量複製作業

### 範例 4：啟動增量複製作業，從 CloudPageBlob 物件到具有 Blob 名稱的目的地 Blob
```
...
此命令會啟動以 Blob 名稱從 CloudPageBlob 物件到目的地 Blob 的增量複製作業

## 參數

### -AbsoluteUri
來源的絕對 Uri。 請注意，如果來源需要任何認證，應在 Uri 中提供認證。
...
### -CloudBlob
來自 Azure 儲存用戶端文件庫的 CloudBlob 物件。 您可以建立或Get-AzStorageBlob Cmdlet。
...
### -CloudBlobContainer
來自 Azure 儲存體用戶端庫的 CloudBlobContainer 物件。 您可以建立或Get-AzStorageContainer Cmdlet。
...
### -內容
來源 Azure 儲存內容。 您可以使用 Cmdlet New-AzStorageContext建立。
...
### -DestCoNtext
目的地 Azure 儲存內容。 您可以使用 Cmdlet New-AzStorageContext建立。
