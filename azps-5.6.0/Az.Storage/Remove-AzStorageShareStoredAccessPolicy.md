...
## 描述
**Remove-AzStorageSharedAccessPolicy** Cmdlet 會從 Azure 儲存空間共用移除儲存的存取策略。

## 例子

### 範例 1：從 Azure 儲存空間共用移除儲存的存取策略
```
PS C:\>Remove-AzStorageShareStoredAccessPolicy -ShareName "ContosoShare" -Policy "GeneralPolicy"
```

此命令會從 ContosoShare 移除名為 GeneralPolicy 的儲存存取策略。
