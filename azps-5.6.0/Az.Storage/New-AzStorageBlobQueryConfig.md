...
## 簡介
建立 Blob 查詢設定檔物件，可用於 Get-AzStorageBlobQueryResult。
...
## 描述
**New-AzStorageBlobQueryConfig** Cmdlet 會建立 Blob 查詢設定檔物件，可用於 Get-AzStorageBlobQueryResult。

## 例子

### 範例 1：建立 Blob 查詢設定，以及查詢 Blob
```powershell
...
此命令會先將輸入組組物件建立為 csv，再將輸出組組物件建立為 json，然後使用 2 組設定檔查詢 Blob。

## 參數

### -AsCsv
表示要建立 CSV 的 Blob 查詢組塊。
