...
## 簡介
新增動作至輸入 ManagementPolicy 動作群組物件，或建立包含該動作的 ManagementPolicy 動作群組物件。 該物件可用於 New-AzStorageAccountManagementPolicyRule。
...
## 描述
**Add-AzStorageAccountManagementPolicyAction** Cmdlet 會新增動作至輸入 ManagementPolicy 動作群組物件，或建立具有該動作的 ManagementPolicy 動作群組物件。

## 例子

### 範例 1：使用 4 個動作建立 ManagementPolicy 動作群組物件，然後將它新增到管理原則規則，並設定為儲存空間帳戶
```
...
第一個命令會建立 ManagementPolicy 動作群組物件，下列 3 個命令會新增 3 個動作至該物件。 然後將它新增到管理原則規則，並設定為儲存空間帳戶。

### 範例 2：在快照和 Blob 版本上建立包含 6 個動作的 ManagementPolicy 動作群組物件，然後將它新增到管理原則規則，並設定為儲存帳戶
```
...
第一個命令會建立 ManagementPolicy 動作群組物件，下列 5 個命令會新增 5 個快照和 Blob 版本的動作至該物件。 然後將它新增到管理原則規則，並設定為儲存空間帳戶。
...
### -DaysAfterModificationGreater方程式
指出上次修改後天數的整數值。
...
### -InputObject
如果輸入 ManagementPolicy 動作物件，就會將動作設定為輸入動作物件。
如果沒有輸入，將會建立新動作物件。
