...
## 描述
**New-AzRmStorageContainer Cmdlet** 會建立儲存 Blob 容器

## 例子

### 範例 1：使用儲存帳戶名稱和容器名稱建立包含中繼資料的儲存 Blob 容器
```
PS C:\>New-AzRmStorageContainer -ResourceGroupName "myResourceGroup" -AccountName "myStorageAccount" -ContainerName "myContainer" -Metadata @{tag0="value0";tag1="value1";tag2="value2"}
```

此命令會建立包含儲存帳戶名稱和容器名稱以及中繼資料的儲存 Blob 容器。

### 範例 2：使用儲存帳戶物件和容器名稱建立具有公用存取為 Blob 的儲存 Blob 容器
```
PS C:\>$accountObject = Get-AzStorageAccount -ResourceGroupName "myResourceGroup" -AccountName "myStorageAccount"
PS C:\>New-AzRmStorageContainer -StorageAccount $accountObject -ContainerName "myContainer" -PublicAccess Blob
```

此命令會建立包含儲存帳戶物件和容器名稱的儲存 Blob 容器，且公用存取為 Blob。

### 範例 3：使用 EncryptionScope 設定建立儲存容器
```
...
此命令會使用 Defalt encryptionScope 建立儲存容器，並且從容器預設值中區塊加密範圍的覆蓋。
然後顯示相關的容器屬性。
