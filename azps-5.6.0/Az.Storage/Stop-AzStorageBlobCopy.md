...
## 描述
**Stop-AzStorageBlobCopy** Cmdlet 會停止對指定目的地 Blob 的複製作業。
...
此命令會從 **Get-AzStorageContainer** 傳遞管線上的容器，以停止複製作業。

### 範例 3：使用管線和管道停止複製Get-AzStorageBlob
```
PS C:\>Get-AzStorageBlob -Container "ContainerName" | Stop-AzStorageBlobCopy -Force
```

此範例會從 Cmdlet 傳遞管線上的容器，以停止Get-AzStorageBlob作業。
...
您可以建立物件，或使用 Get-AzStorageContainer Cmdlet。
...
### -內容
指定 Azure 儲存內容。
您可以使用 Cmdlet 建立New-AzStorageContext內容。
