...
此 Cmdlet 會建立 PostgreSql 伺服器防火牆規則。

### 範例 2：使用 -ClientIPAddress 建立一個新的 PostgreSql 防火牆規則。
```powershell
...
此 Cmdlet 會使用 -ClientIPAddress 建立 PostgreSql 防火牆規則。

### 範例 3：建立新 PostgreSql 防火牆規則以允許所有 IP
```powershell
...
此 Cmdlet 會建立一個新的 PostgreSql 防火牆規則，以允許所有 IP。
