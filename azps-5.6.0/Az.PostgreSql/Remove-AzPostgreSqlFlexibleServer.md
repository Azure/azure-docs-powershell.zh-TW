...
### 範例 1：根據 resourceGroup 和伺服器名稱移除 PostgreSql 伺服器
```powershell
PS C:\> Remove-AzPostgreSqlFlexibleServer -ResourceGroupName PowershellPostgreSqlTest -Name postgresql-test

```

此 Cmdlet 會根據 resourceGroup 和伺服器名稱移除 PostgreSql 伺服器。

### 範例 2：根據身分識別移除 PostgreSql 伺服器
```powershell
...
這些 Cmdlet 會以身分識別移除 PostgreSql 伺服器。
