...
### FileEventTriventParameterSet (預設) 
```
...
## 描述
**New-AzStackEdgeTrigger Cmdlet** 會設定 Stack Edge 裝置上的觸發程式。 
...
### -PeriodicTimerEvent
傳遞此參數以設定 PeriodicTimerEvent Trigger
...
### -排程
需要提高計時器事件的定期頻率。 指定排程 (介於 1 到 365) 、小時 (介於 1 到 23) ，或分鐘 (介於 1 到 59) 。
...
### -ShareId
FileEvent Trigger 中要傳遞的檔案共用識別碼
...
### -StartTime
產生有效觸發的一天中時間。 排程是參照指定最多秒的時間來計算。 如果未指定時區，則時間會視為位於裝置時區。 值一定會以 UTC 時間退回。
...
### Microsoft.Azure.PowerShell.Cmdlets.StackEdge.models.PSStackEdgeTrirr
