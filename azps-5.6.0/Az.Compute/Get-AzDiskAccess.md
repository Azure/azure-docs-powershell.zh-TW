...
## 描述
**Get-Az與Access** Cmdlet 取得磁片存取的屬性
...
此命令會取得資源群組 'ResourceGroup01' 中名為 'DiskAccess01' 的磁片存取資源屬性。

### 範例 2：Get-AzDiskAccess資源群組
```
...
此命令會取得資源群組 "ResourceGroup01" 中所有磁片存取的屬性。
...
此命令會從 'DiskAccessMicrosoft'開始，取得訂閱名稱下所有磁片存取的屬性。

### 範例 5：使用 ResourceId 取得磁片存取。
```
