...
## 描述
**Set-AzSnapshotUpdateCryptencryptionKey** Cmdlet 會設定快照更新物件的磁片加密金鑰屬性。
...
第一個命令會以儲存帳戶類型建立大小為 10 GB 的Premium_LRS快照更新物件。  它也設定 Windows OS 類型，並啟用加密設定。
第二個和第三個命令會設定快照更新物件的磁片加密金鑰和金鑰加密金鑰設定。
最後一個命令會拍攝快照更新物件，並更新資源群組 "ResourceGroup01" 中名稱為 "Snapshot01" 的現有快照。
