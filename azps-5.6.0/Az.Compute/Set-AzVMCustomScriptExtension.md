...
## 描述
**Set-AzTOMCustomScriptExtension** Cmdlet 會新增自訂腳本虛擬機器擴充功能至虛擬機器。
此擴充功能可讓您在虛擬機器上執行自己的腳本。
...
此命令會將自訂腳本新增到名為 VirtualMachine07 的虛擬機器。
腳本檔案會contososcript.exe。
...
### -FileName
指定指令檔名。 如果檔案是儲存在 Azure Blob 儲存體中，檔案名值會區分大小寫。 儲存在 Azure 檔案儲存空間的檔案名不會區分大小寫。
...
### -SecureExecution
表示此 Cmdlet 會確定 *執行* 參數的值並未在伺服器上記錄，或是使用 GET 擴充 API 返回給使用者。
Run *值* 可能包含要安全地傳遞到腳本檔案的機密或密碼。
...
### -TypeHandlerVersion
指定要用於此虛擬機器的擴充功能版本。
若要取得版本，請執行 Get-AzVMExtensionImage Cmdlet，其值為 Microsoft.Compute for *PublisherName* 參數，CustomScriptExtension 為 *Type* 參數。
...
### -VMName
指定虛擬機器的名稱。
此 Cmdlet 會新增此參數指定之虛擬機器的自訂腳本擴充功能。
...
[Get-AzTOMCustomScriptExtension](./Get-AzVMCustomScriptExtension.md)

[Remove-AzTOMCustomScriptExtension](./Remove-AzVMCustomScriptExtension.md)
