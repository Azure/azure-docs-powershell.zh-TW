...
### -Data有Image
資料磁片圖像。   例如@{Source = @{Id=&lt;source_id&gt;};<a0></a0> <a1/&amp;SizeInGB = 100;HostCaching = "ReadOnly" }
...
### -OSAgeImage
作業系統磁片映射，例如 @{Source = @{Id=<source_id>};SizeInGB = 100;HostCaching = "ReadOnly" }
...
如果設定成功，從最新版圖像定義部署的虛擬機器將不會使用此映射版本。
...
### -StorageAccountType
指定要用來儲存影像的儲存空間帳戶類型。 此屬性無法更新。 可用的值Standard_LRS、Standard_ZRS和Premium_LRS。
