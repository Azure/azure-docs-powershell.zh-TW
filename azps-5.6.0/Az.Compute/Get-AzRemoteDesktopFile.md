...
## 描述
**Get-AzRemoteDesktopFile** Cmdlet 會取得遠端桌面通訊協定 (.rdp) 檔案。
...
此命令會針對名為 VirtualMachine07 的虛擬機器，獲得遠端桌面檔案。
該命令將結果儲存在名為 D：\RemoteDesktopFile07.rdp 的檔案中。
...
### -啟動
表示此 Cmdlet 在獲得 .rdp 檔案後，會啟動遠端桌面。
