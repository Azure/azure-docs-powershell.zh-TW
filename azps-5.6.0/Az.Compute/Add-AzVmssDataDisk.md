...
## 描述
**Add-Az VmssDataDataData Cmdlet** 會將資料磁片新增到 VMSS 實例 (虛擬機器縮放) 集。
...
### -DiskIOPSReadWrite
指定受Read-Write磁片的 IOPS 值。 只有在儲存AccountType 已儲存時UltraSSD_LRS。 如果未指定，將會根據 diskSizeGB 指派預設值。
...
### -DiskMBpsReadWrite
指定受管理磁片每秒 MB 的頻寬。 只有在儲存AccountType 已儲存時UltraSSD_LRS。 如果未指定，將會根據 diskSizeGB 指派預設值。
