...
## 描述
**Start-Az Vmss** Cmdlet 會啟動虛擬機器縮放集 (VMSS) 或一組虛擬機器內的所有虛擬機器。
您可以使用 *InstanceId* 參數來選取一組虛擬機器。

## 例子

### 範例 1：在 VMSS 中啟動一組特定的虛擬機器
```
PS C:\> Start-AzVmss -ResourceGroupName "ContosOrg" -VMScaleSetName "ContosoVMSS"-InstanceId "0", "1"
```

此命令會啟動一組由實例識別碼字串陣列所指定的特定虛擬機器，該陣列屬於名為 Contoso VMSS 的 VMSS。

### 範例 2：啟動 VMSS 內的所有虛擬機器
```
PS C:\> Start-AzVmss -ResourceGroupName "ContosOrg" -VMScaleSetName "ContosoVMSS"
```

此命令會啟動所有屬於名為 Contoso VMSS 的虛擬機器。
