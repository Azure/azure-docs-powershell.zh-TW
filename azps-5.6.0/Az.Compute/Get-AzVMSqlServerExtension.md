...
## 描述
**Get-AzCMSqlServerExtension** Cmdlet 會取得 SQL Server 基礎結構的設定做為 (虛擬機器上的 (IaaS) 代理程式。

## 例子

### 範例 1：在虛擬機器上取得 SQL Server 擴充功能設定
```
...
此命令會獲得名為 Contoso VIRTUAL07 的虛擬機器上的 SQL Server 擴充功能設定。
...
此命令會使用 Get-AzVM Cmdlet，在服務 Service08 上獲得名為 ContosoCM22 的虛擬機器。
該命令會使用管線運算子將結果傳遞至目前的 Cmdlet。
目前的命令會獲得該虛擬機器上的 SQL Server IaaS 代理程式設定。

### 範例 3：取得特定 SQL Server 版本的設定
```
...
此命令會獲得名為 Contoso VIRTUAL07 的虛擬機器上 SQL Server 擴充功能版本 1.0 的設定。
