...
## 描述
**Set-Az Vmss** Cmdlet 會針對 VMSS (虛擬機器縮放集設定) 。
此 Cmdlet 支援的唯一動作是重新映射。
...
此命令會重新映射名為 Contoso VMSS 的 VMSS，該 VMSS 屬於名為 ContosoGroup 的資源群組。
...
### -執行Maintenance
表示此 Cmdlet 在 VMSS 中執行維護一或多個虛擬機器。
...
### -重新部署
表示 Cmdlet 會重新部署 VMSS 中的一或多個虛擬機器。
