...
指定是否應該在失敗時自動取代主機。 未提供值時，值預設為 "true"。
...
### -LicenseType
指定將適用于部署在主機上的虛擬機器的軟體授權類型。 可能的值為：無、Windows_Server_Hybrid及Windows_Server_Perpetual。  預設值為 None。
