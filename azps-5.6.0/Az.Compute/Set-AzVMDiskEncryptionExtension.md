...
## 描述
**Set-Az VIRTUAL一cryptionExtension** Cmdlet 可在執行中的基礎結構上啟用加密，做為服務 (在 Azure 中的) IaaS) 虛擬機器。  它會在虛擬機器上安裝磁片加密擴充功能，以啟用加密。 

此 Cmdlet 需要使用者確認，因為啟用加密的步驟之一需要重新開機虛擬機器。

建議您先將工作儲存在虛擬機器上，再執行此 Cmdlet。

Linux：加密 Linux 虛擬機器時需要 **VolumeType** 參數，而且必須設為 ("Os"、"Data" 或 "All") 受 Linux 發佈支援的值。 

Windows： **可能會省略 VolumeType** 參數，在這種情況下，作業會預設為 All;如果 Windows 虛擬機器有 VolumeType 參數，則必須設定為 All 或 OS。
...
### 範例 3：使用 Azure AD 用戶端識別碼和用戶端機密啟用加密
```
$RGName = "MyResourceGroup"
$VMName = "MyTestVM"
...
此範例使用 Azure AD 用戶端識別碼和用戶端金鑰，在 VM 上啟用加密。

### 範例 4：使用 Azure AD 用戶端識別碼和用戶端認證指紋啟用加密
```
$RGName = "MyResourceGroup"
$VMName = "MyTestVM"
...
此範例使用 Azure AD 用戶端識別碼和用戶端認證指紋，在 VM 上啟用加密。

### 範例 5：使用 Azure AD 用戶端識別碼、用戶端金鑰，以及使用金鑰加密金鑰來啟用加密
```
$RGName = "MyResourceGroup"
$VMName = "MyTestVM"
...
此範例使用 Azure AD 用戶端識別碼和用戶端金鑰，在 VM 上啟用加密，並使用金鑰加密金鑰封裝磁片加密金鑰。

### 範例 6：使用 Azure AD 用戶端識別碼、用戶端認證指紋啟用加密，以及使用金鑰加密金鑰來換行磁片加密金鑰
```
$RGName = "MyResourceGroup"
$VMName = "MyTestVM"
#The KeyVault must have enabledForDiskEncryption property set on it
...
此範例使用 Azure AD 用戶端識別碼和用戶端認證指紋來啟用 VM 加密，並使用金鑰加密金鑰封裝磁片加密金鑰。

## 參數

### -AadClientCertThumbprint
指定 Azure AD (Azure AD) 具有寫入 **KeyVault** 機密許可權的應用程式用戶端憑證的指紋。
先決條件是，Azure AD 用戶端憑證之前必須部署至虛擬機器的本機電腦 `my` 憑證存放區。
Add-AzVMSecret Cmdlet 可用來將憑證部署到 Azure 中的虛擬機器。
有關詳細資料，請參閱 **Add-AzMSSECret** Cmdlet 說明。
憑證之前必須部署至虛擬機器本機電腦我的憑證存放區。
...
### -AadClientID
指定有權寫入 **KeyVault** 機密的 Azure AD 應用程式的用戶端識別碼。
...
### -AadClientSecret
指定具有許可權寫入 **KeyVault** 之 Azure AD 應用程式的用戶端機密。
...
### -KeyEncryptionKeyUrl
指定用來封裝和取消封裝虛擬機器加密金鑰的金鑰加密金鑰 URL。
這必須是完整版的 URL。
...
指定 **KeyVault** 的資源識別碼，其中包含用來封裝和取消封裝虛擬機器加密金鑰的金鑰加密金鑰。
這必須是完整版的 URL。
...
### -名稱
指定代表擴充功能之 Azure Resource Manager 資源的名稱。 如果 *省略 Name* 參數，安裝的副檔名為 Windows 虛擬機器上的 AzureCryptEncryption，而 Linux 虛擬機器上的 AzureCryptEncryptionForWindows。
...
### -Passphrase
指定用於加密 Linux 虛擬機器的密碼。
此參數不會用於執行 Windows 作業系統的虛擬機器。
...
### -SequenceVersion
指定虛擬機器加密作業的序號。
這是每個在同一虛擬機器上執行的每個加密作業所唯一的。
您可以使用Get-AzVMExtension Cmdlet 來取回先前使用的序號。
...
[Add-AzMSSecret](./Add-AzVMSecret.md)
