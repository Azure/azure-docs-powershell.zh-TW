...
## 描述
**Remove-Az VmssNetworkInterfaceConfiguration Cmdlet** 會從虛擬機器縮放集 (VMSS) 移除網路介面設定。
...
第一個命令會使用 Get-AzVmss Cmdlet 來獲得 VMSS，然後將它儲存在 $VMSS 變數中。
第二個命令會從 $VMSS 中的集合移除名為 ContosoEvssInterface02 的網路介面$VMSS。
