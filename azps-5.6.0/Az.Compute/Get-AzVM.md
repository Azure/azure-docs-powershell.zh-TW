...
## 描述
**Get-AzCMdlet** 會取得 Azure 虛擬機器的模型視圖或實例視圖。
模型視圖是使用者指定的虛擬機器屬性。
實例視圖是虛擬機器的實例層級狀態。
指定 *狀態* 參數以取得虛擬機器的實例視圖，而不是預設的模型視圖。
...
此命令會獲得名為 VirtualMachine07 之虛擬機器的模型視圖和實例視圖屬性。
...
此命令會獲得名為 VirtualMachine07 的虛擬機器屬性。
此命令會指定 *狀態* 參數。
因此，命令只會獲得實例視圖屬性。
...
此命令會針對資源群組中名為 ResourceGroup11 的所有虛擬機器，獲得屬性。
...
### -DisplayHint
決定虛擬機器物件的顯示方式。
有效的值為： -- 精簡：僅顯示頂層屬性 -- 展開：顯示所有層級的所有屬性
