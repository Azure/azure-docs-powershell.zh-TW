...
## 簡介
Register-AzStackHCI建立代表內部部署組群的 Microsoft.AzureStackHCI 雲端資源，然後向 Azure 登錄內部部署組。
...
### -ComputerName
指定要註冊至 Azure 之內部部署組集中的組名或其中一個集區節點。
...
### -RepairRegistration
使用雲端修復目前的 Azure Stack HCI 註冊。 此 Cmdlet 會刪除集區節點上的本地憑證，以及雲端 Azure AD 應用程式中的遠端憑證，並產生兩者的新取代憑證。 資源群組、資源名稱及其他註冊選項會保留。
