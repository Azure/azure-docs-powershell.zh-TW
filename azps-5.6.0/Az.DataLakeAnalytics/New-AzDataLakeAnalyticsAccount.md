...
## 描述
**New-AzDataLakeAnalyticsAccount** Cmdlet 會建立 Azure Data Lake Analytics 帳戶。

## 例子

### 範例 1：建立 Data Lake Analytics 帳戶
```
...
此命令會建立名為 ContosoAdlAccount的 Data Lake Analytics 帳戶，該帳戶使用 ContosoAdlStore Data Store，位於名為 ContosoOrg 的資源群組中。

## 參數

### -DefaultDataLakeStore
指定要設為預設資料來源的資料湖市集帳戶名稱。
...
### -位置
指定建立 Data Lake Analytics 帳戶的位置。
目前僅支援美國東部 2。
...
### -MaxJobCount
同時在帳戶下所支援的選擇性工作上限。 如果沒有指定，預設值為 3
...
### -QueryStoreRetention
保留工作中繼資料的選擇性天數。 如果沒有指定，預設值為 30 天。
