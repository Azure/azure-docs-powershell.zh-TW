...
## 描述
此New-AzDataLakeAnalyticsCatalogCredential Cmdlet 會建立新認證，以用於 Azure Data Lake Analytics 目錄中，以連接至外部資料源。
...
此命令會使用 HTTPs 通訊協定，為指定的帳號、資料庫、主機和埠建立指定的認證。

### 範例 2：為指定完整 URI 的目錄建立認證
```
PS C:\> New-AzDataLakeAnalyticsCatalogCredential -AccountName "ContosoAdlAccount" `
                  -DatabaseName "databaseName" `
                  -CredentialName "exampleDbCred" `
...
此命令會為指定的帳號、資料庫和外部資料源 URI 建立指定的認證。
