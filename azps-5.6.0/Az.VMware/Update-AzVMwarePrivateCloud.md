...
### -IdentitySource
vCenter單一登入身分識別來源若要建構，請參閱 IDENTITYSOURCE 屬性的 NOTES 區段，並建立雜湊表。
...
IDENTITYSOURCE <IIdentitySource[]>：vCenter單一登入身分識別來源
  - `[Alias <String>]`：網域的 NetBIOS 名稱
  - `[BaseGroupDn <String>]`：群組的基底區別名稱
  - `[BaseUserDn <String>]`：使用者的基本區別名稱
  - `[Domain <String>]`：網域的 dns 名稱
  - `[Name <String>]`：身分識別來源的名稱
  - `[Password <String>]`：Active Directory 使用者的密碼，使用者和群組至少具有基本 DN 的唯讀存取權。
  - `[PrimaryServer <String>]`：主伺服器 URL
  - `[SecondaryServer <String>]`：次要伺服器 URL
  - `[Ssl <SslEnum?>]`：使用 SSL 憑證保護 LDAP 通訊 (LDAPS) 
  - `[Username <String>]`：具有使用者和群組基本 DN 唯讀存取權最低之 Active Directory 使用者的識別碼
