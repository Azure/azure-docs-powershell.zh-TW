...
## 描述
**Set-AzBatchJobSchedule** Cmdlet 會設定 Azure 批次處理服務中的工作排程。
...
第一個命令會使用 **Get-AzBatchJobSchedule** 取得工作，然後將它儲存在$JobSchedule變數中。
第二個命令會修改物件的週期 `$JobSchedule.Schedule` 間隔。
最後一個命令會更新 Batch 服務，以配合 $JobSchedule。
...
### -JobSchedule
指定 **代表工作排程的 PSCloudJobSchedule** 物件。
