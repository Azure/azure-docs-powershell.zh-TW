...
## 描述
**Set-AzBatchJob** Cmdlet 會更新 Azure Batch 工作。
使用 Get-AzBatchJob Cmdlet 取得 **PSCloudJob** 物件。
...
第一個命令會使用 **Get-AzBatchJob** 取得工作，然後將它儲存在$Job變數中。
第二個命令會修改物件$Job規格。
最後一個命令會更新 Batch 服務，以配合 $Job。
...
### -Job
指定此 **Cmdlet 更新批次處理服務的 PSCloudJob。**
