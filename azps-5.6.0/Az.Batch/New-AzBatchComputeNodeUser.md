...
## 描述
**New-AzBatchComputeNodeUser** Cmdlet 在 Azure Batch 計算節點上建立使用者帳戶。

## 例子

### 範例 1：建立具有系統管理認證之使用者帳戶
```
PS C:\>New-AzBatchComputeNodeUser -PoolId "MyPool01" -ComputeNodeId "ComputeNode01" -Name "TestUser" -Password "Password" -ExpiryTime ([DateTime]::Now.AddDays(7)) -IsAdmin -BatchContext $Context
```

此命令在具有 ID ComputeNode01 的計算節點上建立使用者帳戶。
節點位於具有 ID MyPool01 的集中。
使用者名稱為 TestUser、密碼為密碼、帳戶會在七天后過期，且帳戶具有系統管理認證。

### 範例 2：使用管線在計算節點上建立使用者帳戶
```
PS C:\>Get-AzBatchComputeNode "MyPool01" -ComputeNodeId "ComputeNode01" -BatchContext $Context | New-AzBatchComputeNodeUser -Name "TestUser" -Password "Password" -BatchContext $Context
```

此命令會使用 **Get-AzBatchComputeNode** Cmdlet 取得名為 ComputeNode01 的計算節點。
該節點位於具有 ID MyPool01 的集中。
該命令會使用管線運算子，將計算節點傳遞至目前的 Cmdlet。
該命令會建立一個使用者帳戶，該帳戶具有使用者名稱 TestUser 和密碼密碼。
...
### -ComputeNode
指定計算節點作為 **PSComputeNode** 物件，此 Cmdlet 會在此物件上建立使用者帳戶。
