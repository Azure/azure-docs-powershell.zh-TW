...
## 描述
**Get-AzBatchAccount** Cmdlet 會取得目前訂閱中的 Azure Batch 帳戶。 您可以使用 *AccountName* 參數取得單一帳戶，或使用 *ResourceGroupName* 參數取得該資源群組下的帳戶。
...
此命令會獲得名為 pfuller 的批次帳戶。

### 範例 2：取得與資源群組相關聯的批次帳戶
```
...
此命令會獲得與 CmdletExampleRG 資源群組相關聯的批次處理帳戶。

## 參數

### -AccountName
指定帳戶的名稱。
如果您指定帳戶名稱，此 Cmdlet 只會返回該帳戶。
...
### -ResourceGroupName
指定資源組的名稱。
如果您指定資源群組，此 Cmdlet 會獲得指定資源群組下的帳戶。
...
### -標記
以雜湊表格形式建立索引鍵值組。 例如：@{key0="value0";key1=$null;key2="value2"} 此 Cmdlet 會獲得包含此參數指定之標籤的帳戶。
