...
## 描述
**Get-AzBatchTask** Cmdlet 會取得批次工作的 Azure 批次處理工作。
使用 *JobId* 參數或 Job 參數 *指定* 工作。
若要取得單一工作，請指定 *Id* 參數。
您可以指定 *Filter* 參數，以取得符合 OData (開啟資料通訊協定) 的工作。
...
此命令會獲得 Job Job01 下具有 ID Task03 的工作。
使用 Get-AzBatchAccountKey Cmdlet 將上下文指派給$CoNtext變數。

### 範例 2：從指定的工作取得所有已完成的工作
```
...
此命令會從具有識別碼 Job02 的工作中，獲得已完成的工作。
...
### -篩選
指定任務的 OData 篩選子句。
如果您未指定篩選，此 Cmdlet 會針對批次帳戶或工作，會返回所有工作。
