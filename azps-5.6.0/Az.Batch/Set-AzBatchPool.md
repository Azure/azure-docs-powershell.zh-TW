...
## 描述
**Set-AzBatchPool** Cmdlet 會更新 Azure Batch 服務中的集區屬性。
使用 Get-AzBatchPool Cmdlet 取得 **PSCloudPool** 物件。
...
第一個命令會使用 **Get-AzBatchPool** 取得一個資料庫，然後將它儲存在$Pool變數中。
接下來三個命令會修改物件上的$Pool規格。
最後一個命令會更新 Batch 服務，以與 $Pool 中的本地$Pool。
...
### -Pool
指定此 **Cmdlet** 更新批次處理服務的 PSCloudPool。
