...
## 描述
**Remove-AzBatchNodeFile** Cmdlet 會刪除工作或計算節點的 Azure 批次處理節點檔案。
...
此命令會刪除名為 wd\testFile.txt。
該檔案與工作 Job-000001 下具有識別碼工作 26 的工作相關聯。
...
此命令會從具有 ID Pool07 的startup\testFile.txt計算節點中刪除名為 startup\testFile.txt 的節點檔案。
...
此命令會使用 **Get-AzBatchNodeFile 取得節點檔案**。
該檔案與工作 Job-000001 下具有識別碼工作 26 的工作相關聯。
該命令會使用管線，將檔案傳遞至目前的 Cmdlet。
目前的 Cmdlet 會移除節點檔案。
命令會指定 *Force* 參數。
因此，命令不會提示您確認。
...
### -ComputeNodeId
指定包含此 Cmdlet 刪除之 Batch-節點檔案的計算節點識別碼。
...
### -InputObject
指定 **PSNodeFile** 物件，代表此 Cmdlet 刪除的節點檔案。
...
### -PoolId
指定包含此 Cmdlet 移除檔案之計算節點之資料庫的識別碼。
...
### -遞迴
表示此 Cmdlet 會刪除資料夾，以及指定路徑下的所有子資料夾和檔案。
只有在路徑是資料夾時，此 Cmdlet 才具有相關性。
