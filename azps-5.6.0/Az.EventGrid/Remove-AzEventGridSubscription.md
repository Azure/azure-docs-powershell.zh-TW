...
將事件訂閱 \` EventSubscription1 移除 \` 至資源群組 \` \` \` MyResourceGroupName 中的 Azure 事件格線主題 \` Topic1。
...
將事件訂閱 \` EventSubscription1 移除 \` 至資源群組 \` MyResourceGroupName。 \`
...
將事件訂閱 \` EventSubscription1 \` 移除至預設的 Azure 訂閱。
...
將事件訂閱 \` EventSubscription1 \` 移除至事件中樞命名空間。
...
將事件訂閱 \` EventSubscription1 \` 移除至事件格線主題。
