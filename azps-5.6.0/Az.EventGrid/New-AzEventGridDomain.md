...
## 描述
建立新 Azure 事件格線網域。 網域建立之後，應用程式就可以將事件發佈至主題端點。

## 例子

### 範例 1

在資源群組 \` \` MyResourceGroupName 中，于指定的地理位置 \` westus2 中建立事件格線 \` \` 網域網域 \` 1。
...
在資源群組 \` MyResourceGroupName 中，于指定的地理位置 westus2 中建立事件格線網域網域1，並指定標記 \` \` 「部門」和 \` \` \` 「環境」。
