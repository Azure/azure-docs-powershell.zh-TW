...
此命令會獲得訂閱下的所有 Windows IoT 服務。

### 範例 2：在資源群組下取得所有 Windows IoT 服務
```powershell
...
此命令會獲得資源群組下的所有 Windows IoT 服務。

### 範例 3：按名稱取得 Windows IoT 服務
```powershell
...
此命令會按名稱獲得 Windows IoT 服務。

### 範例 4：根據物件取得 Windows IoT 服務
```powershell
...
此命令會按物件獲得 Windows IoT 服務。

### 範例 4：根據管線取得 Windows IoT 服務
```powershell
...
INPUTOBJECT： <IWindowsIotServicesIdentity> 身分識別參數
  ...
  - `[Id <String>]`：資源識別路徑
  - `[ResourceGroupName <String>]`：包含 Windows IoT 裝置服務的資源組名。
