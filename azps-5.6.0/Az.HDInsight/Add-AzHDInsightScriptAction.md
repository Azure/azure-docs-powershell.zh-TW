...
## 描述
**Add-AzHDInsightScriptAction** Cmdlet 會新增腳本動作至由 New-AzHDInsightClusterConfig 建立 HDInsight 組組物件。
腳本動作提供的功能可用來安裝其他軟體，或變更在 Hadoop cluster 上執行的應用程式組式，其使用 Windows PowerShell 或 Bash 腳本 (分別適用于 Windows 或 Linux) 。
部署 HDInsight 組組時，腳本動作會于該組節點上執行，而且會以完成 HDInsight 配置的節點執行。
腳本動作會以系統管理員帳戶許可權執行，並提供組節點的完整存取權限。
您可以提供每一組腳本動作清單，以指定的循序執行。
...
此命令會為您的 hadoop-001 集的 Head 和 Worker 節點新增腳本動作，以在建立集區結束時執行。
