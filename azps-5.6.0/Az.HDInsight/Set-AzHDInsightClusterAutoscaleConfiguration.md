...
## 描述
此 Cmdlet **Set-AzHDInsightClusterAutoscaleConfiguration 會** 設定 Azure HDInsight 叢集的自動縮放設定。

## 例子

### 範例 1：設定 HDInsight cluster 的載入型自動縮放設定
```powershell
...
此命令會設定 Azure HDInsight 套件的載入型自動縮放設定。

### 範例 2：設定 HDInsight 集區之以排程為基礎的自動縮放比例
```powershell
...
此命令會設定 HDInsight 套件的排程自動縮放設定。

### 範例 3：設定另一個已設定自動縮放設定之 HDInsight cluster 的自動縮放設定
```powershell
...
此命令會設定另一個組群的 HDInsight 集區自動縮放設定。
