...
此 Cmdlet 會建立 MySql 伺服器防火牆規則。

### 範例 2：使用 -ClientIPAddress 建立新 MySql 防火牆規則。
```powershell
...
此 Cmdlet 會使用 -ClientIPAddress 建立 MySql 防火牆規則。

### 範例 3：建立新 MySql 防火牆規則以允許所有 IP
```powershell
...
此 Cmdlet 會建立新的 MySql 防火牆規則，以允許所有 IP。
