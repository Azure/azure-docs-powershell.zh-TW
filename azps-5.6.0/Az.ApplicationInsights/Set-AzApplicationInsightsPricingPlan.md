...
## 描述
設定應用程式深入資訊資源的價格計畫和每日資料量資訊。
此 Cmdlet 無法設定 2018 年 4 月之後所建立之應用程式深入見解的定價方案： https://docs.microsoft.com/azure/azure-monitor/app/pricing#legacy-enterprise-per-node-pricing-tier

## 例子

### 範例 1 設定應用程式深入資訊資源的價格計畫和每日資料量資訊
```
...
將定價方案設定為「基本」、將每日資料量上限設定為每天 400 GB，並停止在資源群組「測試群組」中達到資源「測試」上限時傳送通知
