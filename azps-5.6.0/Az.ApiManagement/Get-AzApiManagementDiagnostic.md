...
## 簡介
取得在服務層級或 Api 層級所配置的診斷詳細資料。 診斷是用來記錄 Api 管理閘道的要求/回應。
...
## 描述
**Get-AzApiManagementDiagnostic** 會取得特定範圍內 Api 管理服務中所配置的診斷詳細資料。
...
此命令會獲得 Api 管理服務中所有已配置的診斷。

### 範例 2：在 Api 範圍中取得所有診斷
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
此命令會以 Api 範圍進行所有 `echo-api` 診斷

### 範例 3：取得 Id 指定的 API 範圍診斷
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
此命令會以 `applicationinsights` api 進行診斷 `echo-api` 。
...
### -ResourceId
診斷或 Api 診斷的 Arm 資源識別碼。 如果指定，會嘗試尋找識別碼的診斷。 此參數為必填項。
