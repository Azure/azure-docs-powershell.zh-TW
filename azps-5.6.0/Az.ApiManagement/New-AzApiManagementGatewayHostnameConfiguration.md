...
## 簡介
為現有的閘道建立 hostname configuratin。
...
## 描述
**New-AzApiManagementGatewayHostnameConfiguration** Cmdlet 會為現有的閘道建立 hostname configuratin。
...
此命令會為「g01」閘道建立「h01」主機名稱組。

## 參數

### -CertificateResourceId
現有憑證識別碼的資源識別碼。此參數為必填項。
...
標出以強制執行 NegotiateClientCertificate。
此參數為選擇性。
