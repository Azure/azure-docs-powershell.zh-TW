...
## 描述
**Get-AzApiManagementPolidlet** 會取得指定的範圍策略。
...
此命令會獲得租使用者層級策略，並存到名為 tenantpolicy.xml。
...
此命令會以非 Xml 逸出格式獲得租使用者範圍策略。

## 參數

### -ApiId
指定現有 API 的識別碼。
如果您指定此參數，Cmdlet 會返回 API 範圍策略。
...
### -ApiRevision
API 修訂的識別碼。 此參數為選擇性。 如果未指定，將會從目前使用中的 api 修訂中取回該規則。
...
### -OperationId
指定現有 API 作業的識別碼。
如果您使用 *ApiId* 指定此參數，Cmdlet 會返回作業範圍策略。
