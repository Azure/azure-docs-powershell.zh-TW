...
## 描述
**Publish-AzApiManagementTenant GitConfiguration** Cmdlet 會發佈從 Git 分支到組組資料庫的變更。
您也可以在 Git 分支中驗證變更，而不發佈。
...
此命令會針對組組資料庫驗證 Git 分支中的變更。
它不會發佈變更。

## 參數

### -分支
指定此 Cmdlet 將群組原則部署到群組原則資料庫的 Git 分支名稱。
...
### -PassThru
表示此 Cmdlet 會返回 **PsApiManagementOperationResult** 物件。
