...
## 描述
**Save-AzApiManagementTenant GitConfiguration** Cmdlet 會建立包含存放庫中分支之目前組配置快照的確認，以儲存變更。
...
此命令會儲存變更，以建立存放庫中指定分支之目前組配置快照的提交。

## 參數

### -分支
指定要提交目前群組原則快照的 Git 分支名稱。
...
### -強制
指定此 Cmdlet 會向 Git 存放庫提交目前的群組原則資料庫，即使 Git 存放庫有較新的變更已重寫。
