...
此範例會獲得 Api 管理實例，將它縮放為五個進階單位，然後將額外的三個單位新增到進階區域。
...
此命令會更新現有的 API 管理部署，並加入外部 *VpnType。*

### 範例 3：使用 KeyVault Resource 的機密建立和初始化 PsApiManagementCustomHostNameConfiguration 的實例
```powershell
...
### 範例 4：更新 Publisher 電子郵件、NotificationSender 電子郵件與組織名稱
```powershell
...
### -PassThru
如果作業成功，會傳送更新的 PsApiManagement 到管線。
