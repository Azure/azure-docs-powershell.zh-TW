...
## 描述
**New-AzApiManagementGroup** Cmdlet 會建立 API 管理群組。
...
### -內容
指定 **PsApiManagementCoNtext 物件** 的實例。
...
### -ExternalId
對於外部群組，此屬性包含來自外部身分識別提供者的群組識別碼，例如 Azure Active Directory aad://contoso5api.onmicrosoft.com/groups/12ad42b1-592f-4664-a77b4250-2f2e82579f4c;否則值為 Null。
...
### -類型
群組類型。 自訂群組是使用者定義的群組。 系統群組包含系統管理員、開發人員和來賓。 您無法建立或更新系統群組。  外部群組是來自外部身分識別提供者的群組，例如 Azure Active Directory。 此參數為選擇性，且預設會假設為自訂群組。
