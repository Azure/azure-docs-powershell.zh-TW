...
## 簡介
會建立一個 `PsApiManagementCustomHostNameConfiguration` .
...
## 描述
**New-AzApiManagementCustomHostnameConfiguration** Cmdlet 是建立 **PsApiManagementCustomHostNameConfiguration** 實例的協助程式命令。
此命令會與 New-AzApiManagement 和 Set-AzApiManagement 一起使用。

## 例子

### 範例 1：使用檔案的 Ssl 憑證建立 PsApiManagementCustomHostNameConfiguration 實例並初始化
```powershell
...
此命令會建立並初始化入口網站 **PsApiManagementCustomHostNameConfiguration** 的實例。 接著，它會使用自訂 hostname 設定檔來建立新的 ApiManagement 服務。

### 範例 2：使用 KeyVault Resource 的機密建立和初始化 PsApiManagementCustomHostNameConfiguration 的實例
```powershell
...
### -KeyVaultId
KeyVaultId to the secret stor the Custom SSL Certificate.
