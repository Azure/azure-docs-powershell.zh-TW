...
## 描述
**Get-AzApiManagementApiSchema Cmdlet** 會取得 API 架構的詳細資訊

## 例子

### 範例 1：取得 Api 的所有 Api 架構詳細資料
```powershell
...
此命令會針對特定 ApiManagement 上下文，獲得與 Api `swagger-petstore-extensive` 相關聯的所有 API 架構。

### 範例 2：取得與 Api 相關聯的特定架構
```powershell
PS D:\github\azure-powershell> $context = New-AzApiManagementContext -ResourceId /subscriptions/subid/resourceGroups/resourceGroupName/providers/Microsoft.ApiManagement/service/sdktestapim4163
...
此命令會針對特定 `5cc9cf67e6ed3b1154e638bd` ApiManagement 上下文，獲得與 `swagger-petstore-extensive` Api 相關聯的 API 架構。
...
### -ResourceId
Api 架構的 Arm 資源識別碼。 如果指定，會嘗試根據識別碼尋找 api 架構。 此參數為必填項。
