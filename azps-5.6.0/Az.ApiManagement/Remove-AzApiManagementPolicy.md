...
## 描述
**Remove-AzApiManagementPolidlet** 會從指定的範圍移除 API 管理政策。
...
此命令會從 API 管理移除作業範圍策略。

## 參數

### -ApiId
指定現有 API 的識別碼。
如果您指定此參數，Cmdlet 會移除 API 範圍策略。
...
### -OperationId
指定現有作業的識別碼。
如果您使用 *ApiId* 參數指定此參數，此 Cmdlet 會移除作業範圍策略。
