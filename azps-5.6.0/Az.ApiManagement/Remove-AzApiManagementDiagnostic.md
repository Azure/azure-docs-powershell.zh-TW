...
## 描述
Cmdlet **Remove-AzApiManagementDiagnostic** 會從全域範圍或範圍移除 `DiagnosticId` 指定的診斷 `ApiId` 實體
...
此範例會從 `applicationinsights` Api 管理服務移除診斷。
...
### -InputObject
PsApiManagementDiagnostic 的實例。 此參數為必填項。
