...
## 描述
**Set-AzApiManagementOperation Cmdlet** 會設定 API 作業詳細資料。
...
### -ApiRevision
API 修訂的識別碼。 此參數為選擇性。 如果未指定，將會更新目前使用中的 Api 修訂中的作業。
...
### -TemplateParameters
指定參數 *UrlTemplate* 中定義的陣列或參數。
如果您未指定值，將會根據 UrlTemplate 產生預設值。
使用參數提供描述、類型及其他可能值等參數的更多詳細資料。
...
### -UrlTemplate
指定 URL 範本。
例如：customers/{cid}/orders/{oid}/？date={date}。
