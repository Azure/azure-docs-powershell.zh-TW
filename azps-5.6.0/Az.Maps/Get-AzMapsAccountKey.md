...
## 描述
Cmdlet Get-AzMapsAccountKey提供 Azure 地圖服務帳戶的 API 金鑰。
Azure 地圖帳戶有兩個 API 金鑰：主要和次要。
這些按鍵可啟用與 Azure 地圖服務帳戶端點的互動。
使用New-AzMapsAccountKey (New-AzMapsAccountKey.md) 重新產生金鑰。
...
會針對資源群組 MyResourceGroup 中名為 MyAccount 的帳戶，返回金鑰。
