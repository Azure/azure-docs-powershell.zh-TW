...
## 描述
Cmdlet Update-AzContainerRegistry更新容器註冊表。

## 例子

### 範例 1：為指定的容器登錄啟用系統管理員使用者
```powershell
...
此命令可讓系統管理員使用者使用指定的容器登錄。

### 範例 2：設定指定容器登錄所使用的儲存空間帳戶
```powershell
...
此命令會設定指定的容器登錄，以在同一個訂閱中使用現有的儲存 \` \` 帳戶 mystorageaccount。
