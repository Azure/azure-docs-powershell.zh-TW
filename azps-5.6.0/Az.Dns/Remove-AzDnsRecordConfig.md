...
## 描述
**Remove-AzDnsRecordConfig Cmdlet** 會從記錄集 (DNS) 網域名稱系統。
**RecordSet** 物件是離線物件，而變更不會變更 DNS 回應，直到您執行 Set-AzDnsRecordSet Cmdlet 以將變更持續變更至 Microsoft Azure DNS 服務之後。
若要移除記錄，該記錄類型的所有欄位必須完全相符。
您無法新增或移除 DNS 記錄。
建立 DNS 區域時會自動建立 DNS 記錄，並會在刪除 DNS 區域時自動刪除。
您可以將 **RecordSet 物件** 以參數或管道運算子傳遞至此 Cmdlet。
...
此範例會從現有的記錄集移除 A 記錄。
如果這是記錄集的唯一記錄，結果就會是空白的記錄集。
若要完全移除記錄集，請參閱 Remove-AzDnsRecordSet。
...
此範例會從現有的記錄集移除 CNAME 記錄。
由於 CNAME 記錄集最多隻能包含一筆記錄，因此結果為空白的記錄集。
