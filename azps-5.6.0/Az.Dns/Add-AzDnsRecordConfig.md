...
## 描述
**Add-AzDnsRecordConfig** Cmdlet 會新增網域名稱系統 (DNS) 記錄至 **RecordSet** 物件。
**RecordSet** 物件是離線物件，而變更不會變更 DNS 回應，直到您執行 Set-AzDnsRecordSet Cmdlet 以將變更持續變更至 Microsoft Azure DNS 服務之後。
建立 DNS 區域時，會建立 DNS 記錄，而 DNS 區域刪除時，會移除這些記錄。
您無法新增或移除 DNS 記錄，但可以編輯記錄。
...
由於 CNAME 記錄集最多隻能包含一筆記錄，因此一開始它必須是空白記錄集，或必須使用 Remove-AzDnsRecordConfig 移除現有的記錄。
