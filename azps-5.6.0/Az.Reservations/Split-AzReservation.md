...
## 描述
使用指定的 `Reservation` 數量 `Reservation` 分配將一個分割成兩個 s。
...
使用對應的 `Reservation` 數量將指定的 `Reservation` 專案分割成兩個 s
...
### -ReservationOrderId
包含使用者想要 `ReservationOrder` `Reservation` 分割之識別碼
