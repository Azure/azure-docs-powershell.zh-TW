...
將指定為單一和 `Reservation` InstanceFlexibility 的 AppliedScopeType 更新為 On。
...
將指定給 Shared 和 `Reservation` InstanceFlexibility 的 AppliedScopeType 更新為 Off。

## 參數

### -AppliedScope
要針對此 `Reservation` 進行申請的 SubscriptionId
...
### -InstanceFlexibility
如果有，請更新的 InstanceFlexibility 值 `Reservation` 。 如果未指定，現有值會維持不變。
