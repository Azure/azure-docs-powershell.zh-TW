...
## 描述
清單 `Reservation` 位於單一範圍內 `ReservationOrder` 。
...
清單 `Reservation` 位於指定的範圍內 `ReservationOrder` 。
...
### -ReservationOrderId
包含的 `ReservationOrder` 識別碼 `Reservation` 。 必填。
