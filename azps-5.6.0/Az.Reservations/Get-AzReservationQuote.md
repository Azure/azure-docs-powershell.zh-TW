...
取得目錄之後，客戶就可以根據位置取得不同的產品。 使用這些資訊，正確檢查價格

## 參數

### -AppliedScope
將適用權益的訂閱。 如果 --applied-scope-type 為 Single，則為必填項。 請勿指定 --applied-scope 類型是否為共用。
...
### -SKU
SKU 名稱，使用命令 az 預約目錄顯示來取得 SKU 清單
