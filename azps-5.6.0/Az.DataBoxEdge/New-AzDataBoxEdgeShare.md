...
## 描述
**New-AzDataBoxEdgeShare** Cmdlet 在 Data Box Edge 裝置上建立新共用。
...
### -ClientAccessRight
ClientIds 的讀/寫 Access，例如：@ (@{"ClientId"="192.168.10.10";」AccessRight"="NoAccess"}， @{"ClientId"="192.168.10.11";"AccessRight"="ReadOnly"}) 
...
### -CloudShare
提供現有的 StorageAccountCredential 資源名稱
...
### -ContainerName
容器名稱 (根據指定的資料格式，這代表 Azure 檔案/Pageblob/Block blob) 
...
### -UserAccessRight
提供存取權以及現有使用者名稱，以存取 SMB 共用類型，例如：@ (@{"Username"="user-name-1";"AccessRight"="Read"}， @{"Username"="user-name-2";"AccessRight"="Read"}， @{"Username"="user-name-3";"AccessRight"="Custom"}) 
...
### Microsoft.Azure.PowerShell.Cmdlets.DataBoxEdge.Models.PSDBoxEdgeShare
