...
## 描述
**Invoke-AzDataBoxEdgeDevice Cmdlet** 會以動作來掃描、下載及安裝 Data Box Edge 裝置上的更新。 自動掃描每天在裝置上執行，您可以執行此 Cmdlet 來明確觸發掃描。
