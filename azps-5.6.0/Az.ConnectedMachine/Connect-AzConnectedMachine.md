...
使用 PowerShell 重做連接裝置，將遠端電腦上線。
注意：目前僅支援 Windows 做為目標。
...
### -PSSession
指定時，將電腦上標至 Azure 的命令將會在每個 PSSession 中執行。
注意：這僅適用于 Windows 目前。
