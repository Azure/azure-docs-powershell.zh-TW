...
## 描述
**New-AzPrivateDnsVirtualNetworkLink** Cmdlet 會建立指定的資源群組和私人區域中的新專用網域名稱系統 (DNS) 虛擬網路連結。 您必須為 Name 參數指定唯一的連結名稱，否則 Cmdlet 會返回錯誤。 您可以使用確認 *參數$ConfirmPreference* Windows PowerShell 變數來控制 Cmdlet 是否提示您確認。

## 例子

### 範例 1：建立私人 DNS 虛擬網路連結
```
...
此命令會建立與名為 myzone.com 的私人 DNS 區域及虛擬網路 "myvirtualnetwork" (相關聯的新虛擬網路連結，該連結已在指定的資源群組中的資源群組) 中建立，然後儲存在 $Link 變數中。
