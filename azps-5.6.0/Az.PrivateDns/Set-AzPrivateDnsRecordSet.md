...
## 描述
Cmdlet Set-AzPrivateDnsRecordSet從本地 RecordSet 物件更新 Azure Private DNS 服務中的記錄集。 您可以將 RecordSet 物件傳遞為參數，或是使用管線運算子。 您可以使用確認參數$ConfirmPreference Windows PowerShell 變數來控制 Cmdlet 是否提示您確認。 如果記錄集在 Azure Private DNS 中自本地 RecordSet 物件被取回之後已經變更，則記錄集不會更新。 這可保護同時進行變更。 您可以使用覆寫參數隱藏此行為，無論同時變更，都會更新記錄集。
...
第一個命令使用 Get-AzPrivateDnsRecordSet Cmdlet 取得指定的記錄集，然後將它儲存在 $RecordSet 變數中。 第二個和第三個命令是線下作業，在記錄集新增兩個 A 記錄。 最後一個命令會使用 Set-AzPrivateDnsRecordSet Cmdlet 來提交更新。
...
第一個命令使用 Get-AzPrivateDnsRecordSet Cmdlet 取得指定的記錄集，然後將它儲存在 $RecordSet 變數中。 第二個命令會更新 $RecordSet 中指定的 $RECORDSET。 最後一個命令會使用 Set-AzPrivateDnsRecordSet Cmdlet 在 $RecordSet 中傳播$RecordSet。
...
### -覆寫
請勿使用 RecordSet 參數的 ETag 欄位進行開放式並行檢查。
