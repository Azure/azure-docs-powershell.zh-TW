...
儀表板 <IDashboard> ：共用儀表板資源定義。
  - `Location <String>`：資源位置
  - `[Lens <IDashboardPropertiesLenses>]`：儀表板儀表板。
    - `[(Any) <IDashboardLens>]`：這表示任何屬性都可以新增到這個物件。
  - `[Metadata <IDashboardPropertiesMetadata>]`：儀表板中繼資料。
    - `[(Any) <Object>]`：這表示任何屬性都可以新增到這個物件。
