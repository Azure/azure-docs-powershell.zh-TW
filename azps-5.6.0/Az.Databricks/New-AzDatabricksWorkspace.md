...
此命令會建立一個 Datab一s 工作區，在資源群組中具有自訂的虛擬網路。

### 範例 3：使用啟用加密來建立 Datab一s 工作區
```powershell
...
此命令會建立 Datab workspace，並設定它以準備加密。
如需加密的更多設定，請參閱Update-AzDatabricksWorkspace範例。
...
這是一個布林值，指出 DBFS 根檔案系統是否將啟用次要加密層，並包含平臺管理的資料靜態金鑰。
...
### -VirtualNetworkId
應建立此 Datab一s Cluster 的虛擬網路的識別碼。
