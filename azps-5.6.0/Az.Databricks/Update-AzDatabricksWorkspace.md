...
此命令會更新 Datab一s 工作區的標記。

### 範例 2：在 Datab一s 工作區上啟用加密
```powershell
...
在 Datab一s 工作區上啟用加密需要三個步驟：
1.
如果工作區尚未 `-PrepareEncryption` (，請更新) 。
1.
尋找 `StorageAccountIdentityPrincipalId` 最後一個步驟的輸出。
將金鑰許可權授予主體。
1.
再次更新工作區以填入有關加密金鑰的資訊：
    - `-EncryptionKeySource`
...
### Microsoft.Azure.PowerShell.Cmdlets.Databworks.Models.Api20180401.IWorkspace
