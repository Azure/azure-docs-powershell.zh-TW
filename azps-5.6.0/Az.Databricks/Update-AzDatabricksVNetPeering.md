...
### 範例 1：更新 vnet 對等的 AllowForwardedTraffic
```powershell
...
此命令會更新 vnet 對等的 AllowForwardedTraffic。

### 範例 2：根據物件更新 vnet 對等的 AllowForwardedTraffic
```powershell
...
此命令會更新物件對等的 AllowForwardedTraffic。

## 參數

### -AllowForwardedTraffic
[System.Management.Automation.SwitchParameter] 遠端虛擬網路中是否允許/不允許來自本地虛擬網路中之虛擬機器的轉轉流量。
...
### -AllowGatewayTransit
[System.Management.Automation.SwitchParameter] 如果閘道連結可用於遠端虛擬網路以連結至此虛擬網路。
...
[System.Management.Automation.SwitchParameter] 本地虛擬網路空間的虛擬機器是否能存取遠端虛擬網路空間的虛擬機器。
...
### -UseRemoteGateway
[System.Management.Automation.SwitchParameter] 如果遠端閘道可以在此虛擬網路上使用。
