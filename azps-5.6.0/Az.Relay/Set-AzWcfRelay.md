...
## 描述
此Set-AzWcfRelay Cmdlet 會更新指定 Relay 命名空間中 WcfRelay 的描述。
...
使用指定的命名空間中的新描述更新指定的 WcfRelay。
此範例會以新值更新 UserMetadata 屬性。
...
### -UserMetadata
獲取或設定 usermetadata 是一個預留位置，可儲存 WcfRelay 端點的使用者定義字串資料，例如可用來儲存描述資料，例如團隊清單及其連絡人資訊，也可以儲存使用者定義的設定設定。
