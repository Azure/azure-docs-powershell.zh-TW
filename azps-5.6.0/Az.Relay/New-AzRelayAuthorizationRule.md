...
## 描述
**New-AzRelayAuthorizationRule** Cmdlet 會為指定的轉寄實體 (命名空間/WcfRelay/HybridConnection) 。
...
使用 `AuthoRule1` 命名空間 **的 Listen** 許可權建立 `TestNameSpace-Relay1` 。
...
使用 `AuthoRule1` WcfRelay 的 **「** 聆聽」許可權建立授權規則 `TestWCFRelay1` 。
...
使用 `AuthoRule1` 混合 **式連接的聆聽** 許可權建立 `TestHybridConnection` 。
