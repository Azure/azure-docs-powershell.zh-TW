...
## 描述
**Set-AzRelayAuthorizationRule** Cmdlet 會更新指定轉寄實體 (命名空間/WcfRelay/HybridConnection) 之指定授權規則的描述。

## 例子

### 範例 1.1 - 命名空間與 InputObject
```
PS C:\>
...
從 **命名空間** 中授權規則的存取權新增 `AuthoRule1` `TestNameSpace-Relay1` Send。

### 範例 1.2 - 命名空間與 Rights 參數
```
...
### 範例 2.1 - WcfRelay with InputObject
```
...
將 **Send** 新增到 `AuthoRule1` WcfRelay 授權規則的存取權 `TestWCFRelay1` 。

### 範例 2.2 - WcfRelay with Rights 參數
```
...
### 範例 3.1 - HybridConnection with InputObject
```
...
將 **Send** 新增到 `AuthoRule1` HybridConnection 之授權規則的存取權 `TestHybridConnection` 。

### 範例 3.2 - HybridConnection with Rights 參數
```
