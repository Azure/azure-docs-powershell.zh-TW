...
## 描述
此New-AzRelayHybridConnection Cmdlet 會于指定的 Relay 命名空間中建立混合式連接。
...
在指定的 Relay 命名空間 \` \` TestNameSpace-HybirdConnection 中建立新 HybirdConnection \` TestHybirdConnection2。 \`
...
在指定的 Relay 命名空間 \` \` TestNameSpace-HybirdConnection 中建立新 HybirdConnection \` TestHybirdConnection1。 \`
...
### -需要ClientAuthorization
如果此混合式連接需要用戶端授權，則為 True;否則，False
...
### -UserMetadata
獲取或設定 usermetadata 是一個預留位置，可儲存 HybridConnection 端點的使用者定義字串資料，例如可用來儲存描述資料，例如團隊清單及其連絡人資訊，也可以儲存使用者定義的設定設定。
