...
### -自訂
指定用來描述影像自訂步驟的屬性，例如圖像來源等。若要建構，請參閱 NOTES 區段以自訂屬性及建立雜湊表。
...
### -DefaultProfile
Region HideParameter 用於與 Azure 通訊的認證、帳戶、租使用者和訂閱。
...
### -發佈
此分配會針對影像輸出需要前往的地方進行。
若要建構，請參閱 DISTRIBUTE 屬性的附注區段，並建立雜湊表。
...
### -來源
說明建立、自訂及散發的虛擬機器映射來源。
若要建構，請參閱 SOURCE 屬性的附注區段，並建立雜湊表。
...
### -VMProfileOscioSizeInGb
作業系統磁片的大小 ，以 GB 為單位。
省略或指定 0 以使用 Azure 的預設作業系統磁片大小。
...
自訂<IImageTemplateCustomizer[]>：指定用來描述影像自訂步驟的屬性，例如圖像來源等。
  - `Type <String>`：您想要在影像上使用的自訂工具類型。 例如，"Shell" 可以是 shell 自訂程式
  - `[Name <String>]`：好用名稱，提供此自訂步驟功能相關內容

DISTRIBUTE <IImageTemplateDistributor[]>：此分配會檢查影像輸出必須前往的目標位置。
  - `RunOutputName <String>`：用於相關聯 RunOutput 的名稱。
  - `Type <String>`：分配類型。
  - `[ArtifactTag <IImageTemplateDistributorArtifactTags>]`：發行者建立/更新產品後，將會用於其上的標記。
    - `[(Any) <String>]`：這表示任何屬性都可以新增到這個物件。

來源 <IImageTemplateSource> ：說明建立、自訂及散發的虛擬機器映射來源。
  - `Type <String>`：指定您想要開始的來源影像類型。
