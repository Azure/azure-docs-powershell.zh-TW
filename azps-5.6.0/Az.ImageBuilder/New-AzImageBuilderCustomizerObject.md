...
### -目的地
已建立巢 (目錄結構的檔案絕對路徑) 來源Uri (檔案) 上傳至 VM。
...
### -PowerShellCustomizer
在 Windows 應用程式上的 VM (執行) 。
對應到 Packer Powershell 提供器。
您可以指定其中一個"scriptUri"或"內嵌」。
...
### -RestartCommand
執行重新開機的命令 [預設： '關機 /r /f /t 0 /c packer 重新開機']
...
### -重新開機自訂器
重新開機 VM，並等待它回到 Windows (上) 。
對應到 Packer Windows-restart 提供器。
...
### -RestartTimeout
重新開機超時指定為量級和單位字串，例如 '5m' (5 分鐘) 或 '2h' (2 小時) [預設： '5m']。
...
### -ScriptUri
要執行自訂之 shell 腳本的 URI。
它可以是 github 連結、AZURE 儲存空間的 SAS URI 等等。
...
### -Sha256Checksum
SCRIPTUri 功能變數中提供的 shell 腳本 SHA256 檢查和。
...
### -ShellCustomizer
在 Linux (自訂階段期間執行 shell) 。
對應到 Packer Shell 提供器。
您可以指定其中一個"scriptUri"或"內嵌」。
