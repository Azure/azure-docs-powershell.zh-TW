...
### 範例1：列出群集中的所有 AttachedDatabaseConfigurations
```powershell
...
上述命令會列出 [testnewkustoclusterf] 群集中的所有 AttachedDatabaseConfigurations。

### 範例2：在群集中取得特定 AttachedDatabaseConfiguration
```powershell
...
上述命令會傳回群集 "testnewkustoclusterf" 中名為 "myfollowerconfiguration" 的 AttachedDatabaseConfigurations。
...
### IAttachedDatabaseConfiguration （Kusto）。 Api20200918。
