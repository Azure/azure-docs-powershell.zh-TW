...
## 說明
**新的-AzRmStorageShare** Cmdlet 會修改儲存空間共用。

## 示例

### 範例1：修改儲存空間共用的中繼資料，並使用儲存空間帳戶名稱和共用名稱稱共用配額
```
...
這個命令會修改儲存空間共用的中繼資料，並與儲存空間帳戶名稱和共用名稱稱共用配額，並以傳回的檔案共用物件顯示 [修改結果]。

### 範例2：使用儲存空間帳戶物件和共用名稱稱來修改儲存空間共用上的中繼資料
```
PS C:\>$accountObject = Get-AzStorageAccount -ResourceGroupName "myResourceGroup" -StorageAccountName "myStorageAccount"
PS C:\>$share = Update-AzRmStorageShare -StorageAccount $accountObject -Name "myshare" -Metadata @{tag0="value0";tag1="value1"}
```

這個命令會透過儲存帳戶物件和共用名稱稱來修改儲存空間共用上的中繼資料。

### 範例3：針對含管線的儲存空間帳戶中的所有儲存空間共用修改共用配額
```
...
這個命令會針對含管線的儲存空間帳戶中的所有儲存空間共用，以 5000 GiB 的方式來修改共用配額。

### 範例4：使用 accesstier 修改儲存空間共用（含超酷）
```
...
這個命令會以超酷的方式修改儲存空間共用與 accesstier。

## 參數

### -AccessTier
特定共用的存取層級。 StorageV2 帳戶可以選擇 TransactionOptimized (預設的 [) 、[熱] 和 [超酷]。 FileStorage 帳戶可以選擇 [特優]。
