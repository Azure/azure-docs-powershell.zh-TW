...
## 說明
**AzDataLakeGen2AclRecursive** Cmdlet 會在指定的路徑上以遞迴方式設定 ACL。 輸入 ACL 會完全取代原始的 ACL。

## 示例

### 範例1：以遞迴方式在 directiry 上設定 ACL
```
...
這個命令會先建立含3個 acl 專案的 ACL 物件，然後以遞迴方式在目錄上設定 ACL。

### 範例2：以遞迴方式在 filesystem 的根 directiry 上設定 ACL
```
...
這個命令首先會將 ACL 設定成遞迴到根目錄並失敗，然後在使用者修正失敗的檔案之後，繼續使用 ContinuationToken。

### 範例3：以分塊方式將 ACL 設定成遞迴區塊
```
$token = $null
...
此腳本會依區塊將目錄區塊上的 ACL rescursively 設定成區塊大小為 BatchSize * MaxBatchCount。 此腳本中的組塊大小為200。

### 範例4：在目錄和 ContinueOnFailure 上遞迴設定 ACL，然後從失敗逐一繼續
```
...
這個命令首先會將 ACL 設定為 ContinueOnFailure 的目錄，且有些專案失敗，接著逐一繼續失敗的專案。
