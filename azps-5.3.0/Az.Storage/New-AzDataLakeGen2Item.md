...
這個命令會建立具有指定許可權、Umask、屬性及中繼資料的目錄

### 範例2：建立 (從本機來源檔案) 資料 lake 檔案，而 Cmdlet 在背景執行
```
...
這個命令會建立 (從本機來源檔案) 資料 lake 檔案的上傳，而 Cmdlet 則會在背景執行。
...
### -屬性
指定已建立的目錄或檔案的屬性。 支援的檔案屬性有： CacheControl、ContentDisposition、ContentEncoding、ContentLanguage、ContentMD5、ContentType。
...
### -來源
指定將上傳到 Datalake Gen2 檔案的本機來源檔案路徑。
...
### -Umask
當您建立新專案，而上層目錄沒有預設的 ACL 時，umask 會限制要建立的檔案或目錄的許可權。
產生的許可權是由 p & ^ u 提供，其中 p 是許可權，而您是 umask。
支援符號 (rwxrw-cd-rw-) 。
