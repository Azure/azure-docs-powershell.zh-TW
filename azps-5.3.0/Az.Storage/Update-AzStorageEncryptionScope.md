...
## 說明
**AzStorageEncryptionScope** Cmdlet 會修改儲存空間帳戶的加密範圍。
...
這個命令 updtaes 要使用 Keyvault 加密的加密範圍。
儲存帳戶身分識別需要擁有 keyvault 金鑰的 [取得]、[wrapkey]、[unwrapkey] 許可權。
...
### -KeyvaultEncryption
使用 keySource 做為 Microsoft. Keyvault 建立加密範圍
...
### -StorageEncryption
使用 keySource 建立加密範圍，以儲存為 Microsoft。
