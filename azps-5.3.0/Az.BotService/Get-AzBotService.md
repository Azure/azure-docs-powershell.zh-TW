...
### 範例2：透過 ResourceGroupName 和 Name 取得 BotService
```powershell
...
透過 ResourceGroupName 和名稱取得 BotService

### 範例3：透過 ResourceGroupName 取得所有 BotServices
```powershell
...
透過 ResourceGroupName 取得所有 BotServices

### 範例4：透過 inputObject 取得 BotService
```powershell
...
### IBotServiceIdentity （BotService）的

## 輸出

### IBot （BotService）。 Api20180712。
...
INPUTOBJECT <IBotServiceIdentity> ：身分識別參數
  - `[ChannelName <ChannelName?>]`：頻道資源的名稱。
  - `[ConnectionName <String>]`： Bot 服務連線設定資源的名稱
  - `[Id <String>]`：資源身分識別路徑
  - `[ResourceGroupName <String>]`：使用者訂閱中 Bot 資源群組的名稱。
  - `[ResourceName <String>]`： Bot 資源的名稱。
