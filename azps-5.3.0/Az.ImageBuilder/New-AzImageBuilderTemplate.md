...
### -自訂
指定用來描述影像之自訂步驟的屬性，例如影像來源等。若要建立，請參閱自訂屬性和建立雜湊資料表的備忘稿區段。
...
### -DefaultProfile
地區 HideParameter 用於與 Azure 進行通訊的認證、帳戶、租使用者及訂閱。
...
自訂 <IImageTemplateCustomizer [] >：指定用來描述影像之自訂步驟的屬性，例如影像來源等。
  - `Type <String>`：您想要在影像上使用的自訂工具類型。 例如，"Shell" 可以是 Shell 定制器
  - `[Name <String>]`：易記的名稱，可提供此自訂步驟的功能內容

發佈 <IImageTemplateDistributor [] >：分佈目標，圖像輸出需要移至該物件。
  - `RunOutputName <String>`：要用於關聯 RunOutput 的名稱。
  - `Type <String>`：發佈類型。
  - `[ArtifactTag <IImageTemplateDistributorArtifactTags>]`：由分銷商建立/更新後，會套用到專案的標記。
    - `[(Any) <String>]`：這表示您可以將任何屬性新增至此物件。

來源 <IImageTemplateSource> ：說明建立、自訂及發佈的虛擬機器影像來源。
  - `Type <String>`：指定您要開始使用的來源影像類型。
