...
## 摘要
變更 Azure SQL server 的 Microsoft 支援作業審核設定。
...
## 說明
**AzSqlServerMSSupportAudit** Cmdlet 會變更 Azure SQL Server 的 Microsoft 支援作業審核設定。
若要使用 Cmdlet，請使用 *ResourceGroupName* 和 *ServerName* 參數來識別伺服器。
當 blob 儲存為審核記錄的目的地時，請指定 *StorageAccountResourceId* 參數來判斷審核記錄的儲存空間帳戶。

## 示例

### 範例1：啟用 Azure SQL server 的 blob 儲存 Microsoft 支援作業審核原則
```powershell
...
### 範例6：停用 Azure SQL server 的 log analytics Microsoft 支援作業審核原則
```powershell
...
### 範例7：透過流水線來停用 Azure SQL server 的 log analytics Microsoft 支援作業審核原則
```powershell
...
### 範例8：停用將 Azure SQL server 的 [Microsoft 支援作業] 審核記錄傳送到 [blob 儲存空間]，並啟用將其傳送至記錄分析的功能。
```powershell
...
### 範例9：啟用將 Azure SQL server 的 [傳送 Microsoft 支援作業] 審核記錄傳送到 blob 儲存體、事件中樞和記錄分析。
```powershell
...
指出 [blob 儲存空間] 是否為 Microsoft 支援作業審計記錄的目的地。
...
### -EventHubTargetState
指出事件中樞是否為 Microsoft 支援作業審計記錄的目的地。
...
指出 log analytics 是否為 Microsoft 支援作業審計記錄的目的地。
...
### -PassThru
指定是否要在 Cmdlet 執行結束時輸出 Microsoft 支援作業審核原則
...
### -WorkspaceResourceId
您想要傳送 Microsoft 支援作業審核記錄的 Log analytics 工作區) 的 [工作區識別碼] (資源識別碼。 範例：/subscriptions/4b9e8510-67ab-4e9a-95a9-e2f1e570ea9c/resourceGroups/insights-integration/providers/Microsoft.OperationalInsights/workspaces/viruela2
...
### ServerDevOpsAuditModel （.Sql） .Sql。
