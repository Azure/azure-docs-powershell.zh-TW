...
## 說明
**AzSqlServerMSSupportAudit** Cmdlet 會移除 Azure SQL Server 的 Microsoft 支援作業審核設定。
若要使用 Cmdlet，請使用 *ResourceGroupName* 和 *ServerName* 參數來識別伺服器。

## 示例

### 範例1：移除 Azure SQL server 的 Microsoft 支援作業審核設定
```powershell
PS C:\>Remove-AzSqlServerMSSupportAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01"
```

### 範例2：在管道中移除 Azure SQL server 的 Microsoft 支援審核設定
```
