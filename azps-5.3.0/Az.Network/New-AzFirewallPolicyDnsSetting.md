...
## 說明
**新的 AzFirewallPolicyDnsSetting** Cmdlet 會針對 Azure 防火牆原則建立 DNS 設定物件
...
這個範例會建立設定啟用 dns proxy 的 dns 設定物件。

### 2. 使用 ThreatIntel 模式建立空原則
```powershell
PS C:\> $dnsServers = @("10.10.10.1", "20.20.20.2")
PS C:\> New-AzFirewallPolicyDnsSetting -EnableProxy -Server $dnsServers
```

這個範例會建立一個 dns 設定物件，並設定啟用 dns proxy 及設定自訂 dns 伺服器。
...
### PSAzureFirewallPolicyDnsSettings 中的 [.]
