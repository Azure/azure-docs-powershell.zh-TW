...
若要部署全域負載平衡器，您必須先建立多個物件，且前五個命令會示範如何建立這些物件。
第六個命令會在名為 MyResourceGroup 的資源群組中，建立名為 MyLoadBalancer 的負載平衡器。
第七個與最後一個命令會取得新的負載平衡器，以確保它已成功建立。
請注意，這個範例只顯示如何建立全域負載平衡器。 您也必須使用 New-AzLoadBalancerBackendAddressConfig Cmdlet 來設定它，才能將區域負載等化器前端 ipconfig id 指派給它的後端位址集區
