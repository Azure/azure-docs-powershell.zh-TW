...
## 說明
**新的-AzFirewallNetworkRule** Cmdlet 會建立 Azure 防火牆的網路規則。
...
### 範例2：建立從10.0.0.0 至60.1.5.0：4040的所有 TCP 流量的規則
```powershell
...
### 範例3：建立從任何來源到 10.0.0.0/16 的所有 TCP 與 ICMP 流量的規則
```powershell
$rule = New-AzFirewallNetworkRule -Name "tcp-and-icmp-rule" -Description "Rule for all TCP and ICMP traffic from any source to 10.0.0.0/16" -Protocol TCP,ICMP -SourceAddress * -DestinationAddress "10.0.0.0/16" -DestinationPort *
```

這個範例會建立從任何來源到 10.0.0.0/16 的所有 TCP 流量的規則。 使用者會根據與關聯的規則集合，針對規則允許或拒絕交通。
