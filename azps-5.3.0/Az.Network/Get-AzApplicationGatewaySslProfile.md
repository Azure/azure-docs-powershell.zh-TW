...
## 說明
**AzApplicationGatewaySslProfile** Cmdlet 會取得應用程式閘道的 SSL 設定檔。
...
第一個命令會在名為 ResourceGroup01 的資源群組中，取得名為 ApplicationGateway01 的應用程式閘道，並將它儲存在 $AppGw 變數中。第二個命令會取得 $AppGw 的 SSL 設定檔 SslProfile01，並將它儲存在 $profile 變數中。
