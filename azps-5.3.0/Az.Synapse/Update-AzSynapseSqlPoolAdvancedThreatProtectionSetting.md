...
## 說明
**AzSynapseSqlPoolAdvancedThreatProtectionSetting** Cmdlet 會在 Azure SYNAPSE Analytics SQL pool 上設定高級威脅防護設定。
為了在 SQL 池中啟用高級威脅防護，必須在該 SQL 池中啟用 [審核] 設定。
...
這個命令會在名為 [ContosoWorkspace] 的工作區下，為名為「ContosoSqlPool」的 SQL pool 設定高級威脅防護設定。
...
### PSSqlPoolSecurityAlertPolicy 中的 Synapse。
