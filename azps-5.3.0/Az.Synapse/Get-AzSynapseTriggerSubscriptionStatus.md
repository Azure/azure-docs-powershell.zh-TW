...
## 說明
**AzSynapseTriggerSubscriptionStatus** Cmdlet 會將事件觸發程式的訂閱狀態，取得至指定的外部服務事件。 在傳回的狀態為「已啟用」前，無法啟動觸發程式。
...
這個命令會取得 subscribtion 的觸發程式狀態，稱為 ContosoTrigger 至外部服務事件。
...
這個命令會透過流水線取得稱為「ContosoTrigger」的觸發程式 subscribtion 的狀態。
...
### PSTriggerSubscriptionOperationStatus 中的 Synapse。
