...
## 說明
**AzSynapseSqlPoolRestorePoint** Cmdlet 會永久刪除 Azure SYNAPSE 分析 SQL pool 還原點。
...
這個命令會刪除 Azure Synapse Analytics SQL pool 還原點。
...
這個命令會透過管線刪除 Azure Synapse Analytics SQL pool 還原點。
...
此命令會刪除具有指定資源識別碼的 Azure Synapse Analytics SQL pool 還原點。
...
### -InputObject
SQL pool 還原點建立時間輸入物件，通常是透過管線傳遞。
