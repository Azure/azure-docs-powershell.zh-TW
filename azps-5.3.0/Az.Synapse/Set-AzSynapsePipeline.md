...
## 說明
**AzSynapsePipeline** Cmdlet 會在工作區中建立管線。
...
這個命令會在名為 ContosoWorkspace 的工作區中，建立一個名為 ContosoPipeline 的管線。
該命令根據檔案中 pipeline.js的資訊來基礎。
此檔案包含活動的相關資訊。
...
這個命令會在名為 ContosoWorkspace 至管線的工作區中，建立一個名為 ContosoPipeline 的管線。
該命令根據檔案中 pipeline.js的資訊來基礎。
此檔案包含活動的相關資訊。
