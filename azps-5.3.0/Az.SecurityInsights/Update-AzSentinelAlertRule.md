...
## 說明
**更新-AzSentinelAlertRule** Cmdlet 會更新指定工作區中) 的分析 (警示規則。
您可以使用-InputObject 或-ResourceId 或-AlertId。  您可以更新1個或多個 proprtery parmaters。
...
這個範例會將 **AlertRule** 設定為 [ *已停用* ]，並重新命名為 [ *disabled-AlertRuleDisplayName*]。  所有其他屬性都將保持不變。
...
這個範例使用 InputObject 設定為 [*停* 用] 來更新 **AlertRule** 。  所有其他屬性都將保持不變。
