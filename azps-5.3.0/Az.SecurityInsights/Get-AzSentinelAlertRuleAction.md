...
## 說明
**AzSentinelAlertRuleAction** Cmdlet 會從指定的工作區取得自動回應 (警示規則動作) 。
如果您指定 *ActionId* 和 *AlertRuleId* 參數，則會傳回單一 **AlertRuleAction** 物件。
如果您沒有指定 *ActionId* 參數，則會傳回包含指定工作區中以警報規則之所有動作的陣列。
您可以使用 **action** 物件來更新動作，例如，您可以變更警示規則的 **動作** 。
...
這個範例會在指定的工作區中取得指定之警示規則的所有 **動作** ，然後將其儲存在 $AlertRuleActions 變數中。
...
這個範例會在指定的工作區中取得指定警示規則的 **AlertRuleAction** ，然後將其儲存在 $AlertRuleAction 變數中。
...
### PSSentinelActionResponse 中的 SecurityInsights.。
## 筆記
