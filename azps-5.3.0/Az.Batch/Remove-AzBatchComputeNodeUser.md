...
## 說明
**AzBatchComputeNodeUser** Cmdlet 會從 Azure Batch 計算節點刪除使用者帳戶。

## 示例

### 範例1：不需確認就能從計算節點刪除使用者
```
PS C:\>Remove-AzBatchComputeNodeUser -PoolId "Pool01" -ComputeNodeId "ComputeNode01" -Name "User14" -Force -BatchContext $Context
```

這個命令會從名為 ComputeNode01 的計算節點中刪除名為 User14 的使用者。
Compute 節點位於名為 Pool01 的 pool 中。
這個命令會指定 *Force* 參數。
因此，命令不會提示您進行確認。
