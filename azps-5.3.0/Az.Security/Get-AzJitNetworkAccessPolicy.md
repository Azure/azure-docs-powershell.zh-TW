...
## 說明
即時 (JIT) 網路存取原則可讓您定義原則，就能讓簡單的使用者建立到 VM 的臨時性網路連線。
原則定義哪些埠、通訊協定和來源 IP 位址可以要求與 VM 的連線，以及在連線自動關閉前的最大持續時間。
在原則中，您也可以看到與此原則建立的連線要求。 
...
取得「myService1」資源群組上的所有 JIT 網路存取原則
