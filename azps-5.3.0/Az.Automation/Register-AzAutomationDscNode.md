...
## 摘要
將運行 Windows OS 的 Azure 虛擬機器註冊為自動化帳戶的 DSC 節點。
...
## 說明
**AzAutomationDscNode** Cmdlet 會在 azure 自動化帳戶中，將 Azure 虛擬機器註冊為所需的 Ap 狀態配置 (DSC) 節點。 這個 Cmdlet 只會將運行 Windows OS 的虛擬機器寄存器作為帳戶的自動化 DSC 節點。

如果您需要將節點註冊到不同訂閱中的自動化帳戶，您將需要使用 ARM 範本，而不是 Cmdlet。 如需詳細資訊，請參閱 Azure 自動化 [檔](https://docs.microsoft.com/en-us/azure/automation/automation-dsc-onboarding#registering-virtual-machines-across-azure-subscriptions) 。
