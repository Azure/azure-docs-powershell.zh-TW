---
Module Name: Az.LogicApp
Module Guid: 8a7d44bb-e747-4265-8f22-b3937961d709
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.logicapp
Help Version: 4.0.3.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/LogicApp/LogicApp/help/Az.LogicApp.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/LogicApp/LogicApp/help/Az.LogicApp.md
ms.openlocfilehash: ddad081b0c1609f046aa26b963a04cd50ace8448
ms.sourcegitcommit: 68451baa389791703e666d95469602c5652609ee
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 01/05/2021
ms.locfileid: "98449075"
---
# Az LogicApp 模組
## 說明
本主題顯示邏輯 App Cmdlet 的說明主題。

## Az LogicApp Cmdlet
### [AzIntegrationAccount](Get-AzIntegrationAccount.md)
取得整合帳戶。

### [AzIntegrationAccountAgreement](Get-AzIntegrationAccountAgreement.md)
取得整合帳戶協定。

### [AzIntegrationAccountAssembly](Get-AzIntegrationAccountAssembly.md)
取得整合帳戶元件。

### [AzIntegrationAccountBatchConfiguration](Get-AzIntegrationAccountBatchConfiguration.md)
取得整合帳戶批次設定。

### [AzIntegrationAccountCallbackUrl](Get-AzIntegrationAccountCallbackUrl.md)
取得整合帳戶回撥 URL。

### [AzIntegrationAccountCertificate](Get-AzIntegrationAccountCertificate.md)
從資源群組取得整合帳戶憑證。

### [AzIntegrationAccountGeneratedIcn](Get-AzIntegrationAccountGeneratedIcn.md)
這個 Cmdlet 會針對每個協定檢索產生的交換控制編號的目前值。

### [AzIntegrationAccountMap](Get-AzIntegrationAccountMap.md)
取得整合帳戶對應。

### [AzIntegrationAccountPartner](Get-AzIntegrationAccountPartner.md)
取得整合帳戶合作夥伴。

### [AzIntegrationAccountReceivedIcn](Get-AzIntegrationAccountReceivedIcn.md)
這個 Cmdlet 會針對每個協定和控制編號值來檢索特定接收的交換控制編號。

### [AzIntegrationAccountSchema](Get-AzIntegrationAccountSchema.md)
取得整合帳戶架構。

### [AzLogicApp](Get-AzLogicApp.md)
從資源群組取得邏輯 app。

### [AzLogicAppRunAction](Get-AzLogicAppRunAction.md)
從邏輯 app 執行中取得動作。

### [AzLogicAppRunHistory](Get-AzLogicAppRunHistory.md)
取得邏輯 app 的執行歷程記錄。

### [AzLogicAppTrigger](Get-AzLogicAppTrigger.md)
取得邏輯 app 的觸發程式。

### [AzLogicAppTriggerCallbackUrl](Get-AzLogicAppTriggerCallbackUrl.md)
取得邏輯 App 觸發程式回撥 URL。

### [AzLogicAppTriggerHistory](Get-AzLogicAppTriggerHistory.md)
取得邏輯 app 中的觸發程式歷程記錄。

### [AzLogicAppUpgradedDefinition](Get-AzLogicAppUpgradedDefinition.md)
取得邏輯 app 的升級定義。

### [新-AzIntegrationAccount](New-AzIntegrationAccount.md)
建立整合帳戶。

### [新-AzIntegrationAccountAgreement](New-AzIntegrationAccountAgreement.md)
建立整合帳戶協定。

### [新-AzIntegrationAccountAssembly](New-AzIntegrationAccountAssembly.md)
建立整合帳戶元件。

### [新-AzIntegrationAccountBatchConfiguration](New-AzIntegrationAccountBatchConfiguration.md)
建立整合帳戶批次配置。

### [新-AzIntegrationAccountCertificate](New-AzIntegrationAccountCertificate.md)
建立整合帳戶憑證。

### [新-AzIntegrationAccountMap](New-AzIntegrationAccountMap.md)
建立整合帳戶地圖。

### [新-AzIntegrationAccountPartner](New-AzIntegrationAccountPartner.md)
建立整合帳戶合作夥伴。

### [新-AzIntegrationAccountSchema](New-AzIntegrationAccountSchema.md)
建立整合帳戶架構。

### [新-AzLogicApp](New-AzLogicApp.md)
在資源群組中建立邏輯 app。

### [移除-AzIntegrationAccount](Remove-AzIntegrationAccount.md)
移除整合帳戶。

### [移除-AzIntegrationAccountAgreement](Remove-AzIntegrationAccountAgreement.md)
移除 [整合] 帳戶協定。

### [移除-AzIntegrationAccountAssembly](Remove-AzIntegrationAccountAssembly.md)
移除整合帳戶元件。

### [移除-AzIntegrationAccountBatchConfiguration](Remove-AzIntegrationAccountBatchConfiguration.md)
移除整合帳戶批次設定。

### [移除-AzIntegrationAccountCertificate](Remove-AzIntegrationAccountCertificate.md)
從資源群組中移除整合帳戶憑證。

### [移除-AzIntegrationAccountMap](Remove-AzIntegrationAccountMap.md)
移除整合帳戶對應。

### [移除-AzIntegrationAccountPartner](Remove-AzIntegrationAccountPartner.md)
移除整合帳戶合作夥伴。

### [移除-AzIntegrationAccountReceivedIcn](Remove-AzIntegrationAccountReceivedIcn.md)
這個 Cmdlet 會移除每個協定與控制編號值的特定接收交換控制編號。

### [移除-AzIntegrationAccountSchema](Remove-AzIntegrationAccountSchema.md)
移除整合帳戶架構。

### [移除-AzLogicApp](Remove-AzLogicApp.md)
從資源群組中移除邏輯 app。

### [Set-AzIntegrationAccount](Set-AzIntegrationAccount.md)
修改整合帳戶。

### [Set-AzIntegrationAccountAgreement](Set-AzIntegrationAccountAgreement.md)
修改整合帳戶協定。

### [Set-AzIntegrationAccountAssembly](Set-AzIntegrationAccountAssembly.md)
修改整合帳戶元件。

### [Set-AzIntegrationAccountBatchConfiguration](Set-AzIntegrationAccountBatchConfiguration.md)
修改整合帳戶批次設定。

### [Set-AzIntegrationAccountCertificate](Set-AzIntegrationAccountCertificate.md)
修改整合帳戶憑證。

### [Set-AzIntegrationAccountGeneratedIcn](Set-AzIntegrationAccountGeneratedIcn.md)
更新 Azure 資源群組中 (ICN) 的整合帳戶產生的交換控制編號。

### [Set-AzIntegrationAccountMap](Set-AzIntegrationAccountMap.md)
修改整合帳戶地圖。

### [Set-AzIntegrationAccountPartner](Set-AzIntegrationAccountPartner.md)
修改整合帳戶合作夥伴。

### [Set-AzIntegrationAccountReceivedIcn](Set-AzIntegrationAccountReceivedIcn.md)
更新在 Azure 資源群組中 (ICN) 的整合帳戶收到的交換控制編號。

### [Set-AzIntegrationAccountSchema](Set-AzIntegrationAccountSchema.md)
修改整合帳戶架構。

### [Set-AzLogicApp](Set-AzLogicApp.md)
修改資源群組中的邏輯 app。

### [開始-AzLogicApp](Start-AzLogicApp.md)
在資源群組中執行邏輯 app。

### [停止 AzLogicAppRun](Stop-AzLogicAppRun.md)
取消邏輯 app 的執行。

### [Test-AzLogicApp](Test-AzLogicApp.md)
驗證邏輯應用程式定義。

