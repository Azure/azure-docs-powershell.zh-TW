...
## 說明
這個 Cmdlet 會使用在-PackageUrl 中指定的套件來建立新的應用程式類型版本，這應該可透過 Azure 資源管理員的 REST 端點存取，以在部署期間進行，並包含使用副檔名 sfpkg 封裝和 zipped 的應用程式。 如果應用程式類型尚不存在，此命令會建立它。
...
這個範例會在輸入 "testAppType" 下建立應用程式類型版本 "v1"。 套件中包含的應用程式資訊清單版本應該與版本中所指定的版本相同。
...
### PSApplicationTypeVersion 中的 ServiceFabric。
