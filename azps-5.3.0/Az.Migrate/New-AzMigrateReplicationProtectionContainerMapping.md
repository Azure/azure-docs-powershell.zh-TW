...
提供者針對配對的特定輸入。
若要建立，請參閱 PROVIDERSPECIFICINPUT 屬性和建立雜湊資料表的備忘稿區段。
...
### IProtectionContainerMapping 中的 Api20180110 （）。
...
PROVIDERSPECIFICINPUT <IReplicationProviderSpecificContainerMappingInput> ：提供者針對配對的特定輸入。
  - `[InstanceType <String>]`：類別類型。
