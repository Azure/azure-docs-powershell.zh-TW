...
## 說明
**AzVmssVM** Cmdlet 會修改虛擬機器縮放集 (VMSS) 實例的狀態。
...
### -PerformMaintenance
表示此 Cmdlet 會在 VMSS 中的虛擬機器上執行維護。
...
### -重新部署
表示此 Cmdlet redeploys VMSS 中的虛擬機器。
...
### -SimulateEviction
表示此 Cmdlet 會模擬 VM 規模集中的 [逐出位置] 虛擬機器。
逐出將在呼叫 API 30 分鐘內發生。
