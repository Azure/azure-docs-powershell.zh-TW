...
這個命令會針對名為 MyCache 的 Redis Enterprise Cache，更新資料庫的用戶端通訊協定。
...
這個命令會針對名為 MyCache 的 Redis Enterprise Cache，更新資料庫的用戶端協定和逐出原則。
...
### -ClientProtocol
指定 redis 用戶端是否可使用 TLS 加密或純文字 redis 通訊協定連線。
預設值為 TLS 加密。
...
### -模組
要在此資料庫中啟用的 redis 模組的選擇性集合-只有在建立期間才能新增模組。
若要建立，請參閱模組屬性的 [記事] 區段，並建立雜湊資料表。
...
### IRedisEnterpriseCacheIdentity （RedisEnterpriseCache）的

## 輸出

### IDatabase （RedisEnterpriseCache）。 Api20201001Preview。
...
INPUTOBJECT <IRedisEnterpriseCacheIdentity> ：身分識別參數
  - `[ClusterName <String>]`： RedisEnterprise 群集的名稱。
  - `[DatabaseName <String>]`：資料庫的名稱。
  - `[Id <String>]`：資源身分識別路徑
  - `[Location <String>]`：操作所在的地區。
  - `[OperationId <String>]`：操作的唯一識別碼。
  - `[PrivateEndpointConnectionName <String>]`：與 Azure 資源相關聯的專用端點連接的名稱
  - `[ResourceGroupName <String>]`：資源群組的名稱。
  - `[SubscriptionId <String>]`：取得可唯一識別 Microsoft Azure 訂閱的訂閱認證。 [訂閱識別碼] 會在每個服務呼叫的 URI 中形成一部分。

MODULE <IModule [] >：選用的一組要在此資料庫中啟用的 redis 模組-只有在建立時才能新增模組。
  - `Name <String>`：模組的名稱，例如 "RedisBloom"、"RediSearch"、"RedisTimeSeries"
  - `[Arg <String>]`：模組的設定選項，例如 "ERROR_RATE 0.00 INITIAL_SIZE 400"。
