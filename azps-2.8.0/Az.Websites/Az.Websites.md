---
Module Name: Az.Websites
Module Guid: cc69c625-e961-43f4-8b50-0061eba6e4b6
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.websites
Help Version: 1.0.0.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Websites/Websites/help/Az.Websites.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Websites/Websites/help/Az.Websites.md
ms.openlocfilehash: 174da8669c84442f24866b8ab8831ee70d2d239d
ms.sourcegitcommit: 0b94b9566124331d0b15eb7f5a811305c254172e
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 10/15/2019
ms.locfileid: "93611494"
---
# Az. 網站模組
## 說明
ARM (Azure 資源管理員) Web App 和 App Service 方案命令。

## Az. 網站 Cmdlet
### [編輯-AzWebAppBackupConfiguration](Edit-AzWebAppBackupConfiguration.md)


### [Enter-AzWebAppContainerPSSession](Enter-AzWebAppContainerPSSession.md)
開啟遠端 PowerShell 會話至在指定的網站或槽與指定資源群組中指定的 windows 容器

### [AzAppServicePlan](Get-AzAppServicePlan.md)
在指定的資源群組中取得 Azure App Service 方案。

### [AzAppServicePlanMetric](Get-AzAppServicePlanMetric.md)


### [AzDeletedWebApp](Get-AzDeletedWebApp.md)
在訂閱中取得已刪除的 web 應用程式。

### [AzWebApp](Get-AzWebApp.md)
在指定的資源群組中取得 Azure Web Apps。

### [AzWebAppBackup](Get-AzWebAppBackup.md)


### [AzWebAppBackupConfiguration](Get-AzWebAppBackupConfiguration.md)


### [AzWebAppBackupList](Get-AzWebAppBackupList.md)


### [AzWebAppCertificate](Get-AzWebAppCertificate.md)
取得 Azure Web App 憑證。

### [AzWebAppContainerContinuousDeploymentUrl](Get-AzWebAppContainerContinuousDeploymentUrl.md)
Get-AzWebAppContainerContinuousDeploymentUrl 將會傳回容器連續部署 url

### [AzWebAppMetric](Get-AzWebAppMetric.md)
取得 Azure Web App 度量單位。

### [AzWebAppPublishingProfile](Get-AzWebAppPublishingProfile.md)
取得 Azure Web App 發佈設定檔。

### [AzWebAppSlot](Get-AzWebAppSlot.md)
取得 Azure Web App 槽。

### [AzWebAppSlotConfigName](Get-AzWebAppSlotConfigName.md)
取得 Web 應用程式槽配置名稱的清單

### [AzWebAppSlotMetric](Get-AzWebAppSlotMetric.md)
取得 Azure Web App 槽的指標。

### [AzWebAppSlotPublishingProfile](Get-AzWebAppSlotPublishingProfile.md)
取得 Azure Web App 槽發佈設定檔。

### [AzWebAppSnapshot](Get-AzWebAppSnapshot.md)
取得適用于 web 應用程式的快照。

### [AzWebAppSSLBinding](Get-AzWebAppSSLBinding.md)
取得 Azure Web App 憑證 SSL 系結。

### [新-AzAppServicePlan](New-AzAppServicePlan.md)
在指定的地理位置建立 Azure App 服務方案。

### [新-AzWebApp](New-AzWebApp.md)
建立 Azure Web App。

### [新-AzWebAppAzureStoragePath](New-AzWebAppAzureStoragePath.md)
建立代表要在 Web App 中裝載之 Azure 儲存路徑的物件。 它是用來做為 Set-AzWebApp 與 Set-AzWebAppSlot ( AzureStoragePath) 的參數

### [新-AzWebAppBackup](New-AzWebAppBackup.md)


### [新-AzWebAppContainerPSSession](New-AzWebAppContainerPSSession.md)
New-AzWebAppContainerPSSession 會在指定的網站或槽和指定的資源群組中，建立新的遠端 PowerShell 會話至指定的 windows 容器中。

### [新-AzWebAppDatabaseBackupSetting](New-AzWebAppDatabaseBackupSetting.md)


### [新-AzWebAppSlot](New-AzWebAppSlot.md)
建立 Azure Web App 槽。

### [新-AzWebAppSSLBinding](New-AzWebAppSSLBinding.md)
針對 Azure Web App 建立 SSL 憑證系結。

### [發佈-AzWebApp](Publish-AzWebApp.md)
使用 zipdeploy 從 ZIP、JAR 或 WAR 檔案部署 Azure Web App。

### [移除-AzAppServicePlan](Remove-AzAppServicePlan.md)
移除 Azure 應用程式服務方案。

### [移除-AzWebApp](Remove-AzWebApp.md)
移除 Azure Web App。

### [移除-AzWebAppBackup](Remove-AzWebAppBackup.md)


### [移除-AzWebAppSlot](Remove-AzWebAppSlot.md)


### [移除-AzWebAppSSLBinding](Remove-AzWebAppSSLBinding.md)
從上傳的憑證中移除 SSL 系結。

### [Reset-AzWebAppPublishingProfile](Reset-AzWebAppPublishingProfile.md)


### [Reset-AzWebAppSlotPublishingProfile](Reset-AzWebAppSlotPublishingProfile.md)


### [重新開機-AzWebApp](Restart-AzWebApp.md)
重新開機 Azure Web App。

### [重新開機-AzWebAppSlot](Restart-AzWebAppSlot.md)


### [Restore-AzDeletedWebApp](Restore-AzDeletedWebApp.md)
將已刪除的 web 應用程式還原到新的或現有的 web 應用程式。

### [Restore-AzWebAppBackup](Restore-AzWebAppBackup.md)


### [Restore-AzWebAppSnapshot](Restore-AzWebAppSnapshot.md)
還原 web 應用程式快照。

### [Set-AzAppServicePlan](Set-AzAppServicePlan.md)
設定 Azure App 服務方案。

### [Set-AzWebApp](Set-AzWebApp.md)
修改 Azure Web App。

### [Set-AzWebAppSlot](Set-AzWebAppSlot.md)
修改 Azure Web App 槽。

### [Set-AzWebAppSlotConfigName](Set-AzWebAppSlotConfigName.md)
設定 Web 應用程式槽配置名稱

### [開始-AzWebApp](Start-AzWebApp.md)
啟動 Azure Web App。

### [開始-AzWebAppSlot](Start-AzWebAppSlot.md)
啟動 Azure Web App 槽。

### [停止 AzWebApp](Stop-AzWebApp.md)
停止 Azure Web App。

### [停止 AzWebAppSlot](Stop-AzWebAppSlot.md)
停止 Azure Web App 槽。

### [切換 AzWebAppSlot](Switch-AzWebAppSlot.md)
使用 Web 應用程式交換兩個槽

### [AzWebAppAccessRestrictionConfig](Get-AzWebAppAccessRestrictionConfig.md)
取得 Azure Web App 的存取限制設定。

### [更新-AzWebAppAccessRestrictionConfig](Update-AzWebAppAccessRestrictionConfig.md)
針對 Azure Web App 更新 SCM 網站繼承配置。

### [附加 AzWebAppAccessRestrictionRule](Add-AzWebAppAccessRestrictionRule.md)
將存取限制新增至 Azure Web App。

### [移除-AzWebAppAccessRestrictionRule](Remove-AzWebAppAccessRestrictionRule.md)
移除 Azure Web App 的存取限制