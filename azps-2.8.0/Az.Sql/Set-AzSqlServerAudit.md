...
## 說明
**AzSqlServerAudit** Cmdlet 會變更 Azure SQL server 的審核設定。
...
### 範例9：停用將 Azure SQL server 的審核記錄傳送到 blob 儲存空間，並允許將其傳送至記錄分析。
```
...
### 範例10：啟用將 Azure SQL server 的審核記錄傳送到 blob 儲存體、事件中樞和記錄分析。
```
