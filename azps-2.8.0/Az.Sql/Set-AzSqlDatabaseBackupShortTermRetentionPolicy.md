...
### PolicyByResourceServerDatabaseSet (預設) 
```
...
## 說明
**AzSqlDatabaseBackupShortTermRetentionPolicy** Cmdlet 會取得已登錄至此資料庫的短期保留原則。
...
這個命令會將 database01 至35天的短期保留原則設定為。
...
這個命令會透過 database01 到資料庫物件中的 [管道]，來設定到35天的短期限保留原則。
...
### AzureSqlDatabaseBackupShortTermRetentionPolicyModel 中的 [.Sql]。
