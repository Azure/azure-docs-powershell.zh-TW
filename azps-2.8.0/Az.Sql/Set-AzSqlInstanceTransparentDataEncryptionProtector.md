...
## 說明
Set-AzSqlInstanceTransparentDataEncryptionProtector Cmdlet 會針對 SQL managed 實例設定 TDE 保護程式。 變更 TDE 保護器類型將會旋轉保護器。
...
這個命令會將受管理實例的 TDE 保護器類型更新為 [服務管理]。
...
這個命令會更新指定的受管理實例，以使用受管理的實例金鑰保存庫金鑰，並使用 Id " https://contoso.vault.azure.net/keys/contosokey/01234567890123456789012345678901 " 作為 TDE 保護器。

### 範例3：使用 managed 實例物件將透明資料加密保護器類型設定為 Azure 金鑰保存庫
```powershell
...
### 範例4：使用資源識別碼將透明資料加密保護器類型設定為 Azure 金鑰保存庫
```powershell
PS C:\> $managedInstance = Get-AzSqlInstance -Name 'ContosoManagedInstanceName' -ResourceGroupName 'ContosoResourceGroup'
...
### 範例5：使用管道將透明資料加密保護器類型設定至 Azure Key Vault
```powershell
PS C:\> $managedInstance = Get-AzSqlInstance -Name 'ContosoManagedInstanceName' -ResourceGroupName 'ContosoResourceGroup'
...
### -類型
Azure Sql Database 透明資料加密保護器類型。
...
### AzureRmSqlManagedInstanceTransparentDataEncryptionProtectorModel 中的 [TransparentDataEncryption]
