...
## 說明
**AzDataFactoryV2IntegrationRuntime** Cmdlet 會啟動受管理的專用整合式執行時間。 資源已在作業狀態為「已啟動」的情況下進行預配。
...
這個 Cmdlet 會啟動一個名為「測試專用-ir」的 managed 專用整合執行時間。
...
### PSManagedIntegrationRuntimeStatus 中的 DataFactoryV2。

## 筆記

## 相關連結

[停止 AzDataFactoryV2IntegrationRuntime]()
