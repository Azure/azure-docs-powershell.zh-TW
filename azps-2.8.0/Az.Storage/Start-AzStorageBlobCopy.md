...
### 範例6：將區塊 blob 複製到具有新 blob 名稱的目標容器，並將 [目標 blob StandardBlobTier] 設定為 [封存]，RehydratePriority 為 [高]
```
...
這個命令會將區塊 blob 的複製操作啟動至具有新 blob 名稱的目標容器，並將 [目標 blob StandardBlobTier] 設定為 [封存]，RehydratePriority [高]
