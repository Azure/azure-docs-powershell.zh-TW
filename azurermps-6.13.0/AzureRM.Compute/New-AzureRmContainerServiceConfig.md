...
## 說明
**新的-AzureRmContainerServiceConfig** Cmdlet 會為容器服務建立本機設定物件。
將此物件提供給 New-AzureRmContainerService Cmdlet，以建立容器服務。
...
這個命令會建立一個容器，然後將它儲存在 $Container 變數中。
此命令會為容器服務設定指定各種設定。 命令會使用管線運算子，將 configuration 物件傳遞到 Add-AzureRmContainerServiceAgentPoolProfile Cmdlet。 該 Cmdlet 會新增代理池設定檔。
在 $Container 中為 **AzureRmContainerService** 的 *ContainerService* 參數指定物件。

## 參數

### -AdminUsername
指定要用於 Linux 基礎容器服務的系統管理員帳戶名稱。
...
### -AgentPoolProfile
指定容器服務的代理池設定檔物件陣列。
使用 Add-AzureRmContainerServiceAgentPoolProfile Cmdlet 新增設定檔。
...
### -OrchestratorType
指定容器服務的 orchestrator 類型。
此參數可接受的值為： DCOS 和 Swarm。
...
指定使用 Windows 作業系統之容器服務的系統管理員使用者名稱。
...
### "ContainerServiceOrchestratorTypes" 1 [[21.0.0.0]。計算，版本 =，Culture = 中性，PublicKeyToken = 31bf3856ad364e35 "]]）。）

### System.object

### ContainerServiceAgentPoolProfile [] 的計算模型。
