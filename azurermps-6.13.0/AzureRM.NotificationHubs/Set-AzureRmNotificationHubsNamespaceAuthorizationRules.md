...
## 說明
**AzureRmNotificationHubsNamespaceAuthorizationRules** Cmdlet 會修改指派給通知中心命名空間之 (SAS) 授權規則的共用存取簽章。
...
JSON 檔案是使用類似以下語法的文字檔： {  
    ...
    "版權"： \[  
        ...
        收發  
    \]  
} 與 **AzureRmNotificationHubsNamespaceAuthorizationRules** Cmdlet 搭配使用時，前面的 JSON 範例會修改名為 ContosoAuthorizationRule 的授權規則，讓使用者聆聽並傳送該命名空間的許可權。
