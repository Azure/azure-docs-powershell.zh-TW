...
## 說明
**AzureRmVpnClientIpsecParameter** Cmdlet 會為現有的虛擬網路閘道設定 vpn ipsec 參數。
建立虛擬網路閘道之後，它會在閘道上設定預設的 vpn ipsec 原則組。 在此情況下，指向 [網站使用者] 想要使用特定的自訂 ipsec 原則連線至 VPN 閘道，使用者必須先在 VPN 閘道設定該 ipsec 原則。 **AzureRmVpnClientIpsecParameter** 提供一種方式來執行這項作業。
...
Vpn 用戶端 ipsec 參數。 這個參數值可以使用 PS 命令來構造： [新-AzureRmVpnClientIpsecParameter]
...
參數： VpnClientIPsecParameter (ByValue) 
...
[新-AzureRmVpnClientIpsecParameters](./New-AzureRmVpnClientIpsecParameters.md)
