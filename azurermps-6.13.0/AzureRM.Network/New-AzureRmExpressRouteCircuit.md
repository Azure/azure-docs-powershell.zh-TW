...
## 說明
**新的-AzureRmExpressRouteCircuit** Cmdlet 會建立 Azure express 路由回路。

## 示例

### 範例1：建立新的 ExpressRoute 電路
```
$parameters = @{
...
### 範例2：在 ExpressRoutePort 上建立新的 ExpressRoute 電路
```
$parameters = @{
    Name='ExpressRouteCircuit'
    ResourceGroupName='ExpressRouteResourceGroup'
...
使用此參數可讓您使用傳統的 Azure PowerShell Cmdlet 來管理電路。
...
### -BandwidthInGbps
在 ExpressRoutePort 資源上預配電路時的電路頻寬。
...
### -ExpressRoutePort
在 ExpressRoutePort 資源上預配電路時，對 ExpressRoutePort 資源的參照。
...
### -ServiceProviderName
電路服務提供者的名稱。 這必須符合 Get-AzureRmExpressRouteServiceProvider Cmdlet 所列出的名稱。
...
### -SkuFamily
SKU 系列決定帳單類型。 此參數可能的值為： `MeteredData` 或 `UnlimitedData` 。 請注意，您可以將帳單類型從 MeteredData 變更為 UnlimitedData，但您無法將類型從 UnlimitedData 變更為 MeteredData。
...
### -SkuTier
電路的服務層級。 此參數可能的值為： `Standard` 或 `Premium` 。
...
### [System.object]. 清單 ' 1 [PSPeering，6.4.1.0，，system.object，版本 =，Culture = 中性，PublicKeyToken = null]」）。

### [System.object]. 清單 ' 1 [PSExpressRouteCircuitAuthorization，6.4.1.0，，system.object，版本 =，Culture = 中性，PublicKeyToken = null]」）。
