...
## 說明
**AzureRmVirtualNetworkSubnetConfig** Cmdlet 會將子網配置新增至現有的 Azure 虛擬網路。
...
```
  ...
  這個範例會先建立資源群組做為要建立的資源容器。 接著，它會建立子網設定，並使用它來建立虛擬網路。 然後，使用 Add-AzureRmVirtualNetworkSubnetConfig 將子網新增至虛擬網路的記憶體內表示形式。 [Set-AzureRmVirtualNetwork] 命令會以新的子網來更新現有的虛擬網路。

### 2：在要新增到現有虛擬網路的子網中新增委派
```powershell
...
這個範例會先取得現有的 vnet。
接著，它會在記憶體中建立委派物件。
最後，它會建立一個新的子網，並將該委派新增至 vnet。 修改後的設定就會傳送至伺服器。
...
這個 Cmdlet 會將虛擬網路子網配置新增至此參數指定的物件。
...
### -VirtualNetwork
指定要在其中新增子網配置的 **VirtualNetwork** 物件。
...
### [System.object]. 清單 ' 1 [PSServiceEndpointPolicy，6.7.0.0，，system.object，版本 =，Culture = 中性，PublicKeyToken = null]」）。

### [System.object]。清單 ' 1 [Microsoft.Azure.Commands.Network.Models.PSDelegation，6.7.0.0 "Network，版本 =，Culture = 中性，PublicKeyToken = null]]
