...
## 說明
**AzureRmLoadBalancerRuleConfig** Cmdlet 會設定負載平衡器規則配置的目標狀態。
...
第二個命令使用管線運算子，將負載平衡器 $slb 至 Add-AzureRmLoadBalancerRuleConfig，這會將名為 NewRule 的規則新增至它。
第三個命令會將負載平衡器傳遞至 AzureRmLoadBalancerRuleConfig，這會設定新的規則 **設定** 。
