...
## 摘要
Update-AzureRmVpnGateway 會將可伸縮的 VPN 閘道更新為適當的目標狀態。
...
## 說明
Update-AzureRmVpnGateway 會將可伸縮的 VPN 閘道更新為適當的目標狀態。 AzureRmVpnGateway 是一個軟體定義的連線至 VirtualHub 中的 [網站連線]。 此閘道會根據使用者指定的縮放比例來調整大小和縮放比例。 您可以將連線從稱為「VPNSite」的分支/網站設定為可伸縮閘道。 每個連線都包含2個 Active-Active 隧道
...
建立閘道之後，它會使用 Set-AzureRmVpnGateway 將閘道升級為3個比例單位。
