...
## 說明
**AzureRmManagedApplication** Cmdlet 會更新受管理的應用程式
...
### -識別碼
完全限定的管理應用程式識別碼，包括訂閱。 例如/subscriptions/{subscriptionId}/resourcegroups/{resourceGroupName
...
### 類型
受管理的應用程式類型。
其中一個 marketplace 或 servicecatalog
...
### -參數
受管理的應用程式之參數的 JSON 格式字串。
此路徑可以是包含參數的檔案名或 uri 路徑，或字串形式的參數。
