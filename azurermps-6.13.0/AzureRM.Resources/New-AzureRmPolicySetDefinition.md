...
## 說明
**新的-AzureRmPolicySetDefinition** Cmdlet 會建立一個策略集定義。
...
這個命令會建立一個名為 VMPolicyDefinition 的原則集定義，其中包含 C:\VMPolicy.json 中指定的原則定義。 上面提供了 VMPolicy.js的範例內容。
...
這個命令會建立一個名為 VMPolicyDefinition 的參數化原則集定義，其中包含 C:\VMPolicy.js中指定的原則定義。 上面提供了 VMPolicy.js的範例內容。
...
### -中繼資料
原則集定義的中繼資料。 這可以是包含中繼資料之檔案名的路徑，或以字串形式的中繼資料
