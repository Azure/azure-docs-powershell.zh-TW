...
### 範例1為 application insights 資源建立新的連續匯出配置
```
...
建立新的 application insights 連續匯出設定，以將 "Request" 和 "Trace" 檔案類型匯出至儲存空間在資源群組 "testgroup" 中，在儲存帳戶 "teststorageaccount" 中包含 "testcontainer"。 SAS 權杖必須是有效的，且擁有對容器的寫入權限，否則連續匯出功能將無法運作。如果 SAS 權杖已過期，連續匯出功能將會停止運作。
