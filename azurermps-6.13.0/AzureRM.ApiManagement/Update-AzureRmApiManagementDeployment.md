...
## 說明
**AzureRmApiManagementDeployment** Cmdlet 會更新 API 管理服務的目前部署。

## 示例

### 範例1：更新 ApiManagement 實例的部署
```powershell
PS C:\>Update-AzureRmApiManagementDeployment -ResourceGroupName "Contoso" -Name "ContosoApi" -Sku "Standard" -Capacity 3
```

這個命令會將 API 管理實例的部署更新為三個單位容量標準。

### 範例2：取得 ApiManagement 實例並將它重新縮放
```powershell
...
這個範例會取得 Api 管理實例、將它縮放至5個 premium 單位，然後再將額外的三個單元加到 premium 區域。
...
這個命令會更新現有的 API 管理部署，並加入外部 *VpnType* 。
...
這個命令會更新現有的 API 管理部署，並加入內部 *VpnType* 。

## 參數

### -AdditionalRegions
指定 Azure API 管理的其他部署區域。
...
此參數可接受的值為：
- 所有.
API 管理部署不是任何虛擬網路的一部分。
這是預設值。 
- 外來.
API 管理部署有一個面向外部的虛擬位址。 
- 內部.
API 管理部署有一個面向內部網路的虛擬位址。
...
### PsApiManagementVpnType 中的 ApiManagement。

### ApiManagement 集合. 泛型. IList "1 [PsApiManagementRegion，ApiManagement，Version = 6.1.2.0，Culture = 中性，PublicKeyToken = null]] （區域性 = null）
