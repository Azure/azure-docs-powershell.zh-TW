...
**AzureRmApiManagementApiVersionSet** Cmdlet 會修改 Azure API 管理 API 版本設定。
...
這個命令會更新現有的 API 版本集與版本設定配置 `Header` 和標頭參數 `api-version` 。
...
### -InputObject
PsApiManagementApiVersionSet 的實例。 這個參數是必要的。
...
### -PassThru
如果已指定，則會將代表已修改之 apiVersionSet 的 PsApiManagementApiVersionSet 類型的 ServiceManagement 類型 ApiManagement 寫入輸出。
...
### ServiceManagement. PsApiManagementVersioningScheme （ApiManagement）
...
[AzureRmApiManagementApiVersionSet](./Get-AzureRmApiManagementApiVersionSet.md)

[新-AzureRmApiManagementApiVersionSet](./New-AzureRmApiManagementApiVersionSet.md)
