...
## 說明
**新的-AzureRmApiManagementAuthorizationServer** Cmdlet 會建立 Azure API 管理授權伺服器。
...
### -TokenBodyParameters
使用 **應用程式/x-www 格式 urlencoded** 格式指定其他主體參數。
...
### -TokenEndpointUrl
指定用戶端用來在 exchange 中取得存取權杖的權杖端點 URL，以呈現授權授權或重新整理權杖。
...
### ServiceManagement. PsApiManagementOAuth2AuthrozationServer （ApiManagement）
