...
## 說明
[Helper] 命令來建立 PsApiManagementRegion 的實例。
此命令將與 New-AzureRmApiManagement 命令搭配使用。
...
在 West 美國地區建立外部 VpnType 的 ApiManagement 服務，並在美國中部新增一個區域。

## 參數

### -容量
Azure API Management 服務其他區域的 Sku 容量。
預設值為1。
...
### -VirtualNetwork
Azure API Management 部署區域的虛擬網路設定。
預設值為 [$null]。
...
### PsApiManagementRegion 中的 ApiManagement。
