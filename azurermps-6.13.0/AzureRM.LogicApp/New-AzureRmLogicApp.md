...
## 說明
**新的-AzureRmLogicApp** Cmdlet 會使用邏輯應用程式功能來建立邏輯 app。
...
這個命令會使用 Get-AzureRmResourceGroup Cmdlet 來取得名為 ResourceGroup11 的資源群組。
...
第一個命令會使用 Get-AzureRmLogicApp Cmdlet 來取得名為 LogicApp03 的邏輯 app。
...
### -定義
將您的邏輯 app 的定義指定為 JavaScript 物件 Notataion (JSON) 格式中的物件或字串。
