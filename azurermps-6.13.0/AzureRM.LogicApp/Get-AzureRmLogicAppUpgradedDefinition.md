...
## 說明
**AzureRmLogicAppUpgradedDefinition** Cmdlet 會從資源群組取得架構版本與邏輯 app 的升級定義。
這個 Cmdlet 會傳回代表已升級之邏輯 app 之定義的物件。
指定資源群組名稱、邏輯 app 名稱和目標架構版本。
此模組支援動態參數。
若要使用動態參數，請在命令中輸入。
...
第一個命令會取得已升級至指定目標架構版本之邏輯 app 的定義。
該命令會將定義儲存在 $UpgradedDefinition 變數中。
第二個命令會將 $UpgradedDefinition 的內容顯示為一個字串。
