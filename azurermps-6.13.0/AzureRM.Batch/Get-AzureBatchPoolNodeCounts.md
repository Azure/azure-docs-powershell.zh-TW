...
## 說明
Get-AzureBatchPoolNodeCounts Cmdlet 可讓客戶取得依池分組的每個節點狀態的節點數。 可能的節點狀態是 [建立]、[空閒]、[leavingPool]、[離線]、[已搶佔]、[重新開機]、[重新格式化]、[執行]、[無法 這個 Cmdlet 會採用 PoolId 或 Pool 參數，只篩選指定了 pool id 的 pool。 
