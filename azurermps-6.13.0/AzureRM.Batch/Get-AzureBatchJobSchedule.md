...
## 說明
**AzureBatchJobSchedule** Cmdlet 會針對 *BatchCoNtext* 參數指定的批次帳戶取得 Azure Batch 作業排程。
指定識別碼以取得單一作業排程。
指定 *篩選* 參數，以取得符合開啟資料通訊協定的作業排程 (OData) 篩選。
...
這個命令會取得 ID 為 JobSchedule23 的作業排程。
...
這個命令會透過指定 *Filter* 參數，來取得所有 id 都以 job 開始的作業排程。
...
### -篩選
指定 OData 篩選子句。
這個 Cmdlet 會傳回符合此參數指定之篩選準則的作業排程。
如果您沒有指定篩選，這個 Cmdlet 會傳回批內容的所有作業排程。
