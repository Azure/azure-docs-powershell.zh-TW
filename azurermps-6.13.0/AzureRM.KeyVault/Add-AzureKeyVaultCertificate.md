...
## 說明
**AzureKeyVaultCertificate** Cmdlet 會啟動在 Azure 金鑰保存庫中，在金鑰電子倉庫中註冊憑證的程式。
...
第一個命令使用 New-AzureKeyVaultCertificatePolicy Cmdlet 來建立證書原則，然後將它儲存在 $Policy 變數中。
第二個命令使用 [ **載入 AzureKeyVaultCertificate** ] 來啟動程式來建立證書。
第三個命令使用 Get-AzureKeyVaultCertificateOperation Cmdlet 來輪詢操作，以確認該作業已完成。
最後一個命令會使用 Get-AzureKeyVaultCertificate Cmdlet 來取得憑證。

## 參數

### -CertificatePolicy
指定 **KeyVaultCertificatePolicy** 物件。
