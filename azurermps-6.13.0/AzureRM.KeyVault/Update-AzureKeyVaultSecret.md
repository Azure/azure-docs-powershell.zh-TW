...
## 說明
**AzureKeyVaultSecret** Cmdlet 會更新金鑰保存庫中的秘密可編輯屬性。
...
第三個命令使用 Get-AzureKeyVaultSecret Cmdlet 在指定的金鑰保存庫中取得機密，然後將這些機密傳遞到 **物件** Cmdlet。 **物件** Cmdlet 會篩選以字元開頭的名稱的密碼。 命令會將與篩選器相符的秘訣加入 Update-AzureKeyVaultSecret Cmdlet，從而停用。
...
前三個命令會定義要用於 *VaultName* 、 *名稱* 和 *ContentType* 參數的字串變數。 第四個命令使用 Get-AzureKeyVaultKey Cmdlet 來取得指定的金鑰，並將這些金鑰管道到 Update-AzureKeyVaultSecret Cmdlet，以將其內容類型設為 XML。
