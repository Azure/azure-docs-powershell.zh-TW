...
## 說明
**Export-AzureRmDataLakeStoreChildItemProperties** 是用來報告指定目錄的 ADLS 空間使用量或/和 ACL 使用量，以及它是子目錄和檔案。
...
取得/a. 下所有子目錄和檔案的 ACL 使用量 IncludeFile 可確保針對檔案也會報告使用方式。 在此情況下，HideconsistentAcl 會顯示/a 的 Acl （而不是其子項），因為所有子系都有與/a. 相同的 acl。 如果您的所有 acl 都與根目錄相同，此旗標會跳過子樹的 acl 輸出。
...
### -OutputPath
輸出檔案的路徑。 可以是本機路徑或 Adl 路徑。 預設為本機。 如果 SaveToAdl 是 pecified，則是同一帳戶中的 ADL 路徑
