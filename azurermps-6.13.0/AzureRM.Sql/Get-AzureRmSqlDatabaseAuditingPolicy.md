...
## 說明
**AzureRmSqlDatabaseAuditingPolicy** Cmdlet 會取得 Azure SQL 資料庫的審核原則。
若要使用 Cmdlet，請使用 [ *ResourceGroupName* ]、[ *ServerName* ] 和 [ *DatabaseName* ] 參數來識別資料庫。

## 示例

### 範例1：取得 Azure SQL database 的審核原則，並在其上定義 [資料表審核]
```
...
### 範例2：取得 Azure SQL database 的審核原則，並在其上定義 Blob 審核
```
PS C:\>Get-AzureRmSqlDatabaseAuditingPolicy -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -DatabaseName "Database01"
DatabaseName           : database01
AuditAction            : {}
