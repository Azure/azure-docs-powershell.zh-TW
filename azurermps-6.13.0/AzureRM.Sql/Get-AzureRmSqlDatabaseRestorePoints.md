...
## 說明
**AzureRmSqlDatabaseRestorePoints** Cmdlet 會檢索 Azure SQL 資料倉儲可以還原的不同還原點。
針對 Azure SQL 資料庫，[還原] 視窗是連續的。
這表示資料庫備份保留期間中的任何時間點都可以用來做為還原點。
...
這個命令會傳回 Azure SQL Database （名為 Database01）所有可用的還原點。
...
### AzureSqlDatabaseRestorePointModel 中的 [.Sql]。
