...
## 說明
**AzureRmSqlDatabaseDataMaskingRule** Cmdlet 會將特定的資料遮罩規則從 Azure SQL 資料庫中移除。
您可以使用 *ResourceGroupName* 、 *ServerName* 、 *DatabaseName* 和 *RuleId* 參數來移除資料遮罩規則，以識別此 Cmdlet 移除的規則。
...
這個命令會移除為資料庫 Database01 定義的規則名稱 Rule01。
資料庫位於 Server01 上，且已指派給資源群組 ResourceGroup01。
