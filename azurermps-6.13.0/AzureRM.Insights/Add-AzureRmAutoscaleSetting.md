...
## 說明
**AzureRmAutoscaleSetting** Cmdlet 會建立自動縮放設定。
...
前兩個命令使用 New-AzureRmAutoscaleRule 來建立兩個自動縮放規則，$Rule 1 和 $Rule 2。
第三個和第四個命令使用 New-AzureRmAutoscaleProfile 來建立自動縮放設定檔，$Profile 1 和 $Profile 2，使用 $Rule 1 和 $Rule 2。
...
### [System.object]。清單 ' 1 [5.1.0.0 AutoscaleProfile，，版本 =，Culture = 中性，PublicKeyToken = null]」））））中的集合。

### [System.object]。清單 ' 1 [5.1.0.0 AutoscaleNotification，，版本 =，Culture = 中性，PublicKeyToken = null]」））））中的集合。
