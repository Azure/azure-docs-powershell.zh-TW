...
### -Pool
此資源定義 IP 位址的範圍，這些位址是針對子網中的節點所指派的位址。
若要建立，請參閱 POOL 屬性和建立雜湊資料表的備忘稿區段。
...
[泳池] <IIPPool> ：此資源會定義 IP 位址範圍，從此處將位址指派給子網中的節點。
  - `[Location <String>]`：資源所在的地區。
  - `[Tag <IResourceTags>]`：鍵-值對的清單。
    - `[(Any) <String>]`：這表示您可以將任何屬性新增至此物件。
  - `[AddressPrefix <String>]`：位址首碼。
  - `[EndIPAddress <String>]`：結束 IP 位址。
  - `[NumberOfAllocatedIPAddresses <Int64?>]`：目前已分配 IP 位址的數目。
  - `[NumberOfIPAddresses <Int64?>]`： IP 位址的總數。
  - `[NumberOfIPAddressesInTransition <Int64?>]`：過渡中的目前 IP 位址數。
  - `[StartIPAddress <String>]`：起始 IP 位址。
