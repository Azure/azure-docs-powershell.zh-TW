...
### -BareMetalNode
在群集上用於 ScaleOut 操作的裸機節點描述。
若要建立，請參閱 BAREMETALNODE 屬性和建立雜湊資料表的備忘稿區段。
...
### IBareMetalNodeDescription （FabricAdmin）。 Api20160501。
...
BAREMETALNODE <IBareMetalNodeDescription> ：身分識別參數
  - `[BiosVersion <String>]`： Bios 版本的物理電腦。
  - `[BmciPv4Address <String>]`：物理電腦的 BMC 位址。
  - `[ClusterName <String>]`：群集的名稱。
  - `[ComputerName <String>]`：電腦的名稱。
  - `[MacAddress <String>]`：裸機節點之 MAC 位址的名稱。
  - `[Model <String>]`：物理電腦的模型。
  - `[SerialNumber <String>]`：物理電腦的序列值。
  - `[Vendor <String>]`：物理電腦的廠商。
