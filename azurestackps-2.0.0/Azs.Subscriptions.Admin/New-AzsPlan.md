...
若要建立，請參閱 PLANDEFINITION 屬性和建立雜湊資料表的備忘稿區段。
...
PLANDEFINITION <IPlan> ：方案代表提供租使用者的配額與功能套件。 租使用者可以透過將其存取權升級為底層雲端服務的優惠來取得此方案。
  - `[Location <String>]`：資源的位置
  - `[Description <String>]`：方案的描述。
  - `[DisplayName <String>]`：顯示名稱。
  - `[ExternalReferenceId <String>]`：外部參照識別碼。
  - `[PropertiesName <String>]`：方案名稱。
  - `[QuotaIds <String[]>]`：方案中的配額識別碼。
  - `[SkuIds <String[]>]`： SKU 識別碼。
  - `[SubscriptionCount <Int32?>]`：訂閱數。
