...
如果未指定 UpdateName 值，Get-UpdateRun 將總是要求輸入。
一旦提供，就會輸出所有失敗或成功的 UpdateRun 實例

### 範例2：依 UpdateName Get-AzsUpdateRun
```powershell
...
將會檢索與特定更新相關的所有 UpdateRuns

### 範例2：依名稱 Get-AzsUpdateRun
```powershell
...
將會檢索與特定更新及特定名稱相關聯的所有 UpdateRuns
...
### IUpdateRun （UpdateAdmin）。 Api20160501。
