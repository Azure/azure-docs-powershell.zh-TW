...
### IBackupLocation （BackupAdmin）。 Api20180901。
...
備份 <IBackupLocation> ：備份位置的相關資訊。
  - `[Location <String>]`：資源的位置。
  - `[Tag <IResourceTags>]`： [鍵值] 對的清單。
    - `[(Any) <String>]`：這表示您可以將任何屬性新增至此物件。
  - `[BackupFrequencyInHours <Int32?>]`：排程程式執行備份之頻率的間隔（以小時為單位）。
  - `[BackupRetentionPeriodInDays <Int32?>]`：在儲存位置中備份的保留期間（以天為單位）。
  - `[EncryptionCertBase64 <String>]`：備份加密憑證的 base64 原始資料。
  - `[IsBackupSchedulerEnabled <Boolean?>]`：如果已啟用備份排程程式，則為 True。
  - `[Password <String>]`：存取位置的密碼。
  - `[Path <String>]`：更新位置的路徑
  - `[UserName <String>]`：用於存取位置的使用者名稱。
