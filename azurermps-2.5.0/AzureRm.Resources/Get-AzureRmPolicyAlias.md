...
## 摘要
Get-AzureRmPolicyAlias 檢索及輸出已定義別名並符合指定參數值的 Azure 提供者資源類型。 如果未提供任何參數，則會輸出包含別名的所有提供者資源類型。
...
## 說明
**AzureRmPolicyAlias** Cmdlet 會取得原則別名清單。
策略別名是由 Azure Policy 用來參照資源類型屬性。
提供的參數可根據資源類型或其別名的各種屬性來限制清單中的專案。
如果目標字串包含使用不區分大小寫的比較，則指定的 match 值會相符。
...
列出命名空間與「compute」相符且包含別名的所有提供者資源類型。
...
列出資源類型符合「virtual」且包含別名的所有提供者資源類型。
...
列出資源類型符合「virtual」的所有提供者資源類型，包括不含別名的服務。
...
列出其 namespace 與「compute」相符且資源類型符合「virtual」且包含別名的所有提供者資源類型。
注意：-NamespaceMatch 和-ResourceTypeMatch 提供專屬相符，而其他則是包含。
...
列出所有包含與「virtual」相符的別名的提供者資源類型，或是與「網路」相符路徑的別名。
...
使用 Alpha api 版本列出所有提供者資源類型，或包含含 Alpha api 版本的別名。
...
### -ApiVersion
設定時，會指出要使用的資源提供者 API 版本。 如果未指定，API 版本會自動決定為最新的可用。
```yaml
Type: String
Parameter Sets: (All)
Aliases:
...
### PsResourceProviderAlias 中的 [命令]

## 筆記

* 若要展開別名或任何其他屬性，請將輸出管道 `select -ExpandProperty <property>` 。 例如： `Get-AzureRmPolicyAlias -NamespaceMatch 'Microsoft.Cache' -ApiVersionMatch 'alpha' | select -ExpandProperty Aliases | select -Property Name -ExpandProperty Paths`

* 您可以在輸出中使用其他屬性，並透過將輸出輸送至來顯示 `Format-List` 。 例如： `Get-AzureRmPolicyAlias -NamespaceMatch 'Web' -ResourceTypeMatch site -PathMatch cert | Format-List`
