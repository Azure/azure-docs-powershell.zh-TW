...
## 說明
**AzureRmPolicySetDefinition** Cmdlet 會移除原則定義。
...
第一個命令會使用 Get-AzureRmPolicySetDefinition Cmdlet 來取得原則集定義。
該命令會將它儲存在 $PolicySetDefinition 變數中。
第二個命令會移除由 $PolicySetDefinition 的 **ResourceId** 屬性所識別的原則集定義。
