...
## 說明
**新的-AzureRmAutoscaleRule** Cmdlet 會建立自動縮放規則。
...
第一個命令會針對要求公制建立規則，然後將它儲存在 $Rule 1 變數中。
第二個命令會針對要求公制建立第二個規則，然後將它儲存在 $Rule 2 變數中。
...
### ComparisonOperationType 中的 [管理模型]。

### MetricStatisticType 中的 [管理模型]。

### Double

### TimeAggregationType 中的 [管理模型]。

### System.object

### ScaleDirection 中的 [管理模型]。
