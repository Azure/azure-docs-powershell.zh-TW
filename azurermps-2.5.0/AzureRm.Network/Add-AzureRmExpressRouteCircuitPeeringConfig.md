...
## 說明
**AzureRmExpressRouteCircuitPeeringConfig** Cmdlet 會將對等設定新增至 ExpressRoute 回路。 ExpressRoute 回路使用連線提供者（而不是公用網際網路），將您的內部部署網路連線至 Microsoft 雲端。 請注意，執行 **附加 AzureRmExpressRouteCircuitPeeringConfig** 之後，您必須呼叫 Set-AzureRmExpressRouteCircuit Cmdlet 才能啟動設定。

## 示例

### 範例1：將對等新增至現有的 ExpressRoute 電路
```
