...
## 說明
**AzStorageFileCopyState** Cmdlet 會取得 Azure 儲存空間複製作業的狀態。
它應該在複製目的地檔案上執行。
...
第一個命令會開始將檔案 "contosofile" 複製到 "contosofile_copy"，並輸出 destiantion 檔案物件。 第二個命令管線要取得 AzStorageFileCopyState 的 destiantion 檔案物件，以取得檔案複製狀態。
