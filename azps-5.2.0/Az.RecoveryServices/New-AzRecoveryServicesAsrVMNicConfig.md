...
## 說明
**新的-AzRecoveryServicesAsrVMNicConfig** Cmdlet 會建立一個 ASR NIC config 物件，其中包含容錯移轉與測試容錯移轉相關詳細資料。 如果未傳遞任何資訊，則會從複製受保護的專案挑選對應的值，避免這些值更新為 null。
...
使用針對 NIC 設定的容錯移轉和測試 faiover 網路設定來建立 ASRVmNicConfig 物件。 未傳遞的任何屬性都會從已傳送的受保護專案提取。
...
使用針對 NIC 重新命名設定的測試 faiover 網路設定來建立 ASRVmNicConfig 物件。 未傳遞的任何屬性都會從已傳送的受保護專案提取。


### 範例3

建立一個 ASR NIC config，其中包含容錯移轉與測試容錯移轉相關的配置詳細資料。  (自動生成) 
...
指定與測試容錯移轉 NIC 相關聯之公用 IP 位址的 ID。
...
### RecoveryServices. SiteRecovery. ASRVMNicConfig
