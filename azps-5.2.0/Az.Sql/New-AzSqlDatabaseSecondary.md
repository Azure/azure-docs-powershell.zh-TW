...
## 說明
**新的-AzSqlDatabaseSecondary** Cmdlet 會在用於設定資料庫異地複製時取代 Start-AzSqlDatabaseCopy Cmdlet。 它會將地域複製連結化物件從主要資料庫傳回次要資料庫。

## 示例

### 範例1：建立作用中 Geo-Replication
```powershell
$database = Get-AzSqlDatabase -DatabaseName $databasename -ResourceGroupName $primaryresourcegroupname -ServerName $primaryservername
$database | New-AzSqlDatabaseSecondary -PartnerResourceGroupName $secondaryresourcegroupname -PartnerServerName $secondaryservername -AllowConnections "All"
```

### 範例2：建立作用中 Geo-Replication，並指定合作夥伴資料庫名稱與源資料庫名稱不同
```powershell
$database = Get-AzSqlDatabase -DatabaseName $databasename -ResourceGroupName $primaryresourcegroupname -ServerName $primaryservername
...
指定此 Cmdlet 指派副資料庫的 Azure 資源群組的名稱。
...
### -ResourceGroupName
指定此 Cmdlet 指派主要資料庫的 Azure 資源群組的名稱。
...
### -標記
指定雜湊資料表形式的鍵值對，以與 SQL 資料庫複製連結建立關聯。 例如： @ {key0 = "value0"; key1 = $null; key2 = "value2"}
...
### AzureReplicationLinkModel 中的 [.Sql]。
