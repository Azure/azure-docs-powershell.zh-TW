...
這個命令會使用實例池物件在實例池中建立新的實例。

### 範例4：使用實例池資源識別碼在實例池中建立新的實例
```powershell
...
這個命令會在名為 instancePool0 的實例池中建立新的實例
...
備份儲存空間冗余用來儲存 Sql Azure Managed 實例的備份。 選項包括： Local、Zone 和 Geo
...
### -DnsZonePartner
合作夥伴管理伺服器的資源識別碼，以繼承受管理實例建立的 DnsZone 屬性
