...
## 摘要
從群集中移除服務結構應用程式類型。 這會移除此資源下的所有類型版本。 僅支援 ARM 已部署的應用程式類型。
...
## 說明
這個 Cmdlet 會從群集中移除應用程式類型，並移除此資源下的所有類型版本，但在執行此命令前，必須移除所有的應用程式。
...
這個範例會移除應用程式類型 "testAppType" 以及它底下的所有版本。 如果有使用此類型建立的任何應用程式，命令就會引發例外狀況。
