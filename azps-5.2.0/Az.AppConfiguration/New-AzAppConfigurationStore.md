...
### 範例2：建立 IdentityType 設定為 "UserAssigned" 的 app 設定
```powershell
...
這個命令會建立 app 設定並指派使用者指派的管理身分識別。
請參閱下列步驟的範例， `Update-AzAppConfigurationStore` 以啟用 CMK (cusomer 受管理的金鑰) 。

### 範例3：建立 IdentityType 設定為 "SystemAssigned" 的 app 設定 
```powershell
...
這個命令會建立應用程式設定，並啟用系統指派的與資源相關聯的管理身分識別。
請參閱下列步驟的範例， `Update-AzAppConfigurationStore` 以啟用 CMK (cusomer 受管理的金鑰) 。

### 範例4：建立 IdentityType 設定為 "SystemAssigned，UserAssigned" 的 app 設定
```powershell
PS C:\> $assignedIdentity = New-AzUserAssignedIdentity -ResourceGroupName azpwsh-manual-test -Name assignedIdentity
...
您可以啟用系統指派的管理身分識別，並同時提供使用者指派的身分識別。
請參閱下列步驟的範例， `Update-AzAppConfigurationStore` 以啟用 CMK (cusomer 受管理的金鑰) 。
