...
## 說明
評估在 IoT Edge 部署中定義的目標自訂或系統規格。
預先定義的系統規格是由 Iot 中樞計算且無法自訂。
- 「目標」顯示與部署目標條件相符的 IoT 邊緣裝置。
- 「已套用」會顯示由另一個較高優先順序的部署所無法進行目標的目標 IoT Edge 裝置。
- 「報告成功」顯示已報告已成功部署模組的 IoT Edge 裝置。
- 「報告失敗」會顯示已報告已順利部署一個或多個模組的 IoT Edge 裝置。 
  若要進一步調查錯誤，請遠端連線到這些裝置並查看記錄檔。
...
### PSConfigurationMetricsResult （IotHub.）。
