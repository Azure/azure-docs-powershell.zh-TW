...
## 說明
更新 Vmss VM 的狀態。  目前，唯一允許的更新是新增受管理的資料磁片。

## 示例

### 範例1：使用 New-AzVMDataDisk 將受管理的資料磁片新增至 Vmss VM
```powershell
...
第一個命令會取得現有的受管理磁片。
下一個命令會使用受管理的磁片建立資料磁片物件。
Next 命令會取得資源群組名稱所提供的現有 Vmss VM、Vmss 名稱和實例識別碼。
最後一個命令會透過新增資料磁片來更新 Vmss VM。

### 範例2：使用 Add-AzVMDataDisk 將受管理的資料磁片新增至 Vmss VM
```powershell
PS C:\> $disk = Get-AzDisk -ResourceGroupName $rgname -DiskName $diskname0
PS C:\> $VmssVM = Get-AzVmssVM -ResourceGroupName "myrg" -VMScaleSetName "myvmss" -InstanceId 0
...
指出模型更新或動作 (包括在 VMSS 上啟動的 [擴展]) ，不應該套用到 VMSS VM。
...
### PSVirtualMachineDataDisk [] 的計算模型。
