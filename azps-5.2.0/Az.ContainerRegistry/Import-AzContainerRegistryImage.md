...
將 busybox 匯入到 ACR。 注 
* "library/" 需要在源影像之前新增。 "busybox：最新" => "library/busybox：最新"
* 如果來源註冊表沒有公開提供，則需要認證
...
從來源 ACR 將 busybox 匯入至目標 ACR。 注 
* 如果來源登錄的管理員使用者已停用，則需要認證。
...
### 模式
強制會覆寫任何現有的目標標籤。
NoForce 時，任何現有的目標標籤都會在任何複製開始前失敗。
...
以標籤 ( 「hello-world：最新」 ) 指定影像。

根據 sha256 式資訊清單摘要指定影像， ( " hello-world@sha256:abc123 " ) 。
...
### -TargetTag
表單存放庫的字串清單 \[ ：標記 \] 。
省略 tag 時，會 (使用 [來源] 或 [最新版本] （如果仍省略來源標記）) 。
