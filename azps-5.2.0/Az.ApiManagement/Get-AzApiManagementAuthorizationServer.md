...
## 說明
**AzApiManagementAuthorizationServer** Cmdlet 會取得所有 Azure API 管理授權伺服器或指定的授權伺服器。
ClientSecret 不會包含在結果詳細資料中。 若要取得用戶端密碼，請使用 **AzApiManagementAuthorizationServerClientSecret**。
...
### -ResourceId
授權伺服器的 Arm 資源識別碼。 如果已指定，將會嘗試依據識別碼尋找授權伺服器。 這個參數是必要的。
