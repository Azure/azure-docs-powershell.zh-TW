...
## 說明
如果沒有指定訂閱， **AzApiManagementSubscription** Cmdlet 會取得指定的訂閱或所有訂閱。
[結果詳細資料] 中不會包含按鍵。 若要取得金鑰，請使用 **AzApiManagementSubscriptionKey**。
...
### -範圍
範圍識別碼。 訂閱的範圍（無論是 Api 範圍/apis/{apiId} 或產品範圍/products/{productId}），還是全域 API 範圍/apis 或全域作用中。
