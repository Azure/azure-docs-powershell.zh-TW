...
## 說明
建立或更新資源的安全性評定結果，可以用來變更現有結果的狀態或新增其他資料。
只能用於「CustomerManaged」評定類型，而且只能在建立了相符的評定中繼資料之後才能使用。
...
將訂閱結果標示為「不健康」，以便評估類型「4FB6C0A0-1137-42C7-A1C7-4BD37C91DE8D」-評定類型的詳細資料會在 assessmentMetadata 類型下找到。

## 參數

### -AdditionalData
附加至評定結果的資料，以獲得更佳的調查或狀態明晰性。
...
### -StatusCode
評定結果的 Progremmatic 代碼。
可以是「健康」、「不健康」或「NotApplicable」
