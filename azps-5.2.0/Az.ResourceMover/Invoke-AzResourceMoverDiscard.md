...
在放棄資源前驗證 dependecies。

### 範例2：使用 "MoveResource Name" 做為輸入，放棄資源的移動
```powershell
...
使用「MoveResource Name」做為輸入，放棄資源的移動。

### 範例3：捨棄使用 "SourceARMID" 做為輸入的資源移動
```powershell
