...
### 範例1：取得可在 Windows 中使用 Premium 的位置。 如果未指定任何參數，PlanType 會設定為 "Premium"，而 OSType 則會設定為 [Windows]。
```powershell
...
這個命令會取得可在 Windows 中使用 Premium 的位置。

### 範例2：取得可在 Linux 中使用 Premium 的位置。
```powershell
...
這個命令會取得可在 Linux 中使用 Premium 的位置。

### 範例3：取得可用於 Windows 的消耗量位置。
```powershell
