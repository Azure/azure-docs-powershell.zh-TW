...
## 說明
**AzManagementGroupDeployment** Cmdlet 會取消已啟動但未在管理群組完成的部署。
若要停止部署，部署必須具有不完整的提供狀態（例如 [提供]），而不是已完成的狀態（例如 [已預配] 或 [未成功]）。

若要在管理群組建立部署，請使用 New-AzManagementGroupDeployment Cmdlet。
...
這個命令會在管理群組「myMG」取消執行部署「deployment01」。
...
這個命令會取得管理群組「myMG」的部署「deployment01」，並予以取消。 
