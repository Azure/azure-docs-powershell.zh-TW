...
## 說明
建立 PSServiceDiagnosticSettings 物件。
這可以用來做為 `-InputObject` 參數 `Set-AzDiagnosticSetting`
...
建立 PSServiceDiagnosticSettings 物件。 並建立目標資源的診斷設定。

## 參數

### -DedicatedLogAnalyticsDestinationType
指示是否要將 (匯出到 ODS) 至資源特定 (（如果有的話）) 或 AzureDiagnostics (預設值，則不會顯示) 
...
### -WorkspaceId
要傳送記錄/度量單位之 Log Analytics 工作區的資源識別碼
