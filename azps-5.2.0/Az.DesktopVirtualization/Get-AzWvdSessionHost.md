...
### 範例1：依名稱取得 Windows 虛擬桌面 SessionHost
```powershell
...
這個命令會在主機池中取得 Windows 虛擬桌面 SessionHost。

### 範例2：列出 Windows 虛擬桌面 SessionHosts
```powershell
...
這個命令會在主機池中列出 Windows 虛擬桌面 SessionHosts。
...
### ISessionHost （DesktopVirtualization）。 Api20201019Preview。
