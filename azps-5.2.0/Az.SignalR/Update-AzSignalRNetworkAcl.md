...
## 說明
更新 SignalR 服務的網路 ACL，包括預設動作和公用及私人連線的網路 Acl。

## 示例

### 允許 RESTAPI、ClientConnection 公用網路，並將預設動作設定為 [拒絕]
```powershell
...
### -DefaultAction
SignalR 網路 Acl 的預設動作是 [允許] 或 [拒絕]。 它會決定 [拒絕網路 Acl] 或 [允許網路 Acl] 是否會生效。 例如，如果預設動作為 [允許]，則只有 [拒絕] Acl 所需的事項。
...
### PSSignalRNetworkAcls 中的 SignalR。
