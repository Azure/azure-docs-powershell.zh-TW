...
### 範例1：依範圍和名稱更新 AzCostManagementExport
```powershell
...
依範圍和名稱更新 AzCostManagementExport

### 範例2：由 InputObject 更新 AzCostManagementExport
```powershell
...
以 InputObject 更新 AzCostManagementExport
...
### -DefinitionType
匯出的類型。
請注意，"用法" 相當於 "ActualCost"，且適用于尚不提供費用或攤銷以進行服務預留的匯出。
...
### -ETag
資源的 eTag。
若要處理併發更新案例，此欄位將用來判斷使用者是否要更新最新版本。
