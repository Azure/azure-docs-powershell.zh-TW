...
這個命令會在指定的環境下建立 eventhub 事件來源。

### 範例2：在指定的環境下建立 iothub 事件來源
```powershell
...
這個命令會在指定的環境下建立 iothub 事件來源。

## 參數

### -ConsumerGroupName
包含要讀取事件之分區的事件/iot 中樞的消費者群組的名稱。
...
### -KeyName
可授與事件/iot 中心存取時間數列 Insights 服務存取權的 SAS 金鑰名稱。
...
### IEventSourceResource （TimeSeriesInsights）。 Api20200515。
