...
## 說明
**AzDataShareSubscription** Cmdlet 提供有關在資料共用帳戶中共用訂閱的資訊。 如果提供共用訂閱名稱，則 Cmdlet 會提供特定共用訂閱的相關資訊。 否則，此 Cmdlet 會在 [資料共用] 帳戶中提供共用訂閱清單。
...
此命令會提供在資料共用帳戶 WikiAds 中共用訂閱 AdsShareSubscription 的相關資訊。
