...
## 說明
**AzApplicationGatewayTrustedClientCertificate** Cmdlet 會將信任的用戶端 CA 憑證鏈新增至 Azure 應用程式閘道。
...
第一個命令會取得應用程式閘道，並將它儲存在 $gw 變數中。 第二個命令會建立新的信任用戶端 CA 憑證鏈，並將用戶端 CA 憑證的路徑做為輸入。 第三個命令會使用信任的用戶端憑證來建立 SSL 設定檔。 第四個命令會更新應用程式閘道。
