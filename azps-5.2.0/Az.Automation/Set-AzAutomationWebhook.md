...
## 說明
**AzAutomationWebhook** Cmdlet 會修改 Azure 自動化 runbook 的 webhook。
...
這個命令會設定 webhook 的 [執行物件] 值，並強制在名為 Windows 的混合式輔助角色群組上執行 runbook。
...
這個命令會更新 webhook 的 [執行物件] 值，並強制在 Azure runbook worker 上執行 runbook。 
