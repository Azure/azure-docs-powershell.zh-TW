...
## 說明
**AzAutomationDscNode** Cmdlet 會修改 (DSC) 節點設定的 Ap 所需狀態設定。
Azure 自動化會將 DSC 節點設定儲存為受管理的物件格式， (MOF) 設定檔。
...
這個命令會將名為 NodeConfiguration01 的節點配置指派給具有指定 GUID 的節點。

## 參數

### -AutomationAccountName
指定包含此 Cmdlet 修改設定之 DSC 節點之自動化帳戶的名稱。
...
### -ResourceGroupName
指定此 Cmdlet 修改 DSC 節點設定之資源群組的名稱。
