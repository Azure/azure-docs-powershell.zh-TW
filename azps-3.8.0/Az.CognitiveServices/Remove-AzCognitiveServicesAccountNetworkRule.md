...
### 範例1：使用 IPAddressOrRange 移除數個 IpRules
```
...
這個命令會以 IPAddressOrRange 移除數個 IpRules。

### 範例2：使用 JSON VirtualNetworkRule 物件輸入來移除 VirtualNetworkRule
```
...
這個命令會移除具有 VirtualNetworkRule 物件輸入的 VirtualNetworkRule，並使用 JSON。

### 範例3：移除使用管線的第一個 IpRule
```
...
這個命令會將第一個 IpRule 移除為管線。

### 範例4：使用 VirtualNetworkResourceID 移除多個 VirtualNetworkRules
```
...
這個命令會以 VirtualNetworkResourceID 移除數個 VirtualNetworkRules。 
...
### -IpAddressOrRange
認知服務帳戶在字串中 NetworkRule IpRules IpAddressOrRange。
...
認知服務帳戶在字串中 NetworkRule VirtualNetworkRules VirtualNetworkResourceId。
...
### -VirtualNetworkRule
認知服務帳戶 NetworkRule VirtualNetworkRules。
...
### PSIpRule []. CognitiveServices. []

### PSVirtualNetworkRule []. CognitiveServices. []
