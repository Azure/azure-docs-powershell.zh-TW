...
## 說明
**新的-AzAutomationConnection** Cmdlet 會在 Azure 自動化中建立連線。

## 示例

### 範例1：建立 ConnectionTypeName = Azure 的連線
```
...
### 範例2：建立 ConnectionTypeName = AzureServicePrincipal 的連接
```
...
此命令會在使用 $RunAsAccountConnectionFieldValues 和 ConnectionTypeName = AzureServicePrincipal 的自動化帳戶（名為 AutomationAccount01）中建立名為 Connection13 的 Azure 連線。
此 ConnectionTypeName = AzureServicePrincipal 主要用於 Azure Run As 帳戶。

### 範例3：建立 ConnectionTypeName = AzureClassicCertificate 的連接
```
...
此命令會在使用 $ClassicRunAsAccountConnectionFieldValues 和 ConnectionTypeName = AzureClassicCertificate 的自動化帳戶（名為 AutomationAccount01）中建立名為 Connection14 的 Azure 連線。
此 ConnectionTypeName = AzureClassicCertificate 主要用於 Azure 傳統運行方式帳戶。
