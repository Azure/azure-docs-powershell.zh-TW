...
### 範例 1-依名稱刪除現有的 Kusto 資料庫

```
PS C:\> Remove-AzKustoDatabase -ResourceGroupName testrg -ClusterName mykustocluster -Name mykustodatabase
```

上述命令會刪除在資源群組 "testrg" 中找到的群集 "mykustocluster" 中名為 "mykustodatabase" 的 Kusto 資料庫。

### 範例 2-透過管道刪除現有的 Kusto 資料庫

```
PS C:\> Get-AzKustoDatabase -ResourceGroupName testrg -ClusterName mykustocluster -Name mykustodatabase | Remove-AzKustoDatabase
```

上述命令會在使用 Cmdlet 之資源群組 "testrg" 中找到的群集 "mykustocluster" 中，取得名為 "mykustodatabase" 的 Kusto 資料庫 `Get-AzKustoDatabase` ，然後再將結果設為 [ `Remove-AzKustoDatabase` 刪除]。
