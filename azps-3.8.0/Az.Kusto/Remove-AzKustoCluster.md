...
### 範例 1-依名稱刪除現有的 Kusto 群集

```
PS C:\> Remove-AzKustoCluster -ResourceGroupName testrg -Name mykustocluster
```

上述命令會刪除資源群組 "testrg" 中名為 "mykustocluster" 的 Kusto 群集。

### 範例 2-依管道刪除現有的 Kusto 群集

```
PS C:\> Get-AzKustoCluster -ResourceGroupName testrg -Name mykustocluster | Remove-AzKustoCluster
```

上述命令會在資源群組 "testrg" 中使用 Cmdlet 來取得名為 "mykustocluster" 的 Kusto 群集，然後將 `Get-AzKustoCluster` 結果設為 `Remove-AzKustoCluster` 刪除。
