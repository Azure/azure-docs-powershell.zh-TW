...
## 說明
**更新-AzVmss** Cmdlet 會更新虛擬機器縮放集 (VMSS) 的狀態設定為局部 VMSS 物件的狀態。
...
因 VM 上狀態變更而暫停自動修復的時間長度。 當狀態變更完成後，寬限期即會開始。 這有助於避免過早或意外的修復。 應以 ISO 8601 格式指定持續時間。 預設值為5分鐘 (PT5M) 。
...
將同時修復之 scaleset) 的虛擬機器容量 (產能的百分比。 預設值為20%。
