...
## 說明
**AzVmss** Cmdlet 會取得虛擬機器縮放集 (VMSS) 的模型和實例視圖。
模型視圖是使用者指定的虛擬機器規模設定屬性。
[實例] 視圖是虛擬機器規模集的實例層級狀態。
指定 *InstanceView* 參數，只取得虛擬機器縮放集的實例視圖。
...
由於命令不會指定 *InstanceView* 切換參數，因此 Cmdlet 會取得虛擬機器縮放集的模型視圖。
...
### -OSUpgradeHistory
表示此 Cmdlet 列出虛擬機器規模集的 os 升級歷程記錄。
