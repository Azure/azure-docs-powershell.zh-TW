...
### 範例15：指定個別客戶連絡人參數（而非 CustomerContactDetail 物件），以建立支援票證。 
```powershell
...
### 範例16：建立支援票證，並要求來自 Azure 的 24 x 7 回應。 
```powershell
...
### 範例17：如果您是雲端解決方案提供者 (CSP) ，請代表您的客戶建立支援票證。 CSP 應該先登入其租使用者，然後登入客戶的租使用者，如下列範例所示。 然後，必須使用-CSPHomeTenantId 參數來指定其在建立支援票證時的主租使用者識別碼。  
```powershell
