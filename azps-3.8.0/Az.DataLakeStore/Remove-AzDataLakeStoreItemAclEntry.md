...
## 說明
**AzDataLakeStoreItemAclEntry** Cmdlet 會從資料 Lake Store 中檔案或資料夾的存取控制清單 (ACL) ，移除 (ACE) 中的專案。
...
這個命令會從 root 中移除 Patti 的使用者 ACE，並以遞迴方式從它的所有子目錄和檔案中，將其從帳戶 ContosoADL。
...
### -ShowProgress
如果經過，就會顯示 [進度] 狀態。 只有在已完成遞迴 Acl 移除時才適用。
