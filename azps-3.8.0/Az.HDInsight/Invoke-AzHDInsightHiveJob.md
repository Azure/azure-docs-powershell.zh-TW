...
## 說明
**AzHDInsightHiveJob** Cmdlet 會將 Hive 查詢提交至 Azure HDInsight 群集，並在一個操作中檢索查詢結果。
在呼叫 **AzHDInsightHiveJob** 前，請先使用 Use-AzHDInsightCluster Cmdlet，以指定將用於查詢的群集。
...
### -JobName
指定 Hive 工作的名稱。
如果您沒有指定此參數，這個 Cmdlet 會使用預設值：「Hive： \< 第一個100字元的查詢」 \> 。
