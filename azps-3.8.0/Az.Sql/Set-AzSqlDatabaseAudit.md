...
## 說明
**AzSqlDatabaseAudit** Cmdlet 會變更 Azure SQL 資料庫的審核設定。
...
### 範例3.1：使用 T-sql 謂語，透過高級篩選啟用 Azure SQL 資料庫的 blob 儲存審核原則
```
...
### 範例3.2：從 Azure SQL 資料庫的審核原則中移除 [高級篩選] 設定
```
...
### 範例6：啟用 Azure SQL 資料庫的 log analytics 審核原則
```
...
### 範例7：停用 Azure SQL 資料庫的 log analytics 審核原則
```
...
### 範例8：透過管線停用 Azure SQL 資料庫的 log analytics 審核原則
```
...
### 範例9：停用將 Azure SQL 資料庫的審核記錄傳送到 blob 儲存空間，並允許將其傳送至記錄分析。
```
...
### 範例10：啟用將 Azure SQL 資料庫的審核記錄傳送到 blob 儲存體、事件中樞和記錄分析。
```
...
接收  
提到  
定義要審核之動作的一般形式是： [[物件] 上的 [動作]，[主體] 請注意，上述格式中的 [物件] 可以參照表格、視圖或儲存程式，或整個資料庫或架構等物件。 在後一個情況下，系統會分別使用表單資料庫：： [dbname] 和架構：： [schemaname]。
例如：  
依公開選取 dbo. myTable  
