...
### GetVirtualClusterByResourceGroup (預設) 
```
...
## 說明
**AzSqlVirtualCluster** Cmdlet 會傳回一或多個 Azure SQL 虛擬叢集的相關資訊。
指定虛擬叢集的名稱，只查看該群集的資訊。

## 示例

### 範例1：取得指派給資源群組的所有虛擬叢集
```powershell
...
這個命令會取得指派給資源群組 ResourceGroup01 的所有虛擬叢集的相關資訊。
...
這個命令會在資源群組 ResourceGroup01 中取得虛擬叢集 VirtualCluster1 的相關資訊。
