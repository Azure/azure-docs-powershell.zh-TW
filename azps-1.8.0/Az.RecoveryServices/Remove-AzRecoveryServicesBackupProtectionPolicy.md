...
## 說明
**AzRecoveryServicesBackupProtectionPolicy** Cmdlet 會刪除電子倉庫的備份原則。
在您可以刪除備份保護原則之前，原則不能有任何相關聯的備份專案。
在您刪除原則之前，請確認每個相關專案都與其他原則相關聯。
若要將另一個原則與備份專案建立關聯，請使用 Enable-AzRecoveryServicesBackupProtection Cmdlet。
...
### -原則
指定要移除的備份保護原則。
若要取得 **BackupPolicy** 物件，請使用 Get-AzRecoveryServicesBackupProtectionPolicy Cmdlet。
