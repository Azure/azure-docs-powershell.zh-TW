...
## 說明
**AzIntegrationAccountBatchConfiguration** Cmdlet 會在整合帳戶中建立新的批次配置。
...
使用位於 [$batchConfigurationFilePath] 中所含檔案路徑的本機檔案建立新的批次配置。

### 範例2：使用 JSON 字串建立新的批次配置
```powershell
...
使用包含在「$batchConfigurationContent」中的 JSON 字串來建立新的批次設定。
