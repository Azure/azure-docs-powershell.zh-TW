...
### 範例 1-依名稱暫停現有的 Kusto 群集

```
PS C:\> Suspend-AzKustoCluster -ResourceGroupName testrg -Name mykustocluster
```

上述命令會暫停在資源群組 "testrg" 中找到名為 "mykustocluster" 的 Kusto 群集。

### 範例 2-透過管道暫停現有的 Kusto 群集

```
PS C:\> Get-AzKustoCluster -ResourceGroupName testrg -Name mykustocluster | Suspend-AzKustoCluster
```

上述命令會在使用 Cmdlet 的資源群組 "testrg" 中，找到名為 "mykustocluster" 的 Kusto 群集，然後將 `Get-AzKustoCluster` 結果設為 `Suspend-AzKustoCluster` 暫停狀態。
