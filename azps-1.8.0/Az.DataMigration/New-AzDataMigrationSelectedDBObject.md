...
## 說明
New-AzDataMigrationSelectedDB Cmdlet 會建立一個資料庫資訊物件，其中包含有關來源和目標資料庫的資訊，以及用於遷移的資料表對應資訊。 這個 Cmdlet 可以用來做為 New-AzDataMigrationTask Cmdlet 的參數。
...
### -BackupFileShare
檔案共用：此資料庫的源伺服器資料庫檔案應該備份。
使用此設定來覆寫每個資料庫的檔案共用資訊。
針對伺服器使用完整的功能變數名稱。
...
將遷移類型設定為 SQL Server 以進行 SQL 資料庫遷移。
...
### -MigrateSqlServerSqlDbMi
將遷移類型設定為 SQL Server 以進行 SQL 資料庫 MI 遷移。
