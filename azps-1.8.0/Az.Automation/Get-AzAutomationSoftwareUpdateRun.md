...
## 說明
Get-AzAutomationSoftwareUpdateRun Cmdlet 會傳回軟體更新執行的清單。 因為軟體更新設定有關聯的排程，所以可以多次觸發。 每次觸發排程時，都會產生已建立的更新執行。 [更新執行] 是所有電腦執行結果的匯總。 您可以傳送 SoftwareUpdateConfigurationName 參數，以取得特定軟體更新設定的執行。 若要取得特定的執行，您必須傳遞 name 參數。 您也可以使用特定狀態列出執行，並以特定的 operatins 系統為目標，或傳遞適當的參數，在特定時間之後執行。
