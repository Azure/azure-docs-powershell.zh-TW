...
## 摘要
警告：取消註冊伺服器會導致層疊刪除此伺服器上的所有伺服器端點。 這個命令會登出伺服器的儲存同步處理服務。
...
## 說明
這個命令會從儲存同步處理服務登出伺服器。 警告：取消註冊伺服器會導致層疊刪除此伺服器上的所有伺服器端點。 只有在您確定此伺服器上的任何路徑都要再同步處理時，才應呼叫它。
...
這個命令會登出伺服器，導致此伺服器上所有伺服器端點的串聯刪除。
...
### -InputObject
RegisteredServer 輸入物件，通常是透過管線傳遞。
