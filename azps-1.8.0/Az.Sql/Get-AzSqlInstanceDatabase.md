...
### GetInstanceDatabaseFromInputParameters (預設) 
```
...
## 說明
**AzSqlInstanceDatabase** Cmdlet 會從 Azure Sql Database Managed 實例取得一個或多個 azure sql 資料庫。
...
這個命令會取得實例名為 managedInstance1 的所有資料庫。

### 範例2：依名稱在受管理的實例上取得資料庫
```
...
這個命令會從名為 managedInstance1 的實例取得名為 managedDatabase1 的資料庫。
...
這個命令會取得名為 managedInstance1 的實例上以 "managedDatabase" 為開頭的所有資料庫。
