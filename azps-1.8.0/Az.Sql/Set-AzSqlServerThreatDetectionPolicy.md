...
## 描述
**Set-AzSqlServerThreatDetectionPolicy** Cmdlet 會設定 Azure SQL 伺服器上的威脅偵測策略。
若要在伺服器上啟用威脅偵測，必須在伺服器上啟用稽核策略。
若要使用此 Cmdlet，請指定 *ResourceGroupName* 和 ServerName 參數來識別伺服器。
...
### -StorageAccountName
指定要使用的儲存空間帳戶名稱。 不允許使用萬用字元。 此參數不為必填專案。 未提供此參數時，Cmdlet 會使用先前定義為資料庫威脅偵測策略一部分的儲存空間帳戶。 如果這是資料庫第一次定義at 偵測策略，而且未提供此參數，Cmdlet 將會失敗。
