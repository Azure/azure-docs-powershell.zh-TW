...
## 說明
**AzLoadBalancerRuleConfig** Cmdlet 會將規則配置新增至 Azure 負載平衡器。
...
第二個命令使用管線運算子，將負載平衡器 $slb 至 **Add-AzLoadBalancerRuleConfig** ，這會新增名為 NewRule 的規則配置。
第三個命令會使用新的負載平衡器規則 Config 來更新 azure 中的負載平衡器。

## 參數

### -BackendAddressPool
指定要與負載平衡器規則配置相關聯的後端位址集區。
...
### -LoadBalancer
指定 **LoadBalancer** 物件。
這個 Cmdlet 會將規則配置新增到此參數指定的負載平衡器。
