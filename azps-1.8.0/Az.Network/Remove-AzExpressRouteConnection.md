...
上述將會在 Azure 中的 [testRG] 資源群組中，建立一個資源群組、虛擬 WAN、虛擬網路、虛擬中樞。 在具有2個比例單位的虛擬中樞中，將會建立 ExpressRoute 閘道。

建立閘道之後，就會使用 [New-AzExpressRouteConnection] 命令將它連線至 ExpressRouteSite。
...
與範例1相同，但現在它會從 AzExpressRouteConnection 中移除使用管道物件的連接。
