...
## 簡介
建立新的網路組配置診斷設定檔物件。 此物件會使用指定的準則，在診斷會話期間限制網路問題。
...
## 描述
Cmdlet New-AzNetworkWatcherNetworkConfigurationDiagnosticProfile會建立一個新的診斷設定檔物件。 此物件用於使用指定準則在網路組配置診斷會話期間限制網路問題。
