...
### [AzInterfaceEndpoint](Get-AzInterfaceEndpoint.md)
Get-AzInterfaceEndpoint Cmdlet 會取得介面端點。
...
建立新的網路設定診斷設定檔物件。 這個物件是用來在診斷會話期間使用指定準則來限制網路 confiuration。
...
這個命令可讓使用者建立 Vpn ipsec 原則物件，指定一個或所有值，例如 IpsecEncryption、IpsecIntegrity、IkeEncryption、IkeIntegrity、DhGroup、PfsGroup，以在 VPN 閘道上設定。 這個命令 let 輸出物件是用來為新的/exisitng 閘道設定 vpn ipsec 原則。
