...
## 說明
**AzNetworkInterfaceIPConfig** Cmdlet 會從 Azure 網路介面取得網路介面 IP 配置。
...
第一個命令會取得名為 mynic 的現有網路介面，並將它儲存在 $nic 1 的變數中。 第二個命令會取得此網路介面的名為 ipconfig1 的 IP 配置。
...
### -NetworkInterface
指定包含此 Cmdlet 所取得之網路 IP 配置的 **NetworkInterface** 物件。
