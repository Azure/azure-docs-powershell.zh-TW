...
## 說明
啟用 Az 模組的 AzureRm 首碼別名。 如果指定了-Module，則只有列出的模組才會啟用別名。 否則，所有的 AzureRm 別名都是啟用的。
...
啟用目前 PowerShell 會話的所有 AzureRm 首碼。
...
針對目前的程式和目前的使用者，啟用 Az 的 AzureRm 別名。
