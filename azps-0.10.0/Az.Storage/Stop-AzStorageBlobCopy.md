...
## 說明
**AzStorageBlobCopy** Cmdlet 會停止複製操作至指定的目的地 blob。
...
這個命令會從 **AzStorageContainer** 傳送管線上的容器，以停止複製操作。

### 範例3：使用管線和 Get-AzStorageBlob 停止複製操作
```
PS C:\>Get-AzStorageBlob -Container "ContainerName" | Stop-AzStorageBlobCopy -Force
```

這個範例會從 Get-AzStorageBlob Cmdlet 傳送管線上的容器，以停止複製操作。
...
您可以建立物件或使用 Get-AzStorageContainer Cmdlet。
...
### -內容
指定 Azure 儲存區內容。
您可以使用 New-AzStorageContext Cmdlet 來建立內容。
