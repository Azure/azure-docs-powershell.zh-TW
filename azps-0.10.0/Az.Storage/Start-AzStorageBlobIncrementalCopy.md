...
### -CloudBlob
從 Azure 存儲用戶端文件庫 CloudBlob 物件。 您可以建立它或使用 Get-AzStorageBlob Cmdlet。
...
### -CloudBlobContainer
從 Azure 存儲用戶端文件庫 CloudBlobContainer 物件。 您可以建立它或使用 Get-AzStorageContainer Cmdlet。
...
### -內容
來源 Azure 儲存區內容。 您可以 New-AzStorageContext Cmdlet 來建立它。
...
### -DestCoNtext
[目的地 Azure 儲存區] 內容。 您可以 New-AzStorageContext Cmdlet 來建立它。
