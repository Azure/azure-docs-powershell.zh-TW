...
## 說明
AzVirtualNetworkPeering Cmdlet 會 **設定** 虛擬網路對等。
...
透過將此屬性變更為 $True，就可以使用對等的 VNet 閘道。
不過，對等 VNet 必須已設定閘道，且 **AllowGatewayTransit** 必須具有 $True 的值。
...
形參 "VirtualNetworkPeering" 接受管線中 "PSVirtualNetworkPeering" 類型的值
