...
### -DomainNameLabel
此比例集之公用 Fully-Qualified 功能變數名稱的功能變數名稱標籤 (FQDN) 。 這是功能變數名稱的第一個元件，會自動 assiged 到縮放集。 自動指派的功能變數名稱使用 [表單 (] <DomainNameLabel> 。 <Location>cloudapp.azure.com [) ]。 如果未提供任何值，預設的網功能變數名稱稱標籤將會是 and 的串聯 <ScaleSetName> <ResourceGroupName> 。
...
### -FrontendPoolName
要 usein 比例集 locad 平衡器的前端位址集區名稱。  如果未提供任何值，則會建立新的前端位址集區，其名稱與縮放集相同。
