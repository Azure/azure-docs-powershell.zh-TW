...
## 說明
**AzDnsRecordConfig** Cmdlet 會將網域名稱系統 (DNS) 記錄新增至 **RecordSet** 物件。
...
因為 CNAME 記錄集最多可以包含一筆記錄，所以它必須是空記錄集，或必須使用 AzDnsRecordConfig 移除現有的記錄。
...
這是 **記錄集** 的離線表示形式，在您執行 Set-AzDnsRecordSet Cmdlet 之後，才能變更 DNS 回應。
