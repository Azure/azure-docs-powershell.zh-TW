...
## 說明
將 web 應用程式快照還原至 web 應用程式。 還原快照時，會使用快照中包含的檔案，覆寫 web app 中的所有檔案。 若要同時還原設定，請使用 RecoverConfiguration 切換參數。 您可以將來自單一 web 應用程式的快照還原至相同訂閱中的任何其他 web 應用程式。
...
取得名為「ContosoApp」的 web 應用程式的最新快照，其中包含「Default-Web-WestUS」資源群組中名為「暫存」的槽。 將快照還原至 web 應用程式的「還原」槽。
...
BackupRestore AzureWebAppSnapshot System. WebApps..............。
