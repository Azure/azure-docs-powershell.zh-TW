...
**新-AzureHDInsightPigJobDefinition** 會定義 Azure HDInsight 服務的豬作業。
...
第一個命令會宣告字串值，然後將其儲存在 $0 變數中。

第二個命令會建立豬工作查詢，然後將它儲存在 $QueryString 變數中。

最後一個命令會建立在 $QueryString 中使用查詢的豬作業定義，然後將作業定義儲存在 $PigJobDefinition 變數中。
