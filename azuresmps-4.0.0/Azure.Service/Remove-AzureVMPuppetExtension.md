...
## 說明
**AzureVMPuppetExtension** Cmdlet 會移除在虛擬機器上套用的 Puppet 延伸。

## 示例

### 範例1：移除在虛擬機器上套用的 Puppet 延伸
```
PS C:\> Remove-AzureVMPuppetExtension -VM $VM;
```

這個命令會移除在指定的虛擬機器上套用的 Puppet 延伸。
...
[AzureVMPuppetExtension](./Get-AzureVMPuppetExtension.md)
