...
## 摘要
取得雲端服務中部署之所有虛擬機器或服務中特定虛擬機器的 DSC 副檔名狀態。

## 句法

### GetStatusByServiceAndVMName (預設) 
```
...
## 說明
AzureVMDscExtensionStatus Cmdlet 會針對部署在雲端服務中的所有虛擬機器或服務中的特定虛擬機器， **取得** 所需的狀態設定 (DSC) 副檔名狀態。
...
### VirtualMachineDscExtensionStatusCoNtext WindowsAzure. ServiceManagement。
