...
## 說明
**Start-AzureSiteRecoveryTestFailoverJob** Cmdlet 會開始測試 Azure Site Recovery 保護實體或復原方案的容錯移轉。
您可以使用 **AzureRMSiteRecoveryJob** Cmdlet 來檢查作業是否已成功完成。
...
第一個命令使用 **AzureSiteRecoveryProtectionContainer** Cmdlet 來取得受保護的容器，然後將它儲存在 $ProtectionContainer 變數中。
...
最終命令會針對儲存在 $ProtectionEntity 中的受保護實體啟動測試容錯移轉作業，並指定容錯移轉的方向。
...
這個命令會透過 **AzureSiteRecoveryRecoveryPlan** Cmdlet，為目前的 Azure Site recovery 保存庫取得名為 RecoveryPlan01 的復原方案。
該命令會將計畫儲存在 $RecoveryPlan 變數中。

第二個命令會針對儲存在 $RecoveryPlan 的復原方案啟動測試容錯移轉作業，並指定容錯移轉的方向。
...
### -網路
指定要用於測試容錯移轉的網路物件。
若要取得網路，請使用 **AzureSiteRecoveryNetwork** Cmdlet。
...
[AzureSiteRecoveryNetwork](./Get-AzureSiteRecoveryNetwork.md)
...
[開始-AzureSiteRecoveryUnplannedFailoverJob](./Start-AzureSiteRecoveryUnplannedFailoverJob.md)
