...
## 說明
**新的-AzureInternalLoadBalancerConfig** Cmdlet 會建立 **InternalLoadBalancerConfig** 物件。
當您建立 Azure 虛擬機器部署時，您可以使用內部負載平衡器配置。
...
這個命令會為名為 FrontEndSubnet 的子網建立內部負載平衡器設定。
該命令會將設定儲存在 $IlbConfig 變數中，供您用來進行虛擬機器部署。
...
### -StaticVNetIPAddress
指定此 Cmdlet 包含在配置中的內部負載等化器虛擬網路 IP 位址。
...
### WindowsAzure. ServiceManagement. InternalLoadBalancerConfig
