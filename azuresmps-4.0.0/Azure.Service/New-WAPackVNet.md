...
**新的-WAPackVNet** Cmdlet 會建立虛擬化的網路。
...
第一個命令首先會檢索要新增新虛擬化網路的邏輯網路。
這個邏輯網路名為 ContosoLogicalNetwork01。

第二個和 [上一個] 命令會使用先前檢索的邏輯網路建立虛擬化網路，名稱 (ContosoVNett01) ，以及 (描述) 描述。
...
[移除-WAPackVNet](./Remove-WAPackVNet.md)
