...
**AzureAutomationRunbookDefinition** Cmdlet 會更新 Microsoft Azure 自動化 runbook 的草稿定義。
指定 Windows PowerShell 腳本 ( ps1) 檔案，該檔案包含成為草稿版 runbook 的 runbook。

如果草稿定義已存在，請使用 *Overwrite* 參數強制 Cmdlet 覆寫現有的草稿。

## 示例

### 範例1：覆寫現有的 runbook 草稿定義
```
PS C:\> Set-AzureAutomationRunbookDefinition -AutomationAccountName "Contoso17" -Name "Runbk01" -Path ".\App01.ps1" -Overwrite
```

這個命令會覆寫 runbook 的現有草稿定義。
...
### RunbookDefinition 中的 [.]

## 筆記

## 相關連結

[AzureAutomationRunbookDefinition](./Get-AzureAutomationRunbookDefinition.md)
