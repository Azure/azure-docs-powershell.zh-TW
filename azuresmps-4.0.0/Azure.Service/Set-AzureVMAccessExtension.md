...
## 說明
**AzureVMAccessExtension** Cmdlet 會將虛擬機器 VMAccess 延伸新增至虛擬機器。 VMAccess 延伸可以用來設定暫時的密碼，而且應該在登入電腦後立即變更它。

## 示例

### 範例1：設定套用至指定虛擬機器的 VMAccess 延伸
```
PS C:\> Set-AzureVMAccessExtension -VM $VM -UserName $User -Password $PWD;
```

這個命令會將 VMAccess 延伸套用至指定的虛擬機器，並儲存在 $VM 變數中。

## 參數

### -停用
表示此 Cmdlet 會將延伸狀態設定為 [停用]。
...
### -ForceUpdate
表示當設定尚未更新時，此 Cmdlet 會將配置重新應用到延伸。
