...
## 說明
**AzureVMDiagnosticsExtension** Cmdlet 會從虛擬機器中移除 Microsoft Azure 診斷擴展外掛程式。
您必須將這個 Cmdlet 的輸出傳遞給 **更新-add-azurevm** Cmdlet。

## 示例

### 範例1：從虛擬機器移除 Azure 診斷延伸
```
PS C:\> Remove-AzureVMDiagnosticsExtension -VM $VM | Update-AzureVM
```

此命令會從虛擬機器中移除 Azure 診斷延伸。
