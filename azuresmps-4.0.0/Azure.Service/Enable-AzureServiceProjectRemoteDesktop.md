...
**Enable-AzureServiceProjectRemoteDesktop** Cmdlet 可讓遠端桌面存取雲端服務。
您必須在啟用遠端桌面存取後使用 **Publish AzureServiceProject** Cmdlet 發佈服務，變更才會生效。
...
### -Username
指定透過遠端桌面通訊協定連線到 Azure 中的角色實例時要使用的使用者名稱 (RDP) 。
