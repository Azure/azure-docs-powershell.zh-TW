...
**AzureHDInsightProperties** Cmdlet 會取得 Azure HDInsight 服務專用的屬性，例如可用的 Azure 區域清單、HDInsight 群集版本，以及可用的計算能力。

## 示例

### 範例1：取得訂閱的 HDInsight 屬性
```
PS C:\>$SubName = Get-AzureSubscription -Current | %{ $_.SubscriptionName }
PS C:\> Get-AzureHDInsightProperties -Subscription $SubName
```

第一個命令會取得目前 Azure 訂閱的名稱，然後將其儲存在 $SubName 變數中。

第二個命令會在 $SubName 中取得訂閱的 HDInsight 屬性。
...
### -位置
表示此 Cmdlet 會取得 HDInsight 服務可用的 Azure 區域清單。
...
### -版本
表示此 Cmdlet 會取得訂閱服務中可用的 HDInsight 群集版本清單。
