...
## 說明
**AzureEndpoint** Cmdlet 會取得指派給 Azure 虛擬機器之端點的相關資訊。
...
這個命令會使用 **add-azurevm** Cmdlet 來檢索名為 VirtualMachine12 的虛擬機器的設定。
命令會使用管線運算子將它傳遞到目前的 Cmdlet。
這個 Cmdlet 會取得該虛擬機器的端點資訊。
