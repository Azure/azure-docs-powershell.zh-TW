...
## 說明
**AzPrivateDnsVirtualNetworkLink** Cmdlet 會從指定的資源群組取得與特定專用 DNS 區域相關聯的虛擬網路連結。
如果您指定 *Name* 參數，則會傳回單一 **PSPrivateDnsVirtualNetworkLink** 物件。
如果您沒有指定 *Name* 參數，則會傳回包含與指定資源群組中之區域相關聯之所有連結的陣列。
您可以使用 **PSPrivateDnsVirtualNetworkLink** 物件來更新連結。
...
這個範例會取得與指定資源群組中名為 myzone.com 的專用 DNS 區域相關聯的虛擬網路連結 mylink，然後將其儲存在 $Link 變數中。

### 範例2：取得與資源群組中的某個區域相關的所有連結。
```
...
這個範例會取得與指定資源群組中的專用 DNS 區域「myzone.com」相關聯的所有虛擬網路連結，然後將其儲存在 $Links 變數中。
...
### -名稱
指定要取得的虛擬網路連結的名稱。
如果您沒有指定 *Name* 參數的值，這個 Cmdlet 會取得與指定資源群組中指定的專用 DNS 區域相關聯的所有連結。
