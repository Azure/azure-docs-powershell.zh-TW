...
## 說明
**AzDataCollectionRuleAssociation** Cmdlet 會取得一或多個資料收集規則關聯 (DCRA) 。
...
### 範例1：根據目標資源識別碼取得資料收集規則關聯 (關聯的虛擬機器) 
```
...
這個命令會列出指定目標資源識別碼的所有資料收集規則， (虛擬機器) 。

### 範例2：透過規則 (DCR) 取得資料收集規則關聯
```
...
這個命令會列出指定資源群組和規則 (DCR) 的資料收集規則關聯。

### 範例3：透過 input 物件 (PSDataCollectionRuleResource) 取得資料收集規則關聯
```
...
這個命令會列出指定輸入物件的資料收集規則關聯。

### 範例4：根據目標資源識別碼取得資料收集規則關聯 (關聯的虛擬機器) 與關聯名稱
```
...
這個命令會列出一個 (清單，其中只有一個元素) 資料收集規則關聯。
...
[移除-AzDataCollectionRuleAssociation](./Remove-AzDataCollectionRuleAssociation.md) 
[新-AzDataCollectionRuleAssociation](./New-AzDataCollectionRuleAssociation.md)
