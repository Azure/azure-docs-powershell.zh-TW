...
若要將 DCR 適用于虛擬機器，請為虛擬機器建立關聯。 虛擬機器可能與多個 DCRs 相關聯，而 DCR 可能有多個與它相關聯的虛擬機器。 這可讓您定義一組符合特定要求的 DCRs，並只將它們套用至其套用之虛擬機器。 以下是使用 DCRA [文章的](https://docs.microsoft.com/en-us/azure/azure-monitor/platform/data-collection-rule-azure-monitor-agent) 「設定 Azure 監視器代理程式的資料集合」。
