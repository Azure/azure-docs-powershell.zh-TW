...
## 說明
**AzSqlServer** Cmdlet 會修改 Azure SQL 資料庫伺服器的屬性。
...
這個命令會在名為 server01 的 AzureSQL 伺服器上重設系統管理員密碼。

### 範例2

修改 SQL 資料庫伺服器的屬性。  (自動生成) 
...
### -PublicNetworkAccess
接受標誌、啟用/停用，以指定是否允許公用網路存取伺服器。
停用時，只有透過私人連結建立的連線才能到達此伺服器。
...
### -ServerVersion
指定此 Cmdlet 變更伺服器的版本。 此參數可接受的值為：2.0 和12.0。
...
### -SqlAdministratorPassword
針對資料庫伺服器管理員，指定新的密碼（作為 **SecureString**）。 若要取得 **SecureString**，請使用 Get-Credential Cmdlet。 如需詳細資訊，請輸入 `Get-Help
...
### -標記
指定此 Cmdlet 與伺服器關聯的標記字典。 在伺服器上將雜湊表的格式設定為標記的索引鍵/值對。 例如： @ {key0 = "value0"; key1 = $null; key2 = "value2"}
