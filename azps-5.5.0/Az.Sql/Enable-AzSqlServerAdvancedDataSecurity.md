...
## 說明
**Enable-AzSqlServerAdvancedDataSecurity** Cmdlet 可在伺服器上啟用高級資料安全性。 [高級資料安全性] 是一個整合的安全性套件，其中包括您伺服器的資料分類、漏洞評估及高級威脅防護。  (會自動建立新的儲存空間帳戶來儲存漏洞評量。 如果先前已針對此目的建立儲存空間帳戶，則會改為使用它) 
