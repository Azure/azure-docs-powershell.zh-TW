...
## 說明
**AzSqlServerMSSupportAudit** Cmdlet 會取得 Azure SQL Server 的 Microsoft 支援作業審核設定。
指定 *ResourceGroupName* 和 *ServerName* 參數來識別伺服器。

## 示例

### 範例1：取得 Azure SQL server 的 Microsoft 支援作業審核設定
```
...
### 範例2：透過管線取得 Azure SQL server 的 Microsoft 支援作業審核設定
```
...
### 範例3：取得 Azure SQL server 的 Microsoft 支援作業審核設定
```
PS C:\>Get-AzSqlServerMSSupportAudit -ResourceGroupName "resourcegroup01" -ServerName "server01"
ServerName                          : server01
ResourceGroupName                   : resourcegroup01
