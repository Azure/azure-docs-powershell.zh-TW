...
### DefaultSetInstancePoolParameterSet (預設) 
```
...
## 說明
**AzSqlInstancePool** Cmdlet 會修改 Azure SQL 實例池的屬性。
...
這個命令會設定名為 instancePool0 的實例池的授權類型和/或標記。

### 範例2：使用實例池物件設定實例池授權類型
```powershell
...
這個命令會使用實例池物件來設定實例池的授權類型和/或標記。

### 範例3：使用實例池資源識別碼設定實例池授權類型
```powershell
...
### -LicenseType
決定要使用哪一種授權類型。
可能的值為 BasePrice (，AHB 折扣) 和 LicenseIncluded (沒有 AHB 的折扣) 。
