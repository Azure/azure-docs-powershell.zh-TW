...
## 說明
Enable-AzSqlInstanceDatabaseSensitivityRecommendation Cmdlet 會針對 Azure SQL managed 實例資料庫中的欄啟用敏感度建議。

## 示例

### 範例1：針對 Azure SQL managed 實例資料庫中的特定欄啟用敏感度建議。
```powershell
...
### 範例2：使用管道針對 Azure SQL managed 實例資料庫中的特定欄啟用敏感度建議。
```powershell
