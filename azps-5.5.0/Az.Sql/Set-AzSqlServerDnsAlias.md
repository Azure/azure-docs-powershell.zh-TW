...
## 說明
這個命令正在更新別名指向哪個伺服器。 此命令必須在登入要指向哪個別名所要指向的新伺服器時發出訂閱。
...
這個命令正在更新，且先前指向 oldServer 的別名指向 server newServer
