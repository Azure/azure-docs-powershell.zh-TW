...
## 說明
**AzSqlInstance** Cmdlet 會修改 Azure SQL Database Managed 實例的屬性。

## 示例

### 範例1：使用新的值（AdministratorPassword、-LicenseType、-StorageSizeInGB、-VCore 及-Edition）來設定現有實例
```powershell
...
### 範例2：使用 ComputeGeneration 的新值變更現有的實例硬體產生
```powershell
...
### 範例3：使用 AdministratorPassword、-LicenseType、-StorageSizeInGB 和-VCore 的新值設定現有的實例，在實例池中
```powershell
PS C:\>$InstancePassword = "Newpassword1234"
PS C:\> $SecureString = ConvertTo-SecureString $InstancePassword -AsPlainText -Force
...
這個命令會使用 AdministratorPassword、-LicenseType、-StorageSizeInGB 和-VCore 的新值，為實例池中的實例設定現有的實例
