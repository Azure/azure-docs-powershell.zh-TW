...
### 範例10：停用將 Azure SQL server 的審核記錄傳送到 blob 儲存空間，並允許將其傳送至記錄分析。
```powershell
...
### 範例11：啟用將 Azure SQL server 的審核記錄傳送到 blob 儲存空間、事件中樞和記錄分析。
```powershell
