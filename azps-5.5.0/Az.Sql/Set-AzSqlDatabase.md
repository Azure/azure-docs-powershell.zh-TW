...
## 說明
**AzSqlDatabase** Cmdlet 會在 Azure SQL database 中設定資料庫的屬性。 這個 Cmdlet 可以修改服務層級 (*Edition*) 、效能層 (*RequestedServiceObjectiveName*) ，以及資料庫的 (*MaxSizeBytes*) 的儲存空間大小上限。  此外，您也可以指定 *ElasticPoolName* 參數，將資料庫移至彈性池。 如果資料庫已在彈性池中，您可以使用 *RequestedServiceObjectiveName* 參數，將資料庫從彈性池移出，然後轉化為單一資料庫的效能等級。

## 示例

### 範例1：將資料庫更新為標準 S0 資料庫
```
...
這個命令會將一個名為 Database01 的資料庫，更新為伺服器上名為 Server01 的標準 S0 資料庫。
...
### 範例4：更新現有的一般用途資料庫以 Hyperscale 服務層級
```
...
這個命令會將名為 Database01 的資料庫從一般用途更新為 Hyperscale 服務層級。
...
與資料庫相關聯的唯讀次要副本數目。  僅適用于 Hyperscale 版本。
...
### -MinimumCapacity
資料庫始終已配置的最小產能（如果沒有暫停的話）。
僅限無伺服器的 Azure Sql 資料庫。
...
### -ReadScale
如果已啟用，則在其連線字串中將應用程式意向設定為 readonly 的連線，可能會路由到 readonly 次要複本。 這個屬性僅適用于 Premium 和商務重要的資料庫。
...
### -RequestedServiceObjectiveName
指定要指派給資料庫之服務目標的名稱。 如需服務目標的相關資訊，請參閱 Microsoft 開發人員網路文件庫中的 [AZURE SQL 資料庫服務層級和效能層級](https://docs.microsoft.com/en-us/azure/sql-database/sql-database-dtu-resource-limits-single-databases) 。
...
### -SecondaryType
如果資料庫是副，則為資料庫的次要類型。  有效值為 Geo 且命名。
