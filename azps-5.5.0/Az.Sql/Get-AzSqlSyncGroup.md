...
## 說明
**AzSqlSyncGroup** Cmdlet 會傳回一或多個 Azure SQL 資料庫同步處理群組的相關資訊。
指定同步群組的名稱，只查看該同步群組的資訊。

## 示例

### 範例1：取得指派給 Azure SQL 資料庫之 Azure SQL 同步處理群組的所有實例
```
...
這個命令會取得指派給 Azure SQL 資料庫的所有 Azure SQL 資料庫同步處理群組的相關資訊。

### 範例2：取得 Azure SQL 資料庫同步處理群組的相關資訊
```
...
此命令會取得名為 "SyncGroup01" 的 Azure SQL 資料庫同步處理群組的相關資訊

### 範例3：使用篩選來取得指派給 Azure SQL Database 之 Azure SQL 同步處理群組的所有實例
```
...
此命令會取得指派給以 "SyncGroup" 開頭的 Azure SQL 資料庫的所有 Azure SQL 資料庫同步處理群組的相關資訊。
