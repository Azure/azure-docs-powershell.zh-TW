...
## 說明
Get-AzSqlDatabaseSensitivityClassification Cmdlet 會傳回 Azure SQL 資料庫中資料行的目前資訊類型和敏感度標籤。

## 示例

### 範例1：取得 Azure SQL 資料庫的目前資訊類型及敏感度標籤。
```powershell
...
### 範例2：使用管道取得 Azure SQL 資料庫的目前資訊類型和敏感度標籤。
```powershell
...
### 範例3：取得 Azure SQL 資料庫特定資料行的目前資訊類型與敏感度標籤。
```powershell
...
### 範例4：使用管道取得 Azure SQL Database 之特定資料行的目前資訊類型與敏感度標籤。
```powershell
