...
這個命令會在現有的虛擬網路中建立一個專用的 HSM。

**注意：** 在 `New-AzDedicatedHsm` Azure 上完全預配 HSM 之前，目前有一個限制會傳回它。
因此，在建立新的 HSM 之後，請查詢其狀態， `Get-AzDedicatedHsm` 並確認它在 `Provisioning State` `Succeeded` 使用之前。
...
若要建立，請參閱 NETWORKINTERFACE 屬性和建立雜湊資料表的備忘稿區段。
...
### -SubnetId
/Subscriptions/{SubscriptionId}/resourceGroups/{ResourceGroupName}/... 形式的 ARM 資源識別碼
...
### IDedicatedHsm （DedicatedHsm）。 Api20181031。
...
NETWORKINTERFACE <INetworkInterface [] >：指定與專用 HSM 相關聯之網路介面的資源識別碼清單。
  - `[PrivateIPAddress <String>]`：介面的專用 Ip 位址
