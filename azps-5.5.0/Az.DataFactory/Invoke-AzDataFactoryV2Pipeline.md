...
### -IsRecovery
[復原模式] 旗標。 如果將 [復原模式] 設定為 true，則會執行指定的參照管線，而新的執行將會群組為相同的 groupId。
...
要重新執行的管線執行 ID。 如果指定了 [執行識別碼]，則會使用指定執行的參數來建立新的執行。
...
### -StartActivityName
在 [恢復] 模式中，[重新執行] 就會從這個活動開始。 如果未指定，所有活動將會執行。
...
### -StartFromFailure
從失敗的活動標誌開始重新執行。 如果已指定，則在 [恢復] 模式中，[重新執行] 會從失敗的活動開始。
