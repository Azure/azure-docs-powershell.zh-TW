...
過時. 若要使用自訂的 MSI 端點，請設定環境變數 MSI_ENDPOINT （例如 " http://localhost:50342/oauth2/token "）。 受管理服務的主機名稱。
...
過時. 若要使用自訂的 MSI 端點，請設定環境變數 MSI_ENDPOINT （例如 " http://localhost:50342/oauth2/token "）。受管理服務的埠號碼。
...
過時. 若要使用自訂的 MSI 機密，請 MSI_SECRET 設定環境變數。 受管理的服務登入的權杖。
...
[最大訂閱編號] 可在登入後填入內容。 預設值為25。 若要填入內容的所有訂閱，請將它設為-1。
