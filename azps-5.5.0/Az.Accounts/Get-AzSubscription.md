...
## 說明
Get-AzSubscription Cmdlet 會取得目前帳戶可存取之訂閱的訂閱識別碼、訂閱名稱及家用租使用者。
...
這個命令會取得指定的訂閱，然後設定目前的內容來使用它。 此會話中的所有後續 Cmdlet 都會預設使用 (Contoso 訂閱 1) 的新訂閱。
