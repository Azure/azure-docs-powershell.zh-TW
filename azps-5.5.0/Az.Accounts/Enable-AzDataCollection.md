...
這個 `Enable-AzDataCollection` Cmdlet 是用來加入宣告資料收集。 Azure PowerShell 預設會自動收集遙測資料。 Microsoft 會匯總收集的資料以識別使用方式、找出常見問題，以及改善 Azure PowerShell 的體驗。
Microsoft Azure PowerShell 不會收集任何私人或個人資料。 若要停用資料收集，您必須透過執行明確退出宣告 `Disable-AzDataCollection` 。

## 示例

### 範例1：為目前的使用者啟用資料收集

下列範例顯示如何為目前的使用者啟用資料收集。
