...
## 說明
**AzKeyVaultNetworkRule** Cmdlet 會將金鑰保存庫的存取權授予或限制為一組呼叫者，由其 IP 位址或其所屬的虛擬網路所指定。 規則有可能會限制其他使用者、應用程式或安全性群組的存取權，這些使用者已透過存取原則授與許可權。

請注意， (私人 IP) 位址內的任何 IP 範圍都 `10.0.0.0-10.255.255.255` 無法用來新增網路規則。
...
這個命令會在指定的電子倉庫中新增一個網路規則，以允許從 $myNetworkResId 識別的虛擬網路存取指定的 IP 位址。
