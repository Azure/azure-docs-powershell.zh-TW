...
## 說明
使用 `New-AzKeyVaultRoleAssignment` 命令來授與存取權。
...
所指派的角色，必須使用 RoleDefinitionName pr RoleDefinitionId 參數加以指定。 您可以指定授權存取的範圍。 預設為選取的訂閱。
...
這個範例會將角色「受管理的 HSM 策略系統管理員」指派給 user1@microsoft.com top 作用中的使用者 ""。
