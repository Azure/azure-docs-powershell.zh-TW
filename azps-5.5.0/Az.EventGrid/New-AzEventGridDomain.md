...
## 說明
建立新的 Azure 事件格線網域。 網域建立之後，應用程式就可以將事件發佈到主題端點。

## 示例

### 範例1

\` \` \` \` 在 [資源群組 MyResourceGroupName] 中，在指定的地理位置 Westus2 中 \` \` 建立事件 Grid 網域 Domain1。
...
在 \` \` [ \` \` 資源群組 MyResourceGroupName] 中，以 \` 指定的標籤「 \` 部門」和「環境」，在指定的地理位置 westus2 中建立事件 Grid 網域 Domain1。
