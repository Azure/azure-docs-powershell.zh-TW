...
重新產生對應至 \' \` \` 資源群組 MyResourceGroupName 中的 key Key1 "\ 事件 Grid 網域 Domain1 \` ] 的索引鍵 \` 。
...
\' \` \` 在資源群組 MyResourceGroupName 中， \` \` 使用它的完整資源識別碼，重新產生對應至 key Key2 "\" 的事件 Grid 網域 Domain1 的索引鍵。
...
### DomainSharedAccessKeys 中的 EventGrid。
