...
## 說明
**新的-AzNetworkInterface** Cmdlet 會建立 Azure 網路介面。
...
第一個命令會在第二個命令中檢索用來指派子網的現有指定虛擬網路。
第二個命令會建立名為 IPConfig1 的網路介面 IP 配置，並將設定儲存在名為 $IPconfig 的變數中。
第三個命令會建立一個名為 NetworkInterface1 的網路介面，該介面使用在名為 $IPconfig 的變數中儲存的網路介面 IP 配置。
