...
## 說明
**AzApplicationGatewayTrustedClientCertificate** Cmdlet 會從應用程式閘道中移除受信任的用戶端 CA 憑證鏈物件。
...
第一個命令會取得應用程式閘道，並將它儲存在 $gw 變數中。 第二個命令會從儲存在 $gw 的應用程式閘道中，移除名為「TrustedClientCertificate01」的信任用戶端 CA 憑證鏈。
