...
## 說明
**AzApplicationGatewayTrustedRootCertificate** Cmdlet 會將信任的根憑證新增至 Azure 應用程式閘道。
...
第二個命令會將新的受根信任憑證新增至應用程式閘道，並將根憑證的路徑做為輸入。
第三個命令會使用信任的根憑證來建立新的後端 HTTP 設定，以驗證後端伺服器憑證。
第四個命令會更新應用程式閘道。
