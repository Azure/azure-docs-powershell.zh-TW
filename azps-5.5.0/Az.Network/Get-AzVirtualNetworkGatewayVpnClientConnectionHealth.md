...
## 說明
列出所有連線的 vpn 用戶端連線健康情況。 這包括： vpnConnectionId vpnConnectionDuration vpnConnectionTime publicIpAddress privateIpAddress vpnUserName maxBandwidth egressPacketsTransferred egressBytesTransferred ingressPacketsTransferred ingressBytesTransferred maxPacketsPerSecond
...
針對資源群組 resourceGroup 中名為 gatewayname 的 Azure 虛擬網路閘道，請使用 OpenVpn 來檢索目前連線的 vpn 用戶端連線。 

### 範例2

取得每個 vpn 用戶端連線之 Azure 虛擬閘道的 vpn 用戶端連線健康清單。  (自動生成) 
