...
## 說明
**AzApplicationGatewayPrivateLinkConfiguration** Cmdlet 會將私人連結設定新增至應用程式閘道。
...
第一個命令會建立 privateLinkIpConfiguration，並將它儲存在 $PrivateLinkIpConfiguration 變數中。
第二個命令會取得名為 ApplicationGateway01 的應用程式閘道，並將其儲存于名為 ResourceGroup01 的資源群組，然後將其儲存在 $AppGw 變數中。
第三個命令會針對 $AppGw 中的閘道，新增名為 privateLinkConfig01 的私人連結設定
