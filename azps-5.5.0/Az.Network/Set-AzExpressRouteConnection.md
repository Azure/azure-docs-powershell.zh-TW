...
### ByExpressRouteConnectionName (預設) 
```
...
## 說明
**AzExpressRouteConnection** Cmdlet 會更新在快速路由閘道與內部部署快速路由線路對等之間建立的快速路由連線。
...
建立閘道之後，就會使用 [New-AzExpressRouteConnection] 命令將它連線到內部部署 ExpressRoute 電路對等。

接著，您可以使用 Set-AzExpressRouteConnection 命令來更新連接，以提供不同的 RoutingWeight。
...
### -ResourceId
要刪除之 ExpressRouteConnection 物件的資源識別碼。
