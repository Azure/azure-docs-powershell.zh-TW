...
## 說明
**載入 AzApplicationGatewaySslProfile** Cmdlet 會將 SSL 設定檔新增至應用程式閘道。 SSL 設定檔會套用至 HTTPS 偵聽程式。
...
第二個命令會建立新的 SSL 原則，並將它儲存在 $sslPolicy 變數中。
第三個和第四個命令會建立兩個新的信任用戶端 CA 憑證鏈，並將它們儲存在 $ClientCert 01 和 $ClientCert 02 變數中。
第五個命令會將 SSL 設定檔與 ssl 原則和受信任的用戶端 CA 憑證鏈一起新增至應用程式閘道 $AppGw。
