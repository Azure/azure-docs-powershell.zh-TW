...
上述將會在 Azure 中的 [testRG] 資源群組中，建立一個資源群組、虛擬廣域網路、虛擬網路、虛擬中樞。 在具有2個比例單位的虛擬中樞中，將會建立 ExpressRoute 閘道。

然後它會使用其 resourceGroupName 和閘道名稱取得 ExpressRouteGateway。
...
這個命令會取得所有以「test」開頭的 ExpressRouteGateways。
