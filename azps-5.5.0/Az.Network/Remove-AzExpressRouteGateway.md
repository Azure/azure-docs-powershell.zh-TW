...
這個範例會在美國中部建立資源群組、虛擬 WAN、虛擬中樞、可伸縮的 ExpressRoute 閘道，然後立即將其刪除。 若要在刪除虛擬閘道時取消提示，請使用-Force 標誌。
這將會刪除 ExpressRouteGateway 及其附加的所有 ExpressRouteConnections。
...
這個範例會在 West 中建立一個資源群組、虛擬 WAN、虛擬中樞、可伸縮的 ExpressRoute 閘道，然後立即將其刪除。 此刪除會使用 powershell 管道執行，這會使用 Get-AzExpressRouteGateway 命令所傳回的 ExpressRouteGateway 物件。
