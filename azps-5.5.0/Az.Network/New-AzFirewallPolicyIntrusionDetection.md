...
## 摘要
建立新的 Azure 防火牆原則入侵偵測以與防火牆原則建立關聯
...
## 說明
**新的-AzFirewallPolicyIntrusionDetection** Cmdlet 會建立 Azure 防火牆原則入侵偵測物件。
...
這個範例會使用警示 (偵測) 模式來建立入侵偵測

### 範例 2: 2。 使用簽名覆蓋來建立入侵偵測
```powershell
PS C:\> $signatureOverride = New-AzFirewallPolicyIntrusionDetectionSignatureOverride -Id "123456798" -Mode "Deny"
PS C:\> New-AzFirewallPolicyIntrusionDetection -Mode "Alert" -SignatureOverride $signatureOverride
```

這個範例會使用特定的簽名覆蓋來建立入侵偵測

### 範例 3: 3。 使用設定了 [略過流量] 設定的入侵偵測建立防火牆原則
```powershell
...
### PSAzureFirewallPolicyIntrusionDetection 中的 [.]
