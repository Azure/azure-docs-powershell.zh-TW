...
上述將會建立資源群組、要求公用 IP 位址、建立虛擬網路和子網，以及在 Azure 中建立虛擬網路閘道。
閘道將會在位置 "vnet-gateway" 中的資源群組 "vnet-閘道" 中稱為「myNGW」，並將先前建立的 IP 設定儲存在變數 "ngwIPConfig"、"VPN" 的閘道類型、vpn 類型 "RouteBased" 和 sku "Basic"。 它也會新增位址為 "TestRadiusServer" 的外部 radius 伺服器。 它也會設定由客戶在閘道所指定的自訂路由。

### 範例3：使用 P2S 設定建立虛擬網路閘道
```powershell
New-AzResourceGroup -Location "UK West" -Name "vnet-gateway"
$subnet = New-AzVirtualNetworkSubnetConfig -Name 'gatewaysubnet' -AddressPrefix '10.254.0.0/27'
...
### 範例4：使用 AAD 驗證設定建立虛擬網路閘道，以 VpnClient 虛擬網路閘道。
```powershell
New-AzResourceGroup -Location "UK West" -Name "vnet-gateway"
New-AzVirtualNetworkSubnetConfig -Name 'gatewaysubnet' -AddressPrefix '10.254.0.0/27'
...
上述將會建立資源群組、要求公用 IP 位址、建立虛擬網路和子網，以及在 Azure 中建立虛擬網路閘道。
閘道將會在位置 "vnet-gateway" 中的資源群組 "vnet-閘道" 中稱為「myNGW」，並將先前建立的 IP 設定儲存在變數 "ngwIPConfig"、"VPN" 的閘道類型、vpn 類型 "RouteBased" 和 sku "Basic"。 它也會設定 AAD 驗證配置： AadTenantUri、AadIssuerUri 和 AadAudienceId，以 VpnClient 虛擬網路閘道。

### 範例5：使用 VpnGatewayGeneration 建立虛擬網路閘道
```powershell
New-AzResourceGroup -Location "UK West" -Name "vnet-gateway"
$subnet = New-AzVirtualNetworkSubnetConfig -Name 'gatewaysubnet' -AddressPrefix '10.254.0.0/27'
...
上述將會建立資源群組、要求公用 IP 位址、建立虛擬網路和子網，以及在 Azure 中建立虛擬網路閘道。
閘道將會在位置 "vnet-gateway" 中的資源群組 "vnet-閘道" 中稱為「myNGW」，並將先前建立的 IP 設定儲存在「ngwIPConfig」、「VPN」的閘道類型、vpn 類型「RouteBased」、sku "VpnGw4" 以及 VpnGatewayGeneration Generation2。

### 範例6：使用 IpConfigurationBgpPeeringAddresses 建立虛擬網路閘道
```powershell
...
上述將會建立資源群組、要求公用 IP 位址、建立虛擬網路和子網，以及在 Azure 中建立虛擬網路閘道。
ipconfigurationId1 的閘道 ipconfiguration 剛剛建立並儲存在 ngwipconfig 中。
閘道將會在 [英國西部] 位置的資源群組 "resourcegroup1resourcegroup1" 中稱為「gateway1」，並將先前建立的 IP 配置 Bgppeering 位址儲存在變數「gw1ipconfBgp1」、「VPN」的閘道類型、vpn 類型「RouteBased」、sku "VpnGw4" 以及 VpnGatewayGeneration Generation2 已啟用。
...
要新增的 VpnClientRootCertificates 清單。
...
此 VirtualNetwork VPN 閘道的產生。
如果 GatewayType 不是 VPN，則必須是 None。
