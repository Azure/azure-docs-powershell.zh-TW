...
## 說明
只有在未預配虛擬中樞的路由狀態時，才會重設現有 VirtualHub 資源的路由狀態。
...
使用其 ResourceGroupName 和 CoNtext.resourcename 來重設虛擬中心的路由狀態。
...
使用輸入物件來重設虛擬中心的路由狀態。 輸入物件的類型是 PSVirtualHub。
...
您可以檢索現有的虛擬中心物件，然後將它作為輸入物件傳遞給 Reset-AzHubRouter。
