...
## 說明
**新的-AzApplicationGatewayFirewallCondition** 會建立防火牆自訂規則的符合條件。
...
此命令會使用 $variable 中定義的 match 變數建立新的相符條件，運算子是 Contains 且否定條件為 false，Transfroms 包括小寫和 trim，而 match 值是 abc 與 cde。 新的 [相符] 條件會儲存在 $condition 中。
...
### PSApplicationGatewayFirewallCondition 中的 [.]
