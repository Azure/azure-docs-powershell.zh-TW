...
## 說明
New-AzVpnGateway 建立可伸縮的 VPN 閘道。
這是 VirtualHub 中的網站與網站連線的軟體定義連線。

此閘道會根據在此或 Set-AzVpnGateway Cmdlet 中指定的縮放比例來調整大小並進行縮放。

從稱為 VPNSite 的分支/網站將連接設定為可伸縮閘道。
每個連線都由 2 Active-Active 隧道組成。

VpnGateway 將會位於與參照 VirtualHub 相同的位置。
...
在此 VpnGateway 上啟用路由喜好設定網際網路的標誌。
