...
## 說明
AzApplicationGatewayUrlPathMapConfig Cmdlet 會將 URL 路徑映射陣列的 **設定** 設定為後端伺服器池。
...
第二個命令會更新應用程式閘道中名為 map01 的 URL 路徑對應。
第三個命令會更新應用程式閘道。

## 參數

### -ApplicationGateway
指定此 Cmdlet 設定 URL 路徑對應配置的應用程式閘道。
...
### -名稱
指定此 Cmdlet 針對其設定配置的 URL 路徑對應名稱。
