...
New-AzVirtualHubVnetConnection Cmdlet 會建立對 Azure 虛擬中樞擁有虛擬網路的 HubVirtualNetworkConnection 資源。  (自動生成) 
...
```
上述將會建立新的路由設定，並在路由配置中使用下一個躍點作為指定 IP 位址來建立靜態路由。 然後，就可以使用參數 RoutingConfiguration，將這種路由設定傳遞到 [New-AzVirtualHubVnetConnection] 命令。
