...
## 說明
**AzApplicationGatewayAuthenticationCertificate** Cmdlet 會更新 Azure 應用程式閘道的驗證憑證。
...
第一個命令會取得名為 appGwName 的應用程式閘道，並將結果儲存在 $appgw 變數中。
第二個命令會更新應用程式閘道中名為 cert01 的驗證憑證。
第三個命令會更新應用程式閘道。

## 參數

### -ApplicationGateway
指定此 Cmdlet 更新驗證憑證之應用程式閘道的名稱。
