...
上述命令會傳回「testnewkustocluster」群集中是否存在名為 "mykustodatabase" 的 Kusto 資料庫。

### 範例2：檢查是否有空使用中的 Kusto 資料庫名稱
```powershell
...
上述命令會傳回「testnewkustocluster」群集中是否存在名為 "mykustodatabase2" 的 Kusto 資料庫。
...
### -類型
資源類型，例如 Microsoft. Kusto/群集/資料庫。
