...
### 範例1：檢查使用中的資料庫 principalassignment 名稱是否有空
```powershell
...
上述命令會傳回在群集 "testnewkustocluster" 的資料庫 "mykustodatabase" 中是否存在名為 "myprincipal" 的 PrincipalAssignment。

### 範例2：檢查是否有空使用的資料庫 principalassignment 名稱
```powershell
...
### -類型
資源類型 Kusto/群集/資料庫/principalAssignments。
