...
上述命令會傳回名為 "testnewkustocluster" 的 Kusto 群集是否存在於 "東 US" 區域中。

### 範例2：檢查是否有空使用中的 Kusto 群集名稱
```powershell
...
上述命令會傳回名為 "availablekustocluster" 的 Kusto 群集是否存在於 "東 US" 區域中。
