...
這個命令會在 MariaDB 下建立防火牆規則。

### 範例2：使用-ClientIPAddress 建立新的 MariaDB 防火牆規則。
```powershell
...
這個 Cmdlet 會使用-ClientIPAddress 建立 MariaDB 防火牆規則。

### 範例3：建立新的 MariaDB 防火牆規則，以允許所有 Ip
```powershell
...
這個 Cmdlet 會建立新的 MariaDB 防火牆規則，以允許所有的 Ip。
...
### IFirewallRule （MariaDb）。 Api20180601Preview。
