...
## 說明
**AzBatchJobSchedule** Cmdlet 會在 Azure Batch service 中設定作業排程。
...
第一個命令會使用 **AzBatchJobSchedule** 來取得作業，然後將它儲存在 $JobSchedule 變數中。
第二個命令會修改物件上的定期間隔 `$JobSchedule.Schedule` 。
最後一個命令會更新批次服務，以符合 $JobSchedule 中的本機物件。
