...
### 範例13：使用 RoutingPreference 設定來更新儲存空間帳戶
```powershell
...
這個命令會使用 RoutingPreference 設定來更新儲存空間帳戶： PublishMicrosoftEndpoint 為 false、PublishInternetEndpoint 為 true，且 RoutingChoice 為 MicrosoftRouting。
...
### -RoutingChoice
路由選擇：定義使用者選擇的網路路由類型。 可能的值包括： "MicrosoftRouting"、"InternetRouting"
