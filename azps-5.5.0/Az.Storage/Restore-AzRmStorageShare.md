...
## 說明
如果啟用 [共用虛刪除]， **Restore-AzRmStorageShare** Cmdlet 會在有效的保留天數內還原已刪除的檔案共用。
...
這個命令會先刪除檔案共用，然後列出 [共用] 並查看已刪除的共用版本，最後再將它還原為 [標準] 共用。 需要啟用 AzStorageFileServiceProperty 的 [共用虛刪除]，再刪除 [共用]。
