...
這個命令會從 Filesystem 中列出直接子專案

### 範例2：以遞迴方式從目錄中清單，並提取屬性/ACL
```
...
### 範例3：以多個批次以遞迴方式列出 Filesystem 中的專案
```
...
這個範例使用 *MaxCount* 和 *ContinuationToken* 參數，以多個批次從 Filesystem 中以遞迴方式列出專案。
前四個命令會將值指派給要在範例中使用的變數。
第五個命令指定一個 **Do** 語句，該語句使用 **AzDataLakeGen2ChildItem** Cmdlet 來列出專案。
...
如果 speicify 此參數，在每個清單專案的擁有者和群組欄位中傳回的使用者身分識別值，就會從 Azure Active Directory 物件識別碼轉換為使用者主要名稱。 如果未 speicify 此參數，值將會以 Azure Active Directory 物件識別碼的形式傳回。 請注意，群組和應用程式物件識別碼不會翻譯，因為它們沒有唯一的易記名稱。
...
### -Path
要在指定的 Filesystem 中檢索的路徑。
必須是 [directory1/directory2/"格式的目錄。
