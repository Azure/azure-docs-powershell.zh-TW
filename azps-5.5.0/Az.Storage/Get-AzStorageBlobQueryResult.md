...
## 說明
**AzStorageBlobQueryResult** Cmdlet 會在 blob 內容的 SQL) 語句中套用簡單的結構化查詢語言 (，並將所查詢的資料子集儲存至本機檔案。
...
這個命令 querys 將輸入配置為 csv 的 blob succsssfully，並將輸出配置為 json，並將輸出儲存至本機檔案 "c:\resultfile.json"。
...
這個命令會先取得 blob 快照的 blob 物件，然後查詢 blob 快照，並顯示結果包含查詢錯誤。
...
用來處理查詢輸入文字的配置。 使用新的-AzStorageBlobQueryConfig 建立 configuration 物件。
...
用來處理查詢輸出文字的配置。 使用新的-AzStorageBlobQueryConfig 建立 configuration 物件。
...
### -QueryString
查詢字串，請參閱以下內容中的詳細資料： https://docs.microsoft.com/en-us/azure/storage/blobs/query-acceleration-sql-reference
