...
## 說明
**Disable AzStorageBlobRestorePolicy** Cmdlet 會針對 Azure Storage Blob 服務停用 Blob 還原原則。

## 示例

### 範例1：針對儲存帳戶上的 Azure Storage Blob 服務停用 Blob 還原原則
```powershell
PS C:\> Disable-AzStorageBlobRestorePolicy -ResourceGroupName "myresourcegoup" -StorageAccountName "mystorageaccount"
```

這個命令會針對儲存帳戶上的 Azure Storage Blob 服務停用 Blob 還原原則。
