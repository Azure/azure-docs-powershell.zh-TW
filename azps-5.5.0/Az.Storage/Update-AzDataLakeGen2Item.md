...
## 說明
**更新 AzDataLakeGen2Item** Cmdlet 會更新屬性、中繼資料、許可權、ACL 及擁有者的檔案或目錄。
這個 Cmdlet 只有在已針對儲存空間帳戶啟用階層命名空間時才適用。 此類型的帳戶可使用「-EnableHierarchicalNamespace $true」執行「新-AzStorageAccount」 Cmdlet 來建立。

## 示例

### 範例1：使用3個 ACL 專案建立 ACL 物件，並以遞迴方式更新 Filesystem 中所有專案的 ACL
```
...
這個命令會先建立含3個 acl 專案的 ACL 物件 (使用-InputObject 參數將 acl 專案新增至現有的 ACL 物件) ，然後取得 filesystem 中的所有專案，並更新專案的 acl。

### 範例2：更新檔案中的所有內容，並顯示這些屬性
```
...
這個命令會更新檔案上的所有屬性 (ACL、許可權、擁有者、群組、中繼資料、屬性可以使用任何 conbination) 進行更新，並在 Powershell 主控台中顯示。
...
這個命令會從目錄取得 ACL、更新/新增 ACL 專案，並將其設回目錄。
如果具有相同 AccessControlType/EntityId/DefaultScope 的 ACL 專案不存在，將會新增 ACL 專案，否則就會新增現有 ACL 專案的許可權。

## 參數

### -Acl
設定對檔案和目錄的 POSIX 存取控制許可權。
使用新的 AzDataLakeGen2ItemAclObject 建立此物件。
...
### -許可權
為檔案擁有者、擁有的群組及其他人設定 POSIX 存取許可權。
每個類別都可能被授與 [讀取]、[寫入] 或 [執行] 許可權。
支援符號 (rwxrw-cd-rw-) 。
與 Acl 搭配無效。
...
### -屬性
指定目錄或檔案的屬性。 支援的檔案屬性有： CacheControl、ContentDisposition、ContentEncoding、ContentLanguage、ContentMD5、ContentType。
