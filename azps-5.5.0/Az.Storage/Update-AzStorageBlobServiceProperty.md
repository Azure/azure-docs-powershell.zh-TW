...
## 說明
**AzStorageBlobServiceProperty** Cmdlet 會修改 Azure Storage Blob 服務的服務屬性。

## 示例

### 範例1：將 Blob 服務 DefaultServiceVersion 設定為2018-03-28
```
...
這個命令會將 Blob 服務的 DefaultServiceVersion 設定為2018-03-28。

### 範例2：在儲存空間帳戶的 Blob 服務上啟用 Changefeed
```
...
這個命令可讓 Changefeed 在儲存帳戶的 Blob 服務上變更 [Azure Blob 儲存空間] 中的摘要支援，方法是透過偵聽任何 Blob 層級建立、修改或刪除事件的 GPv2 或 Blob 儲存空間帳戶來運作。 接著，它會針對儲存在儲存空間帳戶內 $blobchangefeed 容器中的 blob 輸出已排序的事件記錄。 序列化的變更會保留為 Apache Avro 檔案，而且可以以非同步和增量方式進行處理。

### 範例3：在儲存空間帳戶的 Blob 服務上啟用版本設定
```
...
### -EnableChangeFeed
[啟用變更儲存空間的摘要記錄] 設定為 [$true]，停用將摘要記錄設為 [$false]。
