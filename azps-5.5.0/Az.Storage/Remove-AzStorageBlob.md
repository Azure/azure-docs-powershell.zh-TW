...
## 說明
**AzStorageBlob** Cmdlet 會將指定的 Blob 從 Azure 中的儲存空間帳戶中移除。
...
這個命令使用星號 ( * ) 萬用字元和管線來檢索 blob 或 blob，然後將它們移除。
...
這個命令會在 VersionId 中移除單一 blob verion。
...
這個命令會將單一的 blob 快照與 SnapshotTime 移除。
...
您可以使用 Get-AzStorageContainer Cmdlet 來取得它。
...
### -DeleteSnapshot
指定要刪除所有快照，但不是基底 blob。
如果未指定此參數，則會將基底與其快照一起刪除。
系統會提示使用者確認刪除操作。
...
指定要讀取之 Cmdlet 的 Azure 設定檔。
如果沒有指定，則會從預設設定檔讀取 Cmdlet。
