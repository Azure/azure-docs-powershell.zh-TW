...
## 說明
**AzStorageBlobContent** Cmdlet 會下載指定的儲存區 blob。
...
### 範例4：取得 blob 物件並將它儲存在變數中，然後使用 blob 物件下載 blob 內容
```
PS C:\>$blob = Get-AzStorageBlob -Container containername -Blob blobname 
PS C:\>Get-AzStorageBlobContent -CloudBlob $blob.ICloudBlob -Destination "C:\test"
```

這個範例會先取得 blob 物件，並將它儲存在變數中，然後使用 blob 物件下載 blob 內容。 
...
您可以建立或使用 Get-AzStorageContainer Cmdlet。
