...
這個命令會取得與 VersionId verion 的單一 blob。
...
這個命令會取得含有 SnapshotTime 的單一 blob 快照。
...
如果為此參數指定一個值，則 Cmdlet 會列出名稱與此參數相符的所有 blob。 這個參數在字串中的任何位置都支援萬用字元。
...
### -IncludeVersion
只有在這個參數存在時，才會列出 Blob 版本，因此預設的 [取得 blob] 不會包含 blob 版本。
