...
## 說明
**AzStorageFileServiceProperty** Cmdlet 會修改 Azure 儲存空間服務的服務屬性。

## 示例

### 範例1：啟用檔案共用 softdelete
```powershell
...
這個命令啟用檔案共用 softdelete 刪除，保留天數為5
...
將儲存帳戶的 [刪除保留原則] 設定為 [$true，停用 [共用刪除保留原則] 設定為 [$false]。
...
### -ShareRetentionDays
設定 [共用 DeleteRetentionPolicy] 的保留天數。
只有在啟用共用刪除保留原則時，才應設定此值。
