...
## 說明
**新的-AzTrafficManagerProfile** Cmdlet 會建立 Azure 流量管理器設定檔。
指定 *Name* 參數及必要的設定。
這個 Cmdlet 會傳回代表新設定檔的本機物件。

這個 Cmdlet 不會設定流量管理器端點。
您可以使用 Add-AzTrafficManagerEndpointConfig Cmdlet 來更新本機設定檔物件。
然後使用 Set-AzTrafficManagerProfile Cmdlet 將變更上傳到流量管理器。
或者，您也可以使用 New-AzTrafficManagerEndpoint Cmdlet 來新增端點。
