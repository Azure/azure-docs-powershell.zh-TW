...
## 說明
新增 vm 延伸至節點類型。 這會將延伸新增至 underliying 的「虛擬電腦規模集資源」。
...
這個命令會將副檔名與 [設定] 和 [受保護的設定] 新增至節點類型。
...
### -Publisher
延伸處理常式發行者的名稱。
這可以使用 Get-AzVMImagePublisher Cmdlet 來取得發行者。
...
您可以使用 Get-AzVMExtensionImageType Cmdlet 來取得延伸類型。
