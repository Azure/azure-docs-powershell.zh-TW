...
## 說明
這個 Cmdlet 會建立不含節點類型的受管理的群集資源。 若要引導群集，需要新增主要節點類型使用 [New-AzServiceFabricManagedNodeType](./New-AzServiceFabricManagedNodeType.md)。
...
這個命令會在 centraluseuap 中使用初始系統管理員用戶端憑證和標準 sku 來建立群集資源。
...
### -Sku
群集的 Sku，選項為基本：它至少會有3個種子節點，且只允許1個節點類型及標準：它至少有5種子節點，且允許多個節點類型。
...
### -UseTestExtension
如果指定 [群集]，將會 crated 服務測試 vmss 延伸。
