...
### 範例1：依名稱建立 Windows 虛擬桌面 HostPool
```powershell
...
這個命令會在資源群組中建立 Windows 虛擬桌面 HostPool。
...
HostPool 的 PersonalDesktopAssignment 類型。
...
### -SsoadfsAuthority
用於簽署 WVD SSO 憑證之客戶 ADFS 伺服器的 URL。
...
### -SsoClientId
使用已登錄的信賴方來頒發 WVD SSO 憑證的 ClientId。
...
Azure KeyVault 的路徑，儲存用於與 ADFS 通訊的秘密。
...
### -SsoCoNtext
包含 ssoCoNtext 秘密之 keyvault 的路徑。
...
### -StartVMOnConnect
[開啟/關閉 StartVMOnConnect 功能] 的 [標誌]。
...
### -VMTemplate
在 hostpool 中 sessionhosts 設定的 VM 範本。
...
### IHostPool （DesktopVirtualization）。 Api20201102Preview。
