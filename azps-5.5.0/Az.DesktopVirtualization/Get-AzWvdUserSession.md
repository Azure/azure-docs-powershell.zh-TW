...
### 範例1：依名稱取得 Windows 虛擬桌面 UserSession
```powershell
...
這個命令會在工作階段主機中取得 Windows 虛擬桌面 UserSession。

### 範例2：列出 Windows 虛擬桌面 UserSessions
```powershell
...
這個命令會在工作階段主機中列出 Windows 虛擬桌面 UserSessions。

### 範例3：在主機池中列出 Windows 虛擬桌面 UserSessions
```powershell
...
這個命令會列出主機池中的工作階段主機中的 Windows 虛擬桌面 UserSessions。
...
### -篩選
OData 篩選運算式。
篩選的有效屬性為 userprincipalname 與 sessionstate。
...
### IUserSession （DesktopVirtualization）。 Api20201102Preview。
