...
### -InputObject
指定需要啟動測試遷移清除的複製伺服器。
您可以使用 Get-AzMigrateServerReplication Cmdlet 來檢索 server 物件，請參閱適用于 INPUTOBJECT 屬性的記事區段，並建立雜湊資料表。
...
INPUTOBJECT <IMigrationItem> ：指定需要啟動測試遷移清除的複製伺服器。 可以使用 Get-AzMigrateServerReplication Cmdlet 來檢索 server 物件
