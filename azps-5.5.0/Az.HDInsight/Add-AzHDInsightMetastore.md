...
## 說明
**AzHDInsightMetastore** Cmdlet 會將 Hive 或 Oozie metastore 新增至 New-AzHDInsightClusterConfig Cmdlet 建立的 HDInsight 設定物件。
Metastore 是 SQL 資料庫，可用於儲存 Hive、Oozie 或兩者的中繼資料。

## 示例

### 範例1：新增 SQL 資料庫 metastore 至群集設定物件
```
...
這個命令會將 SQL 資料庫 metastore 新增到名為 [您的 hadoop-001] 的群集中。

### 範例2：將自訂 Ambari 資料庫新增至群集設定物件
```
PS C:\># Primary storage account info
PS C:\> $storageAccountResourceGroupName = "Group"
PS C:\> $storageAccountResourceId = "yourstorageaccountresourceid"
...
這個命令會將自訂 Ambari 資料庫新增到名為 [hadoop-002] 的群集中。
...
### -DatabaseName
指定要用於此 metastore 的 AzureSQL 伺服器實例上的資料庫。
...
### -MetastoreType
指定 metastore 的類型。
可能的值為 HiveMetastore 或 OozieMetastore。
...
### -SqlAzureServerName
指定要用於此 metastore 的 AzureSQL 伺服器實例。
