...
## 說明
**新的-AzHDInsightClusterAutoscaleScheduleCondition** Cmdlet 會建立以排程為基礎的自動縮放條件。
...
這個命令會在每個星期一、星期三，建立每個09:00 的群集自動縮放至5個工作節點的情況。

### 範例2：使用自動縮放條件啟用群集的以排程為基礎的自動縮放。
```powershell
...
### -WorkerNodeCount
取得或設定自動縮放排程條件的排程 workernode 計數。
...
### AzureHDInsightAutoscaleCondition （即 Azure。
