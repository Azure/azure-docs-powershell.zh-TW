...
訂閱階層配置：訂閱只能有一個此類型的設定。 LowCpuThreshold 和 Exclude 是此類型設定的唯一屬性。

ResourceGroup 層次配置：訂閱中的每個 ResourceGroup 只能有一個設定。 [排除] 是此類型設定的唯一屬性。
