...
## 說明
**AzApiManagementGatewayHostnameConfiguration** Cmdlet 會從現有的閘道中移除主機名稱設定。
...
這個命令會移除現有的閘道主機名稱設定，不會提示使用者進行確認。
...
### -InputObject
PsApiManagementGatewayHostnameConfiguration 的實例。 這個參數是必要的。
...
### -ResourceId
GatewayHostnameConfiguration 的 Arm ResourceId。 這個參數是必要的。
