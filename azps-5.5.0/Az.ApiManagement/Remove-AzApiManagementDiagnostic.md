...
## 說明
Cmdlet **移除-AzApiManagementDiagnostic** 會 `DiagnosticId` 從全域範圍或範圍中移除指定的診斷實體 `ApiId`
...
這個範例會 `applicationinsights` 從 Api 管理服務移除診斷程式。
...
### -InputObject
PsApiManagementDiagnostic 的實例。 這個參數是必要的。
