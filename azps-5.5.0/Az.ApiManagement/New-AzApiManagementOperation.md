...
## 說明
**新的-AzApiManagementOperation** Cmdlet 會建立 API 操作。
...
這個範例會使用 [要求] 與 [回應] 詳細資料建立 API 管理作業。
...
### -ApiRevision
API 修訂的識別碼。 這個參數是選用的。 如果未指定，該作業將會附加到目前使用中的 api 修正程式。
...
### -TemplateParameters
指定 [參數 *UrlTemplate*] 中定義的參數陣列。
如果您沒有指定此參數，則會根據 *UrlTemplate* 產生預設值。
