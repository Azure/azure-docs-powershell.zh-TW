...
這個命令會從指定的 Open 3.0 規格連結中匯入 API。

### 範例5：從開放式 Api 連結匯入 API 至 ApiVersion 集合
...
這個命令會從指定的開啟3.0 規格檔中匯入 API，並建立新的 ApiVersion。
...
### -通訊協定
Web API 通訊協定 (HTTP、HTTPs) 。 提供可用 API 的通訊協定。 這個參數是選用的。 如果有的話，會覆寫 [規格] 檔中指定的通訊協定。
...
### -ServiceUrl
公開 API 之 web 服務的 URL。 這個 URL 將只由 Azure API 管理使用，不會公開。 這個參數是選用的。 如果有的話，會覆寫 [規格] 檔中指定的 ServiceUrl。
