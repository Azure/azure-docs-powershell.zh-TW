...
## 說明
**新的-AzApiManagementLogger** Cmdlet 會建立 Azure API 管理 **記錄器**。
...
這個命令會使用指定的連接字串來建立名為 ContosoSdkEventHub 的記錄器。
...
### ConnectionString
指定以下列開頭的 Azure 事件中心連接字串： `Endpoint=endpoint and key from Azure classic portal`
必須設定連接字串中具有傳送權利的索引鍵。
