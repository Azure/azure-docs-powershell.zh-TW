...
## 說明
**AzApiManagementPolicy** Cmdlet 會從指定的範圍中移除 API 管理原則。
...
這個命令會從 API 管理移除操作範圍原則。

## 參數

### -ApiId
指定現有 API 的識別碼。
如果您指定此參數，則 Cmdlet 會移除 API 作用域原則。
...
### -OperationId
指定現有作業的識別碼。
如果您使用 *ApiId* 參數指定此參數，這個 Cmdlet 會移除操作範圍原則。
