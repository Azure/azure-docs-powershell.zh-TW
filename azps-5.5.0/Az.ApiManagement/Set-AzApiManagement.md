...
**AzApiManagement** Cmdlet 會更新 Azure API 管理服務。

## 示例

### 範例1：取得 API 管理服務並將它縮放至 Premium 並新增區域
```powershell
...
### 範例3：使用 KeyVault 資源中的秘密來建立和初始化 PsApiManagementCustomHostNameConfiguration 的實例
```powershell
...
### 範例4：更新發行者電子郵件、NotificationSender 電子郵件和組織名稱
```powershell
...
### -PassThru
如果操作成功，則會將更新的 PsApiManagement 傳送到管線。
