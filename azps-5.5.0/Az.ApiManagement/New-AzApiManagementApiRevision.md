...
**AzApiManagementApiRevision** Cmdlet 會在 api 管理內容中為現有 API 建立 api 修正。

## 示例

### 範例1：建立 API 的空白 API 修訂
```powershell
...
這個命令會建立 api 的 API 版本 `2` `echo-api` 。

### 範例2：從現有的 Api 建立 API 修訂，並複製所有操作、標籤與原則
```powershell
...
這個命令會建立 api 的 API 版本 `5` `echo-api` 。
...
### -ServiceUrl
在後端服務中公開 API 之 web 服務的 URL。 這個 URL 將只由 Azure API 管理使用，不會公開。 長度必須是1到2000個字元。 這個參數是必要的。
