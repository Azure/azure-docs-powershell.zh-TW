...
## 說明
**新的-AzApiManagementSubscription** Cmdlet 會建立訂閱。
...
### -AllowTracing
判斷是否可在訂閱層級啟用追蹤的標誌。 此為選用的參數，預設值為 $null。
...
### -範圍
訂閱的範圍（無論是 Api 範圍/apis/{apiId} 或產品範圍/products/{productId}），還是全域 API 範圍/apis 或全域作用中。 這個參數是必要的。
