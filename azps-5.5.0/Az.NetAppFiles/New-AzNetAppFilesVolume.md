...
## 說明
**新的-AzNetAppFilesVolume** Cmdlet 會建立 ANF 的音量。
...
這個命令會在「MyAnfPool」池中建立新的 ANF 音量「MyAnfVolume」。
...
### -SecurityStyle
Volume 的安全性樣式。 可能的值包括： "ntfs"、"unix"
...
如果已啟用 (true) 則會包含唯讀的快照目錄，提供每個卷快照的存取權 (預設為 true) 
...
### -SnapshotId
從快照建立音量。 用來識別快照的 UUID v4 或資源識別碼
...
### PSNetAppFilesPool 中的 NetAppFiles。
