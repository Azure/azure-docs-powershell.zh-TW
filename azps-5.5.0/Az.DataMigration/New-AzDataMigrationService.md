...
## 說明
New-AzDataMigrationService Cmdlet 會建立 Azure Database 遷移服務的新實例。 這個 Cmdlet 會採用現有 Azure 資源群組的名稱、要建立的 Azure 資料庫移轉服務新實例的唯一名稱、該實例的提供區域、DMS Worker SKU 的名稱，以及該服務要駐留的 Azure 虛擬子網的名稱。 訂閱名稱不提供任何參數，因為使用者需要指定 Azure 登入會話或執行 Get-AzSubscription 的預設訂閱 SubscriptionName "MySubscription" |Select-AzSubscription 選取另一個訂閱。
