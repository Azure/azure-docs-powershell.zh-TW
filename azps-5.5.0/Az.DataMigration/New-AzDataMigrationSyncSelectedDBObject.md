...
New-AzDataMigrationSyncSelectedDB Cmdlet 會建立一個特定于同步處理案例的資料庫資訊物件，其中包含來源和目標資料庫的相關資訊。
...
這個範例會建立資料庫中繼資料物件來描述 $DatabaseName 到資料庫 $DatabaseName 的遷移設定。  
...
### MigrateSqlServerSqlDbSyncTaskInput 中的 DataMigration。
