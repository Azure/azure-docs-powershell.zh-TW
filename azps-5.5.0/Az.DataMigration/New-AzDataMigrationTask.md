...
## 說明
New-AzDataMigrationTask Cmdlet 會建立資料移轉任務。 這個 Cmdlet 採用任務類型枚舉器、Azure 資源群組、關聯 Azure 資料庫移轉服務的名稱和專案作為輸入的參數。 
...
這個範例腳本示範如何在名為 [myDMSProject] 的專案或名為 [TestService] 的服務中，建立名為 MyMigrationTask 的新資料移轉任務。 
...
### -稍候
是否等待任務完成。 如果已設定旗標，則會在工作完成後再檢查一秒，然後返回使用者可檢查輸出或錯誤的任務屬性。 
```yaml
