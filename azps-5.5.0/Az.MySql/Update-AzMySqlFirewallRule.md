...
這個 Cmdlet 會依名稱更新 MySql 防火牆規則。

### 範例2：依身分識別更新 MySql 防火牆規則。
```powershell
...
這些 Cmdlet 會依身分識別來更新 MySql 防火牆規則。

### 範例3：依 ClientIPAddress 更新 MySql 防火牆規則。
```powershell
PS C:\> $ID = "/subscriptions/<SubscriptionId>/resourceGroups/PowershellMySqlTest/providers/Microsoft.DBforMySQL/servers/mysql-test/firewallRules/rule"
...
這些 Cmdlet 會依 ClientIPAddress 更新 MySql 防火牆規則。
