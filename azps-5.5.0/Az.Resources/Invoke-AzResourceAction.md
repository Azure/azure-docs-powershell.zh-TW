...
## 說明
**AzResourceAction** Cmdlet 會在指定的 Azure 資源上調用動作。
若要取得支援的動作清單，請使用 Azure 資源管理器工具。

## 示例

### 範例1：使用 ResourceId 啟動 VM
...
這個命令會以 {ResourceId} 啟動虛擬機器。

### 範例2：使用 CoNtext.resourcename 來調用 poweroffing VM
...
這個命令會停止使用 {ResourceId} 的虛擬機器。
命令會指定 *Force* 參數，因此不會提示您進行確認。

### 範例3：使用 ResourceId 註冊資源提供者
