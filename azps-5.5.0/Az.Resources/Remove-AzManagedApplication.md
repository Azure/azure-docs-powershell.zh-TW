...
## 說明
**AzManagedApplication** Cmdlet 會移除受管理的應用程式

## 示例

### 範例1：依資源識別碼移除受管理的應用程式
```
PS C:\>$Application = Get-AzManagedApplication -Name "myApp" -ResourceGroupName "myRG"
PS C:\>Remove-AzManagedApplication -Id $Application.ResourceId -Force
```

第一個命令會使用 Get-AzManagedApplication Cmdlet 來取得名為 myApp 的受管理應用程式。
該命令會將它儲存在 $Application 變數中。
第二個命令會移除由 $Application 的 **ResourceId** 屬性所識別的管理應用程式。
