...
## 說明
**新的-AzManagementGroupDeployment** Cmdlet 會在管理群組新增部署。

若要在管理群組新增部署，請指定管理群組、位置及範本。
...
若要將資源新增至資源群組，請使用 **New-AzResourceGroupDeployment** ，這會在資源群組建立部署。
若要將資源新增至訂閱，請使用 **新的-AzSubscriptionDeployment** ，這會在訂閱範圍中建立部署，以部署訂閱層級資源。
若要在租使用者範圍中新增資源，請使用 **新的-AzTenantDeployment** ，這會在租使用者範圍中建立部署。
...
這個命令會使用自訂的範本，以及磁片上的範本檔案（含已定義的 tags 參數），在管理群組「myMG」建立新的部署。
此命令會使用 *TemplateFile* 參數來指定範本和 *TemplateParameterFile* 參數，以指定包含參數和參數值的檔案。

### 範例2：使用 uri 和 SAS 權杖部署儲存在非公用儲存空間帳戶的範本
```
...
```
這個命令會使用 TemplateUri 中的範本來建立新的部署，該範本不是公開的，而且需要權杖參數才能使用 QueryString 參數來提供存取。
執行此命令可有效地使用 url 存取範本 https://example.com/example.json?foo 。
如果您想要在儲存空間帳戶中使用範本，只要將 SAS 權杖提供為 QueryString，就可以使用此功能。

### 範例3：使用自訂範本物件與參數檔案建立部署
```
...
這個命令會使用自訂範本與磁片上已轉換成記憶體中的 hashtable 的範本檔案，在管理群組「myMG」建立新的部署。
前兩個命令會讀取磁片上範本檔案的文字，並將它轉換成記憶體中的 hashtable。
最後一個命令使用 *TemplateObject* 參數來指定此 Hashtable 和 *TemplateParameterFile* 參數，以指定包含參數和參數值的檔案。
...
### -名稱
將要建立之部署的名稱。 如果未指定，則預設為提供範本檔時的範本檔案名;預設為提供範本物件的目前時間（例如 "20131223140835"）。
...
### -QueryString
查詢字串 (例如，要與 TemplateUri 參數搭配使用的 SAS 權杖) 。 會在連結範本時使用
...
會跳過 PowerShell 動態參數處理，檢查提供的範本參數是否包含範本所使用的所有必要參數。
這項檢查會提示使用者提供缺少參數的值，但提供-SkipTemplateParameterPrompt 將會在範本中發現未系結的參數時，立即忽略此提示和錯誤。
在非互動式腳本中，您可以提供 SkipTemplateParameterPrompt，以在不符合所有必要參數的情況下，提供較佳的錯誤訊息。
...
要從 What-If 結果中排除的逗號分隔資源變更類型。 在設定了-WhatIf 或-Confirm 切換時適用。
...
### -WhatIfResultFormat
What-If 的結果格式。 在設定了-WhatIf 或-Confirm 切換時適用。
