...
## 說明
使用 New-AzRoleAssignment 命令來修改現有作業。  
描述可以是任何有效的字串，用來相互 diferentiate。  
如果設定條件，則必須設定條件版本，但如果您要更新不必要的條件。
條件版本可以從1.0 升級到2.0，但不能降級回來。 請小心，因為2.0 沒有與 1.0 retrocompatible。
