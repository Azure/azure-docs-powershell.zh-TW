...
## 說明
**新的-AzPolicySetDefinition** Cmdlet 會建立一個策略集定義。

## 示例

### 範例1：使用原則集檔案建立含中繼資料的原則集定義
```
[
   {
...
這個命令會建立一個名為 VMPolicySetDefinition 的原則集定義，其中繼資料指出其類別是「虛擬機器」，其中包含 C:\VMPolicy.js的中所指定的原則定義。 上面提供了 VMPolicy.js的範例內容。
...
這個命令會建立一個名為 VMPolicySetDefinition 的參數化原則集定義，其中包含 C:\VMPolicy.js中指定的原則定義。 上面提供了 VMPolicy.js的範例內容。
...
這個命令會建立一個名為 VMPolicySetDefinition 的原則集定義，並將原則定義分組，並在 C:\VMPolicy.json] 上指定。 上面提供了 VMPolicy.js的範例內容。
...
### -GroupDefinition
新原則集定義的原則定義群組。 這可以是包含群組之檔案的路徑，或作為 JSON 字串的群組。
...
### -中繼資料
原則集定義的中繼資料。 這可以是包含中繼資料之檔案名的路徑，或作為 JSON 字串的中繼資料。
...
### -參數
原則集定義的參數宣告。
這可以是包含參數宣告之檔案名的路徑，或作為 JSON 字串的參數宣告。
...
### -PolicyDefinition
原則定義。 這可以是包含原則定義的檔案名路徑，或原則定義（JSON 字串）。
