...
## 說明
**新的-AzTenantDeployment** Cmdlet 會在目前的租使用者範圍內新增部署。

若要在租使用者範圍中新增部署，請指定位置和範本。
...
若要在管理群組新增資源，請使用 [ **New-AzManagementGroupDeployment** ]，在管理群組建立部署。
...
這個命令會使用自訂的範本與磁片上的範本檔案（含已定義的 tags 參數），在目前租使用者範圍內建立新的部署。
...
這個命令會在目前的租使用者中建立新的部署，方法是使用自訂範本，以及磁片上已轉換成記憶體中的 hashtable 的範本檔案。
