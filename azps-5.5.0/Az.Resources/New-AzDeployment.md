...
Azure 資源是使用者管理的 Azure 實體。 資源可以駐留在資源群組中，例如資料庫伺服器、資料庫、網站、虛擬機器或儲存空間帳戶。
或者，它可以是訂閱階層資源，例如角色定義、原則定義等。
...
這個命令會使用自訂的範本與磁片上的範本檔案（含已定義的標記參數），在目前的訂閱範圍中建立新的部署。
...
這個命令會使用自訂範本和磁片上已轉換為記憶體中 hashtable 的範本檔案，在目前的訂閱範圍中建立新的部署。
