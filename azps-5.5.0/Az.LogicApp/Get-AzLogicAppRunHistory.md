...
**AzLogicAppRunHistory** Cmdlet 會取得邏輯 app 的執行歷程記錄。
...
這個命令會透過遵循 NextPageLink 來取得名為 LogicApp03 之邏輯 app 的整個執行歷程記錄。
...
這個命令會透過遵循 NextPageLink 並將結果大小限制為兩個頁面，以取得名為 LogicApp03 之邏輯 app 的前兩頁。
每個頁面都包含三十個結果。
