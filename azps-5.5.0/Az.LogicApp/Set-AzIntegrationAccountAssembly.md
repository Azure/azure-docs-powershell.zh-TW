...
## 說明
**AzIntegrationAccountAssembly** Cmdlet 會修改整合帳戶元件。
...
使用位於 [$localAssemblyFilePath] 中所含檔案路徑的本機檔案，修改名為「sampleAssembly」的程式集。
...
使用「$assemblyContent」中所含的位元組陣列來修改名為「sampleAssembly」的程式集。
...
使用位於 URL "$assemblyUrl" 的位元組資料來修改名為 "sampleAssembly" 的元件。 這是建立大型元件的建議方法。
