...
## 說明
**AzLogicApp** Cmdlet 會使用邏輯應用程式功能來執行邏輯 app。
指定名稱、資源群組及觸發程式。
此模組支援動態參數。
若要使用動態參數，請在命令中輸入。
...
這個命令會在名為 ResourceGroup11 的資源群組中執行邏輯 app。
...
### -ResourceGroupName
指定包含此 Cmdlet 啟動之邏輯 app 之資源群組的名稱。
