...
## 說明
Set-AzEventHubNamespace Cmdlet 會更新指定事件中樞命名空間的屬性。
...
更新要建立之命名空間 MyNamespaceName 的標記 \` \` 。
...
### -KeyProperty
索引鍵屬性清單，@ ( @ (KeyName、KeyVaultUri、Keyversion) 、@ (KeyName、KeyVaultUri、Keyversion) # A5
...
### "NamespaceState" 1 [[1.4.3.0]、[]、[]、[Culture = 中性]、[PublicKeyToken = null]]。
