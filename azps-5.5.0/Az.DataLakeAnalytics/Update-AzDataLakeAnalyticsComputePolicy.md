...
## 說明
**更新-AzDataLakeAnalyticsComputePolicy** 會針對 Azure Data Lake Analytics 帳戶中的特定 AAD 實體更新指定的計算原則規則。
...
這個命令會更新帳戶「contosoadla」中名為「myPolicy」的原則，以確保使用者無法提交超過5個分析單元的任何作業。
...
這個命令會在帳戶 "contosoadla" 中建立名為 "myPolicy" 的原則，以確保使用者無法提交超過5個分析單元或優先順序低於100的任何作業
...
此原則每個作業支援的分析單位上限。 必須指定 this、MinPriorityPerJob 或這兩個參數。
...
### -MinPriorityPerJob
此原則的每個作業支援的最低優先順序。 必須指定 this、MaxAnalyticsUnitsPerJob 或這兩個參數。
