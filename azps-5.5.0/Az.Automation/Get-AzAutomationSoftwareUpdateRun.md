...
## 說明
Get-AzAutomationSoftwareUpdateRun Cmdlet 會傳回軟體更新執行的清單。 因為軟體更新設定有關聯的排程，所以可以多次觸發。 每次觸發排程時，都會產生已建立的更新執行。 [更新執行] 是所有電腦執行結果的匯總。 您可以傳送 SoftwareUpdateConfigurationName 參數，以取得特定軟體更新設定的執行。 若要取得特定的執行，您必須傳遞 name 參數。 您也可以以特定狀態列出執行、以特定作業系統為目標，或在特定時間之後傳送適當的參數來執行。

## 示例

### 範例1
這個範例會列出由特定軟體更新設定所觸發的所有更新執行。
...
### UpdateManagement SoftwareUpdateConfiguration 中的 []

### "UpdateManagement" 1 [[OperatingSystemType]、[]、[system.object]、[Version = 1.0.0.0]、[Culture = 中立]、[PublicKeyToken = null]]

### "UpdateManagement" 1 [[SoftwareUpdateRunStatus]、[]、[system.object]、[Version = 1.0.0.0]、[Culture = 中立]、[PublicKeyToken = null]]
