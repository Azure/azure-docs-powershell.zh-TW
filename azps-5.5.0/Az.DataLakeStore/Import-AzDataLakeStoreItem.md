...
## 說明
匯 **入-AzDataLakeStoreItem** Cmdlet 會將本機檔案或目錄上傳到 Data Lake store。
...
這個命令會將檔案上傳 SrcFile.csv 並將它新增到 Data Lake Store 中的 MyFiles 資料夾，並以4的併發性為 File.csv。
...
### -併發性
表示以並行方式上傳的檔案數或區塊數。 根據系統規格，會將預設值計算為最大的精力。
...
### -簡歷
表示檔案 (s) 被覆制為前一個上傳的延續。 這會導致系統嘗試從沒有完全上傳的最後一個檔案繼續。
