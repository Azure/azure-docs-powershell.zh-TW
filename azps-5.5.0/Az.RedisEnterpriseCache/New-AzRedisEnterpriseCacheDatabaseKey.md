...
這個命令會重新產生主要的機密存取金鑰，以用於驗證連線至 Redis Enterprise Cache （名為 MyCache）資料庫的連線。
...
這個命令會重新產生次要機密存取金鑰，以用於驗證名為 MyCache 之 Redis Enterprise Cache 之資料庫的連線。
...
### IAccessKeys （RedisEnterpriseCache）。 Api20201001Preview。
