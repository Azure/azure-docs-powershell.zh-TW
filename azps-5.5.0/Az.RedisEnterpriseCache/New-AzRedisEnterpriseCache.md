...
這個命令會建立名為 MyCache 的 Redis 企業版快取。

### 範例2：使用一些選用的參數建立 Redis 企業版快取
```powershell
...
這個命令會使用一些選用的參數，建立名為 MyCache 的 Redis 企業版快取。
...
### -容量
RedisEnterprise 群集的大小。
根據 SKU 的預設值為2或3。
有效值為 (2、4、6、... ) （適用于企業版 Sku），以及 (3、9、15、... ) 的 Flash Sku。
...
### -Sku
要部署的 RedisEnterprise 群集類型。
可能的值： (Enterprise_E10、EnterpriseFlash_F300 等。 ) 
