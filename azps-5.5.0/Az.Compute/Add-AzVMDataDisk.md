...
## 說明
**載入 AzVMDataDisk** Cmdlet 會將資料磁片新增至虛擬機器。
...
第一個命令會使用 [AzVM](./Get-AzVM.md) Cmdlet 取得名為 VirtualMachine07 的虛擬機器。
...
### -VM
指定要新增資料磁片的本機虛擬機器物件。
您可以使用 **AzVM** Cmdlet 來取得虛擬機器物件。
您可以使用 **新的 AzVMConfig** Cmdlet 來建立虛擬機器物件。
