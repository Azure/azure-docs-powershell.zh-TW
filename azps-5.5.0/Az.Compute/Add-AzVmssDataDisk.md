...
## 說明
**AzVmssDataDisk** Cmdlet 會將資料磁片新增到虛擬電腦規模集 (VMSS) 實例。
...
### -DiskIOPSReadWrite
指定受管理磁片的 Read-Write IOPS。 只能在 StorageAccountType UltraSSD_LRS 時使用。 如果未指定，將根據 diskSizeGB 指派預設值。
...
### -DiskMBpsReadWrite
指定受管理磁片的頻寬（以每秒 MB 為單位）。 只能在 StorageAccountType UltraSSD_LRS 時使用。 如果未指定，將根據 diskSizeGB 指派預設值。
