...
### -DataDiskImage
資料磁片影像。   例如，@ {Source = @ {Id =<source_id>};Lun = 1;SizeInGB = 100;HostCaching = "ReadOnly"}
...
### -OSDiskImage
OS 磁片圖像（例如 @ {Source = @ {Id =<source_id>};SizeInGB = 100;HostCaching = "ReadOnly"}
...
如果已設定，則從最新版本的影像定義部署的虛擬機器將不會使用此影像版本。
...
### -StorageAccountType
指定要用來儲存影像的儲存空間帳戶類型。 這個屬性無法更新。 可用的值 Standard_LRS、Standard_ZRS 及 Premium_LRS。
