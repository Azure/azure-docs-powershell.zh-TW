...
這個命令會在指定的主機群組和指定的位置中建立給定 Sku 的主機。
...
指定是否應在失敗時自動取代主機。 未提供時，該值預設為 [true]。
...
### -LicenseType
指定將套用至主機上部署之 Vm 的軟體授權類型。 可能的值為： None、Windows_Server_Hybrid 和 Windows_Server_Perpetual。  預設值為 [無]。
