...
## 說明
**新的-AzDiskUpdateConfig** Cmdlet 會建立一個可設定的磁片更新物件。
...
### -DiskMBpsReadOnly
將共用磁片全部裝載為 ReadOnly 的所有 Vm 所允許的總輸送量 (MBps) 。 Mbits （MBps）代表每秒數百萬個位元組，就會使用 ISO 符號（10的冪）。
...
### -MaxSharesCount
可以在同一時間附加至磁片的 Vm 數目上限。 大於1的值表示可以同時在多個 Vm 上裝載的磁片。
