...
## 說明
**新的-AzDiskConfig** Cmdlet 會建立可設定的磁片物件。
...
第一個命令會在 Standard_LRS 儲存帳戶類型中，建立一個大小為5GB 的本機空白磁片物件。 它也會設定 Windows OS 類型，並啟用加密設定。 第二個和第三個命令會設定磁片物件的磁片加密金鑰和金鑰加密金鑰設定。 最後一個命令會取得磁片物件，並在 [資源] 群組的 [ResourceGroup01] 中建立名為 "Disk01" 的磁片。
...
第一個命令會建立要上傳的本機磁片物件。
第二個命令會取得磁片物件，並在資源群組 ' ResourceGroup01 ' 中建立名為「Disk01」的磁片。
第三個命令會取得磁片的 SAS Url。
第四個命令會取得磁片的狀態。
如果磁片狀態是 [ReadyToUpload]，使用者可以使用 AzCopy 將磁片從 blob 儲存體上傳到磁片 SAS Url。
在上傳期間，磁片狀態會變更為 [ActiveUpload]。
最後一個命令會吊銷 SAS Url 的磁片存取權。
...
從共用圖庫影像版本建立磁片。  [識別碼] 是共用圖庫影像版本的 id。 只有當來源是資料磁片時，才需要 Lun。

## 參數

### -BurstingEnabled
在預配磁片的效能目標之外啟用突發。 預設會停用暴沖。 不適用於超磁片。
...
### -DiskAccessId
取得或設定在上使用 private 端點的 DiskAccess 資源的 ARM ID。
...
### -DiskIOPSReadOnly
將共用磁片以唯讀方式裝載到所有 Vm 的總 IOPS 數。 一項操作可在4k 與256k 位元組之間進行傳輸。
...
### -DiskMBpsReadOnly
"描述"： "在將共用磁片設為 ReadOnly 的所有 Vm 中，允許的總輸送量 (MBps) 。 Mbits （MBps）代表每秒數百萬個位元組，就會使用 ISO 符號（10的冪）。
...
### -EncryptionType
用來加密磁片資料的金鑰類型。  可用的值為： "EncryptionAtRestWithPlatformKey"、"EncryptionAtRestWithCustomerKey"
...
GalleryImageReference 物件。  如果是從圖庫影像建立，則為必要。 識別碼將是建立磁片所用的共用條樣影像版本的 ARM 識別碼。
如果複本來源是圖庫影像中的其中一個資料磁片，則需要 lun;如果是 null，則會複製影像的 OS 磁片。
...
### -HyperVGeneration
虛擬機器的虛擬機器監控程式產生。 僅適用于作業系統磁片。  允許的值為 V1 和 V2。
...
### -MaxSharesCount
可以在同一時間附加至磁片的 Vm 數目上限。
大於1的值表示可以同時在多個 Vm 上裝載的磁片。
...
### -UploadSizeInBytes
指定 CreateOption 上傳時，包括 VHD 頁尾在內的上傳內容大小。  此值應該在 20972032 (20 MiB + 512 位元組（適用于 vhd 頁尾) 與35183298347520位元組 (32 TiB + 512 位元組）) 。
...
### ImageDiskReference 中的 [Azure. 管理]
