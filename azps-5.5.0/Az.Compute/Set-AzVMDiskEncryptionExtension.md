...
## 說明
**AzVMDiskEncryptionExtension** Cmdlet 可在執行中的基礎結構上加密，成為 Azure 中的服務 (IaaS) 虛擬機器。  它會在虛擬機器上安裝磁片加密延伸，以啟用加密。 

這個 Cmdlet 需要使用者確認，因為啟用加密的其中一個步驟需要重新開機虛擬機器。

建議您在執行此 Cmdlet 前，先將工作儲存在虛擬機器上。

Linux：加密 Linux 虛擬電腦時需要 **VolumeType** 參數，且必須設定為 Linux 發佈支援的「Os」、「資料」或「全部」 ) 值 (。 

Windows：您可以省略 **VolumeType** 參數，在這種情況下，該作業的預設值為 All;如果 Windows 虛擬機器的 VolumeType 參數存在，則必須將它設為 All 或 OS。
...
這個範例會使用流水線輸入傳送參數，以在 VM 上啟用加密，而不需指定廣告認證。
...
這個範例使用 Azure AD 用戶端識別碼和用戶端密碼在 VM 上啟用加密。
...
這個範例使用 Azure AD 用戶端識別碼和用戶端認證指紋在 VM 上啟用加密。
...
這個範例使用 Azure AD 用戶端識別碼和用戶端密碼在 VM 上啟用加密，並使用金鑰加密金鑰來包裝磁片加密金鑰。
...
這個範例使用 Azure AD 用戶端識別碼和用戶端憑證指紋在 VM 上啟用加密，並使用金鑰加密金鑰來包裝磁片加密金鑰。
...
Add-AzVMSecret Cmdlet 可以用來將憑證部署到 Azure 中的虛擬機器。
...
### -名稱
指定代表延伸的 Azure 資源管理員資源的名稱。 如果省略 *Name* 參數，則在 Windows 虛擬機器上，系統會將已安裝的擴充功能命名為 AzureDiskEncryption，並在 Linux 虛擬機器上 AzureDiskEncryptionForLinux。
...
Get-AzVMExtension Cmdlet 可用來檢索以前使用的序列值。
