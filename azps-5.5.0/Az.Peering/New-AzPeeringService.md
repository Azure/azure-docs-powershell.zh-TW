...
使用提供者和對等位置建立對等服務物件。 搭配與 conjuction 搭配使用 `Get-AzPeeringServiceProvider``Get-AzPeeringServiceLocation`
...
### -PeeringLocation
不同于 Azure 區域的物理位置。 使用 Get-AzPeeringServiceLocation [-Country <country> ]
...
對等服務提供者名稱。
在清單中使用 Get-AzPeeringServiceProvider Cmdlet
...
### -Tag
要與 Microsoft InputObject 服務相關聯的標記。
