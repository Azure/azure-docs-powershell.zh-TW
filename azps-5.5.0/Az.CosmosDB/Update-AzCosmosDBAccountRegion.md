...
## 說明
更新 CosmosDB 帳戶的地區。 位置可以是以 PSLocation 類型的物件或以容錯移轉優先順序排序之位置名稱的字串形式提供。
LocationObject 參數預期 (容錯移轉 prioritiies 中的目前位置清單，新增的 LocationObjects 對應至要新增的新位置) 。
Location 參數預期目前位置的清單 (是依容錯移轉優先順序) 與新位置排序。 請注意，我們只支援新增地區。 請提供位置或 LocationObject。
