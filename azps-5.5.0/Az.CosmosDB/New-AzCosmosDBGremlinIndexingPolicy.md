...
## 摘要
建立新的 CosmosDB IndexingPolicy 物件。
...
## 說明
**新的-AzCosmosDBGremlinIndexingPolicy** Cmdlet 會建立 PSIndexingPolicy 類型的新物件。
...
### -CompositePath
PSCompositePath 的物件陣列的陣列（CosmosDB）。
...
### -ExcludedPath
包含 excludedPath 的字串陣列 (指定 JSON 檔內要排除在 Azure Cosmos DB 服務中的路徑。 ) 元素。
...
### -IncludedPath
包含 includedPath 的字串陣列 (指定 JSON 檔內要包含在 Azure Cosmos DB 服務中的路徑。 ) 元素。
...
### -SpatialSpec
CosmosDB 類型的物件陣列。 PSSpatialSpec
