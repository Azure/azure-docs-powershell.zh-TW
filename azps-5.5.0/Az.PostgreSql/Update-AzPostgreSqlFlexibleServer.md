...
如果您想要補救伺服器參數（例如 wait_timeout 或 net_retry_count），請改用 Update-AzPostSqlFlexibleServerConfiguration。
...
### 範例1：依資源群組和伺服器名稱更新 PostgreSql 伺服器
```powershell
...
這個 Cmdlet 會依資源群組和伺服器名稱來更新 PostgreSql server。

### 範例2：依身分識別更新 PostgreSql 伺服器。
```powershell
...
這個 Cmdlet 會依身分識別來更新 PostgreSql 伺服器。
