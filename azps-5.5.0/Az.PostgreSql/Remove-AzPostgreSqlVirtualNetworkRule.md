...
### 範例1：依名稱移除 PostgreSql server 虛擬網路規則
```powershell
PS C:\> Remove-AzPostgreSqlVirtualNetworkRule -Name vnet -ResourceGroupName PostgreSqlTestRG -ServerName PostgreSqlTestServer

```

這個 Cmdlet 會依名稱移除 PostgreSql server 虛擬網路規則。

### 範例2：依身分識別移除 PostgreSql server 虛擬網路規則
```powershell
...
這些 Cmdlet 會依身分識別來移除 PostgreSql server 虛擬網路規則。
