...
### 範例1：依名稱 Updatae 指定的 PostgreSql 設定
```powershell
...
這個 Cmdlet 會依名稱更新指定的 PostgreSql 設定。

### 範例1： Updatae 指定的 PostgreSql 設定（依身分識別）
```powershell
...
這個 Cmdlet 會依身分識別來更新指定的 PostgreSql 設定。
...
### IConfigurationAutoGenerated （PostgreSql）。 Api20200214Preview。
