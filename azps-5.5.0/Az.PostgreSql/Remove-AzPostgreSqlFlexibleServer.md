...
### 範例1：依 resourceGroup 和伺服器名稱移除 PostgreSql 伺服器
```powershell
PS C:\> Remove-AzPostgreSqlFlexibleServer -ResourceGroupName PowershellPostgreSqlTest -Name postgresql-test

```

這個 Cmdlet 會依 resourceGroup 和伺服器名稱移除 PostgreSql 伺服器。

### 範例2：依身分識別移除 PostgreSql 伺服器
```powershell
...
這些 Cmdlet 會依身分識別來移除 PostgreSql 伺服器。
