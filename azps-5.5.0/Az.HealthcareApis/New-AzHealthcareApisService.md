...
### 範例1：在資源群組 MyResourceGroup 中，建立一個名為 MyService 的新 Azure healthcareapis fhir 服務，其中 westus2 優惠輸送量 = 400
```powershell
...
### 範例2：在位置 westus2 的資源群組 MyResourceGroup 中，建立名為 MyService 的新 Azure healthcareapis fhir 服務，並提供 cosmosdb 提供輸送量 = 400 和主要保存庫金鑰 uri "HTTPs:// \<my-keyvault> . vault.azure.net/keys/ \<my-key> "
```powershell
...
### -AllowCorsCredential
HealthcareApis Fhir Services AllowCorsCredentials。
...
### -物件
HealthcareApis Fhir Services 物件。
...
### -CorsHeader
HealthcareApis Fhir 服務的 Cors 標頭清單。
指定可在要求期間使用的 HTTP 標頭。
針對任何標題使用 "*"。
...
### -CorsMaxAge
HealthcareApis Fhir 服務 Cors 最大使用時間。
指定要求中的結果可在數秒內緩存多久。
範例：600代表10分鐘。
...
### -CorsMethod
HealthcareApis Fhir 服務的 Cors 方法清單。
...
### -CorsOrigin
[Cors 來源] 的 HealthcareApis Fhir 服務清單。
...
HealthcareApis Fhir Services CosmosOfferThroughput。
...
### -EnableSmartProxy
HealthcareApis Fhir Services EnableSmartProxy。
