...
## 說明
**AzMarketplaceTerms** Cmdlet 會將指定的發行者 (id 的使用條款物件儲存在 publisher) ，提供識別碼 (產品) 與計畫識別碼 (名稱) 元組。
...
### 範例2
將發行者協定設定為 [Accept] （接受）。 從「AzMarketplaceTerms」 Cmdlet 取得「字詞」參數的值
...
### -InputObject
在 Get-AzMarketplaceTerms Cmdlet 中傳回的詞彙物件。 如果接受的參數為 true，則此為必要參數。
...
### PSAgreementTerms 中的 MarketplaceOrdering。
