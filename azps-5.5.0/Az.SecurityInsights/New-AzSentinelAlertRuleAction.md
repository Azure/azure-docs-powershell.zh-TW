...
## 說明
**新的-AzSentinelAlertRuleAction** Cmdlet 會針對指定工作區中的報警規則建立自動回應。
您必須提供邏輯 App Resorce Id 與觸發 Uri，才能使用邏輯 App 模組找到。
...
這個範例會使用邏輯 App 的屬性，為指定的警示規則建立 **AlertRuleAction** ，然後將其儲存在 $AlertRuleAction 變數中。
