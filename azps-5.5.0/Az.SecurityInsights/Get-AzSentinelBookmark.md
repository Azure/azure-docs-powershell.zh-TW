...
## 說明
**AzSentinelBookmark** Cmdlet 會從指定的工作區取得書簽。
如果您指定 *BookmarkId* 參數，則會傳回單一 **書簽** 物件。
如果您沒有指定 *BookmarkId* 參數，則會傳回包含指定工作區中所有書簽的陣列。
您可以使用 **bookmark** 物件來更新書簽，例如，您可以在 **書簽** 中新增記事。
...
這個範例會取得指定工作區中的所有 **書簽** ，然後將它儲存在 $Bookmarks 變數中。
...
這個範例會在指定的工作區中取得 **書簽** ，然後將它儲存在 $Bookmark 變數中。
