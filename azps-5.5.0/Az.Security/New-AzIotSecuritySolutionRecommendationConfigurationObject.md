...
## 說明
AzIotSecuritySolutionRecommendationConfigurationObject 會為 iot 安全解決方案建立新的建議設定物件。
如此一來，就會設定建議的狀態（無論是已啟用還是停用）。
...
針對建議輸入 "IoT_ACRAuthentication" （狀態設定為 [停用]）建立新的建議配置
...
### PSRecommendationConfiguration 中的 IotSecuritySolutions （Security..。
