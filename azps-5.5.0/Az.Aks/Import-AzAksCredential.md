...
### -系統管理員
取得 [clusterAdmin' kubectl config]，而不是預設的「clusterUser」。
...
### -ConfigPath
要建立或更新的 kubectl config 檔案。
使用 "-" 改為列印 YAML 至 stdout。
預設值：% Home%/.kube/config。
