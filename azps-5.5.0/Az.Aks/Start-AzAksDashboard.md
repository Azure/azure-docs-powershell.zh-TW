...
## 說明
建立 Kubectl SSH 隧道至 managed 群集的儀表板。 SSH 隧道是在稱為 Kubectl-Tunnel 的 PowerShell 作業中設定，可透過執行來找到 `Get-Job` 。 隧道應該可透過存取 [http://127.0.0.1:8001](http://127.0.0.1:8001) 。
...
### -DisableBrowser
在建立 kubectl 埠轉寄之後，請不要使用 pop 來開啟瀏覽器。
