...
## 說明
**AzFrontDoor** Cmdlet 會更新前門負載平衡器。 如果未提供輸入參數，則會使用現有的前蓋的舊參數。

## 示例

### 範例1：使用 FrontDoorName 和 ResourceGroupName 更新現有的前門。
```powershell
...
### 範例4：使用-DisableCertificateNameCheck 切換參數更新現有前門的 BackendPoolSetting 屬性 EnforceCertificateNameCheck
您可以使用 FrontoorName 和 ResourceGroupName、PSFrontDoor 物件或 ResourceId 來識別要更新的前蓋。  (請參閱上述3個範例中的範例) 下列範例會使用 PSFrontDoor 物件。
...
是否要針對所有後端池停用 HTTPS 要求的憑證名稱檢查。 對非 HTTPS 式要求沒有作用。
