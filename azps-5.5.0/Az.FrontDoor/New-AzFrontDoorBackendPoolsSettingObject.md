...
## 說明
**新的-AzFrontDoorBackendpoolsSettingObject** Cmdlet 會建立新的 PSBackendPoolsSettings 物件供前門建立。

## 示例

### 範例1：使用預設值建立 BackendPoolsSettings 物件
```powershell
...
### 範例2：使用使用者指定的值建立 BackendPoolsSettings 物件
```powershell
...
傳送和接收到後端的轉寄要求超時。 當達到超時時間時，要求會失敗並傳回。
...
### PSBackendPoolsSetting 中的 FrontDoor。
