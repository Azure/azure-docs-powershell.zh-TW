...
使用 Cmdlet "New-AzFrontDoorRulesEngineActionObject" 建立 PSRulesEngineAction 物件，以傳送到 "-Action" 參數。
使用 Cmdlet "New-AzFrontDoorRulesEngineMatchConditionObject" 建立 PSRulesEngineMatchCondition 物件，以傳送到 "-MatchCondition" 參數。
...
建立新的 PSRulesEngineRule 物件，並示範如何查看子欄位。
...
傳遞不正確 [優先順序] 值時，預期會出現輸出。

## 參數

### -動作
在符合所有符合條件的情況下，在要求和回應上執行的動作。
...
### -MatchCondition
必須符合才能執行此規則之動作之符合條件的清單。 沒有任何相符的條件，就表示動作將會一直執行。
...
### PSRulesEngineRule 中的 FrontDoor。
