...
## 說明
**Enable-AzFrontDoorCustomDomainHttps** 會啟用自訂網域的 HTTPS。

## 示例

### 範例1：使用 FrontDoorName 和 ResourceGroupName 的自訂網域啟用 HTTPS，並使用 [前門] 管理的憑證。
```powershell
...
在資源群組 "resourcegroup1" 中，使用前門管理的憑證來啟用自訂網域「frontendpointname1-custom-xyz」的 HTTPS。

### 範例2：使用 FrontDoorName 和 ResourceGroupName 在金鑰保存庫中使用自己的憑證，為自訂網域啟用 HTTPS。
```powershell
...
### 範例3：針對使用前門管理憑證的 PSFrontendEndpoint 物件啟用自訂網域的 HTTPS。
```powershell
...
使用由前門管理憑證的 PSFrontendEndpoint 物件，為自訂網域啟用 HTTPS。

### 範例4：針對使用前門管理憑證的自訂網域啟用 HTTPS。
```powershell
...
針對自訂網域啟用 HTTPS，使用資源識別碼 $resourceId 使用前門管理的憑證。
