...
## 說明
**Disable-AzFrontDoorCustomDomainHttps** 會停用自訂網域的 HTTPS。

## 示例

### 範例1：停用 FrontDoorName 和 ResourceGroupName 的自訂網域的 HTTPS。
```powershell
...
針對自訂網域停用 HTTPS "frontendpointname1-custom-xyz"，FrontDoorName 為 "frontdoor1" 且 ResourceGroupName 為 "resourcegroup1"。

### 範例2：針對含 PSFrontendEndpoint 物件的自訂網域停用 HTTPS。
```powershell
...
針對含 PSFrontendEndpoint 物件的自訂網域停用 HTTPS。

### 範例3：針對具有 ResourceId 的自訂網域停用 HTTPS。
```powershell
...
針對自訂網域停用 HTTPS "frontendpointname1-自訂-xyz" 與 ResourceId 為 $resourceId。
