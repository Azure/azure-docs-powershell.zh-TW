...
ms.locfileid: "100135547"
---
# Enable-AzRecoveryServicesBackupAutoProtection

## 摘要
**Enable-AzRecoveryServicesBackupAutoProtection** Cmdlet 會使用提供的原則，為指定實例中的目前及未來的任何 SQL 內容設定自動保護。
...
## 說明
這個命令可讓使用者自動保護所有現有的未受保護的 SQL 資料庫，以及稍後會以指定原則新增的任何資料庫。  由於此指示是要備份所有未來的內容，因此，Azure 備份服務會定期針對任何新的系統來掃描自動保護的容器，並自動保護它們。
...
第二個 Cmdlet 會提取相關的 SQLInstance，這是可保護的專案。 然後，第三個命令會使用 $Pol 中的原則，為此實例設定自動保護。

### 範例2

這個命令可讓使用者自動保護所有現有未受保護的資料庫，以及稍後會以指定原則新增的任何 DB。  (自動生成) 
...
受保護的資源類別。 這個 Cmdlet 支援的值目前為 MAB、AzureWorkload、Add-azurevm
...
### -InputItem
指定可以傳送成輸入的可保護專案物件。 目前支援的值是 "SQLInstance" 類型的 protectableItem 物件。 
...
### -WorkloadType
資源的工作負荷類型。 目前支援的值為 Add-azurevm、WindowsServer、MSSQL
