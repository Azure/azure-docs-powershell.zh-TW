...
## 說明
**Enable-AzRecoveryServicesBackupProtection** Cmdlet 會將保護原則與專案建立關聯，以啟用備份。
如果原則識別碼不存在，或備份專案未與任何原則相關聯，則此命令會預期 policyID。
...
第二個 Cmdlet 會指定要備份的磁片 Lun，並將其儲存在 $inclusionDiskLUNS 變數中。
第三個 Cmdlet 會使用 $Pol 中的原則，為名為 V2VM 的 ARM 虛擬機器設定備份保護原則。

### 範例2

啟用具有指定備份保護原則之專案的備份。  (自動生成) 
...
### -ExclusionDisksList
要在備份中排除的磁片 Lun 清單，其餘部分會自動包含在其中。
...
### -InclusionDisksList
在 [備份] 中包含的磁片 Lun 清單，其餘部分會自動排除在作業系統磁片以外。
...
若要取得 **AzureRmRecoveryServicesBackupProtectionPolicy** 物件，請使用 Get-AzRecoveryServicesBackupProtectionPolicy Cmdlet。
