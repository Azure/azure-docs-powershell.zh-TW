...
**AzRecoveryServicesBackupContainer** Cmdlet 會取得備份容器。 備份容器封裝 modelled 為備份專案的資料來源。
針對容器類型 "Azure VM"，輸出會列出其名稱與以易記名稱參數值傳遞之完全相符的所有容器。 針對其他容器類型，輸出會提供名稱與為易記名稱參數傳遞的值類似的容器清單。
