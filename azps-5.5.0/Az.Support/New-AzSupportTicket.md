...
### CreateSupportTicketWithContactDetailParameterSet (預設) 
```
...
## 說明
這個 Cmdlet 可以用來建立帳單、訂閱管理、配額或技術問題的支援票證。 使用 Get-AzSupportService 和 Get-AzSupportProblemClassification Cmdlet 來識別 Azure 服務，並分別針對您要要求支援的問題分類。 您必須指定下列參數： 
...
您可以使用 New-AzSupportContactProfileObject helper Cmdlet 來建立 CustomerContactDetail 物件。

雲端解決方案提供者可以登入客戶的租使用者，並使用 *CSPHomeTenantId* 參數指定其家用租使用者識別碼，為其客戶的訂閱建立支援票證。

__針對技術入場券：__

若要指定資源名稱，請使用 *TechnicalTicketResourceId* 參數來指定資源的 ARM 資源識別碼。 請參閱下面的範例。 

__針對配額票證：__

若要針對 **計算 VM 核心**、 **批次** **SQL 資料庫** 與 **SQL 資料倉儲** 要求配額增加，請在 [ *QuotaTicketDetail* 物件] 底下提供其他詳細資料。 QuotaTicketDetail 物件由3個屬性組成，如下所述。 如需詳細檔，請 [按一下這裡。](https://aka.ms/supportrpquotarequestpayload)
...
如需如何針對各種配額類型構造負載的詳細檔，請 [按一下這裡](https://aka.ms/supportrpquotarequestpayload)

## 示例

### 範例1：建立帳單或訂閱管理支援票證。 使用 Get-AzSupportService 和 Get-AzSupportProblemClassification 來針對您要要求支援的計費或訂閱管理問題分類來檢索正確的 Guid 
```powershell
...
### 範例2：建立適用于 Windows 資源的虛擬機器的技術支援票證。 使用 Get-AzSupportService 和 Get-AzSupportProblemClassification 來檢索 Windows 版虛擬機器的正確 Guid，以取得您要要求支援的分類問題分類 
```powershell
...
### 範例3：建立配額支援票證以增加特定 VM 系列的虛擬電腦核心配額。 使用 Get-AzSupportService 和 Get-AzSupportProblemClassification，以針對配額計算 VM 核心問題分類來取得正確的 Guid。
```powershell
...
### 範例4：建立配額支援票證，以增加 Batch 帳戶低優先順序核心的配額。 使用 Get-AzSupportService 和 Get-AzSupportProblemClassification，以針對配額批次問題分類來取得正確的 Guid。
```powershell
...
### 範例5：建立配額支援票證，以增加批次帳戶特定 VM 系列的 VM 核心配額。 使用 Get-AzSupportService 和 Get-AzSupportProblemClassification，以針對配額批次問題分類來取得正確的 Guid。
```powershell
...
### 範例6：建立配額支援票證以增加 Batch 帳戶的 Pool quota。 使用 Get-AzSupportService 和 Get-AzSupportProblemClassification，以針對配額批次問題分類來取得正確的 Guid。
```powershell
...
### 範例7：建立配額支援票證以增加批次帳戶的作用中作業與作業排程配額。 使用 Get-AzSupportService 和 Get-AzSupportProblemClassification，以針對配額批次問題分類來取得正確的 Guid。
```powershell
...
### 範例8：建立配額支援票證以增加訂閱的批次帳戶數目。 使用 Get-AzSupportService 和 Get-AzSupportProblemClassification，以針對配額批次問題分類來取得正確的 Guid。
```powershell
...
### 範例9：建立配額支援票證，以增加 SQL 資料庫 Dtu 的配額。 使用 Get-AzSupportService 和 Get-AzSupportProblemClassification，以針對配額 SQL 資料庫問題分類來取得正確的 Guid。
```powershell
...
### 範例10：建立配額支援票證以增加 SQL 資料庫伺服器的配額。 使用 Get-AzSupportService 和 Get-AzSupportProblemClassification，以針對配額 SQL 資料庫問題分類來取得正確的 Guid。
```powershell
...
### 範例11：建立配額支援票證，以增加 SQL 資料倉儲的 Dtu 配額。 使用 Get-AzSupportService 和 Get-AzSupportProblemClassification，以針對配額 SQL 日期倉庫問題分類來取得正確的 Guid。
```powershell
...
### 範例12：建立配額支援票證以增加 SQL 資料倉儲的伺服器配額。 使用 Get-AzSupportService 和 Get-AzSupportProblemClassification，以針對配額 SQL 資料倉儲問題分類來取得正確的 Guid。
```powershell
PS C:\> New-AzSupportTicket -Name "test1" -Title "Test" -Description "Test" -Severity "minimal" -ProblemClassificationId "/providers/Microsoft.Support/services/{quota_service_guid}/problemClassifications/{sql_datawarehouse_problemClassification_guid}" -QuotaTicketDetail @{QuotaChangeRequestVersion = "1.0" ; QuotaChangeRequestSubType = "Servers" ; QuotaChangeRequests = (@{Region = "westus"; Payload = "{`"NewLimit`":200}"})} -CustomerContactDetail @{FirstName = "first" ; LastName = "last" ; PreferredTimeZone = "pacific standard time" ; PreferredSupportLanguage = "en-us" ; Country = "USA" ; PreferredContactMethod = "Email" ; PrimaryEmailAddress = "user@contoso.com"}
...
### 範例13：建立配額支援票證，以增加電腦學習服務低優先順序核心的配額。 使用 Get-AzSupportService 和 Get-AzSupportProblemClassification 來為配額機器學習服務問題分類取得正確的 Guid。
```powershell
...
### 範例14：建立配額支援票證，為電腦學習服務的特定 VM 系列增加 VM 核心配額。 使用 Get-AzSupportService 和 Get-AzSupportProblemClassification 來為配額機器學習服務問題分類取得正確的 Guid。
```powershell
...
### 範例15：建立配額支援票證以增加 Azure SQL Managed 實例的配額。 使用 Get-AzSupportService 和 Get-AzSupportProblemClassification 來針對配額 SQL Managed 實例服務問題分類來取得正確的 Guid。
```powershell
...
### 範例16：指定個別客戶連絡人參數（而非 CustomerContactDetail 物件），以建立支援票證。 
```powershell
...
### 範例17：建立支援票證，並要求來自 Azure 的 24 x 7 回應。 
```powershell
...
### 範例18：如果您是雲端解決方案提供者 (CSP) ，請代表您的客戶建立支援票證。 CSP 應該先登入其租使用者，然後登入客戶的租使用者，如下列範例所示。 然後，必須使用-CSPHomeTenantId 參數來指定其在建立支援票證時的主租使用者識別碼。  
```powershell
...
### -CSPHomeTenantId
這是用來嘗試為客戶建立支援票證之雲端解決方案提供者的「家用租使用者識別碼」。
...
與 SupportTicket 資源相關聯的客戶連絡人詳細資料。
...
自訂的 Peferred 語言。
這必須是有效的語言 contry 程式碼，適用于此處所列的其中一個支援的語言 https://azure.microsoft.com/en-us/support/faq/ 。
...
### -ProblemClassificationId
每個 Azure 服務都有自己的一組問題類別，名為「問題分類」，且與您所遇到的問題類型相對應。
這個參數是 ProblemClassification 資源的 ARM 資源 id。
...
### -嚴重度
支援票證的嚴重度。
這會指出案例的緊急性，然後根據您所擁有的 Azure 技術支援方案服務層級合約來判斷回應時間。
...
這是 Azure 服務資源 (的資源識別碼，例如：建立支援票證的虛擬機器資源或 HDInsight 資源) 。
