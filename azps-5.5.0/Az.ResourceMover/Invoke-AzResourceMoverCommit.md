...
在確認資源前驗證 dependecies。

### 範例2：使用「MoveResource Name」作為輸入來確認移動集合中的資源集
```powershell
...
在移動集合中使用「MoveResource Name」做為輸入來認可資源集

### 範例3：使用 "SourceARMID" 做為輸入，在移動集合中確認資源集
```powershell
...
使用 "SourceARMID" 做為輸入來認可移動集合中的資源集
