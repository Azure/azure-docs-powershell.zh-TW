...
## 說明
**AzWebAppSSLBinding** Cmdlet 會從 Azure Web APP (SSL) 系結來移除安全通訊端層。
...
第一個命令使用 Get-AzWebApp Cmdlet 來建立名為 ContosoWebApp 的 Web App 的物件參考。
該物件參照會儲存在名為 $WebApp 的變數中。
第二個命令使用物件參照和 **AzWebAppSSLBinding** Cmdlet 來移除 SSL 系結。
