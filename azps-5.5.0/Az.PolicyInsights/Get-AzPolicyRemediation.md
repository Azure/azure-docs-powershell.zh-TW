...
## 說明
**AzPolicyRemediation** Cmdlet 會取得作用域或特定修正中的所有原則 remediations。

## 示例

### 範例1：取得目前訂閱中的所有原則 remediations
```
PS C:\> Select-AzSubscription -Subscription "My Subscription"
PS C:\> Get-AzPolicyRemediation
```

這個命令會取得在名為「我的訂閱」的訂閱或其下所建立的所有 remediations。
...
這個命令會從資源群組 ' myResourceGroup ' 取得名為「remediation1」的修正。 將會包含修正資源的詳細資料。

### 範例3：使用選擇性篩選，在管理群組中取得10個原則 remediations
```
...
這個命令會從名為「mg1」的管理群組中取得最多10個原則 remediations。 只會檢索指定原則指派的原則 remediations。
...
### -範圍
資源範圍。 例如，"/subscriptions/{subscriptionId}/resourceGroups/{rgName}"。
