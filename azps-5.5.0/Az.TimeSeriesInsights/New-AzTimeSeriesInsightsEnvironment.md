...
### -容量
Sku 的容量。
針對 Gen1 環境，您可以將這個值變更為支援在已建立環境之後，擴大規模。
...
將用來在環境中對資料進行分區的事件屬性清單。
若要建立，請參閱 PARTITIONKEYPROPERTY 屬性和建立雜湊資料表的備忘稿區段。
...
### -StorageAccountKey
提供存儲帳戶寫入時間數列 Insights 服務寫入存取權的管理金鑰值。
...
在超出環境容量的情況下，時間序列 Insights 服務應採取的行為
...
事件屬性的清單，將用來定義環境的時間序列 id。若要建立，請參閱 TIMESERIESIDPROPERTY 屬性和建立雜湊資料表的備忘稿區段。
...
ISO8601 timespan 指定環境事件將可供來自暖存放區之查詢的天數。
...
### IEnvironmentResource （TimeSeriesInsights）。 Api20200515。
...
PARTITIONKEYPROPERTY <ITimeSeriesIdProperty [] >：將用來在環境中劃分資料的事件屬性清單。
  - `[Name <String>]`：屬性的名稱。
  - `[Type <PropertyType?>]`：屬性的類型。

TIMESERIESIDPROPERTY <ITimeSeriesIdProperty [] >：將用來定義環境的時間序列 id 的事件屬性清單。
  - `[Name <String>]`：屬性的名稱。
  - `[Type <PropertyType?>]`：屬性的類型。
