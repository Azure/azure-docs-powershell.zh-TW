...
### 範例2：依 DigitalTwinsObject 和 CheckNameObject 檢查名稱。
```powershell
...
取得 DigitalTwinsInstance 並建立 Requset 物件，以測試名稱的可用性。
...
從資料庫檢查名稱可用性要求傳回的結果。
若要建立，請參閱 DIGITALTWINSINSTANCECHECKNAME 屬性和建立雜湊資料表的備忘稿區段。
...
### ICheckNameRequest （DigitalTwins）。 Api20201031。

### IDigitalTwinsIdentity （DigitalTwins）的

## 輸出

### ICheckNameResult （DigitalTwins）。 Api20201031。
...
DIGITALTWINSINSTANCECHECKNAME <ICheckNameRequest> ：從資料庫檢查名稱可用性要求傳回的結果。
  - `Name <String>`：資源名稱。
