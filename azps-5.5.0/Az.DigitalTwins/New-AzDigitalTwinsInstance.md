...
### 範例1：依預設建立 AzDigitalTwinsInstance。
```powershell
...
依預設建立 AzDigitalTwinsInstance

### 範例2：依 AzDigitalTwins 物件建立 AzDigitalTwinsInstance。
```powershell
...
依 AzDigitalTwins 物件建立 AzDigitalTwinsInstance
...
### -DigitalTwinsCreate
DigitalTwins 服務的描述。
若要建立，請參閱 DIGITALTWINSCREATE 屬性和建立雜湊資料表的備忘稿區段。
...
DIGITALTWINSCREATE <IDigitalTwinsDescription> ： DigitalTwins 服務的描述。
