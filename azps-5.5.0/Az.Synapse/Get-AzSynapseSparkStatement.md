...
### GetSparkStatementsByIdParameterSet (預設) 
```
...
## 說明
**AzSynapseSparkStatement** Cmdlet 會取得有關 Azure Synapse 分析 Spark 語句的資訊。
...
這個命令會在 Spark 會話下取得所有的 Spark 語句。
...
這個命令會取得具有指定 livy 識別碼的 Spark 語句。
...
這個命令會透過管線透過觸發會話來取得所有的 Spark 語句。
...
### PSSynapseSparkStatement 中的 Synapse。
