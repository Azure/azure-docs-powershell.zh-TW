...
## 說明
**Start AzSynapseTrigger** Cmdlet 會在工作區中啟動觸發程式。 如果觸發程式處於「已停止」狀態，則該 Cmdlet 會啟動觸發程式，而且最終會根據其定義來調用管線。 如果觸發程式已處於「已啟動」狀態，則此 Cmdlet 不會有任何作用。
...
在工作區 ContosoWorkspace 中啟動名為 ContosoTrigger 的觸發程式。
...
在工作區 ContosoWorkspace 透過管線啟動名為 ContosoTrigger 的觸發程式。
