...
## 說明
Enable-AzSynapseSqlPoolSensitivityRecommendation Cmdlet 會針對 SQL 池中的欄啟用敏感度建議。

## 示例

### 範例1：針對 Azure Synapse SQL pool 中的特定資料行啟用靈敏度建議。
```powershell
...
### 範例2：使用管道在特定欄的 Azure Synapse SQL pool 上啟用靈敏度建議。
```powershell
