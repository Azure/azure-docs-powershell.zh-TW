...
## 說明
Get-AzSynapseSqlPoolSensitivityRecommendation Cmdlet 會傳回建議的資訊類型，以及 SQL 池中欄的敏感度標籤。

## 示例

### 範例1：取得 Azure Synapse SQL pool 的建議資訊類型和敏感度標籤。
```powershell
...
### 範例2：使用管道取得 Azure Synapse SQL pool 的建議資訊類型及敏感度標籤。
```powershell
