...
## 說明
**AzSynapseTrigger** Cmdlet 會在工作區中建立觸發程式。 觸發程式是在「已停止」狀態中建立，這表示它們不會立即開始喚醒它們所參照的管線。
...
在工作區 ContosoWorkspace 中建立名為 ContosoTrigger 的新觸發程式。 觸發程式是以「已停止」狀態建立，表示它不會立即啟動。 您可以使用 Cmdlet 來啟動觸發程式 `Start-AzDataFactoryV2Trigger` 。
...
透過管線在工作區 ContosoWorkspace 中建立名為 ContosoTrigger 的新觸發程式。 觸發程式是以「已停止」狀態建立，表示它不會立即啟動。 您可以使用 Cmdlet 來啟動觸發程式 `Start-AzDataFactoryV2Trigger` 。
