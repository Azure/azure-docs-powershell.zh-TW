...
## 說明
**AzSynapseTrigger** Cmdlet 會在工作區中停止觸發程式。 如果觸發程式處於「已啟動」狀態，則該 Cmdlet 會停止觸發程式，而且不會再叫管線。 如果觸發程式已處於「已停止」狀態，則此 Cmdlet 沒有任何作用。
...
在工作區 ContosoWorkspace 中停止名為 ContosoTrigger 的觸發程式。
...
在工作區 ContosoWorkspace 透過管線停止名為 ContosoTrigger 的觸發程式。
