...
## 說明
**AzSynapseSqlDatabase** Cmdlet 會取得有關 Azure SYNAPSE 分析 SQL 資料庫的資訊。
...
這個命令會建立 Azure Synapse 分析 SQL 資料庫。
...
### -排序規則
排序規則定義排序與比較資料的規則，且無法在建立 SQL 池之後進行變更。
預設的排序規則為 [SQL_Latin1_General_CP1_CI_AS]。
