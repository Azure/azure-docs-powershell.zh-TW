...
### 範例1：使用名稱重新產生 blockchain 成員的 Api 金鑰
```powershell
...
這個命令會使用名稱、資源群組，為 blockchain 成員重新產生 Api 金鑰。

### 範例2：使用 idenity 重新產生 blockchain 成員的 Api 金鑰
```powershell
PS C:\> $keyPair = Get-AzBlockchainMemberApiKey -BlockchainMemberName myblockchainhlqc92 -ResourceGroupName bc-rg
...
這個命令會使用身分識別為 blockchain 成員重新產生 Api 金鑰。
