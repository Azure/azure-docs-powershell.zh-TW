...
## 說明
這個命令會建立 Azure 檔案同步處理雲端端點。 雲端端點是對現有 Azure 檔案共用的參照。 它代表檔案共用，並定義 it 參與同步處理已在其中建立雲端端點之同步群組的所有檔案。
...
雲端端點是同步群組的一個有機成員，這是在名為「mySyncGroupName」的現有同步處理群組內建立一個範例。
...
### -名稱
雲端端點的名稱。 透過 Azure 入口網站建立時，名稱會設定為它所參照之 Azure 檔案共用的名稱。
...
### PSCloudEndpoint 中的 StorageSync。
