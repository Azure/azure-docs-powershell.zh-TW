...
SUBSCRIPTIONDEFINITION <ISubscriptionDefinition> ：訂閱物件屬性。
  - `[DelegatedProviderSubscriptionId <String>]`：父 DelegatedProvider 訂閱識別碼。
  - `[DisplayName <String>]`：訂閱名稱。
  - `[ExternalReferenceId <String>]`：外部參照識別碼。
  - `[Id <String>]`：完全限定識別碼。
  - `[OfferId <String>]`：受委派提供者範圍內提供的識別碼。
  - `[Owner <String>]`：訂閱擁有者。
  - `[RoutingResourceManagerType <ResourceManagerType?>]`：路由資源管理器類型。
