...
### -PlanLink
將方案連結和取消連結至提供的定義。
若要建立，請參閱 PLANLINK 屬性和建立雜湊資料表的備忘稿區段。
...
### IPlanLinkDefinition （SubscriptionsAdmin）。 Api20151101。
...
PLANLINK <IPlanLinkDefinition> ：將方案連結和取消連結至提供的定義。
  - `[MaxAcquisitionCount <Int32?>]`：由訂閱者所取得的最大購置計數
  - `[PlanLinkType <PlanLinkType?>]`：方案連結的類型。
