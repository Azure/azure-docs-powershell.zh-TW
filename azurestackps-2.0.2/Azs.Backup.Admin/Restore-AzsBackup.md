...
### -RestoreOption
[還原選項] 的屬性。
若要建立，請參閱 RESTOREOPTION 屬性和建立雜湊資料表的備忘稿區段。
...
### -擁有方
要還原的 Azure 堆疊角色名稱，針對所有結構角色將它設為空白
...
RESTOREOPTION <IRestoreOptions> ：還原選項的屬性。
  - `[DecryptionCertBase64 <String>]`：以 Base64 字串的憑證檔案原始資料。 這應該是具有私用金鑰的 .pfx 檔案。
  - `[DecryptionCertPassword <String>]`：解密憑證的密碼。
  - `[RoleName <String>]`：用於還原的 Azure 堆疊角色名稱，針對所有結構角色將它設為空白
