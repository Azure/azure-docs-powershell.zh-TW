...
* 精簡 Cmdlet 失敗的錯誤訊息。
* 升級的例外狀況處理，以使用 Azure PowerShell 相關的例外狀況。
...
* 依備份服務修改原則驗證限制。
* 已新增復原服務保存庫的區域冗余。 
* Azure Site Recovery VMware 至 Azure 和 HyperV 對 Azure 提供者的鄰近放置群組支援。
* Azure Site Recovery VMware 至 Azure 和 HyperV 對 Azure 提供者的可用性區域支援。
* 針對 UseManagedDisk for HyperV 至 Azure 提供者的支援 Azure Site Recovery
...
* Alexander Schmidt (@devdeer-alex) ，已修正 ( # 14009 的貢獻指導方針連結) 
...
* @Willem-J-an，修正導致 Docker 密碼從 appsettings 中的 Set-azwebapp (位置)  ( # 13866 中移除的 bug) 
