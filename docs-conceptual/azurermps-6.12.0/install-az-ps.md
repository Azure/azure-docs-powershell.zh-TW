...
本文說明如何使用 PowerShellGet 安裝 Azure PowerShell 模組。 在 Az 預覽版本中，不支援任何其他安裝方法。 
...
若要開始使用 Azure PowerShell，您必須使用 [Import-Module](/powershell/module/Microsoft.PowerShell.Core/Import-Module) Cmdlet 將 `Az` 載入您目前的 PowerShell 工作階段，然後使用您的 Azure 認證登入。
...
您可使用 `-RequiredVersion` 引數搭配 `Install-Module` 或 `Import-Module`，以載入特定版本的 `Az` 模組：
