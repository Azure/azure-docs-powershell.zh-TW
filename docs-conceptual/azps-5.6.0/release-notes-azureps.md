...
## <a name="560---march-2021"></a>5.6.0-2021 年3月
#### <a name="azaccounts"></a>Az.Accounts
* 升級 Azure 身分識別，以修正使用 ADFS 認證時 Connect-AzAccount 失敗的問題 [#13560]

#### <a name="azautomation"></a>Az.Automation
* 修正了無法正確序列化字串的問題。 [#14215]
* 已新增 Python3 Runbook 類型的支援

#### <a name="azcompute"></a>Az.Compute
* 已將參數 '-EnableHotpatching ' 新增至 Windows 機器的 ' >set-azvmoperatingsystem ' Cmdlet。 
* 已將參數 '-PatchMode ' 新增至 Cmdlet ' >set-azvmoperatingsystem ' 中的 Linux 參數集。 
* [重大變更]針對 VM 來賓修補功能公開預覽中的使用者所進行的重大變更。
    - 已從 ' LastPatchInstallationSummary ' 物件中移除屬性 ' RebootStatus '。 
    - 已從 ' LastPatchInstallationSummary ' 物件中移除屬性 ' StartedBy '。
    - 已將屬性 ' Kbid ' 重新命名為 ' VirtualMachineSoftwarePatchProperties ' 物件中的 ' KbId '。 
    - 已將屬性 ' 修補程式 ' 重新命名為 ' VirtualMachineAssessPatchesResult ' 物件中的 ' availablePatches '。 
    - 已將物件 ' SoftwareUpdateRebootBehavior ' 重新命名為 ' VMGuestPatchRebootBehavior ' （.....）。
    - 已將物件 ' InGuestPatchMode ' 重新命名為 ' WindowsVMGuestPatchMode ' （.....）。
* [重大變更]已移除所有 ' >microsoft.containerservice ' Cmdlet。 容器服務 API 已于2020年1月淘汰。 
    - ' AzContainerServiceAgentPoolProfile '
    - ' AzContainerService '
    - ' AzContainerService '
    - ' AzContainerServiceConfig '
    - ' AzContainerService '
    - ' AzContainerServiceAgentPoolProfile '
    - ' AzContainerService '

#### <a name="azcontainerregistry"></a>Az.ContainerRegistry
* 固定的驗證 `Connect-AzContainerRegistry`

#### <a name="azcosmosdb"></a>Az. CosmosDB
* 引進了資料庫帳戶 Cmdlet 的 NetworkAclBypass 和 NetworkAclBypassResourceIds。
* 已將 ServerVersion 參數引進至 AzCosmosDBAccount。
* 引進了資料庫帳戶 Cmdlet 的 BackupInterval 和 BackupRetention

#### <a name="azdatafactory"></a>Az.DataFactory
* 已將 ADF .Net SDK 版本更新為4.14。0

#### <a name="azmigrate"></a>Az. 遷移
* Az. 遷移 GA
* 從目前執行的外部腳本，將 Initialize-AzMigrateReplicationInfrastructure 併入 Az. 遷移模組中的 Cmdlet。
* 將 AzMigrateServerReplication 的一些參數設為不區分大小寫 New-AzMigrateDiskMapping。
* 新增了調整設備變更的支援，以處理新的 V3 金鑰。

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* 已針對檔案共用還原中的目標儲存體帳戶新增 null 檢查。

#### <a name="azresources"></a>Az.Resources
* 已新增 Bicep 語言的 Azure 資源部署支援
* 修正了 ' AzTenantDeployment ' 和 ' AzManagementGroupDeployment ' 中的 TemplateSpec 部署問題
* 已在 ' Test-Az * 部署 ' Cmdlet 中新增 '-QueryString ' 參數的支援
* 修正了 ' New-Az * 部署 ' 與 '-QueryString ' 搭配使用時，動態參數的問題
* 已新增 '-TemplateParameterObject ' 參數的支援，但在 ' New-Az * 部署 ' Cmdlet 中使用 '-TemplateSpecId ' 參數
* 修正嘗試部署不存在的範本規格時所收到的不正確錯誤訊息

#### <a name="azstorage"></a>Az.Storage
* 升級至19.0.0，以支援新的 API 版本2021-01-01。
* NetworkRuleSet 中支援的資源存取規則
    - 'Update-AzStorageAccountNetworkRuleSet'
    - 'Add-AzStorageAccountNetworkRule'
    - ' AzStorageAccountNetworkRule '
* 管理原則中支援的 Blob 版本和附加 Blob 類型
    - 'Add-AzStorageAccountManagementPolicyAction'
    - ' AzStorageAccountManagementPolicyFilter '
    - 'Set-AzStorageAccountManagementPolicy'
* 使用 AllowSharedKeyAccess 支援的建立/更新帳戶
    - 'New-AzStorageAccount'
    - 'Set-AzStorageAccount'
* 使用 RequireInfrastructureEncryption 支援的建立加密範圍
    - 'New-AzStorageEncryptionScope'
* 以加密範圍同步支援的複製區塊 blob
    - ' AzStorageBlob '
* 修正 Get-AzStorageBlobContent 在 Linux 和 MacOS 上使用錯誤的目錄分隔符號字元的問題 [#14234]

#### <a name="azwebsites"></a>Az.Websites
* 引進了一個選項，可為 ' Set-azwebapp ' 提供自訂的超時 
* 已新增對 App Service 環境的支援
    - ' AzAppServiceEnvironment '
    - ' AzAppServiceEnvironment '
    - ' AzAppServiceEnvironment '
    - ' AzAppServiceEnvironmentInboundServices '

### <a name="thanks-to-our-community-contributors"></a>感謝我們的社群參與者
* @alunmj、小型拼寫、格式化變更 (#14155) 
* @chakra146，更新 Add-AzLoadBalancerInboundNatPoolConfig.md (#14231) 
* Ehrnst () 的聖馬丁 @ehrnst ，修正了 Cmdlet (#14112 的打字錯誤) 
* Jan David Narkiewicz (@jdnark) 
  * 使用的範例 New-AzAks 是舊版 Cmdlet 和 AzAksCluster 的別名。 已將範例變更為使用 New-AzAksCluster 這是此檔頁面的目標 Cmdlet。  (#14088) 
  * 類型 fox：已將 SshKeyVaule 變更為 SshKeyValue (#14087) 
* Ivan Kulezic (@kukislav) ，新增 sql mi 維護設定範例 (#14216) 
* @webguynj，更新 Set-AzNetworkSecurityRuleConfig.md (#14176) 
* Yannick Dils (@yannickdils) ，將額外的 Cmdline 新增至範例，以將變更套用至負載平衡器 (#14185) 

## <a name="550---february-2021"></a>5.5.0-2021 年2月
#### <a name="azaccounts"></a>Az.Accounts
* 例外狀況中追蹤的 CloudError 程式碼
* 執行 ' Clear Set-azcoNtext ' 時引發 ' CoNtextCleared ' 事件

#### <a name="azaks"></a>Az.Aks
* 精簡 Cmdlet 失敗的錯誤訊息。
* 已將例外狀況處理升級為使用 Azure PowerShell 相關的例外狀況。
* 修正使用者無法使用提供的服務主體來建立 Kubernetes 叢集的問題。 [#13938]

#### <a name="azautomation"></a>Az.Automation
* 已修正處理 ' PSCustomObject ' 和 ' Array ' 的問題。

#### <a name="azcompute"></a>Az.Compute
* 已將參數 '-EnableAutomaticUpgrade ' 新增至 ' >remove-azvmextension ' 和 ' >add-azvmssextension '。
* 已從 ' >get-azvmimage ' Cmdlet 檔中移除 FilterExpression 參數。 
* 已將取代訊息新增至 >microsoft.containerservice Cmdlet：
    - ' AzureRmContainerServiceAgentPoolProfileCommand '
    - ' AzContainerService '
    - ' AzContainerService '
    - ' AzContainerServiceConfig '
    - ' AzContainerService '
    - ' AzContainerServiceAgentPoolProfile '
    - ' AzContainerService '
* 已將參數 '-BurstingEnabled ' 新增至 ' New-azdiskconfig ' 和 ' AzDiskUpdateConfig '
* 已將 '-GroupByApplicationId ' 和 '-GroupByUserAgent ' 參數新增至 ' Export-AzLogAnalyticThrottledRequest ' 和 ' Export-AzLogAnalyticRequestRateByInterval ' Cmdlet。
* 已將 ' VMParameterSet ' 參數設定為 ' >remove-azvmextension ' Cmdlet。 已將新的參數 '-VM ' 新增至此參數集。 

#### <a name="azcontainerregistry"></a>Az.ContainerRegistry
* 已將 Cmdlet 新增至支援的存放庫、資訊清單和標記作業：
    - ' AzContainerRegistryRepository '
    - ' AzContainerRegistryRepository '
    - ' AzContainerRegistryRepository '
    - ' AzContainerRegistryManifest '
    - ' AzContainerRegistryManifest '
    - ' AzContainerRegistryManifest '
    - ' AzContainerRegistryTag '
    - ' AzContainerRegistryTag '
    - ' AzContainerRegistryTag '

#### <a name="azdatabricks"></a>Az.Databricks
支援-在建立 Databricks 工作區時 EnableNoPublicIP

#### <a name="azfrontdoor"></a>Az.FrontDoor
* 已將 FrontDoorId 新增至屬性
* 已將 JSON 排除專案和 RequestBodyCheck 支援新增至受控規則

#### <a name="azhdinsight"></a>Az.HDInsight
* 已將新的參數 '-EnableComputeIsolation ' 和 '-ComputeIsolationHostSku ' 新增至 Cmdlet ' >new-azhdinsightcluster '，以支援計算隔離功能
* 已在類別 AzureHDInsightCluster 中新增屬性 ' ComputeIsolationProperties ' 和 ' ConnectivityEndpoints '。

#### <a name="azkeyvault"></a>Az.KeyVault
* 在透過 BYOK 檔案匯入金鑰時，支援指定金鑰類型和曲線名稱

#### <a name="aznetwork"></a>Az.Network
* 已新增新的 Cmdlet，以將舊的產品名稱 ' virtual 路由器 ' 取代為新名稱 ' route server ' （未來）。
    - ' AzRouteServer '
    - ' AzRouteServer '
    - ' AzRouteServer '
    - ' AzRouteServer '
    ...
    - ' AzRouteServerPeer '
    - ' AzRouteServerPeer '
    - 已將取代屬性警告新增至舊的 Cmdlet。
* 修正 ExpressRouteLink MacSecConfig 中的 Bug。 已將新的屬性 ' SciState ' 新增至 ' PSExpressRouteLinkMacSecConfig '
* 更新 Get-AzVirtualNetworkGatewayConnectionIkeSa 的格式清單和格式資料表視圖

#### <a name="azpolicyinsights"></a>Az.PolicyInsights
* 撤銷在 powershell 中所做的變更，以增加要求的資料列限制。 已移除不正確的支援分頁語句

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* 依備份服務修改原則驗證限制。
* 已新增復原服務保存庫的區域冗余。 
* Azure Site Recovery 支援將 VMware 至 Azure 和 HyperV 的鄰近放置群組提供給 Azure 提供者。
* 適用于 VMware 至 Azure 和 HyperV 至 Azure 提供者之可用性區域的 Azure Site Recovery 支援。
* 適用于 HyperV 到 Azure 提供者之 UseManagedDisk 的 azure Site Recovery 支援

#### <a name="azresources"></a>Az.Resources
* 已移除 New-AzRoleAssignment 和 Set-AzRoleAssignment 上的主體類型，因為目前的對應中斷了某些案例

#### <a name="azsql"></a>Az.Sql
* 已將 MaintenanceConfigurationId 新增至 ' >set-azsqldatabase '、' >set-azsqldatabase '、' New-AzSqlElasticPool ' 和 ' Set-AzSqlElasticPool '
* 修正了在提供 Predicateexpression 結構引數時，' Set-AzSqlServerAudit ' 中的回歸

#### <a name="azstorage"></a>Az.Storage
* 建立/更新儲存體帳戶中支援的 RoutingPreference 設定
    - 'New-AzStorageAccount'
    - 'Set-AzStorageAccount'
* 已將 Azure 12.8.0 升級為
* 已將12.6.0 升級為共用
* 將 DataLake 升級至12.6。0

#### <a name="azwebsites"></a>Az.Websites
* 已將匯入金鑰保存庫憑證的支援新增至 WebApp。

### <a name="thanks-to-our-community-contributors"></a>感謝我們的社群參與者
* @atul-ram，更新 Set-AzEventHub.md (#13921) 
* Christoph Bergmeister [MVP] (@bergmeister) 、Set-AzDataLakeGen2AclRecursive.md-修正打字錯誤 (directiry > 目錄)  (#14082) 
* Alexander Schmidt (@devdeer-alex) ，已修正貢獻方針 (#14009 的中斷連結) 
* @JiangYuchun，更新 Get-AzApplicationGatewayAuthenticationCertificate.md (#13972) 
* Sebastian Olsen (@Spacebjorn) ，已更正的範例命令 (#13901) 

## <a name="540---january-2021"></a>5.4.0-2021 年1月
#### <a name="azaccounts"></a>Az.Accounts
* 在 debug 訊息上顯示正確的用戶端要求識別碼 [#13745]
* 已新增常見的 Azure PowerShell 例外狀況類型
* 支援的儲存體 API 2019-06-01
...
#### <a name="azcosmosdb"></a>Az. CosmosDB
* ' Az. CosmosDB ' 模組正式推出
* 限制 New-AzCosmosDBAccount Cmdlet，以對現有的資料庫帳戶進行更新呼叫。
* SqlContainer 中的 AnalyticalStorageTTL 簡介。
...
* 修正了「AzLogicAppTriggerHistory」和「AzLogicAppRunAction」只會在第一頁結果中取得 [#9141] 的問題

#### <a name="azmonitor"></a>Az.Monitor
* 已新增資料收集規則的 Cmdlet： 
    - ' AzDataCollectionRule '
    - ' AzDataCollectionRule '
    - ' AzDataCollectionRule '
    - ' AzDataCollectionRule '
    - ' AzDataCollectionRule '
* 已新增資料收集規則關聯的 Cmdlet
    - ' AzDataCollectionRuleAssociation '
    - ' AzDataCollectionRuleAssociation '
    - ' AzDataCollectionRuleAssociation '
...
* 已更新 Cmdlet，以在 VpnGateway 資源上設定 NATRule，並將其與 VpnSiteLinkConnection 資源建立關聯。
    - ' AzVpnGateway '
    - ' AzVpnGateway ' 
    - ' AzVpnSiteLinkConnection '
* 已更新 Cmdlet，可讓您在虛擬網路閘道連線上設定 ConnectionMode。
    - 'New-AzVirtualNetworkGatewayConnection'
    - 'Set-AzVirtualNetworkGatewayConnection'
* 已更新 ' AzFirewallPolicyApplicationRule ' Cmdlet：
    - 已新增參數 TargetUrl
    - 已新增參數 TerminateTLS
* 已新增 Azure 防火牆 Premium 功能的新 Cmdlet
    - ' AzFirewallPolicyIntrusionDetection '
    - ' AzFirewallPolicyIntrusionDetectionBypassTraffic '
    - ' AzFirewallPolicyIntrusionDetectionSignatureOverride '
* 已更新 New-AzFirewallPolicy Cmdlet：
    - 已新增參數-SkuTier
    - 已新增參數-身分識別
    - 已新增參數-UserAssignedIdentityId
    - 已新增參數-IntrusionDetection
    - 已新增參數-TransportSecurityName
    - 已新增參數-TransportSecurityKeyVaultSecretId
* 已新增新的 Cmdlet，以提取虛擬網路閘道連線的 IKE 安全性關聯。
    - ' AzVirtualNetworkGatewayConnectionIkeSa '
* 為 p2sVpnGateway 新增了多個驗證支援
    - 已更新 New-AzVpnServerConfiguration 和 Update-AzVpnServerConfiguration，以允許設定多個驗證參數。
* 已更新 ' AzVpnGateway ' 和 ' AzP2sVpnGateway ' Cmdlet：
    - 已新增參數 EnableRoutingPreferenceInternetFlag

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* 已新增跨區域還原功能。  
* 當目標專案為可用性群組時，封鎖取得工作負載設定。

#### <a name="azresources"></a>Az.Resources
* 在新的 Az * 部署 Cmdlet 中新增-QueryString 參數的支援

#### <a name="azsql"></a>Az.Sql
* 已將 ' AzSqlInstanceDatabaseLogReplay ' Cmdlet 設為同步，已新增-AsJob 旗標

#### <a name="azstorage"></a>Az.Storage
* 修正當清單 blob 與-IncludeVersion 時，ContinuationToken 絕不會是 null
    - 'Get-AzStorageBlob'

#### <a name="azwebsites"></a>Az.Websites
* 已新增對 App Service 受控憑證的支援
    - ' AzWebAppCertificate '
    - ' AzWebAppCertificate '
* 已修正導致 Docker 密碼從 ' Set-azwebapp ' 和 ' Set-azwebappslot ' 中的 appsettings 中移除的問題

### <a name="thanks-to-our-community-contributors"></a>感謝我們的社群參與者
* Ivan Akcheurov (@ivanakcheurov) ，更新 Set-AzSecurityWorkspaceSetting.md (#13877) 
* @javiermarasco、更新範例 (#13837) 
...
* Michael James (@mikejwhat) ，可讓 Get-AzLogicAppTriggerHistory 和 Get-AzLogicAppRunAction 傳回超過30個結果 (#13846) 
* @Willem-J-an，修正錯誤，導致在 Set-azwebapp (位置中從 appsettings 移除 Docker 密碼)  (#13866) 

## <a name="530---december-2020"></a>5.3.0 - 2020 年 12 月
#### <a name="azaccounts"></a>Az.Accounts
* 已修正 Windows PowerShell 中不會遵守 Http Proxy 的問題 [#13647]
* 已改善所產生模組中長時間執行作業的偵錯記錄

#### <a name="azautomation"></a>Az.Automation
* 已修正 'Start-AzAutomationRunbook' 參數無法將 PSObject 所包裝的字串轉換為 JSON 字串的問題 [#13240]
* 已修正 New-AzAutomationUpdateManagementAzureQuery Cmdlet 的 Location 完成碼

#### <a name="azcompute"></a>Az.Compute
* 已將新參數集 'VMParameterSet' 中的新參數 'VM' 新增至 'Get-AzVMDscExtensionStatus' 和 'Get-AzVMDscExtension' Cmdlet。 

#### <a name="azdatabricks"></a>Az.Databricks
* 已修正可能導致 'New-AzDatabricksVNetPeering' 在完全佈建之前就傳回的問題 (https://github.com/Azure/autorest.powershell/issues/610)

#### <a name="azdatafactory"></a>Az.DataFactory
* 已修正 SupportsShouldProcess 問題的 'Invoke-AzDataFactoryV2Pipeline' 命令

#### <a name="azdesktopvirtualization"></a>Az.DesktopVirtualization
* 已將 StartVMOnConnect 屬性新增至 hostpool。

#### <a name="azhdinsight"></a>Az.HDInsight
* 已新增屬性：AzureHDInsightHostInfo 類別中的 Fqdn 和 EffectiveDiskEncryptionKeyUrl。

#### <a name="azkeyvault"></a>Az.KeyVault
* 已將新的參數 '-AsPlainText' 新增至 'Get-AzKeyVaultSecret'，以直接以純文字傳回祕密 [#13630]
* 已支援從受管理的 HSM 完整備份中選擇性地還原金鑰 [#13526]
* 已修正一些小問題 [#13583] [#13584]
* 已在 SecretManagement 模組中新增遺失的 'Get-Secret' 傳回物件
* 已修正可能導致不使用預設存取原則來建立保存庫的問題 [#13687]

#### <a name="azkusto"></a>Az.Kusto
* 已將 API 版本更新為 2020-09-18。

#### <a name="aznetwork"></a>Az.Network
* 已修正在 ExpressRouteCircuit 案例中移除對等互連和連線 Cmdlet 時所發生的問題
    - 'Remove-AzExpressRouteCircuitPeeringConfig' 和 'Remove-AzExpressRouteCircuitConnectionConfig'

#### <a name="azpolicyinsights"></a>Az.PolicyInsights
* 已針對 Get-AzPolicyState 新增可傳回編頁結果的支援

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* 已啟用 SQL 的 softdelete 功能。
* 已修正 SQL AG 還原並移除容器名稱檢查。
* 已變更 Azure 檔案儲存體備份項目的容器名稱格式。
* 已新增復原服務保存庫的 CMK 功能支援。

#### <a name="azresources"></a>Az.Resources
* 已修正 'New-AzureManagedApplication' 和 'Set-AzureManagedApplication' 中的 NullRef 例外狀況問題。
* 已將 Azure Resource Manager SDK 更新為使用最新的 DeploymentScripts 正式發行 API 版本：2020-10-01。

#### <a name="azservicefabric"></a>Az.ServiceFabric
* 已修正 'Add-AzServiceFabricNodeType'。 已在建立虛擬機器擴展集之前，將節點類型新增至 Service Fabric 叢集。

#### <a name="azsql"></a>Az.Sql
* 已修正 'InstanceFailoverGroup' 命令的參數描述。
* 已更新用來從 SQL 資料分類命令的識別碼中擷取 schemaName、tableName 和 columnName 的邏輯。
* 已修正 'Get-AzSqlDatabaseImportExportStatus' 中的 Status 和 StatusMessage 欄位以符合文件
* 已新增 Microsoft 支援作業 (DevOps) 稽核 Cmdlet：Get-AzSqlServerMSSupportAudit、Set-AzSqlServerMSSupportAudit、Remove-AzSqlServerMSSupportAudit

#### <a name="azstorage"></a>Az.Storage
* 已支援儲存體帳戶的建立/更新/取得/列出 EncryptionScope
    - 'New-AzStorageEncryptionScope'
    - 'Update-AzStorageEncryptionScope'
    - 'Get-AzStorageEncryptionScope'
* 已支援使用 [加密範圍] 設定來建立容器和上傳 Blob
    - 'New-AzRmStorageContainer'
    - 'New-AzStorageContainer'
    - 'Set-AzStorageBlobContent'

### <a name="thanks-to-our-community-contributors"></a>感謝我們的社群參與者
* Andreas Wolter (@AndreasWolter)，移除行銷語言，提供更好的篩選範例 (#13671)
* Tidjani Belmansour (@BelRarr)，更新 Get-AzBillingInvoice.md (#13634)
* David Klempfner (@DavidKlempfner)
  * 修正拼字錯誤 (#13677)
  * 更新 PSMetricNoDetails.cs (#13676)
* @kilobyte97，「remove Cmdlet」的錯誤修正以刪除設定 (#13655)
* @kongou-ae，更新 Set-AzFirewall.md (#13727)
* @MasterKuat，修正文件中標題和程式碼之間的交換 (#13666)
* NickT (@nukeulis)，更新 Set-AzContext.md (#13702)
* @PaulHCode，更新 Start-AzJitNetworkAccessPolicy.md - 修正範例以顯示所示範的適當 Cmdlet (#13713)
* Ryan Borstelmann (@ryanborMSFT)，移除了訂用帳戶識別碼 (#13715)
...
## <a name="520---december-2020"></a>5.2.0 - 2020 年 12 月
#### <a name="azaccounts"></a>Az.Accounts
* 如果無法從基礎程式庫取得，則會受控以剖析原始權杖中的 ExpiresOn 時間
* 已改善互動式驗證無法使用時的警告訊息

#### <a name="azapimanagement"></a>Az.ApiManagement
* [重大變更] 根據預設，'New-AzApiManagementProduct' 沒有訂用帳戶限制。

#### <a name="azcompute"></a>Az.Compute
* 已編輯 Get-AzVm，以在檢查節流之前，先依據 '-Name' 進行篩選，因為資源太多。 
* 新的 Cmdlet 'Start-AzVmssRollingExtensionUpgrade'。

#### <a name="azcontainerregistry"></a>Az.ContainerRegistry
* 支援 'Get-AzContainerRegistryUsage' 的管線輸入參數 'Name' 與管線輸入中的值 [#13605]
* 改善 'Connect-AzContainerRegistry' 的例外狀況

#### <a name="azdatafactory"></a>Az.DataFactory
* 已將 ADF .Net SDK 版本更新為 4.13.0
...
#### <a name="azkeyvault"></a>Az.KeyVault
* 在設定金鑰保存庫存取原則時，支援 'all' 做為選項
* 支援 SecretManagement 模組的新版本 [#13366]
* 支援 SecretManagementModule 中 'SecretValue' 的 ByteArray、String、PSCredential 和 Hashtable [#12190]
* [重大變更] 重新設計與受控 HSM 相關 Cmdlet 的 API 介面。

#### <a name="azmonitor"></a>Az.Monitor
* 已將 'New-AzAutoscaleProfile' 的參數 'Rule' 變更為接受空白清單。 [#12903]
* 已新增新的 Cmdlet，以支援建立更有彈性的診斷設定：
    * 'Get-AzDiagnosticSettingCategory'
    * 'New-AzDiagnosticSetting'
    * 'New-AzDiagnosticDetailSetting'

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* 將說明文字和參數集名稱變更為 'Restore-AzRecoveryServicesBackupItem' Cmdlet。

#### <a name="azresources"></a>Az.Resources
* 已將 '-Tag' 參數支援新增至 'Set-AzTemplateSpec' 和 'New-AzTemplateSpec'
* 已將標籤顯示支援新增至範本規格的預設格式器 

#### <a name="azservicefabric"></a>Az.ServiceFabric
* 已將範例新增至具有 SettingsSectionDescription param 的 'Set-AzServiceFabricSetting'
* 已將應用程式相關的 Cmdlet 更新為呼叫支援僅適用於 ARM 部署的資源
* 標示為已淘汰的叢集憑證 Cmdlet 'Add-AzureRmServiceFabricClusterCertificate' 和 'Remove-AzureRmServiceFabricClusterCertificate'

#### <a name="azsql"></a>Az.Sql
* 已將 SecondaryType 新增至下列內容： 
    - 'New-AzSqlDatabase'
    - 'Set-AzSqlDatabase'
    - 'New-AzSqlDatabaseSecondary'
* 已將 HighAvailabilityReplicaCount 新增至下列內容： 
    - 'New-AzSqlDatabase'
    - 'Set-AzSqlDatabase'
* 使 ReadReplicaCount 成為下列內容中的 HighAvailabilityReplicaCount 別名： 
    - 'New-AzSqlDatabase'
    - 'Set-AzSqlDatabase'

#### <a name="azstorage"></a>Az.Storage
* 支援上傳 Azure 檔案大小上限至 4 TiB
    - 'Set-AzStorageFileContent'
* 已將 Azure.Storage.Blobs 升級至 12.7.0
* 已將 Azure.Storage.Files.Shares 升級至 12.5.0
* 已將 Azure.Storage.Files.DataLake 升級至 12.5.0

#### <a name="azstoragesync"></a>Az.StorageSync
* 已新增具有下載原則和本機快取模式的同步階層處理原則功能

#### <a name="azwebsites"></a>Az.Websites
* 防止重複存取限制規則

### <a name="thanks-to-our-community-contributors"></a>感謝我們的社群參與者
* Andrew Dawson (@dawsonar802)，升級 Get-AzKeyVaultCertificate.md - 取得憑證並將其另存為 pfx 區段，以使用 PowerShell Core (#13557)
* @iviark，醫療保健 APIs Powershell BYOK 更新 (#13518)
* John Duckmanton (@johnduckmanton)，修正 TagPatchOperation 的拼寫 (#13508)
...
* @SteppingRazor，New-AzApiManagementProduct：將 SubscriptionsLimit 參數的預設值變更為 None (#13457)
* Steve Burkett (@SteveBurkettNZ)，修正範例中 WorkspaceResourceId 參數的打字錯誤 (#13589)

## <a name="510---november-2020"></a>5.1.0 - 2020 年 11 月
#### <a name="azaccounts"></a>Az.Accounts
* 已修正使用 ''Connect-AzAccount -DeviceCode' 時，可能不會遵守 TenantId 的問題 [#13477]
* 已新增 Cmdlet 'Get-AzAccessToken'
* 已修正無法存取使用者設定檔路徑時發生錯誤的問題
* 已修正 Connect-AzAccount 期間發生 Write-Object 錯誤的問題 [#13419]
* 已將參數 'ContainerRegistryEndpointSuffix' 新增至：'Add-AzEnvironment'、'Set-AzEnvironment' 
* 支援點擊 <kbd>CTRL</kbd>+<kbd>C</kbd> 來中斷登入
* 已修正導致 'Connect-AzAccount -KeyVaultAccessToken' 無法運作的問題 [#13127]
* 已修正 'Invoke-AzRestMethod' 中 Null 參考和方法不區分大小寫的問題

#### <a name="azaks"></a>Az.Aks
* 已修正使用者無法使用服務主體建立新 Kubernetes 叢集的問題。 [#13012]

#### <a name="azappconfiguration"></a>Az.AppConfiguration
* 'Az.AppConfiguration' 模組的正式推出

#### <a name="azdatafactory"></a>Az.DataFactory
* 已改善 'New-AzDataFactoryV2LinkedServiceEncryptedCredential' 命令的錯誤訊息

#### <a name="azdatalakestore"></a>Az.DataLakeStore
* 已將 ADLS 資料平面 SDK 更新為 1.2.4-alpha。 變更： https://github.com/Azure/azure-data-lake-store-net/blob/preview-alpha/CHANGELOG.md#version-124-alpha

#### <a name="azdesktopvirtualization"></a>Az.DesktopVirtualization
* 已新增 MSIX Package Cmdlet 和更新應用程式 Cmdlet。

#### <a name="azeventhub"></a>Az.EventHub
* 已修正 EventHub 叢集沒有標記時的叢集命令
* 已更新 AzEventHubGeoDRConfiguration 的 PartnerNamespace 命令解說文字 

#### <a name="azhdinsight"></a>Az.HDInsight
* 已將 'ResourceProviderConnection' 和 'PrivateLink' 參數新增至 'New-AzHDInsightCluster' Cmdlet 以支援轉送輸出和私人連結功能
* 已將 'AmbariDatabase' 參數新增至 'New-AzHDInsightCluster' Cmdlet，以支援自訂 Ambari 資料庫功能
* 將接受值 'AmbariDatabase' 新增至 'Add-AzHDInsightMetastore' Cmdlet 的 'MetastoreType' 參數

#### <a name="aziothub"></a>Az.IotHub
* 允許在 IoT 中樞建立 Cmdlet 中使用標記。

#### <a name="azkeyvault"></a>Az.KeyVault
* 支援更新金鑰保存庫標記

#### <a name="azlogicapp"></a>Az.LogicApp
* 已修正 Get-AzLogicAppRunHistory 只會取得結果第一頁的問題
...
* 已更新下列 Cmdlet 
    - 'New-AzLoadBalancerFrontendIpConfigCommand'、'Set-AzLoadBalancerFrontendIpConfigCommand'、'Add-AzLoadBalancerFrontendIpConfigCommand'：
        - 已新增 PublicIpAddressPrefix 屬性
        - 已新增 PublicIpAddressPrefixId 屬性
* 已將新屬性新增至下列 Cmdlet，以允許進行全域負載平衡
    - 'New-AzLoadBalancer'：
        - 已新增 SKU 層屬性
    ...
    - 'New-AzLoadBalancerBackendAddressConfig'：
        - 已新增 LoadBalancerFrontendIPConfigurationId 屬性
* 已更新規劃來取代下列 Cmdlet 的警告   -'New-AzVirtualHubRoute'   -'New-AzVirtualHubRouteTable'   -'Add-AzVirtualHubRoute'   -'Add-AzVirtualHubRouteTable'   -'Get-AzVirtualHubRouteTable'   -'Remove-AzVirtualHubRouteTable'
* 已新增規劃來取代下列 Cmdlet 的 'RouteTable' 引數警告   -'New-AzVirtualHub'   -'Set-AzVirtualHub'   -'Update-AzVirtualHub'
* 已將 '-MinScaleUnits' 和 '-MaxScaleUnits' 引數設為 'Set-AzExpressRouteGateway' 中的選擇性項目
* 已新增 Cmdlet 來支援應用程式閘道的相互驗證和 SSL 設定檔
...
#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* 指定原則 BackupTime 的格式為 UTC。
* 修改 Get-AzRecoveryServicesBackupJobDetails Cmdlet 中的中斷性變更警告。
* 更新 Set-AzRecoveryServicesBackupProtectionPolicy Cmdlet 的範例指令碼解說文字。

#### <a name="azresources"></a>Az.Resources
* 已修正 What-If 會顯示兩個大小寫不同的資源群組範圍的問題
* 已將 'Export-AzResourceGroup' 更新為使用 SDK。
* 已將文化特性資訊新增至 parse 方法

#### <a name="azsql"></a>Az.Sql
* 已修正 Set-AzSqlDatabaseAudit 不支援超大規模資料庫資料庫和資料庫版本無法判斷的問題
* 已將 MaintenanceConfigurationId 新增至 New-AzSqlInstance' 和 'Set-AzSqlInstance'
* 已修正 GetAzureSqlDatabaseReplicationLink.cs 中的錯誤 (bug)：PartnerServerName 參數會以值進行檢查，而非索引鍵

#### <a name="azwebsites"></a>Az.Websites
* 新增最新存取限制功能的支援：ServiceTag、multi-ip 和 http-headers

### <a name="thanks-to-our-community-contributors"></a>感謝我們的社群參與者
* John Q. Martin (@johnmart82)，新增防火牆必要條件資訊 (#13385)
* Manikandan Duraisamy (@madurais-msft)，修正 PublicSubnetName 引數 (#13417)
* @mahortas，更新 -HostNames 參數值 (#13349)
* @MariachiForHire，新增支援的 TrafficAnalyticsInterval 值 (#13304)
* Michael James (@mikejwhat)，允許 Get-AzLogicAppRunHistory 傳回超過 30 個項目 (#13393)
...
#### <a name="azaks"></a>Az.Aks
* 透過對 [ListClusterAdmin](/rest/api/aks/managedclusters/listclusteradmincredentials) 和 [ListClusterUser](/rest/api/aks/managedclusters/listclusterusercredentials) API 的呼叫，取代舊 [AccessProfile API](/rest/api/aks/managedclusters/getaccessprofile) 的使用方式
