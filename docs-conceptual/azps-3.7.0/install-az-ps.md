...
Azure PowerShell 可在 Windows 上與 PowerShell 5.1 或更新版本搭配運作，或在所有平台上與 PowerShell Core 6.x 和更新版本搭配運作。 您應為您的作業系統安裝可使用的[最新版 PowerShell Core](/powershell/scripting/install/installing-powershell#powershell-core)。 在 PowerShell Core 上執行時，Azure PowerShell 沒有額外的需求。
...
使用 PowerShellGet Cmdlet 是慣用的安裝方法。 這個方法在 Windows、macOS、Linux 平台上的做法相同。 從 PowerShell 工作階段執行下列命令︰
...
> [!WARNING]
> 我們不支援同時為 PowerShell 5.1 for Windows 安裝 AzureRM 和 Az 模組。 如果您需要在系統上保留可用的 AzureRM，請安裝適用於 PowerShell Core 6.x 或更新版本的 Az 模組。

首先，[下載 PowerShell Core 6.x 或更新版本](/powershell/scripting/install/installing-powershell-core-on-windows)。

然後，從 PowerShell Core 工作階段中，只為目前使用安裝 Az 模組。 這是建議的安裝範圍。
...
* 將模組下載到您網路中的另一個位置，並使用該位置做為安裝來源。
  這可讓您將單一伺服器或檔案共用上的 PowerShell 模組 (已使用 PowerShellGet 部署) 快取到任何已中斷連線的系統上。 了解如何透過[使用本機 PowerShellGet 存放庫](/powershell/scripting/gallery/how-to/working-with-local-psrepositories)設定本機存放庫，並安裝在中斷連線的系統上。
