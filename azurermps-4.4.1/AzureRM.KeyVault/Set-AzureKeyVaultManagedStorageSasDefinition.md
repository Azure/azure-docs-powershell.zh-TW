...
使用保存庫 [vault1] 中的主要保存庫管理的儲存空間帳戶 ' 帳戶 1 ' 設定 ad hoc 服務 blob sas 定義 ' sas1」。

### 範例2：設定 ad hoc 帳戶 sas 定義
```
...
使用保存庫 [vault1 "中的主要保存庫管理的儲存空間帳戶 ' 帳戶 1 ' 設定 ad hoc blob sas 定義 ' sas1」。

### 範例3：使用 hashtable 設定 sas 定義
```
...
使用 hashtable 來設定保存庫 [vault1] 中的主要 blob sas 定義 ' sas1 ' （含金鑰儲存管理儲存帳戶 ' 帳戶1」）。
...
### -ApiVersion
指定要用來執行使用帳戶 SAS URI 發出之要求的儲存服務版本。
...
### -IPAddressOrRange
IP 或 IP 範圍 ACL (存取控制) 清單，以供 Azure 存儲接受的要求。
...
### -許可權
拒絕. 值包含 "Query"、"Add"、"Update"、"Process"
...
### -ResourceType
此 SAS 權杖適用的資源類型。 值包括 "Service"、"Container"、"Object"
...
### -服務
此 SAS 權杖適用的服務類型。 值包含 "Blob"，"File"，"Queue"，"Table"
...
指定已簽署的儲存服務版本，以用來驗證使用 SAS 權杖所進行的要求。
...
[Azureâ€‹ RM。€‹ Keyâ€‹ Vault](/powershell/module/azurerm.keyvault/)
