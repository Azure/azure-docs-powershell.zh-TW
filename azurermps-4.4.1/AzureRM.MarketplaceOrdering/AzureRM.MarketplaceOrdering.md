...
## 說明
本節中的主題會將 azure 資源管理器中的 azure PowerShell Cmdlet MarketplaceOrdering 到 Azure 資源管理員 (ARM) 架構中。 MarketplaceOrdering 命名空間中存在 Cmdlet。 這些 Cmdlet 可讓 azure 使用者接受 marketplace 的法律條款，進一步允許 programmtic 部署這些方案。 使用者也可以拒絕已接受的一組法律詞彙。
...
針對指定的發行者識別碼 (Publisher) 中接受或拒絕法律條款，提供識別碼 (產品) 與方案 id (名稱) 。 請使用 Get-AzureRmMarketplaceTerms 取得合約條款。
