...
### 從 WebService 實例定義建立新的 Azure ML webservice。
```
...
## 說明
Update-AzureRmMlWebService Cmdlet 可讓您更新 web 服務的非靜態屬性。
這個 Cmdlet 可以做為修補程式操作。
只傳遞您想要修改的屬性。
...
在這裡，我們會變更描述、主要便捷鍵，以及在執行時間期間為 web 服務的所有追蹤啟用 [診斷集合]。
...
這個範例會先建立一份 web 服務定義，只包含要更新的欄位，然後使用 ServiceUpdates 參數呼叫 Update-AzureRmMlWebService 來套用它們。

## 參數

### -資產
組成 web 服務的一組資產 (例如模組、資料集) 。
...
### -輸入
Web 服務輸入 (s) 的定義，提供為 Swagger 架構構造。
...
### -IsReadOnly
指定此 web serviceis 唯讀。
設定之後，web 服務就可以再更新，包括變更這個屬性的值，而且只能刪除。
...
### -輸出
Web 服務輸出 (s) 的定義，提供為 Swagger 架構構造。
...
### -參數
為 web 服務定義的全域參數值集，指定為全域參數名稱- \> 預設值集合。
如果沒有指定預設值，則認為該參數是必要的。
...
### -ServiceUpdates
將一組更新套用至以 web 服務定義實例形式提供的 web 服務。
只會修改非靜態欄位。
...
### WebService
"ServiceUpdates" 參數接受來自管線的 "WebService" 類型的值
