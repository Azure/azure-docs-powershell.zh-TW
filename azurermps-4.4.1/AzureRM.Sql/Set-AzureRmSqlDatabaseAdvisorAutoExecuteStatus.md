...
## 說明
**AzureRmSqlDatabaseAdvisorAutoExecuteStatus** Cmdlet 會修改 Azure SQL Database Advisor 的自動執行屬性。
這個 Cmdlet 目前支援啟用、停用和預設值。
...
這個命令會將名為 CreateIndex 的 advisor 的自動執行狀態變更為 [啟用]。

## 參數

### -AdvisorName
指定此 Cmdlet 修改狀態的顧問名稱。
...
這個 Cmdlet 會傳回 **AzureSqlDatabaseAdvisorModel** 物件。
