...
## 說明
**AzureRmSqlDatabaseAdvisor** Cmdlet 會針對 Azure sql 資料庫取得一個或多個 Azure Sql database 顧問。
...
這個命令會針對屬於名為 [wi-東] 的伺服器的名稱為 WIRunner 的所有顧問列出該資料庫。
...
這個命令會取得名為 WIRunner 之資料庫的名為 CreateIndex 的顧問。
...
這個命令會以回應中包含的建議動作來取得名為「WIRunner」的資料庫所有顧問。
由於命令使用 *ExpandRecommendedActions* 參數，因此 Cmdlet 會取得回應的建議動作。
...
這個命令會從名為 WIRunner 的資料庫中取得名為 CreateIndex 的顧問，且回應中包含其建議的動作。
由於命令使用 *ExpandRecommendedActions* 參數，因此 Cmdlet 會取得回應的建議動作。
...
### AzureSqlDatabaseAdvisorModel 中的 [.Sql] 命令

## 筆記
* 關鍵字： azure，azurerm，arm，資源，管理，管理員，sql，資料庫，mssql，advisor
