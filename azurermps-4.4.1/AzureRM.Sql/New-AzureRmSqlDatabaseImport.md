...
## 說明
**新的-AzureRmSqlDatabaseImport** Cmdlet 會從 Azure 儲存空間帳戶將 bacpac 檔案匯入到新的 Azure SQL 資料庫。
可能會傳送 [取得匯入資料庫狀態] 要求，以取得此要求的狀態資訊。

## 示例

### 範例1：建立 bacpac 檔案的匯入要求
```
...
這個命令會建立匯入要求，以便將 bacpac 匯入到新資料庫。
...
### -AuthenticationType
指定用來存取伺服器的驗證類型。
如果未設定驗證類型，此參數預設為 SQL。

此參數可接受的值為：

- 語句.
SQL 驗證。
將 *AdministratorLogin* 和 *AdministratorLoginPassword* 參數設定為 SQL 系統管理員的使用者名稱和密碼。 
- ADPassword.
Azure Active Directory 驗證。
將 *AdministratorLogin* 和 *AdministratorLoginPassword* 設定為 Azure Active Directory 系統管理員的使用者名稱和密碼。
