...
## 說明
**AzureRmServiceBusQueueAuthorizationRule** Cmdlet 會更新特定服務匯流排佇列之指定授權規則的描述。
...
將 **管理** 新增到佇列授權規則的存取權限 `SBAuthoRule1` `SB-Queue_exampl1` 。

## 參數

### -AuthorizationRuleName
授權規則名稱。 如果未指定 **-AuthruleObj** ，則為必要。
...
### -許可權
權力;例如 @ ( 「聆聽」、「傳送」、「管理」 ) 。 如果未指定 ' AuthruleObj」，則為必要。
