...
## 說明
Add-AzureAnalysisServicesAccount Cmdlet 是用來登入 Azure Analysis Services 伺服器實例
...
這個範例會將 $UserCredential 變數所指定的帳號新增至 westcentralus.asazure.windows.net Analysis Services 環境。
...
第一個命令會取得應用程式服務主體認證，然後將它們儲存在 $ApplicationCredential 變數中。
第二個命令會將 $ApplicationCredential 變數和 TenantId 所指定的應用程式服務主體帳戶新增至 westcentralus.asazure.windows.net Analysis Services 環境。
...
這個範例會將 ApplicationId、TenantId 和 CertificateThumbprint 所指定的應用程式服務主體帳戶新增至 westcentralus.asazure.windows.net Analysis Services 環境。
...
### -RolloutEnvironment
要登入之 Azure Analysis Services 環境的名稱。 考慮到伺服器的完整名稱（例如 asazure://westcentralus.asazure.windows.net/testserver），此變數的正確值將會 westcentralus.asazure.windows.net
...
### AnalysisServices. Dataplane. AsAzureProfile

## 筆記
別名： Login-AzureAsAccount
