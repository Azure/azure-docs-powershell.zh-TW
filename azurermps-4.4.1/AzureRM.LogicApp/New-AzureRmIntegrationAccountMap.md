...
## 說明
**新的-AzureRmIntegrationAccountMap** Cmdlet 會建立整合帳戶對應。
這個 Cmdlet 會傳回代表整合帳戶對應的物件。
指定整合帳戶名稱、資源組名稱、對應名稱和地圖定義。
...
這個命令會在指定的資源群組中建立整合帳戶地圖。
命令會指定由前一個命令儲存在 $MapContent 變數中的地圖定義。

## 參數

### -ContentType
指定整合帳戶對應的內容類型。
這個 Cmdlet 支援 application/xml 作為地圖內容類型。
...
### -MapFilePath
指定整合帳戶對應的定義檔路徑。 請指定此參數或 *MapDefinition* 參數。
