...
## 說明
匯 **入-AzureRmAutomationRunbook** Cmdlet 會匯入 Azure 自動化 runbook。 指定 wps_2 腳本的路徑 (. ps1 ) 要匯入 wps_2 及 wps_2 工作流程 runbook 的檔案，或針對圖形 runbook ( graphrunbook) 檔案。 檔案的名稱會變成 runbook 的名稱。 針對 wps_2 工作流程 runbook，此腳本必須包含與檔案名稱相符的單一 wps_2 工作流程定義。
...
第二個命令會將一個名為 GraphicalRunbook06 的圖形 runbook 匯入到名為 AutomationAccount01 的自動化帳戶中。
該命令也會指派儲存在 $Tags 中的標籤。

## 參數

### -AutomationAccountName
指定此 Cmdlet 要匯入 runbook 之自動化帳戶的名稱。
...
### -Path
指定此 Cmdlet 所匯入的 ps1 或 graphrunbook 檔案的路徑。
...
### -ResourceGroupName
指定此 Cmdlet 要匯入 runbook 之資源群組的名稱。
