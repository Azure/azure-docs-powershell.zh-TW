...
## 說明
**AzureRmRecoveryServicesBackupJobDetails** Cmdlet 會取得指定作業的 Azure 備份作業詳細資料。
...
第一個命令會在 vault 中取得失敗作業的陣列，然後將它們儲存在 $Jobs 陣列中。

第二個命令會在 $Jobs 中取得失敗作業的作業詳細資料，然後將它們儲存在 $JobDetails 變數中。
