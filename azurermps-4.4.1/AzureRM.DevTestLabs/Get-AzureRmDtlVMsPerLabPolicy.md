...
## 說明
**AzureRmDtlVMsPerLabPolicy** Cmdlet 會取得實驗室的每個實驗室原則的虛擬機器，這可讓您設定實驗室所允許的虛擬機器總數。
這個 Cmdlet 會傳回原則的啟用或停用狀態，以及您在原則中設定的實驗中所允許的虛擬機器總數量。
...
這個 Cmdlet 會傳回指定可在 lab 中建立的虛擬機器數上限的原則。
