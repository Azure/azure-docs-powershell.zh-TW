...
## 摘要
針對指定事件中樞命名空間上的授權規則，建立新的主要或次要金鑰。
...
## 說明
New-AzureRmEventHubNamespaceKey Cmdlet 會針對指定事件中樞命名空間上的指定授權規則，重新生成主要或輔助金鑰。
...
\` \` 在事件中心命名空間 \` MyNamespaceName 中 \` ，使用資源群組 \` MyResourceGroupName \` 重新產生授權規則 MyAuthRuleName 的主鍵。
...
### -RegenerateKeys
要重新產生： \` PrimaryKey \` 或 \` SecondaryKey \` 的按鍵。
