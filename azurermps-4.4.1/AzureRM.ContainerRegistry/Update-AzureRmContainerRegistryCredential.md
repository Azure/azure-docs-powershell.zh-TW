...
## 說明
**AzureRmContainerRegistryCredential** Cmdlet 會為容器註冊重新產生登入認證。
...
這個命令會為指定的容器註冊表重新產生登入認證。 管理員使用者必須啟用容器 `MyRegistry` 登錄，才能重新產生登入認證。
...
### PSContainerRegistry
參數 "Registry" 接受管線中 "PSContainerRegistry" 類型的值
