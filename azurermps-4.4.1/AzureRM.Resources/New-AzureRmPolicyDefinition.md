...
## 說明
**AzureRmPolicyDefinition** Cmdlet 會建立原則定義，其中包含 JavaScript) 格式 (JSON 物件符號中的原則規則。
...
這個命令會建立一個名為 VMPolicyDefinition 的原則定義，其中包含 C:\VMPolicy.json 中指定的原則規則。
...
這個命令會建立名為 VMPolicyDefinition 的原則定義。
