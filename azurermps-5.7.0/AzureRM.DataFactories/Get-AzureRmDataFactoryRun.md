...
## 說明
**AzureRmDataFactoryRun** Cmdlet 會取得 Azure 資料工廠中資料集的資料切片執行。
資料工廠中的資料集是由在時間軸上的扇面組成。
扇面的寬度是依排程（每小時或每天）來決定。
Run 是切片的處理單位。
如果發生重試，可能會有一個或多個切片執行，或因失敗而重新執行您的切片時。
切片是由它的開始時間所識別。
若要取得扇面的開始時間，請使用 Get-AzureRmDataFactorySlice Cmdlet。
...
*StartDateTime* 必須以 ISO8601 格式指定，如下列範例所示： 
...
### [System.object]。清單 ' 1 [Microsoft.WindowsAzure.Commands.Utilities.PSDataSliceRun，WindowsAzure. 實用程式，版本 = 0.8.2.0，Culture = 中性，PublicKeyToken = 31bf3856ad364e35]]
