...
## 說明
[ **取得 AzureRmDataFactoryV2TriggerRun** ] 命令會傳回指定的觸發程式在指定時間範圍內觸發程式執行的詳細資訊。
...
這個命令會顯示在「2017-09-01」和「2019-09-30」之間開始「WikiADF」的「WikiTrigger」執行的相關資訊。
...
觸發程式執行開始之後或之後的時間，以 ISO8601 格式執行。
...
觸發程式執行開始之前或之前的時間，以 ISO8601 格式執行。
...
### [System.object]。清單 ' 1 [DataFactoryV2. PSTriggerRun，DataFactoryV2，版本 = 0.1.9.0，Culture = 中性，PublicKeyToken = null]]。）））
