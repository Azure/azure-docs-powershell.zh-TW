...
## 說明
AzureRmRedisCache Cmdlet 會取得指定的 Azure Redis 快 **取** 。
如果您沒有指定參數，此操作會取得目前訂閱的每個 Redis 快取。
...
這個命令會取得名為 myexists 的 Redis 快取。

### 範例2：取得資源群組中的每個 Redis 快取
```
...
這個命令會取得指定資源群組中的每個 Redis 快取。

### 範例3：在目前的訂閱中取得每個 Redis 快取
```
...
### -ResourceGroupName
指定包含要取得之 Redis 快取的資源群組的名稱。

如果您只指定 *ResourceGroupName* 參數，此操作會取得指定資源群組中的每個 Redis 快取。
...
### RedisCacheAttributes 中的 RedisCache。
傳回 **RedisCacheAttributes** 物件的陣列。
