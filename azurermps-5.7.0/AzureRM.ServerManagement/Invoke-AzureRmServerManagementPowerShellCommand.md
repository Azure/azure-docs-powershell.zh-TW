...
## 說明
**AzureRmServerManagementPowerShellCommand** Cmdlet 會在由 Azure Server 管理閘道管理的節點上執行 Windows PowerShell 腳本區塊。
...
指定目標節點上的 Windows PowerShell 運行空間名稱。
...
### -會話
指定此 Cmdlet 用來連接至目標節點的 **會話** 物件。

您可以指定此參數，而不是 *ResourceGroupName* 、 *NodeName* 、 *SessionName* 和 *PowerShellSessionName* 參數。
