...
## 說明
AzureRmVMDscExtension Cmdlet 會在資源群組中的虛擬機器上 **設定** Windows PowerShell 所需的狀態設定 (DSC) 延伸。
...
先前已使用 **Publish AzureRmVMDscConfiguration** 上傳 Sample.ps1.zip 檔案。
...
先前是使用 **Publish AzureRmVMDscConfiguration** 上傳的 Sample.ps1.zip。

## 參數

### -ArchiveBlobName
指定先前由 Publish-AzureRmVMDscConfiguration Cmdlet 上傳的設定檔名稱。
...
### -版本
指定 Set-AzureRmVMDscExtension 套用設定的 DSC 副檔名版本。
