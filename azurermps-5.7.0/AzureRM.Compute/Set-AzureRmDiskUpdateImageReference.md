...
## 說明
**AzureRmDiskUpdateImageReference** Cmdlet 會在磁片更新物件上設定 image 參照屬性。
...
第一個命令會在 Premium_LRS 儲存帳戶類型中，建立一個大小為10GB 的本機磁片更新物件。  它也會設定 Windows OS 類型。
第二個命令會針對磁片更新物件設定影像識別碼和邏輯單元號碼0。
