...
## 說明
**AzureRmOperationalInsightsStorageInsight** Cmdlet 會從工作區刪除儲存空間洞察力。
...
第一個命令使用 Get-AzureRmOperationalInsightsWorkspace Cmdlet 來取得名為 MyWorkspace 的工作區，然後將它儲存在 $Workspace 變數中。第二個命令會從 $Workspace 移除名為 MyStorageInsight 的儲存空間洞察力，而不會提示您進行確認。
