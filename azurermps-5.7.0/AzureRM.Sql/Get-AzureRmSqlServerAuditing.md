...
## 說明
**AzureRmSqlServerAuditing** Cmdlet 會取得 Azure SQL server 的 blob 審核原則。
指定 *ResourceGroupName* 和 *ServerName* 參數來識別資料庫。
這個 Cmdlet 會傳回在指定的 Azure SQL server 中定義的 Azure SQL 資料庫所使用的原則。

## 示例

### 範例1：取得 Azure SQL server 的審核設定
```
