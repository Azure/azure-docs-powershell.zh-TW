...
## 說明
Set-AzureRmAnalysisServicesServer Cmdlet 會修改 Analysis Services 伺服器的實例
...
修改 resourcegroup testgroup 中名為 testserver 的伺服器，將標記設為 key1： value1 和 key2： value2 與 administrator testuser1@contoso.com

## 參數

### -系統管理員
代表要在伺服器上設定為系統管理員之以逗號分隔的使用者或群組清單字串。
使用者或群組必須指定 UPN 格式，例如 user@contoso.comgroups@contoso.com
...
### -DisableBackup
[切換到停用備份 blob] 容器。
若要重新啟用備份 blob 容器，請以-BackupBlobContainerUri 提供備份 blob 容器 Uri。
...
### AnalysisServicesServer 中的 [Azure. 管理]
