...
## 說明
**AzureRmAutoscaleHistory** Cmdlet 會取得與自動縮放設定相關的事件歷程記錄。
...
這個命令會取得與目前訂閱相關聯的所有自動縮放相關事件。

### 範例2：針對特定資源的 GetAutoscaleHistory
```
...
這個命令會取得與資源識別碼所識別之特定資源相關聯的所有自動縮放相關事件， (實質上是 ResourceUri) 。
...
### OutputClasses> PSEventData<的清單中。
