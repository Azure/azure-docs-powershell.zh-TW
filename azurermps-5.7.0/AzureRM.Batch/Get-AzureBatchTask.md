...
## 說明
**AzureBatchTask** Cmdlet 會取得批次作業的 Azure Batch 工作。
使用 *JobId* 參數或 *作業* 參數指定作業。
若要取得單一任務，請指定 *Id* 參數。
您可以指定 *篩選* 參數，以取得符合開啟資料通訊協定 (OData) 篩選的任務。
...
這個命令會從具有識別碼 Job02 的作業中取得已完成的工作。
...
### -篩選
指定任務的 OData 篩選子句。
如果您沒有指定篩選，這個 Cmdlet 會傳回批次帳戶或工作的所有任務。
