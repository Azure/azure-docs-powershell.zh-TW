...
## 說明
**AzApiManagementApiSchema** Cmdlet 會取得 API 架構的詳細資料

## 示例

### 範例1：取得 Api 所有 Api 架構的詳細資料
```powershell
...
這個命令會取得與 Api 相關的所有 API 架構 `swagger-petstore-extensive` ，以用於特定的 ApiManagement 內容。

### 範例2：取得與 Api 相關聯的特定架構
```powershell
PS D:\github\azure-powershell> $context = New-AzApiManagementContext -ResourceId /subscriptions/subid/resourceGroups/resourceGroupName/providers/Microsoft.ApiManagement/service/sdktestapim4163
...
這個命令會取得 `5cc9cf67e6ed3b1154e638bd` 與 api 相關的 api 架構 `swagger-petstore-extensive` ，以用於特定的 ApiManagement 內容。
...
### -ResourceId
Api 架構的 Arm 資源識別碼。 如果已指定，將會嘗試依據識別碼尋找 api 架構。 這個參數是必要的。
