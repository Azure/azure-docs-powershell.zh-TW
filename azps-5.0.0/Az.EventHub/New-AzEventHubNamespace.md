...
## 說明
New-AzEventHubNamespace Cmdlet 會建立一個類型事件中樞的新命名空間。
...
\` \` \` \` 在 \` \` 已啟用 Kafka 和 AutoInflate 的 [資源群組] MyResourceGroupName 中，在指定的地理位置 MyLocation 中建立事件中心命名空間 MyNamespaceName。

### 範例4： ZoneRedundant 啟用的命名空間
```powershell
