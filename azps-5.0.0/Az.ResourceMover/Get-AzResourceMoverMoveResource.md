...
### 範例2：使用移動資源名稱，在移動集合中取得特定資源的詳細資料。
```powershell
...
使用移動資源名稱，在移動集合中取得特定資源的詳細資料。

### 範例3：使用諸如： SourceResourceName、SourceId、MoveState、IsResolveRequired、ProvisioningState 等篩選，在移動集合中取得特定資源的詳細資料
```powershell
...
若要在移動集合中取得特定資源的詳細資料，請使用 armid、moveStatusMoveState (驗證) 等。
...
### -篩選
要套用在工序上的篩選。
例如，您可以使用 $filter = Properties/ProvisioningState eq "Succeeded"。
