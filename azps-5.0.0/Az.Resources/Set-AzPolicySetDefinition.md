...
這個命令會更新名為 VMPolicySetDefinition 的原則集定義中繼資料，以指出其類別是「虛擬機器」。
...
這個命令會更新名為 VMPolicySetDefinition 的原則集定義群組。

### 範例4：使用雜湊資料表更新原則集定義的群組
```
$groupsJson = ConvertTo-Json @{ name = "group1", displayName = "Virtual Machine Security" }, @{ name = "group2" }
PS C:\> Set-AzPolicySetDefinition -Name 'VMPolicySetDefinition' -GroupDefinition $groupsJson
```

這個命令會使用雜湊資料表來構造群組，以更新名為 VMPolicySetDefinition 的原則集定義群組。
...
### -GroupDefinition
更新之原則集定義的原則定義群組。 這可以是包含群組之檔案的路徑，或作為 JSON 字串的群組。
...
### -中繼資料
更新之原則集定義的中繼資料。 這可以是包含中繼資料之檔案名的路徑，或作為 JSON 字串的中繼資料。
...
### -參數
更新之原則集定義的參數宣告。 這可以是包含參數宣告之檔案名或 uri 的路徑，或作為 JSON 字串的參數宣告。
