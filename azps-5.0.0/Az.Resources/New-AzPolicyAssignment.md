...
第一個命令會使用 Get-AzSubscription Cmdlet 來取得名為 Subscription01 的訂閱，並將其儲存在 $Subscription 變數中。
第二個命令會使用 Get-AzPolicyDefinition Cmdlet 來取得名為 VirtualMachinePolicy 的原則定義，並將它儲存在 $Policy 變數中。
最終命令會將原則指派給訂閱範圍字串所識別之訂閱層級的 $Policy。
...
第一個命令會使用 Get-AzSubscription Cmdlet 來取得名為 Subscription01 的訂閱，並將其儲存在 $Subscription 變數中。
第二個命令會使用 Get-AzPolicyDefinition Cmdlet 來取得名為 VirtualMachinePolicy 的原則定義，並將它儲存在 $Policy 變數中。
最終命令會將原則指派給訂閱範圍字串所識別之訂閱層級的 $Policy。 作業是使用 _DoNotEnforce_ 的 EnforcementMode 值來設定，亦即在建立或更新資源期間未強制執行原則效果。
...
### -EnforcementMode
原則指派的強制模式。 目前，有效值為預設值，DoNotEnforce。
