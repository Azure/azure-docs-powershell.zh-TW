...
## 說明
針對裝置 SAS 權杖，原則參數只是用來存取 device registry。 因此，原則應該具備對註冊表的讀取存取權。
針對 IoT 中樞權杖，該原則就是 SA 的一部分。
...
使用 iothubowner 原則和主鍵來產生 IoT 中樞 SAS 權杖。
...
使用 registryRead 原則和輔助金鑰來產生 IoT 中樞 SAS 權杖。
...
使用 iothubowner 原則產生裝置 SAS 權杖，以存取 {iothub_name} 裝置註冊。
...
使用 iothubowner 原則產生模組 SAS 權杖，以存取 {iothub_name} 裝置註冊。
