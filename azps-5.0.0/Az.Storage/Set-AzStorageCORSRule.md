...
## 說明
**AzStorageCORSRule** Cmdlet 會針對 Azure 儲存空間服務的類型，設定跨來源資源分享 (CORS) 規則。
此 Cmdlet 的儲存服務類型為 Blob、資料表、佇列和檔案。
這個 Cmdlet 會覆寫現有的規則。
若要查看目前的規則，請使用 Get-AzStorageCORSRule Cmdlet。
...
第一個命令會使用 **AzStorageCORSRule** Cmdlet 取得 Blob 類型的目前 CORS 規則。
...
### -CorsRules
指定 CORS 規則的陣列。
您可以使用 Get-AzStorageCORSRule Cmdlet 來檢索現有規則。
