...
## 說明
**AzSynapseTriggerSubscription** Cmdlet 會將事件觸發程式從觸發程式的指定訂閱至指定的外部服務事件。
...
這個命令會將觸發程式（名為 ContosoTrigger 的觸發程式）訂閱到觸發程式的指定事件。
...
這個命令會將觸發程式（名為 ContosoTrigger 的觸發程式）從觸發程式（透過管線）的指定事件訂閱。
