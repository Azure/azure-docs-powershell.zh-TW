...
## 說明
**AzCognitiveServicesAccountNetworkRuleSet** Cmdlet 會更新認知服務帳戶的 NetworkRule 屬性
...
這個命令會以 JSON 更新 NetworkRule、輸入規則的所有屬性。
...
這個命令會清理 NetworkRule 的認知服務帳戶規則， (其他屬性不會變更) 。

## 參數

### -DefaultAction
認知服務帳戶 NetworkRule DefaultAction。 預設值 `Deny` 。
...
### PSNetworkRuleSet （CognitiveServices.）。
