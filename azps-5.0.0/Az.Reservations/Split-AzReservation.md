...
## 說明
`Reservation`以指定的數量分配分割 a 到兩個 `Reservation` 。
...
將指定的 `Reservation` 數量分割成兩 `Reservation` 秒，並將相對應的數量分割
...
### 數量
兩個 s 之 [數量] 欄位的逗號分隔整數 `Reservation`
...
### -ReservationOrderId
`ReservationOrder`包含 `Reservation` 要分割之使用者的識別碼
