...
這個 Cmdlet 會列出指定 PostgreSql 伺服器中的所有設定。

### 範例2：依名稱取得指定的 PostgreSql 設定
```powershell
...
這個 Cmdlet 透過名稱取得指定的 PostgreSql 配置。
...
### IConfiguration （PostgreSql）。 Api20171201。
