...
## 說明
這個命令會從同步處理群組中刪除指定的雲端端點。 Azure 檔案共用雲端端點參照在此程式中保持不變。 此命令僅供解除授權時用。 移除雲端端點是破壞性的操作。 如果沒有至少一個雲端端點存在，伺服器端點就無法進行同步處理。 不應該執行此作業來解決同步處理問題。 如果此 Azure 檔案共用再次新增至同一個同步處理群組（作為雲端端點），可能會導致衝突檔案和其他不預期的結果。
