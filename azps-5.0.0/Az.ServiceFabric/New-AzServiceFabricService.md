...
## 說明
這個 Cmdlet 可讓您在指定的應用程式下建立無狀態或有狀態的服務。 服務應該會在應用程式資訊清單中結束，且類型應該與資訊清單中的相同。 應用程式名稱應該是服務名稱的首碼。
...
這個範例會在所有節點) 上，建立一個新的無狀態服務「testApp ~ testService1」，並 (實例計數-1。
...
這個範例會使用5個節點的目標，建立新的狀態服務「testApp ~ testService2」。
...
### -DefaultMoveCost
指定移動的預設成本。
較高的成本可讓群集資源管理員在嘗試平衡群集時移動複製副本的可能性不大
...
指示服務使用命名的分區配置。
使用此模型的服務通常會有可在限制集內 bucketed 的資料。
使用做為命名的分區鍵的一些資料欄位的常見範例是地區、郵遞區號、客戶群組或其他業務界限。
...
指示服務使用 UniformInt64 分區配置。
這表示每個分區都擁有一個範圍的 int64 鍵。
