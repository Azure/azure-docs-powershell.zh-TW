...
建立具有5個節點的主要節點類型，並指定放置屬性、容量、應用程式和暫時埠。
...
### -PlacementProperty
將位置標記套用至節點類型中的節點做為索引鍵/值組，可以用來指出特定服務 (工作負荷) 應該如何執行。
更新這將會覆寫目前的值。
...
### -VmImageOffer
Azure 虛擬機器 Marketplace 影像的優惠類型。
預設值： WindowsServer。
...
### -VmImageSku
Azure 虛擬機器 Marketplace 影像的 SKU。
預設值： 2019-資料中心。
