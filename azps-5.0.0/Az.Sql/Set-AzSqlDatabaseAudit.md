...
### 範例3：使用 T-sql 謂詞來啟用 Azure SQL 資料庫的 blob 儲存審核原則（含高級篩選）
```powershell
...
### 範例4：從 Azure SQL 資料庫的審核原則中移除 [高級篩選] 設定
```powershell
...
### 範例7：啟用 Azure SQL 資料庫的 log analytics 審核原則
```powershell
...
### 範例8：停用 Azure SQL 資料庫的 log analytics 審核原則
```powershell
...
### 範例9：透過管線停用 Azure SQL 資料庫的 log analytics 審核原則
```powershell
...
### 範例10：停用將 Azure SQL 資料庫的審核記錄傳送到 blob 儲存空間，並允許將其傳送至記錄分析。
```powershell
...
### 範例11：啟用將 Azure SQL 資料庫的審核記錄傳送到 blob 儲存體、事件中樞和記錄分析。
```powershell
