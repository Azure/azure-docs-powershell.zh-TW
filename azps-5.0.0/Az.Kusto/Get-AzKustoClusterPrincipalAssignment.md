...
### 範例1：列出所有 Kusto 群集 principalAssignment
```powershell
...
上述命令會列出群集 "testnewkustocluster" 中的所有 principalAssignment。

### 範例2：透過名稱取得 Kusto 群集 principalAssignment
```powershell
...
上述命令會傳回群集 "testnewkustocluster" 中名為「kustoprincipal1」的 Kusto 群集 principalAssignment。
...
### IClusterPrincipalAssignment （Kusto）。 Api20200614。
