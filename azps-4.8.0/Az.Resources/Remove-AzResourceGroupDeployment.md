...
**AzResourceGroupDeployment** Cmdlet 會移除 Azure 資源群組部署及任何相關聯的作業。

## 示例

### 範例1：移除與 ResourceId 的資源群組部署
...
這個命令會移除具有部署之完全限定資源識別碼的資源群組部署。
成功的移除會傳回 true。

### 範例2：移除含 ResourceGroupName 和資源組的資源群組部署
...
這個命令會移除具有提供的 ResourceGroupName 和資源組的資源群組部署。
成功的移除會傳回 true。
