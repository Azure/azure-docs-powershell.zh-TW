...
## 說明
**新的-AzSynapseFirewallRule** Cmdlet 會建立 Azure Synapse 分析防火牆規則。
...
這個命令會在 [工作區 ContosoWorkspace] 下建立名為 [ContosoFirewallRule] 的防火牆規則。
...
這個命令會透過管線在工作區下建立名為 ContosoFirewallRule 的防火牆規則。
...
這個命令會建立允許所有 azure ip 在工作區下方的防火牆規則。

## 參數

### -AllowAllAzureIP
建立特殊的防火牆規則，以允許所有 Azure Ip 都能存取。
