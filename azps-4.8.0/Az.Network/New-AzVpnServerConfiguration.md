...
## 說明
**新的-AzVpnServerConfiguration** Cmdlet 可讓您使用不同的 VpnProtocols、VpnAuthenticationTypes、IpsecPolicies 建立新的 VpnServerConfiguration，並根據客戶對網站連線的需求，設定選取的 vpn 驗證類型相關參數。
...
上述命令會建立新的 VpnServerConfiguration，並以 VpnAuthenticationType 作為憑證。

### 範例2

建立新的 VpnServerConfiguration，以指向網站連線。  (自動生成) 
