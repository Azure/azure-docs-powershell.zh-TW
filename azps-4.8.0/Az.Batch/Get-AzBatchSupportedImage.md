...
## 說明
**AzBatchSupportedImage** Cmdlet 會取得 Azure Batch 帳戶中提供的支援虛擬機器影像。
...
第一個命令會使用 **AzBatchAccountKey** 來取得包含訂閱之便捷鍵的批次帳戶內容。
該命令會將內容儲存在 $CoNtext 變數中，以用於下一個命令。
第二個命令會取得該 Batch 帳戶的所有可用支援影像。
...
### -篩選
為支援的圖像指定 OData 篩選子句。
如果您沒有指定篩選，這個 Cmdlet 會傳回批次帳戶所支援的所有影像。
...
### Microsoft.Azure.Commands.Batch。PSImageInformation
