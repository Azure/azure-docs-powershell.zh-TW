...
上述命令會針對群集 "testnewkustocluster" 中的資料庫 "mykustodatabase" 更新名為 "myeventhubdc" 的現有 EventHub 資料連線。

### 範例2：更新現有的 EventGrid 資料連線
```powershell
...
上述命令會針對群集 "testnewkustocluster" 中的資料庫 "mykustodatabase"，更新名為 "myeventgriddc" 的現有 EventGrid 資料連線。

### 範例3：更新現有的 IotHub 資料連線
```powershell
...
上述命令會針對群集 "testnewkustocluster" 中的資料庫 "mykustodatabase"，更新名為 "myiothubdc" 的現有 IotHub 資料連線。

### 範例4：透過身分識別來更新現有的 EventHub 資料連線
```powershell
...
### 範例5：透過身分識別更新現有的 EventGrid 資料連線
```powershell
...
### IDataConnection （Kusto）。 Api20200614。
