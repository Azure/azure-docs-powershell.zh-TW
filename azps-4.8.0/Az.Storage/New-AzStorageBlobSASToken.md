...
## 說明
**AzStorageBlobSASToken** Cmdlet 會針對 Azure 儲存區 blob 產生 (SAS) 權杖的共用存取簽章。

## 示例

### 範例1：使用完整 blob 許可權產生 blob SAS 標記
```
PS C:\>New-AzStorageBlobSASToken -Container "ContainerName" -Blob "BlobName" -Permission rwd
```

這個範例會產生含完整 blob 許可權的 blob SAS 權杖。

### 範例2：使用生命時間產生 blob SAS 標記
```
...
這個範例會產生含生命時間的 blob SAS 標記。

### 範例3：根據 OAuth 驗證產生含儲存內容的使用者身分識別 SAS 權杖
```
...
這個範例會根據 OAuth 驗證產生一個含儲存內容的使用者身分識別 blob SAS 標記
...
### -CloudBlob
指定 **CloudBlob** 物件。
若要取得 **CloudBlob** 物件，請使用 [AzStorageBlob](./Get-AzStorageBlob.md) Cmdlet。
...
### -內容
指定儲存內容。
當儲存內容是以 OAuth 驗證為基礎時，會產生使用者身分識別 blob SAS 權杖。
...
### -ExpiryTime
指定共用存取簽章的到期時間。
當儲存內容是以 OAuth 驗證為基礎時，到期時間必須是與目前時間的7天，且不得早于目前時間。
...
### -許可權
指定儲存 blob 的許可權。 請務必注意，這是字串，就像是 `rwd` 讀取、寫入及刪除) 的 (。 
