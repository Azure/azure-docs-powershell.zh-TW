...
## 說明
**AzStorageObjectReplicationPolicy** Cmdlet 會建立或更新儲存空間帳戶中指定的物件複製原則。

## 示例

### 範例1：將物件複製原則設定為目的地和來源帳戶。
```
...
這個命令會將物件複製原則設定為目的地和來源帳戶。
首先建立2個物件複製原則規則，並將原則設定為2個規則至 [目的地帳戶]。 然後，將 [物件複製原則] 從 [目的地帳戶] 設定為 [來源帳戶]。
...
### -PolicyId
物件複製原則識別碼。它應該是 GUID 或「預設」。
如果未輸入 PolicyId，則會使用「預設」，這表示建立新的原則，而建立的原則中將會傳回新原則的識別碼。
...
### PSObjectReplicationPolicy 中的 [管理]。
