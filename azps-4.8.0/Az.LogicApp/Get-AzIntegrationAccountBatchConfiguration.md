...
## 說明
**AzIntegrationAccountBatchConfiguration** Cmdlet 會從整合帳戶取得批次設定。
...
取得名為「sampleBatchConfig」的批次配置，該設定會位於包含在資源群組 "sampleResourceGroup" 的整合帳戶 "sampleIntegrationAccount" 中。

### 範例2：依參數列出整合帳戶中的所有批次設定
```powershell
...
取得位於整合帳戶 "sampleIntegrationAccount" 中的所有批次設定（包含在資源群組 "sampleResourceGroup" 中）。
