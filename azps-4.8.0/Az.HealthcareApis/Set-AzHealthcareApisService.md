...
### 範例2：使用 cosmosdb OfferThroughput = 500 更新資源群組 MyResourceGroup 中名為 MyService 的現有 healthcareapis 服務。
...
### -CorsHeader
HealthcareApis Fhir 服務的 Cors 標頭清單。 指定可在要求期間使用的 HTTP 標頭。 針對任何標題使用 *。
...
### -CorsMaxAge
HealthcareApis Fhir 服務 Cors 最大使用時間。 指定要求中的結果可在數秒內緩存多久。 範例：600代表10分鐘。
...
### -CorsOrigin
[Cors 來源] 的 HealthcareApis FhirService 清單。 [Cors 來源] 的 HealthcareApis Fhir 服務清單。 指定可存取此 API 的來源網站 Url，或使用 * 以允許來自任何網站的存取權。
...
### PSHealthcareApisService 中的 HealthcareApisService。
