...
在開始移動資源之前驗證 dependecies。

### 範例2：使用 "MoveResource Name" 作為輸入來啟動移動集合中的資源集合
```powershell
...
在移動集合中，使用 "MoveResource Name" 做為輸入的資源集合開始移動。

### 範例3：使用 "SourceARMID" 做為輸入，在移動集合中開始移動一組資源
```powershell
...
在移動集合中使用 "SourceARMID" 做為輸入的資源集合開始移動。
