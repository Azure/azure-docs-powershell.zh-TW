...
## 說明
**新的-AzDataFactoryHub** Cmdlet 會在指定的 azure 資源群組和指定的資料工廠中，以指定的檔案定義來建立一個中樞，以供 Azure 資料工廠使用。
建立中樞之後，您可以使用它來儲存及管理群組中的連結服務，而且您可以在中樞中新增管線。
...
這個命令會在資源群組 ADFResourceGroup 和名為 ADFDataFactory 的資料工廠中，建立名為 ContosoDataHub 的中樞。

## 參數

### -DataFactory
指定 **PSDataFactory** 物件。
這個 Cmdlet 會為此參數指定的資料工廠建立中樞。
...
### 檔案
指定包含中樞說明的 JavaScript 物件符號 (JSON) 檔的完整路徑。
...
### -Force
表示此 Cmdlet 會取代現有的中樞，而不會提示您進行確認。
