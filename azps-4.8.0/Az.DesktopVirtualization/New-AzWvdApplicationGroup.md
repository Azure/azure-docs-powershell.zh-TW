...
### 範例1：依名稱建立 Windows 虛擬桌面 ApplicationGroup
```powershell
...
這個命令會在資源群組中建立 Windows 虛擬桌面 ApplicationGroup。

### 範例2：依名稱建立 Windows 虛擬桌面 ApplicationGroup
```powershell
PS C:\> New-AzWvdApplicationGroup -ResourceGroupName ResourceGroupName `
                            -Name ApplicationGroupName `
                            -Location 'eastus' `
...
### -HostPoolArmPath
ApplicationGroup 的 HostPool 臂路徑。
...
### IApplicationGroup （DesktopVirtualization）。 Api20191210Preview。
