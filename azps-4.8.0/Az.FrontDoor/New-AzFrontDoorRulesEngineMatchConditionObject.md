...
符合的值。
運算子會套用到這裡的每個值，或語義。
如果有任何專案與指定運算子的變數相符，此符合條件就會視為相符。
...
### -MatchVariable
[相符] 變數。
可能的值為 IsMobile、RemoteAddr、RequestMethod、QueryString、PostArg、RequestUri、RequestPath、RequestFileName、RequestfilenameExtension、RequestHeader、RequestBody、RequestScheme、、、
...
### -運算子
描述要套用至 match 條件的運算子。
可能的值為 Any、IPMatch、GeoMatch、等於號、Contains、小於、GreaterThan、LessThanOrEqual、GreaterThanOrEqual、BeginsWith、EndsWith、、、。
...
### -轉換
在匹配前套用的轉換清單。 可能的個別轉換值為小寫、大寫、Trim、UrlDecode、UrlEncode、RemoveNulls。
...
### PSRulesEngineMatchCondition 中的 FrontDoor。
