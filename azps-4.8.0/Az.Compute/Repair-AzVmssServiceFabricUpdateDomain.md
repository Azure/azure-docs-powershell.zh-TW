...
## 說明
強制執行手動平臺更新網域逐步更新在 service fabric 虛擬電腦規模集中更新虛擬機器。
...
這個命令會針對由資源群組名稱和縮放集名稱指定的虛擬機器規模集，強制執行針對 UD 0 的服務結構更新行走。
...
這個命令會針對由 VM 比例集物件指定的虛擬機器規模集，強制執行針對 UD 1 的服務結構更新行走。
...
這個命令會針對資源識別碼指定的虛擬電腦規模集，強制執行針對 UD 2 的服務結構更新行走。
...
### PSRecoveryWalkResponse 中的 [計算] 命令。
