...
## 說明
Get-AzSqlInstanceDatabaseSensitivityRecommendation Cmdlet 會傳回 Azure SQL managed 實例資料庫中資料行的建議資訊類型和敏感度標籤。

## 示例

### 範例1：取得 Azure SQL managed 實例資料庫的建議資訊類型和敏感度標籤。
```powershell
...
### 範例2：使用管道取得 Azure SQL managed 實例資料庫的建議資訊類型和敏感度標籤。
```powershell
