...
## 說明
**Clear AzSqlDatabaseAdvancedThreatProtectionSetting** Cmdlet 會從 AzureAzure SQL 資料庫中移除高級威脅防護設定。
若要使用這個 Cmdlet，請指定 *ResourceGroupName* 和 *ServerName* 參數來識別這個 Cmdlet 移除其設定的資料庫。
...
這個命令會從名為 Server01 的伺服器上名為 Database01 的資料庫中，移除 [高級威脅防護] 設定。

## 參數

### -DatabaseName
指定應該移除高級威脅防護設定的資料庫名稱。
