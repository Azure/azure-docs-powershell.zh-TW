...
## 說明
Disable-AzSqlDatabaseSensitivityRecommendation Cmdlet 會停用資料庫中資料行的 (關閉) 敏感度建議。

## 示例

### 範例1：停用 Azure SQL 資料庫中指定資料行的敏感度建議。
```powershell
...
### 範例2：在使用管道的 Azure SQL 資料庫的欄上，停用敏感度建議。
```powershell
...
### 範例3：使用管道針對 Azure SQL 資料庫中的特定欄停用靈敏度建議。
```powershell
