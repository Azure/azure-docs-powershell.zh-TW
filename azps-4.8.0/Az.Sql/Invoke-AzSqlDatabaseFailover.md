...
## 說明
Invoke-AzSqlDatabaseFailover Cmdlet 會針對 Azure SQL 資料庫進行容錯移轉。 如果資料庫位於彈性池中，此命令會將指定的資料庫容錯移轉，而不會影響相同彈性池中的其他資料庫。
...
這個命令會將名為 "Server01" 的伺服器上名為 "Database01" 的主要複本容錯移轉
...
這個命令會將名為 "Server01" 的伺服器上名為 "Database01" 的可讀取輔助副本容錯移轉
...
### -PassThru
成功執行時，傳回 true。  根據預設，這個 Cmdlet 不會產生任何輸出。
