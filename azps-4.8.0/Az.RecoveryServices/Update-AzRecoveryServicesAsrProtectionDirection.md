...
## 說明
**AzRecoveryServicesAsrProtectionDirection** Cmdlet 會在完成認可式容錯移轉作業之後，更新指定 Azure Site Recovery 物件的複製方向。
...
針對指定的復原方案啟動更新方向作業，並傳回用於追蹤作業的 ASR 工作物件。
...
在目標 azure 區域中由保護容器對應所定義的特定複製受保護專案，並在與 VM) 相同地區中使用快取儲存空間 (，啟動更新方向作業。
...
針對由保護容器對應的目標 azure 區域中指定的受保護專案及提供磁碟複製設定，啟動更新方向作業。
...
針對由保護容器對應的目標 azure 區域中指定的加密複製受保護專案，以及提供磁碟複製設定，啟動更新方向作業。
...
在目標 azure 區域中由保護容器對應所定義的特定複製受保護專案，以及使用 [快取儲存空間] 與 [VM) 與鄰近性位置] 群組中的 [快取] (，啟動更新方向作業。
...
指定要在容錯移轉之後用來恢復 VM 的磁片加密秘密電子倉庫識別碼 (Azure 磁片加密) 。
...
### -KeyEncryptionKeyUrl
指定要在容錯移轉之後用來恢復 VM (Azure 磁片加密) 的磁片加密金鑰 URL。
...
要將此虛擬機器容錯移轉至其中的 [恢復鄰近性] 位置群組的資源識別碼。
...
### -VmmToVmm
針對在兩個 VMM 管理的 Hyper-v 網站之間受保護的 Hyper-v 虛擬機器，更新複製方向。
